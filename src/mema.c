...
mema_addr_t g_MemaHeapStart = 0;
u32 g_MemaHeapSize = 0;
struct memaheap g_MemoryAllocations = {0};
void *g_MemoryAllocationDebugData = NULL;

// Swap two allocations.
void memaSwap(memaspace *a, memaspace *b) {
    mema_addr_t tempaddr = a->addr;
    u32 tempsize = a->size;
    a->addr = b->addr;
    a->size = b->size;
    b->addr = tempaddr;
...
memaspace *memaSearch(struct memaheap *heap)
{
    ...
    for (i = 0; i < (ALLOCATIONS_LENGTH - 1); i++) {
        while (curr <= &heap->spaces[ALLOCATIONS_LENGTH - 2]) {
            ...
            if ((mema_addr_t)curr[1].addr < (mema_addr_t)curr[0].addr) {
...
void _memaFree(mema_addr_t addr, s32 size)
{
...
void memaReset(void *heapaddr, u32 heapsize)
{
   struct memaspace *space;
	...
	g_MemoryAllocations.end1.addr = (mema_addr_t)-1;
	g_MemoryAllocations.end1.size = 0;
	g_MemoryAllocations.end2.addr = (mema_addr_t)-1;
	...
	g_MemoryAllocations.spaces[0].addr = g_MemaHeapStart = (mema_addr_t) heapaddr;
...
mema_addr_t memaGrow(mema_addr_t addr, u32 amount)
{
    memaspace *curr = &g_MemoryAllocations.spaces[2];

    while (curr->addr != (mema_addr_t)-1)
    {    
...
{
	_memaFree((mema_addr_t) addr, size);
}
...
void memaDump(void)
{
    ...
    for (curr = &g_MemoryAllocations.spaces[0], tot = 0; curr->addr != (mema_addr_t)-1; curr++)
    {
        tot += curr->size;
    }
    ...
    {
        for (curr = &g_MemoryAllocations.spaces[0]; curr->addr != (mema_addr_t)-1; curr++)
        {
        ...
        for (curr = &g_MemoryAllocations.spaces[0],acc=0; curr->addr != (mema_addr_t)-1; curr++)
        {
            if (curr->size == lim)
            {
                if (count < 200)
                {
                    pos += ge007_sprintf(pos, "%d ", ((curr->size + 512) / 1024));
                }
                else if (count == 200)
                {
                    pos += ge007_sprintf(pos, "...");
                }
...
void mema7000A2F8(void (*func)(mema_addr_t, memaspace*)) {
    memaspace *curr = &g_MemoryAllocations.spaces[0];
    while (curr->addr != (mema_addr_t)-1) {
