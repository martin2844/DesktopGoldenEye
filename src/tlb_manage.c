...
void tlbmanageEstablishManagementTable(void)
{
    ...
    g_tlbmanageTlbAllocatedBlock = (u8 (*)[TLB_BLOCK_SIZE])(((uintptr_t)sp_boot & ~((uintptr_t)PAGE_SIZE - 1)) - (MAPPING_TABLE_COUNT * PAGE_SIZE));
    g_tlbmanageMappingTableEnd = (uintptr_t)&g_tlbManagementTable + (uintptr_t)(s32)0xFFC08000;
}
...
#ifdef NATIVE_PORT
    /* On PC, segment symbols are u32 variables storing ROM offsets.
     * Use ROM_SPAN/ROM_OFFSET pattern (*(u32*)&) to read the value. */
    romCopy(tlbEntryPointer, (void *)(uintptr_t)(*(u32 *)&_gameSegmentRomStart + (u32)maskedAddress), TLB_BLOCK_SIZE);
#else
