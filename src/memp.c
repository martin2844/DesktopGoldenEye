...
#ifdef NATIVE_PORT
void mempCheckMemflagTokens(intptr_t poolAreaStart, intptr_t poolAreaSize)
#else
 ...
 */
// ido_recomp: saves bytes to $s0 changing frame from 0x18 to 0x20.
// Original keeps bytes in $a2 without saving, relying on jal paths
// leading to infinite loops. Also regalloc and branch condition inversions.
#ifdef NONMATCHING
void *mempAllocBytesInBank(u32 bytes, u8 heap)
{
#ifdef NATIVE_PORT
    intptr_t pos;
    bytes = (bytes + 15) & ~(u32)15;
#else
    s32 pos;
#endif

    for (;;) {
        pos = g_mempPools[heap].pos;

        if (pos == 0) {
            needmemallocation = 1;
#ifdef NATIVE_PORT
            fprintf(stderr, "[MEMP] mempAllocBytesInBank: pool %d pos==0, alloc %u bytes failed\n", heap, bytes);
            return NULL;
#else
            for (;;) {}
#endif
...
#ifdef NATIVE_PORT
        if ((uintptr_t)g_mempPools[heap].end < (uintptr_t)pos) {
#else
        if ((u32)g_mempPools[heap].end < (u32)pos) {
#endif
            nulled_mempLoopAllMemBanks();
#ifdef NATIVE_PORT
            fprintf(stderr, "[MEMP] mempAllocBytesInBank: pool %d end < pos, alloc %u bytes failed\n", heap, bytes);
            return NULL;
#else
            for (;;) {}
#endif
...
#ifdef NATIVE_PORT
        if ((uintptr_t)g_mempPools[heap].end < (uintptr_t)(pos + bytes)) {
#else
        if ((u32)g_mempPools[heap].end < (u32)(pos + bytes)) {
#endif
            heap = MEMPOOL_PERMANENT;
#ifdef NATIVE_PORT
            if ((uintptr_t)g_mempPools[MEMPOOL_PERMANENT].end < (uintptr_t)(g_mempPools[MEMPOOL_PERMANENT].pos + bytes)) {
#else
            if ((u32)g_mempPools[MEMPOOL_PERMANENT].end < (u32)(g_mempPools[MEMPOOL_PERMANENT].pos + bytes)) {
#endif
                nulled_mempLoopAllMemBanks();
#ifdef NATIVE_PORT
                fprintf(stderr, "[MEMP] mempAllocBytesInBank: permanent pool overflow, alloc %u bytes failed\n", bytes);
                return NULL;
#else
                for (;;) {}
#endif
        ...
        g_mempPools[heap].pos = pos + bytes;
        g_mempPools[heap].prevpos = pos;
        return (void *)pos;
    }
}
#else
...
#ifdef NATIVE_PORT
    /* Free native model allocations when resetting the stage pool.
     * On N64, model data lives inside the pool and is implicitly freed.
     * The PC port allocates native structs with calloc (64-bit layout differs
     * from N64 binary), so they must be explicitly freed here. */
    if (bank == MEMPOOL_STAGE) {
        extern void modelConvertFreeAll(void);
        modelConvertFreeAll();
    }
#endif
...
void mempNullNextEntryInBank(u8 bank) {
    ...
    if (g_mempPools[bank].pos != 0) {
#ifdef NATIVE_PORT
        /* Same cleanup as mempResetBank — this path is taken during
         * the game loop's level transition (boss.c main loop). */
        if (bank == MEMPOOL_STAGE) {
            extern void modelConvertFreeAll(void);
            modelConvertFreeAll();
        }
