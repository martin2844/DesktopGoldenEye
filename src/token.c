...
#ifdef TARGET_N64
char g_TokenString[G_TOKEN_STRING_LEN * (s32)sizeof(u32)];
#else
char g_TokenString[G_TOKEN_STRING_LEN * (s32)sizeof(u32)] __attribute__((aligned(4)));
#endif
s32 g_TokenCount = 1;
...
s32 tokenReadIo(void)
{
    ...
    if (rmonGetToken()) {
        ...
        for (ptr = (u32 *)g_TokenString,
             end = (u32 *)(g_TokenString + sizeof(g_TokenString));
             ptr != end;
             ptr++) {
