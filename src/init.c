...
#if defined(GE007_N64_ORACLE) && !defined(NATIVE_PORT)
#define GE007_N64_ORACLE_ENABLED 1
#endif
...
#ifdef GE007_N64_ORACLE_ENABLED
static void n64OracleEmitBootMarker(void)
{
    osSyncPrintf("[GE007_ORACLE] event=boot_mainproc\n");
}
#endif
...
void init(void)
{
...
#ifdef NATIVE_PORT
    inflate_code_size = ROM_SPAN(_codeSegmentRomStart, _inflateSegmentRomStart);
#else
    ...
    {
#ifdef NATIVE_PORT
        osPiRawStartDma(OS_READ, (u32)(uintptr_t)ROM_OFFSET(_alt_startSegmentRomStart), ROM_OFFSET(_alt_startSegmentStart), inflate_code_size - MAXCODESIZE);
#else
...
void mainproc(void *args)
{
    ...
    if (tokenReadIo())
    {
        osStopThread(&rmonThread);
    }

    osSetThreadPri(&rmonThread, MAIN_THREAD_PRIORITY);
