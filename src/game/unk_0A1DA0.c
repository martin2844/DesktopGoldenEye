...
#ifdef NATIVE_PORT
static s8 portStoreWrappedByteFromFloat(f32 value)
{
    s32 clamped = (s32)value;
    u8 raw;
    s8 out;
...
#ifdef NONMATCHING
void sub_GAME_7F0A1DA0(f32 *arg0, f32 *arg1, f32 *arg2, f32 *arg3, f32 arg4, f32 arg5, f32 arg6, f32 arg7, f32 arg8, f32 arg9) {
    f32 norm1x, norm1y, norm1z;
    f32 norm2x, norm2y, norm2z;
    f32 len1, len2;
    f32 invLen;
    f32 scaledArg4, scaledArg5, scaledArg6, scaledArg7;
    f32 theta;
    f32 range1, range2;
    s32 halfBuf;
    f32 sqrtResult;
    ...
    norm1y = arg1[1];
    norm1z = arg1[2];
    len1 = sqrtf(norm1x * norm1x + norm1y * norm1y + norm1z * norm1z);

    invLen = 1.0f / len1;
    scaledArg4 = arg4 * len1;
    scaledArg5 = arg5 * len1;
    norm1x *= invLen;
    norm1y *= invLen;
    norm1z *= invLen;
    ...
    norm2y = arg2[1];
    norm2z = arg2[2];
    len2 = sqrtf(norm2x * norm2x + norm2y * norm2y + norm2z * norm2z);

    invLen = 1.0f / len2;
    scaledArg6 = arg6 * len2;
    scaledArg7 = arg7 * len2;
    norm2x *= invLen;
    norm2y *= invLen;
    norm2z *= invLen;

    theta = atan2f(arg3[0], arg3[2]);

    range1 = scaledArg5 - scaledArg4;
    range2 = scaledArg7 - scaledArg6;

    halfBuf = SHATTERED_WINDOW_PIECES_BUFFER_LEN / 2;
    sqrtResult = sqrtf((range1 * range2) / (f32)halfBuf);

    gridTotal = (s32)sqrtResult;
    half = gridTotal / 2;
    halfF = (f32)half;

    center1 = scaledArg4 + halfF;
    center2 = scaledArg6 + halfF;

    centerX = arg0[0] + center1 * norm1x + center2 * norm2x;
    centerY = arg0[1] + center1 * norm1y + center2 * norm2y;
    centerZ = arg0[2] + center1 * norm1z + center2 * norm2z;

    snd = sndPlaySfx(g_musicSfxBufferPtr, 71, NULL);
    chrobjSndCreatePostEventDefault(snd, (coord3d *)arg0);
        ...
        for (j = 0; j < rows; j++) {
            if (cols > 0) {
                ...
                for (i = 0; i < cols; i++) {
                    ...
                    pos.x = centerX + fi * norm1x + fj * norm2x;
                    pos.y = centerY + fi * norm1y + fj * norm2y;
                    pos.z = centerZ + fi * norm1z + fj * norm2z;

                    randF = (f32)(u32)randomGetNext() * (1.0f / 4294967296.0f);
                    size = (randF * 0.7f + 0.1f) * sqrtResult;

                    sub_GAME_7F0A2160(&pos, theta, size);
                }
            }
        }
    }
...
void update_broken_windows(void) {
    ...
    if (g_ClockTimer < 15) {
        timescale = (f32)g_ClockTimer;
    } else {
        timescale = 15.0f;
    }

    for (i = 0; i < SHATTERED_WINDOW_PIECES_BUFFER_LEN; i++) {
        piece = &ptr_shattered_window_pieces[i];

        if (piece->piece <= 0) {
            continue;
        }

        frames = (s32)timescale;
        piece->piece += frames;

        piece->field_0x10 += piece->field_0x28 * timescale;
        piece->field_0x14 += piece->field_0x2c * timescale;
        piece->field_0x18 += piece->field_0x30 * timescale;
        piece->x += piece->field_0x1c * timescale;
        piece->z += piece->field_0x24 * timescale;

        for (j = 0; j < frames; j++) {
            piece->y += piece->field_0x20;
            piece->field_0x20 -= 0.1f;
        }
        ...
        if (piece->y < -30000.0f || piece->y > 30000.0f) {
            piece->piece = 0;
        }
    }
}
...
Gfx * sub_GAME_7F0A2C44(Gfx *gdl) {
    ...
    gdl->words.w0 = 0xBB001001;
    gdl->words.w1 = 0x0D800D80;
    gdl++;

    gdl->words.w0 = 0xBA001402;
    gdl->words.w1 = 0x00100000;
    gdl++;

    gdl->words.w0 = 0xBA001001;
    gdl->words.w1 = 0x00010000;
    gdl++;

    gdl->words.w0 = 0xB6000000;
    gdl->words.w1 = 0x00003000;
    gdl++;

    gdl->words.w0 = 0xBA000C02;
    gdl->words.w1 = 0x00002000;
    gdl++;

    gdl->words.w0 = 0xB7000000;
    gdl->words.w1 = 0x00060000;
    gdl++;

    gdl->words.w0 = 0x01030040 | (bondviewGetField10E0GbiFlags() << 16);
    ...
    for (i = 0; i < SHATTERED_WINDOW_PIECES_BUFFER_LEN; i++) {
        ...
        if (piece->piece > 0) {
            ...
            gdl->words.w0 = 0xBF000000;
            gdl->words.w1 = 0x00000A14;
            gdl++;
        }
        piece_offset += 0x68;
    }
...
void sub_GAME_7F0A2F30(struct damage_display_parent *arg0, s32 arg1, s32 arg2, f32 arg3)
{
    ...
    for (i=0; i<23; i++)
    {
        ...
        for (var_s4 = 0; var_s4 < 2; var_s4++)
        {
            ...
            if (arg2 > 0)
            {
                arg0[i].items[var_s4].unk0C = portStoreWrappedByteFromFloat(96.0f - (cosf(temp_f24) * 96.0f));
                arg0[i].items[var_s4].unk0D = portStoreWrappedByteFromFloat(127.0f - (cosf(temp_f24) * 127.0f));
                arg0[i].items[var_s4].unk0E = 0xFF;
            }
            else if (arg2 < 0)
            {
                arg0[i].items[var_s4].unk0D = portStoreWrappedByteFromFloat(127.0f - (cosf(temp_f24) * 127.0f));
                arg0[i].items[var_s4].unk0E = portStoreWrappedByteFromFloat(32.0f - (cosf(temp_f24) * 32.0f));
            }
...
#ifdef NATIVE_PORT
/* Static DL buffers for watch menu sub-DLs that overflow their struct fields on 64-bit.
 * On N64, Gfx is 8 bytes; on 64-bit it's 16. The struct arrays are sized for N64
 * and overflow into padding fields. We replace them with properly sized buffers. */
#define WATCH_BAR_DL_MAX 48  /* health/armor bar: ~37 commands max */
Gfx g_pcArmorBarDL[WATCH_BAR_DL_MAX];
Gfx g_pcHealthBarDL[WATCH_BAR_DL_MAX];
Gfx g_pcGreenBackdropDL[12];  /* page-select rectangles: 5*2+1 = 11 commands */
Gfx g_pcStaticDL[4];          /* static bar: 1*2+1 = 3 commands */
#endif

#ifdef NONMATCHING
// Builds a display list for loading texture tiles. For each tile row, emits
// a G_LOADBLOCK command and conditionally a G_RDPPIPESYNC (every 2 iterations
// when index < 9, every 4 when >= 9). Appends G_ENDDL at the end.
// Matches with original IDO. ido_recomp: branch scheduling differs.
Gfx *sub_GAME_7F0A3330(Gfx *gdl, void *arg1, s32 arg2) {
    s8 i;
    s32 half;
#ifdef NATIVE_PORT
    uintptr_t addr;
...
#ifdef NATIVE_PORT
    gdl->words.w0 = 0xB8000000;
    gdl->words.w1 = 0x00000000;
    gdl++;
    return gdl;
#else
    cmd = (u32 *)gdl;
...
void sub_GAME_7F0A33F8(struct WatchVertex *arg0, s32 arg1, f32 arg2, s32 arg3) {
    ...
    if (end >= 7) {
        ...
        for (i = 7; i <= end; i += 2) {
            angle = (f32)i * 3.1415927f / total_f;

            sin_result = (s16)(s32)(sinf(angle) * 520.0f * arg2);
            cos_result = (s16)(s32)(cosf(angle) * 520.0f * arg2);

            ptr->coord1.x = sin_result + 1;
            ptr->coord1.y = 0;
            ptr->coord1.z = -cos_result;
            ptr->coord2.x = 0;
            ptr->coord2.y = 0;
            ptr->coord2.z = 0;
            ptr->color.r = (u8)(u32)(0.0f - cosf(angle) * 0.0f);
            ptr->color.g = (u8)(u32)(44.0f - cosf(angle) * 20.0f);
            ptr->color.b = (u8)(u32)(0.0f - cosf(angle) * 0.0f);
            ptr->color.a = 176;
            ptr++;

            if (i != 0 && i < arg1) {
                ptr->coord1.x = 1 - sin_result;
                ptr->coord1.y = 0;
                ptr->coord1.z = -cos_result;
                ptr->coord2.x = 0;
                ptr->coord2.y = 0;
...
#ifdef NONMATCHING
// Builds RSP/RDP display list commands for texture tile loading.
// When arg3 != 0 (tile mode): emits SP DMA + tile setup for 8 tiles with descending offsets.
// When arg3 == 0 (simple mode): emits 8 SP DMA + tile commands with ascending source addresses.
// Both paths end with RDP Full Sync.
...
Gfx *sub_GAME_7F0A3978(Gfx *gdl, void *arg1, s32 unused_arg2, s32 arg3) {
    ...
    if (arg3 != 0) {
        uintptr_t original_arg1 = (uintptr_t)arg1;
        uintptr_t dmem_base = (uintptr_t)arg1 + 0x10;

        gdl->words.w0 = 0x04300040;
        gdl->words.w1 = dmem_base + 0xE0;
        gdl++;

        gdl->words.w0 = 0x04040010;
        gdl->words.w1 = original_arg1;
        gdl++;

        gdl->words.w0 = 0xB1000003;
        gdl->words.w1 = 66;
        gdl++;
    ...
    if (arg3 != 0) {
        original_arg1 = (u32 *)arg1;
        dmem_base = (u8 *)arg1 + 0x10;

        /* SP DMA: set end address */
        dl[0] = 0x04300040;
        dl[1] = (u32)(dmem_base + 0xE0);
        dl += 2;

        /* SP DMA read: source = original arg1 */
        dl[0] = 0x04040010;
        dl[1] = (u32)original_arg1;
        dl += 2;
        ...
        /* Load 8 tiles: offsets descend from tile 7 to tile 0 */
        i = 7;
        do {
            dl[0] = 0x04300040;
            dl[1] = (u32)(dmem_base + (i << 5));
            dl += 2;
        ...
        /* Simple mode: 8 iterations with ascending source */
        i = 0;
        do {
            dl[0] = 0x04300040;
            dl[1] = (u32)arg1;
 ...
 *
 * NONMATCHING: leaf function, ido_recomp register allocation differs.
 * Instruction ordering before loop and register choices do not match.
 */
...
#ifdef NONMATCHING
// Initializes a bullet spark/dust cloud entry. Sets effect type properties
// (frame count, scale, image pointer), copies RGBA color from D_80040960[type],
// copies position from pos, and computes random velocity direction and magnitude.
// type selects: 4=flare, 1=smoke, 3=scattered_explosion, 6=lens_flare, default=smoke.
// ido_recomp: register allocation differences.
...
void sub_GAME_7F0A3C08(void *entry_ptr, coord3d *pos, s32 type, f32 scale, s16 extra) {
    ...
    angle = (f32)(u32)randomGetNext() * (1.0f / 4294967296.0f) * 6.2831855f;
    ...
    {
        /* Pack RGBA into unk28 (was accessed as raw bytes at N64 offset 0x28) */
        u8 *rgba = (u8 *)&entry->unk28;
        rgba[0] = D_80040960[type].r;
        rgba[1] = D_80040960[type].g;
        rgba[2] = D_80040960[type].b;
        rgba[3] = D_80040960[type].a;
    }
    ...
    rand01 = (f32)(u32)randomGetNext() * (1.0f / 4294967296.0f);
    factor = 1.0f + rand01 * 0.25f;
    scale = scale * factor * 1.41412f;
    entry->unk24 = scale;

    entry->unk1c = cosf(angle) * scale;
    entry->unk20 = sinf(angle) * scale;
}
#else
...
#ifdef NONMATCHING
// Searches dword_CODE_bss_8007A170 array for a free slot (unk04 == 0),
// initializes it via sub_GAME_7F0A3C08, returns the slot or NULL if full.
...
void *sub_GAME_7F0A3E1C(coord3d *arg0, s32 arg1, f32 arg2, s16 arg3) {
    bondstruct_unk_8007A170 *ptr = &dword_CODE_bss_8007A170[0];
...
#else
extern void texSelect(Gfx **gdlptr, struct sImageTableEntry *tconfig, u32 arg2, s32 arg3, u32 ulst);
extern s32 camIsPosInScreen(coord3d *vec_scale, f32 norm_scale);
extern Mtxf *currentPlayerGetMatrix10D4(void);
extern void *dynAllocate7F0BD6C4(s32 count);
extern struct coord3d *getRoomPositionByIndex(s32 index);
...
void sub_GAME_7F0A3F04(bondstruct_unk_8007A170 *arg0, Gfx **gdl_ptr, s32 arg2) {
    ...
    f32 scaleA;
    f32 scaleB;
    coord3d mrow0_scaleA;
    coord3d mrow0_scaleB;
    coord3d mrow1_scaleA;
    coord3d mrow1_scaleB;
    u8 *frameData;
    Gfx *gfxTemp;
    ...
    vtxtemplate.v.cn[0] = *((u8 *)&arg0->unk28 + 0);
    vtxtemplate.v.cn[1] = *((u8 *)&arg0->unk28 + 1);
    vtxtemplate.v.cn[2] = *((u8 *)&arg0->unk28 + 2);
    vtxtemplate.v.cn[3] = *((u8 *)&arg0->unk28 + 3);

    scaleA = *(f32 *)&arg0->unk1c;
    scaleB = *(f32 *)&arg0->unk20;

    frameIndex = (s32)((f32)arg0->unk00 * *(f32 *)&arg0->unk08);

    posX = *(f32 *)&arg0->unk10;
    posY = *(f32 *)&arg0->unk14;
    posZ = *(f32 *)&arg0->unk18;

    mrow0_scaleA.x = mtx->m[0][0] * scaleA;
    mrow0_scaleA.y = mtx->m[0][1] * scaleA;
    mrow0_scaleA.z = mtx->m[0][2] * scaleA;

    mrow0_scaleB.x = mtx->m[0][0] * scaleB;
    mrow0_scaleB.y = mtx->m[0][1] * scaleB;
    mrow0_scaleB.z = mtx->m[0][2] * scaleB;

    mrow1_scaleA.x = mtx->m[1][0] * scaleA;
    mrow1_scaleA.y = mtx->m[1][1] * scaleA;
    mrow1_scaleA.z = mtx->m[1][2] * scaleA;

    mrow1_scaleB.x = mtx->m[1][0] * scaleB;
    mrow1_scaleB.y = mtx->m[1][1] * scaleB;
    mrow1_scaleB.z = mtx->m[1][2] * scaleB;
    ...
    vertices[0].v.ob[2] = (posZ - mrow0_scaleA.z - mrow1_scaleB.z) * get_room_data_float1() - roomPos->z;

    byteOffset = frameIndex * 12;

    vertices[0].v.tc[0] = frameData[byteOffset + 4] << 5;
    vertices[0].v.tc[1] = 0;

    vertices[1].v.ob[0] = (posX + mrow0_scaleB.x - mrow1_scaleA.x) * get_room_data_float1() - roomPos->x;
    vertices[1].v.ob[1] = (posY + mrow0_scaleB.y - mrow1_scaleA.y) * get_room_data_float1() - roomPos->y;
    vertices[1].v.ob[2] = (posZ + mrow0_scaleB.z - mrow1_scaleA.z) * get_room_data_float1() - roomPos->z;
    vertices[1].v.tc[0] = 0;
    vertices[1].v.tc[1] = 0;

    vertices[2].v.ob[0] = (posX + mrow0_scaleA.x + mrow1_scaleB.x) * get_room_data_float1() - roomPos->x;
    vertices[2].v.ob[1] = (posY + mrow0_scaleA.y + mrow1_scaleB.y) * get_room_data_float1() - roomPos->y;
    vertices[2].v.ob[2] = (posZ + mrow0_scaleA.z + mrow1_scaleB.z) * get_room_data_float1() - roomPos->z;
    vertices[2].v.tc[0] = 0;
    vertices[2].v.tc[1] = frameData[byteOffset + 5] << 5;

    vertices[3].v.ob[0] = (posX - mrow0_scaleB.x + mrow1_scaleA.x) * get_room_data_float1() - roomPos->x;
    vertices[3].v.ob[1] = (posY - mrow0_scaleB.y + mrow1_scaleA.y) * get_room_data_float1() - roomPos->y;
    vertices[3].v.ob[2] = (posZ - mrow0_scaleB.z + mrow1_scaleA.z) * get_room_data_float1() - roomPos->z;

    vertices[3].v.tc[0] = frameData[byteOffset + 4] << 5;
    vertices[3].v.tc[1] = frameData[byteOffset + 5] << 5;

    /* gSPClearGeometryMode(gdl++, 0x2000) */
    gdl->words.w0 = 0xB7000000;
    gdl->words.w1 = 0x00002000;
    gdl++;

    /* gSPMatrix(gdl++, ..., G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH) */
    gdl->words.w0 = 0x01030040 | (bondviewGetField10E0GbiFlags() << 16);
    gfxTemp = gdl;
    gdl++;
    gfxTemp->words.w1 = osVirtualToPhysical((void *)get_BONDdata_field_10E0());

    gdl = applyRoomMatrixToDisplayList(gdl, roomIndex);

    texSelect(&gdl, (struct sImageTableEntry *)(frameData + byteOffset), 4, arg2, 2);

    /* gSPVertex(gdl++, vertices, 4, 0) */
    gdl->words.w0 = 0x04300040;
    gdl->words.w1 = osVirtualToPhysical(vertices);
    gdl++;

    /* gSP2Triangles / custom triangle command */
    {
        Gfx *_g = (Gfx *)(gdl++);
        _g->words.w0 = 0xB1000032;
        _g->words.w1 = 0x00002010;
    }

    /* gSPMatrix(gdl++, projectionMatrix, G_MTX_PROJECTION | G_MTX_LOAD | G_MTX_NOPUSH) */
    gdl->words.w0 = 0x01030040;
    gdl->words.w1 = osVirtualToPhysical((void *)currentPlayerGetProjectionMatrix());
    gdl++;
...
void sub_GAME_7F0A45D8(void) {
    // NONMATCHING: 2 of 10 instructions differ due to ido_recomp scheduling.
    // addiu $v1 and addiu $v0 (instructions 3-4) are swapped in order.
    // Original: lui v0, lui v1, addiu v1, addiu v0 (reverse completion order)
    // ido_recomp: lui v0, lui v1, addiu v0, addiu v1 (same completion order)
    // All other 8 instructions match byte-identically.
    u8 *ptr = dword_CODE_bss_8007A4E0;

    do {
        *(s16 *)(ptr + 4) = 0;
        ptr += 0x3C;
    } while (ptr < dword_CODE_bss_8007B098);
}
#else
#ifndef VERSION_EU
GLOBAL_ASM(
...
#ifdef NONMATCHING
// Searches dword_CODE_bss_8007A4E0 array (stride 0x3c) for a free slot (s16 at +4 == 0),
// initializes it via sub_GAME_7F0A3C08, copies position and scale, returns slot or NULL.
// Matches with original IDO. ido_recomp: reverse global address completion order.
void *sub_GAME_7F0A4600(void *arg0, coord3d *pos, s32 arg2, f32 arg3, f32 arg4, s16 arg5) {
    ...
    do {
        if (*(s16 *)((u8 *)ptr + 4) == 0) {
            ...
            *(f32 *)((u8 *)ptr + 0x2C) = pos->x;
            *(f32 *)((u8 *)ptr + 0x30) = pos->y;
            *(f32 *)((u8 *)ptr + 0x34) = pos->z;
            *(f32 *)((u8 *)ptr + 0x38) = arg4;
            return ptr;
        }
...
#ifdef NONMATCHING
// Updates bullet spark/dust cloud entries in dword_CODE_bss_8007A4E0 array.
// Each entry (stride 0x3c): s32 timer_accum at +0, s16 timer_limit at +4,
// f32 position[3] at +0x10, f32 velocity[3] at +0x1c, f32 target[3] at +0x2c, f32 y_limit at +0x38.
// Advances timer, interpolates position toward target, clears entry when expired or below y_limit.
// ido_recomp: reverse global address completion order for dword_CODE_bss_8007A4E0/dword_CODE_bss_8007B098.
extern void sub_GAME_7F057D88(f32 *arg0, f32 *arg1, f32 arg2);
void sub_GAME_7F0A46A0(void) {
    ...
    end = dword_CODE_bss_8007B098;

    do {
        if (*(s16 *)(ptr + 4) > 0) {
            sum = *(s32 *)ptr + g_ClockTimer;
            *(s32 *)ptr = sum;
            if (sum >= 0) {
                if (sum >= *(s16 *)(ptr + 4)) {
                    *(s16 *)(ptr + 4) = 0;
                } else {
                    sub_GAME_7F057D88((f32 *)(ptr + 0x10), (f32 *)(ptr + 0x2c), g_GlobalTimerDelta);
                    if (*(f32 *)(ptr + 0x14) < *(f32 *)(ptr + 0x38)) {
                        *(s16 *)(ptr + 4) = 0;
                    }
                }
            }
...
#ifdef NONMATCHING
// Iterates all entries in dword_CODE_bss_8007A4E0 array (stride 0x3c),
