...
#ifdef NATIVE_PORT
extern void fileLoad(struct ModelFileHeader *header, char *name);
#endif

// Copied from another file. Might need code reorganization to prevent the use of extra externs.
extern ALSoundState *dword_CODE_bss_80075DB0;
...
void init_ejected_cartridges(void) {
    ...
    while (ejected_cartridge[i].header != 0) {
        ...
        if (ejected_cartridge[i].header->RootNode == NULL) {
            printf("[EJECT] WARNING: model '%s' has NULL RootNode\n",
                   ejected_cartridge[i].text);
        }
        i++;
    }
