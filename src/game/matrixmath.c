...
#ifdef NATIVE_PORT
/* On PC, always use standard 65536.0 scale so mtx_n64_to_float can
 * convert back correctly (it divides by 65536.0).  The N64-specific
 * custom scale in D_80032310[0] was a precision optimisation for the
 * RSP's 16.16 fixed-point hardware that doesn't apply to PC floats. */
#    define FTOFIX32(x) (long)((x)*65536.0f)
#else
...
void matrix_4x4_f32_to_s32(f32 mf[4][4], s32 ms[4][4])
{
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 2; j++)
        {
            ...
            *(af++) = (((u32)e1 << 16) & 0xffff0000) | (e2 & 0xffff);
        }
}
...
void sub_GAME_7F058E78(Mtxf *arg0, Mtxf *arg1) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 2; j++) {
            e1 = (*ai & 0xFFFF0000) | (((unsigned int)*af >> 16) & 0xFFFF);
            e2 = ((unsigned int)(*ai++) << 16) | (*af++ & 0xFFFF);
#ifdef NATIVE_PORT
            arg1->m[i][j * 2] = (f32)(s32)e1 / 65536.0f;
            arg1->m[i][j * 2 + 1] = (f32)(s32)e2 / 65536.0f;
#else
            arg1->m[i][j * 2] = (f32)(s32)e1 / D_80032310[0];
            arg1->m[i][j * 2 + 1] = (f32)(s32)e2 / D_80032310[j];
#endif
        }
    }
}
...
#ifdef NONMATCHING
// Converts a 4x4 fixed-point matrix from 16.16 format into the N64 RSP split format
// where integer and fractional parts are stored in separate halves.
// Source is 16 s32 values in 16.16 fixed-point. Output is 16 u32 values where
// pairs (src[i], src[i+8]) are packed into dst[i*2] (int parts) and dst[i*2+1] (frac parts).
// Assembly does this as 2 outer iterations of 4 unrolled pairs each.
// ido_recomp: loop unrolling differences.
void sub_GAME_7F059334(s32 *src, s32 *dst) {
    s32 i;
    s32 int_val;
    s32 frac_val;
    ...
    for (i = 0; i < 8; i++) {
        int_val = src[i];
        frac_val = src[i + 8];
        dst[i * 2 + 0] = (int_val & 0xFFFF0000) | ((u32)frac_val >> 16);
        dst[i * 2 + 1] = ((u32)int_val << 16) | (frac_val & 0xFFFF);
    }
}
#else
GLOBAL_ASM(
