#include <ultra64.h>
#ifdef NATIVE_PORT
#include "byteswap.h"  /* read_be16 (portable ntohs replacement; also
                         * avoids an unaligned/strict-aliasing pointer pun) */
#endif



#ifdef NONMATCHING
// RLE decoder: 8-bit indexed color. Each entry is 2 bytes: [count, value].
// Writes 1 byte per pixel. Header: u16 width, u16 height, data starts at +0xa.
...
void sub_GAME_7F01B0E0(u8 *header, u8 *output) {
     ...
     * read_be16() returns a signed value, so cast to u16 first (matches the
     * (u16)read_be16(...) idiom used elsewhere, e.g. front.c) before widening
     * to s32 -- this reproduces ntohs()'s unsigned-16 semantics exactly. */
    remaining = (s32)(u16)read_be16(header) * (s32)(u16)read_be16(header + 2);
#else
...
#ifdef NONMATCHING
// RLE decoder: 16-bit RGBA5551. Each entry is 4 bytes: [count, c1, c2, c3].
// Packs to 16-bit: (c3 >> 3) << 11 | (c2 >> 3) << 6 | (c1 >> 3) << 1 | 1
...
#ifdef NONMATCHING
// RLE decoder: 32-bit RGBA. Each entry is 4 bytes: [count, c1, c2, c3].
// Writes c3, c2, c1, 0xFF per pixel (4 bytes). Header: u16 width, u16 height, data at +0xa.
// Matches with original IDO. ido_recomp: register allocation differs (uses $s0
