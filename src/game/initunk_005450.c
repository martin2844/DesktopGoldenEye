...
{
    int i;
    for (i = 0; i < RENDER_SLOT_COUNT; i++) {
        pc_render_slot_pool[i].model = NULL;
        pc_render_slot_pool[i].node = NULL;
        pc_render_slot_pool[i].sort_z = 0.0f;
        pc_render_slot_pool[i].next = (i + 1 < RENDER_SLOT_COUNT) ? &pc_render_slot_pool[i + 1] : NULL;
        pc_render_slot_pool[i].prev = (i > 0) ? &pc_render_slot_pool[i - 1] : NULL;
    }
