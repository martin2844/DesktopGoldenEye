...
// bss
//CODE.bss:80069550
struct s_display_list_something *barrelDisplayListPtr;
//CODE.bss:80069554
...
//CODE.bss:80069590
uintptr_t virtualaddress;  /* Must be pointer-sized on 64-bit PC (s32 truncates heap addresses) */
...
{
#ifdef NATIVE_PORT
    static int skip_intro_backdrop = -1;
    if (skip_intro_backdrop < 0) {
        skip_intro_backdrop = (getenv("GE007_SKIP_INTRO_BACKDROP") != NULL) ? 1 : 0;
    }
    if (skip_intro_backdrop) {
        return gdl;
    }
#endif
...
Gfx *insert_sniper_sight_eye_intro(Gfx *gdl)
{
    ...
    s32 tex_x = floorFloat((viGetX() * g_TitleX) / 1280.0f);

#ifdef NATIVE_PORT
    static int skip_intro_sniper = -1;
    {
        static int trace_eye_title = -1;
        if (skip_intro_sniper < 0) {
            skip_intro_sniper = (getenv("GE007_SKIP_INTRO_SNIPER") != NULL) ? 1 : 0;
        }
        if (trace_eye_title < 0) {
            trace_eye_title = (getenv("GE007_TRACE_EYE_TITLE") != NULL) ? 1 : 0;
        }
        if (skip_intro_sniper) {
            return gdl;
        }
        if (trace_eye_title) {
            fprintf(stderr,
                    "[EYE-TITLE] mode=%u viX=%d titleX=%.3f tex_x=%d counter=%d\n",
                    (unsigned)gunbarrel_mode, viGetX(), g_TitleX, tex_x, intro_eye_counter);
            fflush(stderr);
        }
    }
#endif
    ...
    return sub_GAME_7F007CC8(gdl, tex_x, &sp3C, &sp30);
}
...
#ifdef NONMATCHING
Gfx *sub_GAME_7F007F30(Gfx *gdl, s32 arg1, Mtxf *arg2) {
    ...
    #define ANIM_START_FRAME 137
    #define ANIM_SPEED_FRAME 212
    #define SFX_TRIGGER_FRAME 230
#else
    #define ANIM_START_FRAME 114
    #define ANIM_SPEED_FRAME 176
    #define SFX_TRIGGER_FRAME 191
#endif

    memset(&renderData, 0, sizeof(renderData));
    renderData.zbufferenabled = D_8002A800;
    renderData.flags = D_8002A804;
    sfxPlayed = 0;
    i = 0;

    if (arg1 > 0) {
        for (i = 0; i < arg1; i++) {
            if (dword_CODE_bss_80069594 >= 0) {
                dword_CODE_bss_80069594++;

                if (dword_CODE_bss_80069594 == ANIM_START_FRAME) {
                    modelSetAnimation(chrModelInstance,
                        ANIM_ADDR(bond_eye_fire),
                        0, 2.0f, 0.91f, 16.0f);
                }

                if (dword_CODE_bss_80069594 == ANIM_SPEED_FRAME) {
                    modelSetAnimSpeed(chrModelInstance, 1.6f, 8.0f);
                }
            }

            modelTickAnimQuarterSpeed(chrModelInstance, 1, 1);

            if (dword_CODE_bss_80069594 == SFX_TRIGGER_FRAME) {
                sndPlaySfx(g_musicSfxBufferPtr, 111, NULL);
                sfxPlayed = 1;
            }
        }
        i = 0;
    ...
    gunModel = gunModelInstance;
    node = modelGetSwitchNodeSafe(gunModel->obj, 0);
    if (node != NULL) {
        modelGetNodeRwData(gunModel, node)->Gunfire.visible = sfxPlayed;
        gunModel = gunModelInstance;
        node = modelGetSwitchNodeSafe(gunModel->obj, 2);
    } else {
        node = modelGetSwitchNodeSafe(gunModel->obj, 2);
    }

    if (node != NULL) {
        modelGetNodeRwData(gunModel, node)->Raw.unk00 = sfxPlayed;
    }
    ...
    jointlist = sub_GAME_7F06B120(jointlist, gunModelInstance);
    sub_GAME_7F06B29C(jointlist);
    sorted = sub_GAME_7F06BB28(jointlist);

    renderData.PropType = 7;
    renderData.zbufferenabled = FALSE;
    renderData.flags = 1;
    renderData.envcolour.word = ((gBloodColour.rgba[0] << 0x18) |
                                 (gBloodColour.rgba[1] << 0x10) |
                                 (gBloodColour.rgba[2] << 0x08));
    renderData.fogcolour.word = 0xFFFFFF00;
    renderData.gdl = gdl;
    drawjointlist(&renderData, sorted);
    ...
    modelSetDistanceDisabled(0);
    sub_GAME_7F06B248(sorted);

#ifndef NATIVE_PORT
    /* Convert chr model matrices from f32 to fixed-point */
    chrModel = chrModelInstance;
    {
        s32 matrix_count = modelGetRenderPosCount(chrModel);

        if (matrix_count > 0) {
            for (i = 0; i < matrix_count; i++) {
                matrix_4x4_copy((Mtxf *)((u8 *)chrModel->render_pos + (i << 6)), &tempMtx1);
                matrix_4x4_f32_to_s32((f32 (*)[4])&tempMtx1, (s32 (*)[4])((u8 *)chrModel->render_pos + (i << 6)));
            }
            i = 0;
        }
    }

    /* Convert gun model matrices from f32 to fixed-point */
    gunModel = gunModelInstance;
    {
        s32 matrix_count = modelGetRenderPosCount(gunModel);

        if (matrix_count > 0) {
            for (i = 0; i < matrix_count; i++) {
                matrix_4x4_copy((Mtxf *)((u8 *)gunModel->render_pos + (i << 6)), &tempMtx2);
                matrix_4x4_f32_to_s32((f32 (*)[4])&tempMtx2, (s32 (*)[4])((u8 *)gunModel->render_pos + (i << 6)));
            }
        }
    }
#endif
...
#undef ANIM_START_FRAME
#undef ANIM_SPEED_FRAME
#undef SFX_TRIGGER_FRAME
}
#else
Gfx *sub_GAME_7F007F30(Gfx*, s32, Mtxf *);
...
Gfx *insert_bond_eye_intro(Gfx *gdl) {
    ...
    u16 perspNorm;
#ifdef NATIVE_PORT
    static int skip_intro_bond = -1;
    if (skip_intro_bond < 0) {
        skip_intro_bond = (getenv("GE007_SKIP_INTRO_BOND") != NULL) ? 1 : 0;
    }
    if (skip_intro_bond) {
        return gdl;
    }
...
enum {
    RAREWARE_DL_MAIN_OFFSET = 0x43E8,
    RAREWARE_DL_TEXT_OFFSET = 0x44B0,
    RAREWARE_DL_GLOW_OFFSET = 0x4758,
    RAREWARE_TEX_MAIN_OFFSET = 0x4FE8,
    RAREWARE_TEX_GLOW_OFFSET = 0x5FF0
};

static void *rarewareSegmentPtr(u32 offset)
{
...
Gfx *load_display_rare_logo(Gfx *gdl, s32 arg1, s32 arg2, s32 arg3, s32 arg4) {
    ...
    gDPLoadTextureBlock(gdl++, rarewareSegmentPtr(RAREWARE_TEX_MAIN_OFFSET), G_IM_FMT_RGBA, G_IM_SIZ_16b, 32, 32, 0, (G_TX_NOMIRROR | G_TX_WRAP), (G_TX_NOMIRROR | G_TX_WRAP), 5, 5, G_TX_NOLOD, G_TX_NOLOD);
    gDPSetPrimColor(gdl++, 0, 0, arg4, arg4, arg4, 0xFF);
    gSPDisplayList(gdl++, rarewareSegmentPtr(RAREWARE_DL_MAIN_OFFSET));
    gSPDisplayList(gdl++, rarewareSegmentPtr(RAREWARE_DL_TEXT_OFFSET));
    gDPLoadTextureBlock(gdl++, rarewareSegmentPtr(RAREWARE_TEX_GLOW_OFFSET), G_IM_FMT_RGBA, G_IM_SIZ_16b, 32, 32, 0, (G_TX_NOMIRROR | G_TX_WRAP), (G_TX_NOMIRROR | G_TX_WRAP), 5, 5, G_TX_NOLOD, G_TX_NOLOD);
    gDPSetPrimColor(gdl++, 0, 0, ((arg4 * 0xF0) / 0xFF), ((arg4 * 0xD0) / 0xFF), ((arg4 * 0xF0) / 0xFF), 0xFF);
    gSPDisplayList(gdl++, rarewareSegmentPtr(RAREWARE_DL_GLOW_OFFSET));

    return gdl;
}

/* u32 values in segment_stubs.c — NOT pointers. Declaring as void* causes
 * size mismatch on 64-bit (8 vs 4 bytes), making ROM_OFFSET read garbage. */
extern u32 _rarewarelogoSegmentRomStart;
extern u32 _rarewarelogoSegmentStart;
extern u32 _rarewarelogoSegmentEnd;
#ifdef NATIVE_PORT
void sub_GAME_7F008B58(uintptr_t address, s32 size) {
#else
void sub_GAME_7F008B58(s32 address, s32 size) {
...
#ifdef NATIVE_PORT
    {
        ...
        size_t segment_size = (size_t)ROM_SPAN(_rarewarelogoSegmentStart, _rarewarelogoSegmentEnd);
        romCopy((void *)virtualaddress, ROM_OFFSET(_rarewarelogoSegmentRomStart), (u32)segment_size);
        gfx_register_n64_dl_region((void *)virtualaddress, segment_size);
    }
    if (getenv("GE007_DEBUG") != NULL) {
        fprintf(stderr,
                "[RARE-INIT] vaddr=%p size=0x%X dl0=%p text=%p dl1=%p tex0=%p tex1=%p\n",
                (void *)virtualaddress,
                size,
                rarewareSegmentPtr(RAREWARE_DL_MAIN_OFFSET),
                rarewareSegmentPtr(RAREWARE_DL_TEXT_OFFSET),
                rarewareSegmentPtr(RAREWARE_DL_GLOW_OFFSET),
                rarewareSegmentPtr(RAREWARE_TEX_MAIN_OFFSET),
                rarewareSegmentPtr(RAREWARE_TEX_GLOW_OFFSET));
        fflush(stderr);
    }
#else
...
#ifdef NONMATCHING
// https://decomp.me/scratch/TODO 97%
// NONMATCHING: ido_recomp reloads a1 from stack into t7 instead of using a1 directly.
// This frees a1 for reuse (unknown2 address), cascading into different register allocation
// throughout the function. Target uses v1 for unknown2 and lui/addiu/sw(v0) for global store.
// Compiled uses a1 for unknown2 and lui at/sw(at) for global. 38/39 instructions.
extern u32 unknown2;
extern u32 _ramromDam1SegmentRomStart;
void sub_GAME_7F01B0E0(u8 *, u8 *);
...
#ifdef NATIVE_PORT
    romCopy((void *)*addr, ROM_OFFSET(unknown2), ALIGN64_V2(ROM_SPAN(unknown2, _ramromDam1SegmentRomStart)));
#else
    romCopy((void *)*addr, (void *)&unknown2, ALIGN64_V2((u32)&_ramromDam1SegmentRomStart - (u32)&unknown2));
#endif
    sub_GAME_7F01B0E0(dword_CODE_bss_80069588, dword_CODE_bss_8006958C);
#ifdef NATIVE_PORT
    if (getenv("GE007_DUMP_EYE_BUFFER") != NULL)
    {
        FILE *fp = fopen("/tmp/ge007_eye_buffer_runtime.pgm", "wb");

        if (fp != NULL)
        {
            fwrite("P5\n440 299\n255\n", 1, 16, fp);
            fwrite(dword_CODE_bss_8006958C, 1, 440 * 299, fp);
            fclose(fp);
        }
    }
#endif
...
void initializeGunBarrelIntro(u8 *gfxBuffer, s32 bufferSize)
{
    ...
    barrelDisplayListPtr = (struct s_display_list_something *)gfxBuffer;
    bufferSize -= 0x200;
    gfxBuffer += 0x200;
    ...
    gunbarrelDlEnd = sub_GAME_7F01BFF8(gunbarrelgfxListPointer, (Vtx *)OS_K0_TO_PHYSICAL(barrelDisplayListPtr), 0x1E);
    gunbarrelDlBytes = ALIGN16_a((uintptr_t)((u8 *)gunbarrelDlEnd - (u8 *)gunbarrelgfxListPointer));
#ifdef NATIVE_PORT
    if (getenv("GE007_DEBUG") != NULL) {
        size_t gunbarrelCmdCount = ((u8 *)gunbarrelDlEnd - (u8 *)gunbarrelgfxListPointer) / sizeof(Gfx);
        size_t gunbarrelCmdLimit = gunbarrelCmdCount < 20 ? gunbarrelCmdCount : 20;
        size_t gunbarrelCmdIndex;

        fprintf(stderr,
                "[GUNBARREL-DL] start=%p end=%p bytes=0x%zX cmds=%zu next=%p\n",
                (void *)gunbarrelgfxListPointer,
                (void *)gunbarrelDlEnd,
                gunbarrelDlBytes,
                gunbarrelCmdCount,
                (void *)(gfxBuffer + gunbarrelDlBytes));
...
#endif
    bufferSize -= gunbarrelDlBytes;
    gfxBuffer += gunbarrelDlBytes;
#ifdef NATIVE_PORT
    gfx_register_extra_pc_dl(gunbarrelgfxListPointer, gunbarrelDlBytes);
#endif
    ...
    load_object_fill_header(c_item_entries[BODY_Brosnan_Tuxedo].header, (const char *)c_item_entries[BODY_Brosnan_Tuxedo].filename, gfxBuffer, bufferSize, &texturePool);
    
    temp_t9 = ((get_pc_buffer_remaining_value((const char *)c_item_entries[BODY_Brosnan_Tuxedo].filename) + 0x3F) | 0x3F) ^ 0x3F;
    bufferSize -= temp_t9;
    gfxBuffer += temp_t9;
    
    load_object_fill_header(c_item_entries[BODY_Male_Pierce_Bond_Tuxedo].header, (const char *)c_item_entries[BODY_Male_Pierce_Bond_Tuxedo].filename, gfxBuffer, bufferSize, &texturePool);
    
    temp_t9 = ((get_pc_buffer_remaining_value((const char *)c_item_entries[BODY_Male_Pierce_Bond_Tuxedo].filename) + 0x3F) | 0x3F) ^ 0x3F;
    bufferSize -= temp_t9;
    gfxBuffer += temp_t9;
...
#undef S_7F008E80_ANIM_SPEED
...
#ifdef NATIVE_PORT
    animation = ANIM_ADDR(bond_eye_walk);
#else
    ...
    load_object_fill_header(PitemZ_entries[PROP_CHRWPPK].header, (const char *)PitemZ_entries[PROP_CHRWPPK].filename, gfxBuffer, bufferSize, &texturePool);
    
    temp_t9 = ((get_pc_buffer_remaining_value((const char *)PitemZ_entries[PROP_CHRWPPK].filename) + 0x3F) | 0x3F) ^ 0x3F;
    bufferSize -= temp_t9;
    gfxBuffer += temp_t9;
    ...
    gunModelInstance->attachedto_objinst = modelGetSwitchNodeSafe(chrModelInstance->obj, 3);
...
Gfx *sub_GAME_7F009254(Gfx *gdl) {
    D_8002A7D0 = (1 - D_8002A7D0);
#ifdef NATIVE_PORT
    static int skip_intro_blood_overlay = -1;
    static int skip_intro_flash_overlay = -1;
    static int skip_intro_fade_overlay = -1;
    if (skip_intro_blood_overlay < 0) {
        skip_intro_blood_overlay =
            (getenv("GE007_SKIP_INTRO_BLOOD_OVERLAY") != NULL) ? 1 : 0;
    }
    if (skip_intro_flash_overlay < 0) {
        skip_intro_flash_overlay =
            (getenv("GE007_SKIP_INTRO_FLASH_OVERLAY") != NULL) ? 1 : 0;
    }
    if (skip_intro_fade_overlay < 0) {
        skip_intro_fade_overlay =
            (getenv("GE007_SKIP_INTRO_FADE_OVERLAY") != NULL) ? 1 : 0;
    }
#endif
