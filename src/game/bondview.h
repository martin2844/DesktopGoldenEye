...
/**
 * PLAYER_MODEL(p) — returns Model* for player p on both platforms.
 * On N64, model is embedded in the player struct (Model model), so &p->model gives Model*.
 * On PC, model is a pointer (Model *model), so p->model already IS Model*.
 */
#ifdef NATIVE_PORT
#define PLAYER_MODEL(p) ((p)->model)
#define PLAYER_WATCH_MODEL(p) ((p)->watch_model)
#else
#define PLAYER_MODEL(p) (&(p)->model)
#define PLAYER_WATCH_MODEL(p) ((Model *)&((p)->something_with_watch_object_instance))
#endif
...
struct hand
{
  ...
  */
  ...
  f32 field_B64;
#ifdef NATIVE_PORT
  /* field_B68..B6C: embedded Model struct for weapon in hand.
   * field_B70: ModelFileHeader* (weapon model header — stored as s32 on N64).
   * field_B74: Mtxf* (allocated matrices — stored as s32 on N64). */
  uintptr_t field_B68;
  uintptr_t field_B6C;
  uintptr_t field_B70;
  uintptr_t field_B74;
...
enum PlayerViewMode
{
    PLAYER_VIEWMODE_GAMEPLAY = 0,
    PLAYER_VIEWMODE_FROZEN_INTRO_CAMERA = 1,
};

struct player
{
  /* PlayerViewMode; 0 = gameplay camera, 1 = frozen intro/cinema camera.
   * Offset 0x0000. */
  /* 0x0000 */ s32 unknown;
   ...
   * Offset 0x005c (N64). Stores Mtx/Mtxf pointers from dynAllocateMatrix.
   */
#ifdef NATIVE_PORT
  intptr_t field_5C;
  intptr_t field_60;
...
#ifdef NATIVE_PORT
  Model *watch_model;
  union ModelRwData **watch_rwdata;
#else
  ...
  */
  ...
  Mtxf* field_10CC;
#ifdef NATIVE_PORT
  intptr_t field_10D0; // ptr — stores Mtxf* on N64
#else
  s32 field_10D0; // ptr
#endif
  ...
  Mtxf* projmatrixf;
#ifdef NATIVE_PORT
  intptr_t field_10E0; // ptr — stores Mtx* on N64
#else
  s32 field_10E0; // ptr
#endif
#ifdef NATIVE_PORT
  intptr_t field_10E4; // ptr — stores LookAt* on N64
#else
  s32 field_10E4; // ptr
#endif
  Mtxf* field_10E8;
  ...
  f32 field_1280;
#ifdef NATIVE_PORT
  uintptr_t players_cur_animation;
#else
  ...
  */
  ...
  f32 field_2A0C;
#ifdef NATIVE_PORT
  uintptr_t ptr_text_first_mp_award;
  uintptr_t ptr_text_second_mp_award;
#else
...
#ifdef TARGET_N64
static s32 playerHasFrozenIntroCamera(const struct player *player)
#else
static inline s32 playerHasFrozenIntroCamera(const struct player *player)
#endif
{
    return player != NULL && player->unknown == PLAYER_VIEWMODE_FROZEN_INTRO_CAMERA;
}
...
#ifdef NATIVE_PORT
void setFontTables(uintptr_t arg0, uintptr_t arg1);
#else
...
void set_BONDdata_autoaim_x(s32 enabled);
s32 get_BONDdata_autoaim_x(void);
void set_BONDdata_autoaim_y(s32 enabled);
...
void bondviewApplyLevelVisibilityScaleToModelView(Mtxf *matrix);
void bondviewRegisterLevelVisibilityScaledRenderPos(Model *model);
s32 bondviewField10E0IsFloat(void);
s32 bondviewGetField10E0GbiFlags(void);
void bondviewTransformManyPosToViewMatrix(RenderPosView *arg0, s32 arg1);
intptr_t sub_GAME_7F078474(void);
intptr_t get_BONDdata_field_10E0(void);
...
void     bondviewClearUpperTextDisplayFlag(int flag);
...
#ifdef NATIVE_PORT
void portTankAimAtCoord(f32 x, f32 y, f32 z);
#endif
s32 bondviewGetRandomSpawnPadIndex(void);
void bondviewApplyVertaTheta(void);
void bondviewSyncViewBasisFromHead(void);
void bondviewPrimeSpawnHeadState(void);
