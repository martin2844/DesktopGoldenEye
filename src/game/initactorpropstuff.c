...
u32 sub_GAME_7F0001F0(void *ani, int aniid, int end) {
    short asStack8[4];
    u16 result = 0;
...
s32 sub_GAME_7F000290(void *ani, s32 start, s32 end) {
    s32 result = 0;
    s16 localdata[4];

    while (start < end) {
        sub_GAME_7F06D2E4(0, 0, &skeleton_guard, ani, start, localdata);
        result += localdata[2];
        start++;
    }
...
#ifdef NATIVE_PORT
s32 sub_GAME_7F00032C(struct attackanimconfig *entries) {
    u8 *entry;
    s32 count;
    s32 anim_offset;
    u32 raw_val;
    ...
    do {
        /* On PC, anonymous_0 is s32 (4 bytes) — can't hold a 64-bit pointer.
         * Convert offset to pointer for the sub_GAME_7F0001F0 call,
         * but DON'T write the pointer back into the struct (would overflow).
         * Keep anonymous_0 as the raw offset; callers use ANIM_FROM_OFFSET(). */
        void *anim_ptr = (void *)((uintptr_t)anim_offset + (uintptr_t)ptr_animation_table);
        floor_val = floorFloatToInt(*(f32 *)(entry + 4));
        raw_val = sub_GAME_7F0001F0(anim_ptr, 0, floor_val);
        speed = *(f32 *)(entry + 4);
        angle_units = (u16)(raw_val & 0xffff);

        if (speed > 0.0f) {
            if (angle_units < 32768u) {
                *(f32 *)(entry + 8) = (f32)(u32)angle_units * (6.2831855f / 65536.0f) / speed;
            } else {
                *(f32 *)(entry + 8) = ((f32)(u32)angle_units * (6.2831855f / 65536.0f) - 6.2831855f) / speed;
            }
        } else {
            *(f32 *)(entry + 8) = 0.0f;
        }

        anim_offset = *(s32 *)(entry + 0x48);
        entry += 0x48;
        count++;
    } while (anim_offset != 0);
...
s32 sub_GAME_7F00032C(struct attackanimconfig *entries) {
    ...
    do {
        *(void **)entry = (void *)((u32)anim_offset + (u32)ptr_animation_table);
        floor_val = floorFloatToInt(*(f32 *)(entry + 4));
        raw_val = sub_GAME_7F0001F0(*(void **)entry, 0, floor_val);
        speed = *(f32 *)(entry + 4);
        angle_units = (u16)(raw_val & 0xffff);
...
#ifdef NONMATCHING
// Matches with original IDO (make IDO_RECOMP=NO). ido_recomp generates
// tighter delay slot filling and eliminates the redundant load after beqz.
s32 sub_GAME_7F0004D0(struct struck_animation_table *arr) {
    s32 count = 0;

    while (arr->anonymous_0 != 0) {
#ifdef NATIVE_PORT
        arr->anonymous_0 = (uintptr_t)arr->anonymous_0 + (uintptr_t)ptr_animation_table;
#else
        arr->anonymous_0 = (void *)((u32)arr->anonymous_0 + (u32)ptr_animation_table);
#endif
        count++;
        arr++;
    }
...
void somethingwith_weapon_animation_groups(void) {
    ...
    if (entry->id != -1) {
        do {
            if (entry->field_1C != NULL) {
                entry->field_20 = sub_GAME_7F0004D0(entry->field_1C);
            }
            if (entry->field_24 != NULL) {
                entry->field_28 = sub_GAME_7F0004D0(entry->field_24);
            }
            entry++;
        } while (entry->id != -1);
    }

    sub_GAME_7F0004D0(D_8002DEBC);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_rifle_firing_animation_groups);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_pistol_firing_animation_groups);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_doubles_firing_animation_groups);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_crouched_rifle_firing_animation_groups);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_crouched_pistol_firing_animation_groups);
    sub_GAME_7F00046C((struct attackanimgroup **)ptr_crouched_doubles_firing_animation_groups);
    sub_GAME_7F00032C((struct attackanimconfig *)D_80030078);
    sub_GAME_7F00032C((struct attackanimconfig *)D_80030660);

    base = (u8 *)ptr_animation_table + 0x4018;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_80030984 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x40D4;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_80030988 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x4070;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_8003098C = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x8204;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_80030990 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x777C;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_80030994 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x77D4;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_80030998 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x84C4;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_8003099C = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x8520;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_800309A0 = (f32)frames * 0.1f / (f32)count;

    base = (u8 *)ptr_animation_table + 0x77D4;
    count = *(u16 *)(base + 4);
    frames = sub_GAME_7F000290(base, 0, count - 1);
    D_800309A4 = (f32)frames * 0.1f / (f32)count;
}
#else
GLOBAL_ASM(
.late_rodata
