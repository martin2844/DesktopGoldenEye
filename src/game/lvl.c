...
static int lvlTracePhaseEnabled(void)
{
    ...
    if (enabled < 0) {
        enabled = (getenv("GE007_TRACE_FRAME_PHASES") != NULL) ? 1 : 0;
    }
    return enabled;
}

static int lvlTracePhaseAfterFrame(void)
{
    static int after_frame = -2;
    if (after_frame < -1) {
        const char *env = getenv("GE007_TRACE_FRAME_PHASES_AFTER_FRAME");
        after_frame = env ? atoi(env) : 0;
    }
    return after_frame;
}

static void lvlTracePhasePrintf(const char *phase, s32 before_tris)
{
    if (!lvlTracePhaseEnabled()) {
        return;
    }

    if (g_frame_count_diag < lvlTracePhaseAfterFrame()) {
        return;
    }

    fprintf(stderr,
            "[FRAME-PHASE] frame=%d phase=%s delta=%d total=%d\n",
            g_frame_count_diag,
            phase,
...
extern u32 sub_GAME_7F0B61DC(s32 roomID, u32 *data, s32 size);
extern s_room_info g_BgRoomInfo[];
extern s32 g_MaxNumRooms;
extern intptr_t ptr_bg_data;
extern bg_room_data *ptr_bgdata_room_fileposition_list;

/* Fly camera from platform_sdl.c */
extern float g_pcCamX, g_pcCamY, g_pcCamZ;
extern float g_pcCamYaw, g_pcCamPitch;

/* PC camera world-to-screen matrix — overrides the game's broken camera.
 * Built in the PC room renderer from g_pcCamX/Y/Z + g_pcCamYaw/Pitch. */
static Mtxf s_pcWorldToScreen;
static int s_pcWorldToScreenValid = 0;

/* PC room loading state */
static int pc_rooms_loaded = 0;
#define PC_ROOM_BUF_SIZE (256 * 1024)  /* 256KB per room */
#define PC_MAX_LOADED_ROOMS 200

/* Track allocations per room for proper cleanup on stage transitions */
static struct {
    int roomID;
    u8 *vtx_buf;
    u8 *dl_buf;
    u8 *sec_buf;
} pc_room_allocs[PC_MAX_LOADED_ROOMS];
static int pc_num_loaded_rooms = 0;
int g_portRoomRenderFallbackFrame = -1;
int g_portRoomRenderFallbackRooms = 0;
int g_portRoomRenderFallbackTotal = 0;

/**
 * Load one room's vertex and display list data on PC.
 * Each room gets its own malloc'd buffers.
 * Returns 1 on success, 0 on failure.
 */
static int pc_load_room(int roomID) {
    static int trace_pc_room_loads = -1;
    if (roomID < 0 || roomID >= g_MaxNumRooms) return 0;
    if (g_BgRoomInfo[roomID].csize_point_index_binary == 0) return 0;
    if (g_BgRoomInfo[roomID].csize_primary_DL_binary == 0) return 0;
    if (trace_pc_room_loads < 0) {
        trace_pc_room_loads = (getenv("GE007_TRACE_PC_ROOM_LOADS") != NULL);
    }
    if (pc_num_loaded_rooms >= PC_MAX_LOADED_ROOMS) {
        fprintf(stderr, "[PC-BG] WARNING: hit %d-room cap, room %d skipped\n",
                PC_MAX_LOADED_ROOMS, roomID);
        return 0;
    }

    u8 *vtx_buf = (u8 *)malloc(PC_ROOM_BUF_SIZE);
    u8 *dl_buf = (u8 *)malloc(PC_ROOM_BUF_SIZE);
    if (!vtx_buf || !dl_buf) { free(vtx_buf); free(dl_buf); return 0; }

    /* Load vertex data */
    s32 vtx_result = sub_GAME_7F0B5FAC(roomID, vtx_buf, PC_ROOM_BUF_SIZE);
    if (vtx_result < 0) {
        free(vtx_buf); free(dl_buf);
        return 0;
    }

    /* Load display list data */
    u32 dl_result = sub_GAME_7F0B609C(roomID, (int *)dl_buf, PC_ROOM_BUF_SIZE);
    if ((s32)dl_result < 0) {
        free(vtx_buf); free(dl_buf);
        return 0;
    }
    ...
    if (trace_pc_room_loads) {
        fprintf(stderr,
                ...
                (unsigned)g_BgRoomInfo[roomID].usize_point_index_binary,
                dl_buf,
    ...
    /* Track allocations for cleanup on stage transitions */
    pc_room_allocs[pc_num_loaded_rooms].roomID = roomID;
    pc_room_allocs[pc_num_loaded_rooms].vtx_buf = vtx_buf;
    pc_room_allocs[pc_num_loaded_rooms].dl_buf = dl_buf;
    pc_room_allocs[pc_num_loaded_rooms].sec_buf = sec_buf;
    pc_num_loaded_rooms++;
    return 1;
}

/**
 * Reset PC room loader state for stage transitions.
 * Without this, pc_rooms_loaded stays 1 and pc_num_loaded_rooms
 * accumulates across stages, eventually hitting the 200-room cap.
 */
static void pc_room_loader_reset(void) {
    extern int  ge007_shoot_out_lights_enabled(void);
    extern void clear_light_fixturetable_in_room(s32 room_index);
    /* Free all room buffers allocated by pc_load_room().
     * The GFX translator's N64 DL region table is also stale at this point
     * since a new stage is being loaded, but it resets itself on new DL runs. */
    for (int i = 0; i < pc_num_loaded_rooms; i++) {
        /* Shoot-out-the-lights: clear each room's light_fixture_table entries
         * before its DL buffer is freed (no table entry may outlive its DL
         * buffer). Gated => byte-identical when the flag is off. See W4.E1.T1. */
        if (ge007_shoot_out_lights_enabled()) {
            clear_light_fixturetable_in_room(pc_room_allocs[i].roomID);
        }
        free(pc_room_allocs[i].vtx_buf);
        free(pc_room_allocs[i].dl_buf);
        free(pc_room_allocs[i].sec_buf);  /* NULL-safe */
        pc_room_allocs[i].vtx_buf = NULL;
        pc_room_allocs[i].dl_buf = NULL;
        pc_room_allocs[i].sec_buf = NULL;
    }
    pc_num_loaded_rooms = 0;
    pc_rooms_loaded = 0;

    /* W1.E1: the render-side room-normal cache is keyed on the pool pointers
     * freed above; drop it so a recycled malloc address on the next stage
     * cannot serve stale normals (cache-invalidation notification only — no
     * render-state readback, so R1-clean; same direction as the DL region
     * table note above). */
    {
        extern void gfx_room_normals_reset(void);
        gfx_room_normals_reset();
    }
}
...
#ifdef NATIVE_PORT
/* Per-player fractional look-pixel carry (audit §3.1): the gamepad look term is
 * a small float truncated to int mdx/mdy every frame; without a carry the
 * crosshair stays dead until ~59% stick deflection in aim mode. Indexed by
...
#ifdef NATIVE_PORT
    size = ROM_SPAN(_fontdlSegmentRomStart, _fontdlSegmentRomEnd);
#else
...
#ifdef NATIVE_PORT
    romCopy(ptr_font_DL, ROM_OFFSET(_fontdlSegmentRomStart), size);
#else
...
#ifdef NATIVE_PORT
    pc_room_loader_reset();
    /* Clear stale N64 DL region registrations — room buffers were just freed.
     * Model files will re-register during setup load. */
    gfx_clear_n64_dl_regions();
    /* Signal to bossMainloop that DLs from this frame are stale.
     * Any pending rspGfxTaskStart should be skipped. */
    extern int g_pc_stage_reload_pending;
    g_pc_stage_reload_pending = 1;
    /* §3.1: reset gamepad look fractional carry on stage load / mode change. */
...
#ifdef NATIVE_PORT
    /* Initialize font table copies that bottom-left HUD text uses.
     * bondviewResetIntroCameraMessageDialogs normally does this during camera transitions,
     * but our PC init path skips those transitions. */
...
Gfx* lvlRender(Gfx* DL)
{
    ...
    {
    /* Load room DL data on first non-title frame (needed by both rendering paths) */
    {
        static int pre_room_log = 0;
        if (getenv("GE007_VERBOSE") && pre_room_log < 2) {
            extern s32 g_FogSkyIsEnabled;
            printf("[PRE_ROOM] stage=%d title=%d maxrooms=%d loaded=%d fogEnabled=%d\n",
                   g_CurrentStageToLoad, LEVELID_TITLE, g_MaxNumRooms, pc_rooms_loaded, g_FogSkyIsEnabled);
            fflush(stdout);
            pre_room_log++;
        }
    }
    if (g_CurrentStageToLoad != LEVELID_TITLE && g_MaxNumRooms > 0 && !pc_rooms_loaded) {
        /* Ensure fog environment is loaded BEFORE rooms, so the room DL LUT
         * rewrite in bg.c uses the fog-enabled LUT (CCRMLUT_PRIMARY_ADDFOG)
         * instead of the non-fog LUT.  On N64, fogLoadLevelEnvironment runs
         * during the intro cinematic which happens before the first render.
         * On PC, the intro is skipped and fog loads later in bondviewSetCameraMode.
         * This races with room loading — fix by loading fog here first. */
        {
            extern s32 g_FogSkyIsEnabled;
            if (!g_FogSkyIsEnabled) {
                s32 stage = bossGetStageNum();
                if (getenv("GE007_VERBOSE")) {
                    printf("[LVL_FOG_FIX] pre: g_FogSkyIsEnabled=%d stage=%d g_CurrentStageToLoad=%d\n",
                           g_FogSkyIsEnabled, stage, g_CurrentStageToLoad);
                    fflush(stdout);
                }
                fogLoadLevelEnvironment(stage, 0);
                if (getenv("GE007_VERBOSE")) {
                    printf("[LVL_FOG_FIX] post: g_FogSkyIsEnabled=%d\n", g_FogSkyIsEnabled);
                    fflush(stdout);
                }
            }
        }
        int room, loaded = 0;
        /* N64 DL regions were cleared at level load start (lvlInitSomeStageData).
         * Model files loaded during setup are already re-registered.
         * Room registrations add to the list here. */
        for (room = 1; room < g_MaxNumRooms; room++) {
            if (pc_load_room(room)) loaded++;
        }
        /* In fly camera mode, override position to a known good room location.
         * In gameplay camera mode, lvlViewMoveTick syncs from player position. */
        if (g_pcDebugFlyCamera && loaded > 0 && ptr_bgdata_room_fileposition_list) {
            int target_rooms[] = {55, 60, 70, 80, 40, 30, 20, 10, 1, -1};
            int chosen = -1;
            for (int t = 0; target_rooms[t] >= 0; t++) {
                int r = target_rooms[t];
                if (r < g_MaxNumRooms && ptr_bgdata_room_fileposition_list[r].pPriMappingBin != 0) {
                    chosen = r;
                    break;
                }
            }
            if (chosen >= 0) {
                bg_room_data *rd = &ptr_bgdata_room_fileposition_list[chosen];
                g_pcCamX = rd->pos.x;
                g_pcCamY = rd->pos.y + 50.0f;
                g_pcCamZ = rd->pos.z;
            }
        }
        /* Re-run the fog LUT rewrite on the loaded room DL data.
         * bg.c's sub_GAME_7F0B6368 did this during setup, but pc_load_room
         * decompresses the DL data AGAIN from ROM, overwriting bg.c's
         * rewritten render modes with the original non-fog values.
         * Reapply the fog LUT here so the final DL data has G_RM_FOG_SHADE_A. */
        {
            extern s32 g_FogSkyIsEnabled;
            if (g_FogSkyIsEnabled) {
                for (int ri = 0; ri < pc_num_loaded_rooms; ri++) {
                    int rm = pc_room_allocs[ri].roomID;
                    if (g_BgRoomInfo[rm].ptr_expanded_mapping_info) {
                        bgLoadFromDynamicCCRMLUT(
                            g_BgRoomInfo[rm].ptr_expanded_mapping_info,
                            (Gfx *)((u8 *)g_BgRoomInfo[rm].ptr_expanded_mapping_info +
...

    {
         ...
         * In debug mode, WASD/mouse drives camera.
         * In gameplay mode, camera synced from game state above. */
        static Vp  s_pcVp __attribute__((aligned(8)));
        static float s_camViewF[4][4];

        /* PC: skip viClearZBufCurrentPlayer — it emits G_SETCOLORIMAGE to redirect
         * fill operations to the Z-buffer, but our GL path doesn't track color image
         * state, so the fill rect would paint over the color buffer. We already
         * clear Z via glClear in gfx_run_dl. */
        DL = video_related_F(DL);
        gDPSetScissor(DL++, G_SC_NON_INTERLACE, 0, 0, (s16)viGetX(), (s16)viGetY());

        if (pc_flycam_bg_renderer) {
            s_pcVp.vp.vscale[0] = 320 * 4;
            s_pcVp.vp.vscale[1] = 240 * 4;
            s_pcVp.vp.vscale[2] = 0x1FF * 4;
            s_pcVp.vp.vscale[3] = 0;
            s_pcVp.vp.vtrans[0] = 320 * 4;
            s_pcVp.vp.vtrans[1] = 240 * 4;
            s_pcVp.vp.vtrans[2] = 0x1FF * 4;
            s_pcVp.vp.vtrans[3] = 0;

            /* Build matrices matching the game's rendering pipeline:
             * Game MV = Scale(1/ls) + Translate(room_pos/ls - player_world)
             * Game Proj = CameraView × Perspective
             * We replicate this: camera in world space, room MV has scale */
            extern f32 level_scale;
            float inv_ls = (level_scale > 0.001f) ? (1.0f / level_scale) : 1.0f;

            /* Camera position in world space.
             * g_pcCamX/Y/Z are in stan-space, synced from prop->pos in
             * lvlViewMoveTick (gameplay) or driven by WASD (fly camera).
             * g_pcCamYaw/Pitch are synced from vv_theta/vv_verta (gameplay)
             * or driven by mouse (fly camera). Both modes use the same path. */
            float cam_wx = g_pcCamX * inv_ls;
            float cam_wy = g_pcCamY * inv_ls;
            float cam_wz = g_pcCamZ * inv_ls;

            {
                float sx = sinf(g_pcCamYaw), cx = cosf(g_pcCamYaw);
                float sp = sinf(g_pcCamPitch), cp = cosf(g_pcCamPitch);
                guLookAtF(s_camViewF,
                          0.0f, 0.0f, 0.0f,
                          -sx * cp, sp, -cx * cp,
                          0.0f, 1.0f, 0.0f);
            }

            /* Combined projection = CameraView × GamePerspective.
             * Use the game's projection (set by video_related_F above) so that
             * room rendering and weapon rendering use the same projection.
             * sub_GAME_7F0876C4 builds weapon matrices from this same projection. */
            /* Store as raw float — registered with gfx_register_float_matrix_range
             * so the GBI interpreter treats them as float, not N64 fixed-point. */
            static float s_pcProjMtxF[4][4] __attribute__((aligned(8)));
            {
                extern f32 g_viProjectionMatrixF[4][4];
                /* Row-vector multiply: combined = view × persp */
                for (int i = 0; i < 4; i++)
                    for (int j = 0; j < 4; j++) {
                        s_pcProjMtxF[i][j] = 0.0f;
                        for (int k = 0; k < 4; k++)
                            s_pcProjMtxF[i][j] += s_camViewF[i][k] * g_viProjectionMatrixF[k][j];
                    }
            }
            ...
            /* Fog setup — use game's environment or default blue-gray */
            {
                ...
                u8 fb = envp->Blue;
                {
                    static int fog_log = 0;
                    if (getenv("GE007_VERBOSE") && fog_log < 2) {
                        printf("[FOG] env RGB=(%d,%d,%d) clouds=%d farIntensity=%d\n",
                               fr, fg, fb, envp->Clouds, (int)envp->FarIntensity);
                        fflush(stdout);
                        fog_log++;
                    }
                }
            ...
            gSPViewport(DL++, &s_pcVp);
            gSPMatrix(DL++, (Mtx *)s_pcProjMtxF, G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_PROJECTION | G_MTX_FLOAT_PORT);
            gSPSegment(DL++, SPSEGMENT_BG_DL, ptr_bg_data);

            {
                static float s_roomMtxPoolF[PC_MAX_LOADED_ROOMS][4][4] __attribute__((aligned(8)));
                int tris_before_rooms = g_tri_count_diag;

                /* Camera in stan-space for distance culling against room positions */
                float cam_sx = cam_wx * level_scale;
                float cam_sy = cam_wy * level_scale;
                float cam_sz = cam_wz * level_scale;

                float cull_radius = 10000.0f;
                float cull_r2 = cull_radius * cull_radius;
                int rendered = 0;
                int max_rooms_debug = 200; /* set lower to debug, 200 = unlimited */
                for (int ri = 0; ri < pc_num_loaded_rooms && rendered < max_rooms_debug; ri++) {
                    int rm = pc_room_allocs[ri].roomID;
                    bg_room_data *rd = &ptr_bgdata_room_fileposition_list[rm];
                    float dx = rd->pos.x - cam_sx;
                    float dy = rd->pos.y - cam_sy;
                    float dz = rd->pos.z - cam_sz;
                    if (dx*dx + dy*dy + dz*dz > cull_r2) continue;
                    if (!g_BgRoomInfo[rm].ptr_expanded_mapping_info) continue;
                    if (!g_BgRoomInfo[rm].ptr_point_index) continue;

                    /* Build room MV matching the game's setupRoomTransformationMatrix:
                     * Scale = 1/ls (diagonal)
                     * Translation = room_pos/ls - cam_world
                     * Store as raw float — skip guMtxF2L to avoid GBI
                     * heuristic misdetecting fixed-point as float. */
                    memset(s_roomMtxPoolF[rendered], 0, sizeof(s_roomMtxPoolF[0]));
                    s_roomMtxPoolF[rendered][0][0] = inv_ls;
                    s_roomMtxPoolF[rendered][1][1] = inv_ls;
                    s_roomMtxPoolF[rendered][2][2] = inv_ls;
                    s_roomMtxPoolF[rendered][3][0] = rd->pos.x * inv_ls - cam_wx;
                    s_roomMtxPoolF[rendered][3][1] = rd->pos.y * inv_ls - cam_wy;
                    s_roomMtxPoolF[rendered][3][2] = rd->pos.z * inv_ls - cam_wz;
                    s_roomMtxPoolF[rendered][3][3] = 1.0f;

                    gSPMatrix(DL++, (Mtx *)s_roomMtxPoolF[rendered],
                              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW | G_MTX_FLOAT_PORT);
                    gSPSegment(DL++, SPSEGMENT_BG_VTX,
                ...
                {
                    static int rlog = 0;
                    if (getenv("GE007_VERBOSE") && rlog < 3) {
                        printf("[ROOMS] loaded=%d rendered=%d cull_r=%.0f cam_stan=(%.1f,%.1f,%.1f)\n",
                               pc_num_loaded_rooms, rendered, cull_radius,
                               cam_sx, cam_sy, cam_sz);
                        fflush(stdout);
                        rlog++;
                    }
                }
                if (getenv("GE007_TRACE_PC_BG_TRIS")) {
                    static int tri_log_frame = -1;
                    if (tri_log_frame != g_frame_count_diag) {
                        tri_log_frame = g_frame_count_diag;
                        fprintf(stderr,
                                "[PC-BG-TRIS] frame=%d rendered=%d delta=%d total=%d cam_stan=(%.1f,%.1f,%.1f)\n",
                                g_frame_count_diag,
                                rendered,
                                g_tri_count_diag - tris_before_rooms,
                                g_tri_count_diag,
                                cam_sx, cam_sy, cam_sz);
                    }
                }
        ...
        /* Build world-to-screen matrix from the PC camera for prop/guard rendering.
         * This overrides field_10CC (the game's camera, which may be wrong due to
         * stubs in sub_GAME_7F0876C4). Uses the same camera as room rendering. */
        if (pc_flycam_bg_renderer) {
            extern f32 level_scale;
            float fly_inv_ls = (level_scale > 0.001f) ? (1.0f / level_scale) : 1.0f;
            float fly_wx = g_pcCamX * fly_inv_ls;
            float fly_wy = g_pcCamY * fly_inv_ls;
            float fly_wz = g_pcCamZ * fly_inv_ls;
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    s_pcWorldToScreen.m[i][j] = s_camViewF[i][j];
            s_pcWorldToScreen.m[3][0] = -(fly_wx * s_camViewF[0][0] + fly_wy * s_camViewF[1][0] + fly_wz * s_camViewF[2][0]);
            s_pcWorldToScreen.m[3][1] = -(fly_wx * s_camViewF[0][1] + fly_wy * s_camViewF[1][1] + fly_wz * s_camViewF[2][1]);
            s_pcWorldToScreen.m[3][2] = -(fly_wx * s_camViewF[0][2] + fly_wy * s_camViewF[1][2] + fly_wz * s_camViewF[2][2]);
            s_pcWorldToScreen.m[3][3] = 1.0f;
            s_pcWorldToScreenValid = 1;
            g_CurrentPlayer->field_10CC = &s_pcWorldToScreen;
        ...
        /* Tick props/guards so they update AI/animation state.
         * In fly camera mode the original room-render path is skipped, so do the
         * update here. Gameplay mode still ticks through the normal path below. */
        if (g_pcDebugFlyCamera) {
            determing_type_of_object_and_detection();
        }

    }
    } /* end NATIVE_PORT block */
    /* Always run gameplay path for game logic (rooms already rendered above) */
#endif
    if (g_CurrentStageToLoad == LEVELID_TITLE)
    {
    ...
    else
    {
...
#ifdef NATIVE_PORT
        /* Skip per-player gameplay render when level has no BG data (stub levels) */
        if (g_MaxNumRooms == 0) goto lvl_render_end;
#endif
        ...
        for(i = 0; i < pcount; i++)
        {
             ...
             * because it emits G_SETCOLORIMAGE which our GL path doesn't track.
             * Z-buffer is cleared via glClear in gfx_run_dl. */
            {
                s32 tris_before_video = g_tri_count_diag;
                DL = video_related_F(DL);
                lvlTracePhasePrintf("video_related_F", tris_before_video);
            }
#else
            DL = viClearZBufCurrentPlayer(DL);
            DL = video_related_F(DL);
...
#ifdef NATIVE_PORT
            /* Sync field_488.pos from collision_position and prop->pos before camera setup.
             * MoveBond updates collision_position but NOT field_488.pos.
             * sub_GAME_7F087A08 reads field_488.pos for the guard camera matrix.
             * Without this sync, guards render with the spawn-position camera. */
            if (g_CurrentPlayer->prop) {
                g_CurrentPlayer->field_488.pos.f[0] = g_CurrentPlayer->prop->pos.f[0];
                g_CurrentPlayer->field_488.pos.f[1] = g_CurrentPlayer->field_488.collision_position.f[1];
                g_CurrentPlayer->field_488.pos.f[2] = g_CurrentPlayer->prop->pos.f[2];
                {
                    static int sync_log2 = 0;
                    if (ge_dbg_enabled() && (sync_log2 < 3 || (g_frame_count_diag % 120 == 0 && sync_log2 < 10))) {
                        sync_log2++;
                        fprintf(stderr, "[LVL_POS_SYNC] f=%d prop=(%.1f,%.1f,%.1f) colpos=(%.1f,%.1f) pos_after=(%.1f,%.1f)\n",
                            g_frame_count_diag,
                            g_CurrentPlayer->prop->pos.f[0], g_CurrentPlayer->prop->pos.f[1], g_CurrentPlayer->prop->pos.f[2],
                            g_CurrentPlayer->field_488.collision_position.f[0], g_CurrentPlayer->field_488.collision_position.f[2],
                            g_CurrentPlayer->field_488.pos.f[0], g_CurrentPlayer->field_488.pos.f[2]);
                        fflush(stderr);
                    }
                }
            }
            ...
            {
#ifdef NATIVE_PORT
                s32 tris_before_bond_view = g_tri_count_diag;
                DL = sub_GAME_7F087A08(DL);
                lvlTracePhasePrintf("sub_GAME_7F087A08", tris_before_bond_view);
#else
                DL = sub_GAME_7F087A08(DL);
#endif
            }
...
#ifdef NATIVE_PORT
            {
                int rendered_rooms = 0;
                int ri;

                for (ri = 1; ri < g_MaxNumRooms; ri++) {
                    if (g_BgRoomInfo[ri].room_rendered) {
                        rendered_rooms++;
                    }
                }
...
#endif
            /* room_rendered pre-marking is handled by the PC room renderer
             * block above (gated on pc_flycam_bg_renderer) before we reach here. */
            determing_type_of_object_and_detection();
            ...
            /* Debug dump: execute after all guards have been ticked/rendered */
            {
                ...
                extern void debugDumpOverlayTick(void);
                debugDumpExecute();
                debugDumpOverlayTick();
            }
#endif
            ...
            {
                StandTile *tile1 = stanMatchTileName((char *)tokenFind(1, "-stanshow_"));
                if (tile1)
                {
                    DL = sub_GAME_7F0B3034(DL);
                    DL = sub_GAME_7F0B3024(DL, tile1->points, 0xFF0000FF);
                }
            }

            if (tokenFind(2, "-stanshow_"))
            {
                StandTile *tile2 = stanMatchTileName((char *)tokenFind(2, "-stanshow_"));
                if (tile2)
                {
                    DL = sub_GAME_7F0B3034(DL);
                    DL = sub_GAME_7F0B3024(DL, tile2->points, 0xFF00FF);
                }
            }

            if (tokenFind(3, "-stanshow_"))
            {
                StandTile *tile3 = stanMatchTileName((char *)tokenFind(3, "-stanshow_"));
                if (tile3)
                {
                    DL = sub_GAME_7F0B3034(DL);
                    DL = sub_GAME_7F0B3024(DL, tile3->points, 0xFFFF);
                }
            }
            ...
            if (get_debug_render_raster() == DEB_BOND_VIEW)
            {
                 ...
                 * 1. PROJECTION = game perspective (not the combined view×persp used for rooms)
                 * 2. MODELVIEW = weapon lookAt (field_5C, computed by sub_GAME_7F0876C4)
                 * This ensures weapon model DLs (which G_MTX MUL with current MODELVIEW)
                 * render in front of the camera, not at a room position. */
...
lvl_render_end:
    ...
    /* Debug text overlay removed — was "GE007 PC PORT" test for font pipeline verification */
...
void lvlManageMpGame(void)
{
    ...
    s32 elimination_order = 0;
    s32 copy_g_clockTimer;
    s32 mp_players_over_point_limit;
    s32 finished_death_count = 0;
    s32 prev_finished_death_count = 0;
...
#ifdef NATIVE_PORT
        /* Cap tick count.  On N64, speedgraphframes is typically 1-2
         * (game runs at 30fps with occasional drops).  On PC, stalls
         * from asset loading or window focus changes can produce huge
         * spikes (observed: 13).  Cap to 4 ticks (≈133ms at 30fps)
         * to prevent movement/gravity integration blowups.
         * First frame after load is further capped to 1 tick. */
        if (get_is_ramrom_flag() != 0) {
            /* RAMROM playback records full speedframes; clamping them diverges
             * from ROM replay timing and invalidates boundary seed checks. */
        } else if (D_80048380 == 0 && g_ClockTimer > 1) {
            g_ClockTimer = 1;
        } else if (g_ClockTimer > 4) {
            g_ClockTimer = 4;
        }
#endif
        D_80048380 += 1;
    ...
    /* Section C: Multiplayer game-ending logic */
    if ((getPlayerCount() >= 2) && (g_CurrentStageToLoad != LEVELID_TITLE))
    {
        /* C1: Mission state 6 transition — count dead/finished players */
        ...
        if (mp_game_time_limit > 0)
        {
            ...
            {
                g_MpSoundStateRelated = sndPlaySfx(g_musicSfxBufferPtr, 0xA1, g_MpSoundStateRelated);
            }
                ...
                if (var_player_count2 > 0)
                {
                    ...
                    if (var_player_count2_again != 0)
                    {
                        for (j=0; j<var_player_count2_again; j++)
                        {
                            ...
                            killed_count += g_playerPlayerData[j].kill_counts[0];
                        }
                    }
                    else
                    {
                        for (j=0; j<var_player_count2_again; j++)
                        {
                            ...
                            killed_count =
                                g_playerPlayerData[PLAYER_1].kill_counts[0] +
                                g_playerPlayerData[PLAYER_2].kill_counts[0] +
                                g_playerPlayerData[PLAYER_3].kill_counts[0] +
                                g_playerPlayerData[PLAYER_4].kill_counts[0];
                        }
                    }
                }
    ...
    /* Section D: MP time accumulation (after gameplay logic, matching N64 ordering) */
    ...
    /* Section E: Idle tracking / input timer */
    ...
    /* Section F: Z-buffer + title/gameplay branching */
    viSetUseZBuf(1);
...
void lvlManageMpGame(void)
{
    ...
    s32 mp_player_field424_count;
    s32 mp_player_currently_in_dying_animation;
    s32 elimination_order;
    s32 copy_g_clockTimer;
    s32 mp_players_over_point_limit;
    s32 finished_death_count;
    s32 prev_finished_death_count;
...
#ifdef NATIVE_PORT
    {
        ...
        if (!g_pcDebugFlyCamera && g_CurrentPlayer->prop != NULL) {
            ...
            if (grx != 0 || gry != 0) {
                ...
                /* §3.1: accumulate the fractional look pixels across frames so
                 * sub-1px/frame deflection isn't truncated to zero. FpsScale is
                 * folded in BEFORE accumulation, so the carry stays in
                 * output-pixel units and is bounded < 1px. */
                f32 fx = nx * gp_scale * fps + s_gpLookRemX[local_player_number];
                f32 fy = ny * gp_scale * fps + s_gpLookRemY[local_player_number];
                int ix = (int)fx;   /* trunc toward zero — symmetric, pairs with the subtraction */
                int iy = (int)fy;
                ...
                /* Stick at rest (or zeroed by !live_look_allowed / g_freezeInput
                 * above): drop the carry so it can never drift the view. */
                s_gpLookRemX[local_player_number] = 0.0f;
                s_gpLookRemY[local_player_number] = 0.0f;
            }

            if (mdx != 0 || mdy != 0) {
                extern float g_pcMouseSensitivity;
                extern float g_pcMouseSensAim;
                ...
                if (in_tank_flag == 1) {
                    ...
                    while (g_TankTurretAngle < 0.0f)
                        g_TankTurretAngle += 6.28318530f;
                    while (g_TankTurretAngle >= 6.28318530f)
                        g_TankTurretAngle -= 6.28318530f;

                    g_CurrentPlayer->vv_theta =
                        (g_TankOrientationAngle + g_TankTurretOrientationAngleRad) * (180.0f / 3.14159265f);
                } else {
             ...
             * Only when gameplay camera is active (not fly camera). */
            if (!g_pcDebugFlyCamera) {
                f32 ls = get_room_data_float1();
                if (playerHasFrozenIntroCamera(g_CurrentPlayer)) {
                    coord3d intro_view = {
                        g_CurrentPlayer->pos2.x - g_CurrentPlayer->pos.x,
                        g_CurrentPlayer->pos2.y - g_CurrentPlayer->pos.y,
                        g_CurrentPlayer->pos2.z - g_CurrentPlayer->pos.z,
                    };
                    f32 intro_view_len = sqrtf(intro_view.x * intro_view.x
                                             + intro_view.y * intro_view.y
                                             + intro_view.z * intro_view.z);
                    f32 intro_view_inv_len = intro_view_len > 0.0001f ? 1.0f / intro_view_len : 0.0f;
                    f32 intro_fx = intro_view.x * intro_view_inv_len;
                    f32 intro_fy = intro_view.y * intro_view_inv_len;
                    f32 intro_fz = intro_view.z * intro_view_inv_len;

                    g_pcCamX = g_CurrentPlayer->pos.x * ls;
                    g_pcCamY = g_CurrentPlayer->pos.y * ls;
                    g_pcCamZ = g_CurrentPlayer->pos.z * ls;
