...
//8008C720
#ifdef NATIVE_PORT
/* On N64, the original code used &ptr_texture_alloc_start as a struct texpool*,
 * relying on adjacent BSS globals to form the struct fields. On 64-bit this
 * doesn't work because pointer sizes differ. Use a real struct instead. */
struct texpool g_MainTexPool;
struct texpool *ptr_texture_alloc_start = &g_MainTexPool;
#else
...
u16 *texGetPalette(s32 texturenum, s32 *numcolours_out) {
    for (s32 i = 0; i < g_TexPaletteCacheCount; i++) {
        if (g_TexPaletteCache[i].texturenum == texturenum) {
            if (numcolours_out) *numcolours_out = g_TexPaletteCache[i].numcolours;
            return g_TexPaletteCache[i].palette;
        }
    }
    return NULL;
}
...
{
    out[0] = ((rgba16 >> 11) & 0x1f) * 255 / 31;
    out[1] = ((rgba16 >> 6) & 0x1f) * 255 / 31;
    out[2] = ((rgba16 >> 1) & 0x1f) * 255 / 31;
    out[3] = (rgba16 & 1) ? 255 : 0;
}
...
int portDumpTextureToPpm(s32 texturenum, const char *path)
{
    ...
    tex = texFindInPool(texturenum, ptr_texture_alloc_start);

    if (tex == NULL || tex->data == NULL || tex->width == 0 || tex->height == 0) {
        return -2;
    }

    texel_count = tex->width * tex->height;
    rgba = malloc((size_t)texel_count * 4u);
    ...
    if (tex->gbiformat == G_IM_FMT_RGBA) {
        if (tex->depth == G_IM_SIZ_32b) {
            for (i = 0; i < texel_count; i++) {
                u32 c;
                memcpy(&c, &tex->data[i * 4], sizeof(c));
            ...
            for (i = 0; i < texel_count; i++) {
                portRgba16ToRgba32(((u16 *)tex->data)[i], &rgba[i * 4]);
            }
        } else {
            free(rgba);
            return -4;
        }
    } else if (tex->gbiformat == G_IM_FMT_IA) {
        if (tex->depth == G_IM_SIZ_16b) {
            for (i = 0; i < texel_count; i++) {
                u16 ia;
                memcpy(&ia, &tex->data[i * 2], sizeof(ia));
                u8 intensity = ia >> 8;
                u8 alpha = ia & 0xff;
        ...
        } else if (tex->depth == G_IM_SIZ_4b) {
            for (i = 0; i < texel_count; i++) {
                u8 ia = tex->data[i / 2];
                ia = (i & 1) ? (ia & 0x0f) : (ia >> 4);
                rgba[i * 4 + 0] = ((ia >> 1) & 0x07) * 255 / 7;
                rgba[i * 4 + 1] = ((ia >> 1) & 0x07) * 255 / 7;
                rgba[i * 4 + 2] = ((ia >> 1) & 0x07) * 255 / 7;
                rgba[i * 4 + 3] = (ia & 0x01) ? 255 : 0;
            }
        } else {
            free(rgba);
            return -4;
        }
    } else if (tex->gbiformat == G_IM_FMT_I) {
        if (tex->depth == G_IM_SIZ_8b) {
            for (i = 0; i < texel_count; i++) {
                u8 intensity = tex->data[i];
                rgba[i * 4 + 0] = intensity;
                rgba[i * 4 + 1] = intensity;
                rgba[i * 4 + 2] = intensity;
                rgba[i * 4 + 3] = 255;
            ...
            for (i = 0; i < texel_count; i++) {
                u8 intensity = tex->data[i / 2];
                intensity = (i & 1) ? (intensity & 0x0f) : (intensity >> 4);
                intensity *= 17;
                rgba[i * 4 + 0] = intensity;
                rgba[i * 4 + 1] = intensity;
                rgba[i * 4 + 2] = intensity;
        ...
        s32 numcolours = 0;
        u16 *palette = texGetPalette(texturenum, &numcolours);
        ...
        for (i = 0; i < texel_count; i++) {
            ...
            if (tex->depth == G_IM_SIZ_4b) {
                idx = tex->data[i / 2];
                idx = (i & 1) ? (idx & 0x0f) : (idx >> 4);
            } else {
                idx = tex->data[i];
            }

            pal_entry = (idx < numcolours) ? palette[idx] : palette[0];

            if (tex->lutmodeindex == (G_TT_IA16 >> G_MDSFT_TEXTLUT)) {
                u8 intensity = pal_entry >> 8;
                u8 alpha = pal_entry & 0xff;
                rgba[i * 4 + 0] = intensity;
                rgba[i * 4 + 1] = intensity;
                rgba[i * 4 + 2] = intensity;
                rgba[i * 4 + 3] = alpha;
            } else {
                portRgba16ToRgba32(pal_entry, &rgba[i * 4]);
            }
        }
    } else {
        free(rgba);
    ...
    ge007_sprintf(alpha_path, "%s.alpha.pgm", path);
    alpha_fp = fopen(alpha_path, "wb");
    ...
    for (i = 0; i < texel_count; i++) {
        ...
        fwrite(&rgba[i * 4], 1, 3, fp);
        if (alpha_fp != NULL) {
            fputc(alpha, alpha_fp);
        }
    }
    fclose(fp);
    if (alpha_fp != NULL) {
    ...
    fprintf(stderr,
        "[TEX_DUMP] tex=%d %dx%d fmt=%d depth=%d alpha[min=%d max=%d nonzero=%d/%d] alpha_path=%s\n",
        texturenum, tex->width, tex->height, tex->gbiformat, tex->depth,
        alpha_min, alpha_max, alpha_nonzero, texel_count, alpha_path);
    fflush(stderr);
    free(rgba);
    return 0;
}
...
s32 texInflateZlib(u8 *src, u8 *dst, s32 arg2, s32 forcenumimages, struct texpool *arg4)
{
    ...
    u8 scratch2[0x800];
#ifdef NATIVE_PORT
    u8 scratch[0x4200]; /* struct huft is 16 bytes on 64-bit vs 8 on N64 */
#else
    u8 scratch[0x2100];
#endif
    u16 palette[0x100];
...
#ifdef NATIVE_PORT
    /* Cache the palette so gfx_handle_settex can look it up for CI textures */
    texStorePalette(arg4->rightpos->texturenum, palette, numcolours);
#endif
...
s32 texFindClosestColourIndexRGBA(u16 *palette, s32 numcolours, s32 r, s32 g, s32 b, s32 a)
{
    ...
    while (var_v1 - var_a1 >= 2)
    {
        ...
        pal_color = palette[temp_a0]; /* N64 asm uses stride-1 (sll a0,1 = u16 offset), not *4 */
        pal_r = ((s32) pal_color >> 0xB) & 0x1F;
        pal_g = ((s32) pal_color >> 0x6) & 0x1F;
        pal_b = ((s32) pal_color >> 0x1) & 0x1F;
        pal_a = pal_color & 1;

        temp_s4 = (pal_r * pal_r) + (pal_g * pal_g) + (pal_b * pal_b) + (pal_a * 0x3C1);
    ...
    {
        pal_color = palette[var_v1]; /* stride-1, not *4 — see MIPS asm */
        pal_r = (((s32) pal_color >> 0xB) & 0x1F) - r;
        pal_g = (((s32) pal_color >> 0x6) & 0x1F) - g;
        pal_b = (((s32) pal_color >> 0x1) & 0x1F) - b;
        pal_a = 0x3C1;
        if (a == (pal_color & 1))
        {
            pal_a = 0;
        ...
        temp_s4 = (pal_r * pal_r) + (pal_g * pal_g) + (pal_b * pal_b) + (pal_a);
    ...
    for (i = 0; i < numimages; i++)
    {
        ...
        switch (compmethod)
        {
            ...
            case TEXCOMPMETHOD_LOOKUP:
                value = texBuildLookup((s8 *)lookup, g_TexFormatBitsPerPixel[format]);
        ...
        case TEXFORMAT_RGB24:
            for (i = 0; i < srcheight; i += 2)
            {
                ...
                for (j = 0; j < alignedsrcwidth; j += 2)
                {
                    ...
                    dst32[j >> 1] = (u32)r << 24 | (u32)g << 16 | (u32)b << 8 | (u32)a;
                }
    ...
    switch (format)
    {
        case TEXFORMAT_RGBA32:
            for (y = 0; y < height; y++)
            {
                for (x = 0; x < width; x++)
                {
                    dst32[x] = (u32)src[pos] << 24 | (u32)src[pos + mult] << 16 | (u32)src[pos + mult * 2] << 8 | src[pos + mult * 3];
                    pos++;
                }
...
{
#ifdef NATIVE_PORT
	return (uintptr_t)arg0->rightpos - (uintptr_t)arg0->leftpos;
#else
...
extern u32 _imagesSegmentRomStart;
    ...
    if (tex == NULL)
    {
...
#ifdef NATIVE_PORT
        /* On PC, bitfield layout differs from N64 — read the field directly
         * instead of the s32* cast which relies on big-endian bitfield packing. */
        thisoffset = (s32)g_Textures[g_TexNumToLoad].dataoffset;
        nextoffset = (s32)g_Textures[g_TexNumToLoad + 1].dataoffset;
#else
        ...
        if (TRUE)
        {
            ...
            romCopy(alignedcompbuffer,
                    (void *)((uintptr_t)ROM_OFFSET(_imagesSegmentRomStart) + (thisoffset & 0xfffffff8)),
