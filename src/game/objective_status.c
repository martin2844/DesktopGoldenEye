...
#ifdef NATIVE_PORT
extern int get_keyanalyzer_flag(void);
extern bool sub_GAME_7F050D30(PropRecord *prop, coord3d *arg1, struct coord2d *arg2, struct coord2d *arg3);
#endif

static struct objective_entry *objectiveGetEntry(s32 objectiveIndex)
{
    if (objectiveIndex < 0 || objectiveIndex >= OBJECTIVES_MAX) {
        return NULL;
    }

    return objective_ptrs[objectiveIndex];
}

#ifdef NATIVE_PORT
static u32 objectiveEntryGetPackedWord(const struct objective_entry *entry, size_t offset)
{
    u32 word = 0;

    memcpy(&word, ((const u8 *)entry) + offset, sizeof(word));

    return word;
}

static u16 objectiveEntryGetTextId(const struct objective_entry *entry)
{
    return (u16)(objectiveEntryGetPackedWord(entry, 8) & 0xffff);
}

static u8 objectiveEntryGetFlags(const struct objective_entry *entry)
{
    return (u8)((objectiveEntryGetPackedWord(entry, 12) >> 8) & 0xff);
}

static s8 objectiveEntryGetDifficulty(const struct objective_entry *entry)
{
    return (s8)(objectiveEntryGetPackedWord(entry, 12) & 0xff);
}
#else
static u16 objectiveEntryGetTextId(const struct objective_entry *entry)
{
...
{
    struct setup_objective_text *curentry;
    u8 * textptr;

    textptr = NULL;

    for (curentry = ptr_last_briefing_setup_entry_type23; curentry != 0; curentry = curentry->next)
    {
        if (page == curentry->unk04)
        {
            textptr = langGet(curentry->unk08);
            break;
        }
    }
...
{
    struct objective_entry *entry = objectiveGetEntry(objectiveIndex);

    if (entry == NULL) {
        return 0;
    }

    return objectiveEntryGetTextId(entry);
}


u8 * get_text_for_objective(int objectiveIndex)
{
    u16 textid = objectiveGetTextId(objectiveIndex);
...
s32 objectiveGetFlags(s32 objectiveIndex)
{
    ...
    return objectiveEntryGetFlags(entry);
}
...
s32 get_difficulty_for_objective(s32 objectiveIndex)
{
    ...
    if (entry != NULL) {
        return objectiveEntryGetDifficulty(entry);
    }
    return 0;
}
...
/* AI command bytecode calls this entry point with two arguments; the objective
 * status logic only uses the objective index. */
#ifdef NATIVE_PORT
...
OBJECTIVESTATUS get_status_of_objective(s32 objectiveNum) //#MATCH
{
    ...
    if (objectiveNum < 10)
    {
        ...
        else
        {
            ...
            for (objective = (MissionObjectiveRecord *)&objective_ptrs[objectiveNum]->id;
                 objective->type != PROPDEF_OBJECTIVE_END;
                 objective = (MissionObjectiveRecord *)((PropDefHeaderRecord *)objective + sizepropdef((PropDefHeaderRecord *)objective)))
            {
                {
    ...
    for (i = 0; i <= objective_count; i++) {
        ...
        if (objectiveStatuses[i] != status) {
            ...
            if (get_difficulty_for_objective(i) <= lvlGetSelectedDifficulty()) {
                if (j_text_trigger != 0)
                {
                    ge007_sprintf(buffer, "%s \x80%c ", langGet(getStringID(LMISC, MISC_STR_2C_OBJECTIVE)), availableindex + 0x1A);
                }
                else
                {
                    ge007_sprintf(buffer, "%s %c: ", langGet(getStringID(LMISC, MISC_STR_2C_OBJECTIVE)), availableindex + 0x61);
                }
                if (status == OBJECTIVESTATUS_COMPLETE)
                {
                    strcat(buffer, (char *)langGet(getStringID(LMISC, MISC_STR_2D_COMPLETED_LF)));
                }
                else if (status == OBJECTIVESTATUS_INCOMPLETE)
                {
                    strcat(buffer, (char *)langGet(getStringID(LMISC, MISC_STR_2E_INCOMPLETE_LF)));
                }
                else if (status == OBJECTIVESTATUS_FAILED)
                {
                    strcat(buffer, (char *)langGet(getStringID(LMISC, MISC_STR_2F_FAILED_LF)));
                }
#ifdef VERSION_US
    ...
    for (; criteria != NULL; criteria = criteria->next)
    {
        if (criteria->flag == 0)
        {
            ...
            if (target_object != NULL)
            {
                ...
                {
                    sub_GAME_7F03F948(&sp84, &sp7C, &sp74, (struct coord2d *)&sp64.right, (struct coord2d *)&sp64.left);
