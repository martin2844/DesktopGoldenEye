...
{
    s32 stick_x = joyGetStickXInRange(player, -2, 1);

    if ((joyGetButtonsPressedThisFrame(player, R_JPAD|R_CBUTTONS)) || ((stick_x >= 1  && (g_CurrentPlayer->mpjoywascentre)))) {
        return 1;
    }
...
s32 mpwatchIsPlayerPressingLeft(s32 player)
{
    ...
    if ((joyGetButtonsPressedThisFrame(player, L_JPAD|L_CBUTTONS)) || ((stick_x < -1 && (g_CurrentPlayer->mpjoywascentre)))) {
        return 1;
    }
...
s32 mpFindMaxInt(int numplayers, int score_a, int score_b, int score_c, int score_d)
{
  s32 aux;
  s32 result;

  if ((score_a < score_b) ||
     ((score_b == score_a && ((randomGetNext() & 1))))) {
    result = 1;
    aux = score_b;
  }
  else {
...
s32 mpFindMinInt(int numplayers, int score_a, int score_b, int score_c, int score_d)
{
  s32 aux;
  s32 result;
    ...
    for (i = 0; i < player_count; i++)
    {
        ...
        while (numdone == 0)
        {
            if (metrics[i].awards & (1 << awardindex))
            {
                ...
                g_playerPointers[i]->ptr_text_first_mp_award = (uintptr_t)langGet(g_AwardNames[awardindex]);
                numdone = 1;
            }
        ...
        while (numdone < 2)
        {
            ...
            {
                metrics[i].awards &= ~(1 << awardindex);
                g_playerPointers[i]->ptr_text_second_mp_award = (uintptr_t)langGet(g_AwardNames[awardindex]);
                numdone = 2;
            }
...
Gfx *display_text_for_playerdata_on_MP_menu(Gfx *gdl, s32 x, s32 y, s32 value, TEXTCOLORS text_color) {
    ...
    ge007_sprintf(score_text, "%d", value);

    textMeasure(&sp50, &sp54, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
    ...
    switch (text_color) {
        ...
        case GREEN_NORMAL:
            viX = viGetX();
            viY = viGetY();
            gdl = textRender(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0xFF00B0, viX, viY, 0, 0);
            break;
        
        case GREEN_HIGHLIGHT:
            viX = viGetX();
            viY = viGetY();
            gdl = textRenderGlow(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0xA0FFA0F0, 0x7000A0, viX, viY, 0, 0);
            break;
        
        case RED_NORMAL:
            viX = viGetX();
            viY = viGetY();
            gdl = textRender(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0xFF4040B0, viX, viY, 0, 0);
            break;
        
        case RED_HIGHLIGHT:
            viX = viGetX();
            viY = viGetY();
            gdl = textRenderGlow(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0xFFA0A0F0, 0x700000A0, viX, viY, 0, 0);
            break;
        
        case BLUE_NORMAL:
            viX = viGetX();
            viY = viGetY();
            gdl = textRender(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0x4040FFB0, viX, viY, 0, 0);
            break;
        
        case BLUE_HIGHLIGHT:
            viX = viGetX();
            viY = viGetY();
            gdl = textRenderGlow(gdl, &sp5C, &sp58, score_text, ptrFontBankGothicChars, ptrFontBankGothic, 0xA0A0FFF0, 0x70A0, viX, viY, 0, 0);
            break;
    }
...
s32 get_points_for_mp_player(s32 arg0) {
    ...
    team = g_playerPlayerData[arg0].have_token_or_goldengun;
    ...
    switch (scenario) {
        ...
        case SCENARIO_LTK:
            for (i = 0; i < playerCount; i++) {
                if (i != arg0) {
                    points += g_playerPlayerData[arg0].kill_counts[i];
                } else {
                    points -= g_playerPlayerData[i].kill_counts[arg0];
                }
            }
            points += g_playerPlayerData[arg0].killed_gg_owner_count * (playerCount - 2);
            break;
        case SCENARIO_YOLT:
            points = 4 - g_playerPlayerData[arg0].order_out_in_yolt;
            break;
        case SCENARIO_TLD:
            points = g_playerPlayerData[arg0].flag_counter;
            break;
        case SCENARIO_2v2:
        case SCENARIO_3v1:
        case SCENARIO_2v1:
            for (i = 0; i < playerCount; i++) {
                if (g_playerPlayerData[i].have_token_or_goldengun != team) {
                    continue;
                }
                for (j = 0; j < playerCount; j++) {
                    if (g_playerPlayerData[j].have_token_or_goldengun == team) {
...
#ifdef NONMATCHING
void write_playerrank_to_buffer(char *buffer, s32 player_index) {
    s32 scenario;
    s32 count;
    s32 scores[4];
    s32 indices[4];
    ...
    /* bubble sort descending by score, swapping indices to match */
    for (i = 0; i < count; i++) {
        for (j = 0; j < count - 1; j++) {
    ...
    /* find position of player_index in sorted indices */
    i = 0;
    for (j = 0; j < count; j++) {
        if (indices[j] == player_index) {
            i = j;
            break;
        }
    }

    /* count ties above: find first entry with same score as ours */
    rank = 0;
    if (i >= 0) {
        s32 my_score = scores[i];
        s32 k;
    ...
    switch (rank) {
        case 0:
            snprintf(buffer, 256, "%s", (char *)langGet(getStringID(LMPMENU, MPMENU_STR_11_RANK1ST)));
            break;
        case 1:
            snprintf(buffer, 256, "%s", (char *)langGet(getStringID(LMPMENU, MPMENU_STR_12_RANK2ND)));
            break;
        case 2:
            if (scenario == SCENARIO_2v2 || scenario == SCENARIO_2v1) {
                snprintf(buffer, 256, "%s", (char *)langGet(getStringID(LMPMENU, MPMENU_STR_12_RANK2ND)));
            } else {
                snprintf(buffer, 256, "%s", (char *)langGet(getStringID(LMPMENU, MPMENU_STR_13_RANK3RD)));
            }
            break;
        case 3:
            if (scenario == SCENARIO_3v1) {
                ...
                snprintf(buffer, 256, "%s", (char *)langGet(getStringID(LMPMENU, MPMENU_STR_14_RANK4TH)));
            }
            break;
    }
}
...
s32 mpwatchShouldDisplayRank(s32 player_index)
{
  switch(get_scenario())
  {
    case SCENARIO_NORMAL:
...
s32 mpwatchShouldDisplayScore(s32 player_index)
{
  switch(get_scenario())
  {
    case SCENARIO_NORMAL:
...
Gfx *mp_watch_menu_display(Gfx *arg0) {
    s32 player_num;          // sp+D4
    s32 player_count;        // sp+D0
    s32 text_height;         // sp+BC
    s32 text_width;          // sp+C0
    s32 x;                   // sp+CC
    s32 y;                   // sp+C8
    s32 viewleft_offset;     // sp+A8
    u8 *text;                // sp+A4
    s32 points[4];           // sp+94..A0
    s32 i;                   // sp+90
    s32 self_color;          // sp+8C
    s32 same_team_color;     // sp+88
    s32 diff_team_color;     // sp+84
    s32 scenario;            // sp+80
    s32 text_height2;        // sp+7C
    s32 text_width2;         // sp+78
    s32 x_offset;            // sp+74
    s32 x2;                  // sp+70
    s32 y2;                  // sp+6C
    u8 *press_start_text;    // sp+60
    s32 total_kills;         // sp+58
    s16 view_tmp;            // sp+56
    s32 text_color;          // sp+50
    u8 *header_text;         // sp+4C
    s32 *pts_ptr;            // sp+44
    s32 sp64;                // sp+64
    s32 sp68;                // sp+68
    s16 viX;
    ...
    switch (g_CurrentPlayer->mpmenumode) {
        ...
        case MENU_SCORES:
            if (!g_gameOverFlag) {
                text = langGet(getStringID(LMPMENU, MPMENU_STR_15_PLAY));
            } else if (time_trigger_alt_gameover_msg) {
                text = langGet(getStringID(LMPMENU, MPMENU_STR_16_GAMEOVER));
            } else {
                text = langGet(getStringID(LMPMENU, MPMENU_STR_17_STARTTTOEXIT));
            }
            break;
        case MENU_7:
            text = (u8 *)ascii_MP_watch_menu_BLANK;
            break;
        case MENU_PAUSE:
            if (g_pausedFlag) {
                text = langGet(getStringID(LMPMENU, MPMENU_STR_18_PAUSED));
                if (get_cur_playernum() == who_paused) {
                    highlight = 1;
                }
            } else {
                text = langGet(getStringID(LMPMENU, MPMENU_STR_19_PAUSE));
            }
            break;
        case MENU_EXIT:
        case MENU_6:
            text = langGet(getStringID(LMPMENU, MPMENU_STR_1A_EXIT));
            x = viGetViewLeft() + viewleft_offset + 0x41;
            break;
    }

    textMeasure(&text_height, &text_width, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
    x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x50;
    y = viGetViewTop() - (text_height >> 1) + 0x16;

    if (highlight) {
        viX = viGetX();
        arg0 = textRenderGlow(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                              0xA0FFA0F0, 0x007000A0, viX, viGetY(), 0, 0);
    } else {
        viX = viGetX();
        arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                          0x00FF00B0, viX, viGetY(), 0, 0);
    }

    if (mpwatchMenuCanGoLeft()) {
        s32 left_x = (s16)(viGetViewLeft() << 16);
        if (g_gameOverFlag) {
            text_color = 10;
        } else {
            text_color = 0;
        }
        x = left_x + viewleft_offset - text_color + 0x28;
        if (g_gameOverFlag) {
            x = x - 8;
        }
        y = viGetViewTop() + 0x16;
        ...
        if (!dword_CODE_bss_8008C708) {
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, ascii_MP_watch_menu_left_chevron,
                              ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);
        } else {
            viX = viGetX();
            arg0 = textRenderGlow(arg0, &x, &y, ascii_MP_watch_menu_left_chevron,
                                  ptrFontBankGothicChars, ptrFontBankGothic,
                                  0xA0FFA0F0, 0x007000A0, viX, viGetY(), 0, 0);
        }
    }
    ...
    if (mpwatchMenuCanGoRight()) {
        s32 right_x = (s16)(viGetViewLeft() << 16);
        if (g_gameOverFlag) {
            text_color = 10;
        } else {
            text_color = 0;
        }
        x = text_color + right_x + viewleft_offset + 0x70;
        if (g_gameOverFlag) {
            x = x + 8;
        }
        y = viGetViewTop() + 0x16;
        ...
        if (!dword_CODE_bss_8008C708) {
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, ascii_MP_watch_menu_right_chevron,
                              ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);
        } else {
            viX = viGetX();
            arg0 = textRenderGlow(arg0, &x, &y, ascii_MP_watch_menu_right_chevron,
                                  ptrFontBankGothicChars, ptrFontBankGothic,
                                  0xA0FFA0F0, 0x007000A0, viX, viGetY(), 0, 0);
        }
    }

    // SCORES or PAUSE: mpmenumode == 3 or 4
    if (g_CurrentPlayer->mpmenumode == MENU_SCORES || g_CurrentPlayer->mpmenumode == MENU_PAUSE) {
        if (player_count > 0) {
            pts_ptr = &points[0];
            for (i = 0; i < player_count; i++) {
                *pts_ptr = get_points_for_mp_player(i);
                pts_ptr++;
            }
        }
        ...
        if (mpwatchShouldDisplayRank(i)) {
            write_playerrank_to_buffer(buf, player_num);
            textMeasure(&text_height, &text_width, buf, ptrFontBankGothicChars, ptrFontBankGothic, 0);
            x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x50;
            y = viGetViewTop() - (text_height >> 1) + 0x25;
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, buf, ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);
        }
        ...
        if (mpwatchShouldDisplayScore(i)) {
            scenario = get_scenario();
            text = langGet(getStringID(LMPMENU, MPMENU_STR_1B_SCORES));
            textMeasure(&text_height, &text_width, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
            x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x50;
            y = viGetViewTop() - (text_height >> 1) + 0x35;
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);

            if (scenario == SCENARIO_2v2 || scenario == SCENARIO_3v1 || scenario == SCENARIO_2v1 ||
                scenario == SCENARIO_TLD || scenario == SCENARIO_MWTGG) {
                if (g_playerPlayerData[player_num].have_token_or_goldengun == 0) {
                    self_color = 3;
                    same_team_color = 2;
                    diff_team_color = 4;
                } else {
            ...
            if (player_count == 2) {
                s32 left, top;
                left = (s16)(viGetViewLeft() << 16);
                top = (s16)(viGetViewTop() << 16);
                if (player_num == 0) {
                    text_color = self_color;
                } else {
                    if (g_playerPlayerData[0].have_token_or_goldengun ==
                        g_playerPlayerData[player_num].have_token_or_goldengun) {
                        text_color = same_team_color;
                    } else {
                        text_color = diff_team_color;
                    }
                }
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x46, points[0], text_color);
                ...
                if (player_num == 1) {
                    ...
                    if (g_playerPlayerData[1].have_token_or_goldengun ==
                        g_playerPlayerData[player_num].have_token_or_goldengun) {
                        text_color = same_team_color;
                    } else {
                        text_color = diff_team_color;
                ...
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x56, points[1], text_color);
            } else {
                s32 left, top;
                left = (s16)(viGetViewLeft() << 16);
                top = (s16)(viGetViewTop() << 16);
                ...
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x46, points[0], text_color);
                ...
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x60, top + 0x46, points[1], text_color);
                ...
                if (player_num == 2) {
                    ...
                    if (g_playerPlayerData[2].have_token_or_goldengun ==
                        g_playerPlayerData[player_num].have_token_or_goldengun) {
                        text_color = same_team_color;
                    } else {
                        text_color = diff_team_color;
                ...
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x56, points[2], text_color);
                ...
                if (player_count == 4) {
                    ...
                    if (player_num == 3) {
                        ...
                        if (g_playerPlayerData[3].have_token_or_goldengun ==
                            g_playerPlayerData[player_num].have_token_or_goldengun) {
                            text_color = same_team_color;
                        } else {
                            text_color = diff_team_color;
                    ...
                    arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                        left + viewleft_offset + 0x60, top + 0x56, points[3], text_color);
                }
            }
        }
        goto render_exit_menu;
    }

    // KILLS: mpmenumode == 2
    if (g_CurrentPlayer->mpmenumode == MENU_KILLS) {
        if (!g_gameOverFlag) {
            i = (g_stopPlayFlag == 0);
        } else {
            i = 1;
        ...
        header_text = langGet(getStringID(LMPMENU, MPMENU_STR_1C_P));
        text = langGet(getStringID(LMPMENU, MPMENU_STR_1D_KILLS));
        ge007_sprintf(buf, ascii_pnum_KILLS, header_text, player_num + 1, text);
        textMeasure(&text_height, &text_width, buf, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x50;
        y = viGetViewTop() - (text_height >> 1) + 0x35;
        viX = viGetX();
        ...
        if (player_count == 2) {
            if (player_num != 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x46,
                    g_playerPlayerData[player_num].kill_counts[0], GREEN_NORMAL);
            }
            if (player_num != 1) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x56,
                    g_playerPlayerData[player_num].kill_counts[1], GREEN_NORMAL);
            }
        } else {
            if (player_num != 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x46,
                    g_playerPlayerData[player_num].kill_counts[0], GREEN_NORMAL);
            }
            if (player_num != 1) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x60, top + 0x46,
                    g_playerPlayerData[player_num].kill_counts[1], GREEN_NORMAL);
            }
            if (player_num != 2) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x56,
                    g_playerPlayerData[player_num].kill_counts[2], GREEN_NORMAL);
            }
            if (player_count == 4 && player_num != 3) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x60, top + 0x56,
                    g_playerPlayerData[player_num].kill_counts[3], GREEN_NORMAL);
            }
        }
        goto render_exit_menu;
    }
    ...
    if (g_CurrentPlayer->mpmenumode == MENU_LOSSES) {
        ...
        header_text = langGet(getStringID(LMPMENU, MPMENU_STR_1C_P));
        text = langGet(getStringID(LMPMENU, MPMENU_STR_1E_LOSSES));
        ge007_sprintf(buf, ascii_pnum_LOSSES, header_text, player_num + 1, text);
        textMeasure(&text_height, &text_width, buf, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x50;
        y = viGetViewTop() - (text_height >> 1) + 0x35;
        viX = viGetX();
        arg0 = textRender(arg0, &x, &y, buf, ptrFontBankGothicChars, ptrFontBankGothic,
                          0xFF4040B0, viX, viGetY(), 0, 0);

        if (player_count == 2) {
            // Player 0 killed current player how many times
            if (player_num != 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x46,
                    g_playerPlayerData[player_num].kill_counts[0], GREEN_NORMAL);
            } else if (g_playerPlayerData[0].kill_counts[player_num] > 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x46,
                    g_playerPlayerData[player_num].kill_counts[0], RED_HIGHLIGHT);
            }

            // Player 1 killed current player how many times
            if (player_num != 1) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x56,
                    g_playerPlayerData[1].kill_counts[player_num], GREEN_NORMAL);
            } else if (g_playerPlayerData[1].kill_counts[player_num] > 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x50, top + 0x56,
                    g_playerPlayerData[1].kill_counts[player_num], RED_HIGHLIGHT);
            }
        } else {
            // Player 0
            if (player_num != 0) {
            ...
            if (player_num != 2) {
                ...
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x56,
                    g_playerPlayerData[player_num].kill_counts[2], GREEN_NORMAL);
            } else if (g_playerPlayerData[2].kill_counts[player_num] > 0) {
                s32 left = (s16)(viGetViewLeft() << 16);
                s32 top = (s16)(viGetViewTop() << 16);
                arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                    left + viewleft_offset + 0x40, top + 0x56,
                    g_playerPlayerData[2].kill_counts[player_num], RED_HIGHLIGHT);
            }
            ...
            if (player_count == 4) {
                if (player_num != 3) {
                    ...
                    arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                        left + viewleft_offset + 0x60, top + 0x56,
                        g_playerPlayerData[player_num].kill_counts[3], GREEN_NORMAL);
                } else if (g_playerPlayerData[3].kill_counts[player_num] > 0) {
                    s32 left = (s16)(viGetViewLeft() << 16);
                    s32 top = (s16)(viGetViewTop() << 16);
                    arg0 = display_text_for_playerdata_on_MP_menu(arg0,
                        left + viewleft_offset + 0x60, top + 0x56,
                        g_playerPlayerData[3].kill_counts[player_num], RED_HIGHLIGHT);
                }
            }
        }
        goto render_exit_menu;
    ...
    if (g_CurrentPlayer->mpmenumode == MENU_GOWOC) {
        x_offset = viewleft_offset;
        if (player_count >= 3) {
            if (player_num & 1) {
                x_offset = viewleft_offset + 7;
            } else {
                x_offset = viewleft_offset - 7;
            }
        }

        text = langGet(getStringID(LMPMENU, MPMENU_STR_1F_WEAPONOFCHOICE));
        textMeasure(&text_height2, &text_width2, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + x_offset - (text_width2 >> 1) + 0x50;
        y = viGetViewTop() - (text_height2 >> 1) + 0x25;
        viX = viGetX();
        arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                          0x00FF00B0, viX, viGetY(), 0, 0);

        text = (u8 *)frontGetPlayersFavoriteWeaponInHand(player_num, 0);
        textMeasure(&text_height2, &text_width2, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + x_offset - (text_width2 >> 1) + 0x50;
        {
            s32 top_val = (s16)(viGetViewTop() << 16);
            if (j_text_trigger) {
                i = 4;
            } else {
                i = 0;
            }
            y = i + top_val - (text_height2 >> 1) + 0x35;
        }
        viX = viGetX();
        arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                          0x00FF00B0, viX, viGetY(), 0, 0);

        if (g_CurrentPlayer->ptr_text_first_mp_award != 0) {
            text = (u8 *)(uintptr_t)g_CurrentPlayer->ptr_text_first_mp_award;
            textMeasure(&text_height2, &text_width2, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
            x = viGetViewLeft() + x_offset - (text_width2 >> 1) + 0x50;
            y = viGetViewTop() - (text_height2 >> 1) + 0x4b;
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);
        }

        if (g_CurrentPlayer->ptr_text_second_mp_award != 0) {
            text = (u8 *)(uintptr_t)g_CurrentPlayer->ptr_text_second_mp_award;
            textMeasure(&text_height2, &text_width2, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
            x = viGetViewLeft() + x_offset - (text_width2 >> 1) + 0x50;
            y = viGetViewTop() - (text_height2 >> 1) + 0x58;
            viX = viGetX();
            arg0 = textRender(arg0, &x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic,
                              0x00FF00B0, viX, viGetY(), 0, 0);
        }
...
render_exit_menu:
    if (g_CurrentPlayer->mpmenumode == MENU_6) {
        text = langGet(getStringID(LMPMENU, MPMENU_STR_20_CANCEL));
        textMeasure(&text_height, &text_width, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x36;
        y = viGetViewTop() - (text_height >> 1) + 0x36;
        ...
        text = langGet(getStringID(LMPMENU, MPMENU_STR_21_CONFIRM));
        textMeasure(&text_height, &text_width, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = viGetViewLeft() + viewleft_offset - (text_width >> 1) + 0x68;
        y = viGetViewTop() - (text_height >> 1) + 0x36;
...
health_bar_section:
...

    {
        ...
        if (player_count > 0) {
            j = 0;
            if ((player_count & 3) != 0) {
                s32 rem = player_count & 3;
                struct player_data *pd = &g_playerPlayerData[0];
                do {
                    total_kills += pd->kill_counts[player_num];
                    pd++;
                    j++;
                } while (j != rem);
                if (j == player_count) {
            ...
            {
                struct player_data *pd = &g_playerPlayerData[j];
                do {
                    total_kills += pd[0].kill_counts[player_num];
                    total_kills += pd[1].kill_counts[player_num];
                    total_kills += pd[2].kill_counts[player_num];
                    total_kills += pd[3].kill_counts[player_num];
                    j += 4;
                    pd += 4;
    ...
    after_sum:
        if (get_scenario() == SCENARIO_YOLT) {
            if (total_kills >= 2) {
                return arg0;
            }
        }

        arg0 = bgScissorCurrentPlayerViewDefault(arg0);
        arg0 = microcode_constructor(arg0);

        press_start_text = langGet(getStringID(LMPMENU, MPMENU_STR_22_PRESSSTART_LF));
        textMeasure(&sp64, &sp68, press_start_text, ptrFontBankGothicChars, ptrFontBankGothic, 0);

        view_tmp = viGetViewLeft();
        x2 = (viGetViewWidth() >> 1) + view_tmp - (sp68 >> 1);
        view_tmp = viGetViewTop();
        y2 = (viGetViewHeight() >> 1) + view_tmp - (sp64 >> 1);

        viX = viGetX();
        arg0 = textRender(arg0, &x2, &y2, press_start_text,
                          ptrFontBankGothicChars, ptrFontBankGothic,
                          0x00FF00B0, viX, viGetY(), 0, 0);
        arg0 = combiner_bayer_lod_perspective(arg0);
    }
