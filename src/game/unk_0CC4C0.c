...
#ifdef NONMATCHING
Gfx *texWriteTileLods(Gfx *gdl, struct tex *tex, s32 smode, s32 tmode, s32 offset, s32 startlod, s32 tmembase) {
    s32 endlod;
    s32 lod;
    s32 currentTile;
    s32 masks;
    ...
    endlod = tex->maxlod + startlod;

    if (startlod < endlod) {
        lod = startlod - startlod;
        currentTile = startlod;
        ...
        do {
            ...
            sizeInBytes = texGetSizeInBytes(tex, lod);

            unk0c_02 = tex->unk0c_02;

            if (texSetLutMode(tex->lutmodeindex << G_MDSFT_TEXTLUT)) {
                gDPSetTextureLUT(gdl++, tex->lutmodeindex << G_MDSFT_TEXTLUT);
            }

            if (texTrySetTileState(currentTile, tex->gbiformat, tex->depth, line, tmembase, smode, tmode, masks, maskt, lod, lod)) {
                gDPSetTile(gdl++, tex->gbiformat, tex->depth, line, tmembase, currentTile, 0,
                    texModeToGbiMode(tmode), maskt, lod,
                    texModeToGbiMode(smode), masks, lod);
            }

            uls = (offset == 2 && !unk0c_02) ? 2 : 0;
            ult = (offset == 2 && !unk0c_02) ? 2 : 0;

            width = texGetWidthAtLod(tex, lod);
            lrs = ((offset == 2 && !unk0c_02) ? 2 : 0) + ((width - 1) << 2);

            height = texGetHeightAtLod(tex, lod);
            lrt = ((offset == 2 && !unk0c_02) ? 2 : 0) + ((height - 1) << 2);

            if (texTrySetTileSize(currentTile, uls, ult, lrs, lrt)) {
                gDPSetTileSize(gdl++, currentTile, uls, ult, lrs, lrt);
            }

            lod++;
            currentTile++;
            tmembase += sizeInBytes;
        } while (currentTile != endlod);
    }
...
Gfx *texWriteLoadToTmemZero(Gfx *gdl, struct tex *tex) {
	...
	if (tex->lutmodeindex == 0) {
		...
		if (texTrySetTileState(7, tex->gbiformat, depth, 0, 0, 0, 0, 0, 0, 0, 0)) {
			gDPSetTile(gdl++, tex->gbiformat, depth, 0, 0, 7, 0,
					0, 0, 0, 0, 0, 0);
		}
		...
		a2 = (u32)(0x3ff - tex->unk0a) < (u32)len ? (0x3ff - tex->unk0a) : 0;
		tmp = len - a2;

		if (texTrySetTileState(7, 0, 0, 1, 256, 0, 0, 0, 0, 0, 0)) {
			gDPSetTile(gdl++, 0, 0, 1, 256, 7, 0, 0, 0, 0, 0, 0, 0);
		}

		gDPLoadSync(gdl++);
		gDPLoadTLUT07(gdl++, tmp, a2, tex->unk0a + tmp, a2);
		gDPPipeSync(gdl++);
	}
...
#ifdef NONMATCHING
s32 texLoadFromGdl(Gfx *src, s32 size, Gfx *dst, struct texpool *pool)
{
    s32 count;
    Gfx *gdl;
    struct tex *tex;
    ...
    s32 shiftt;
    s32 min;
    struct LvlMpUnknown *entry;
    struct LvlMpUnknown *tbl;
    s32 secondTexNum;
    s8 mwIdx;
    ...
    while (count > 0) {
        ...
        if (cmdByte >= 0xC1) {
            if (cmdByte == 0xE7) {
                pipeSyncWritten = 1;
                gdl->words.w0 = src->words.w0;
                gdl->words.w1 = src->words.w1;
                gdl++;
                src++;
            } else {
                gdl->words.w0 = src->words.w0;
            ...
            switch (cmdByte) {
                case 0xC0:
                    ...
                    if (isLight != 0) {
                        save_ptrDL_enpoint_to_current_init_lightfixture_table(gdl);
                        isLight = 0;
                    }

                    texturenum = src->words.w1 & 0xFFF;

                    if (texturenum == D_800483C4 && D_800483C8 != NULL) {
                        entry = D_800483C8 + texturenum;
                        if (entry->unk_0 < 0xFF) {
                            hasOverride = 1;
                        } else {
                            hasOverride = 0;
                    ...
                    gdl = texWriteTextureCmd(gdl, prevTexGdl, tex, firstTexCmd);
                    firstTexCmd = 0;
                    ...
                    switch (word0 & 7) {
                        case 0:
                            ...
                            tbl = D_800483C8;
                            if (tbl != NULL) {
                                entry = tbl + texturenum;
                                if (entry->unk_0 == 0xFF) {
                                    entry->unk_0 = min;
                                    entry->unk_1 = (entry->unk_1 & 0x1F) | ((15 - shifts) << 5);
                                    entry->unk_1 = (entry->unk_1 & 0xE3) | (((15 - shiftt) << 2) & 0x1C);
                                }
                            }

                            if (hasOverride) {
                                entry = D_800483C8 + texturenum;
                                shifts = 15 - (entry->unk_1 >> 5);
                                shiftt = 15 - ((*(u32 *)entry << 11) >> 29);
                                min = entry->unk_0;
                                gdl = texHandleType0(gdl, tex, smode, tmode, offset, shifts, shiftt, min);
                                gdl->words.w0 = 0xBA001402;
                                gdl->words.w1 = 0x00100000;
                                (gdl + 1)->words.w0 = 0xBA001001;
                                (gdl + 1)->words.w1 = 0x00010000;
                                (gdl + 2)->words.w0 = 0xBA001102;
                                (gdl + 2)->words.w1 = 0x00040000;
                                gdl += 3;
                            } else {
                                gdl = texHandleType0(gdl, tex, smode, tmode, offset, shifts, shiftt, min);
                            }
                        ...
                        case 1:
                            secondTexNum = (src->words.w1 >> 12) & 0xFFF;
                            texLoadFromTextureNum(secondTexNum, pool);
                            {
                                struct tex *tex2;
                                tex2 = texFindInPool(secondTexNum, pool);
                                if (tex2 != NULL) {
                                    word0 = src->words.w0;
                                    word1 = src->words.w1;
                                    smode = (word0 >> 22) & 3;
                                    ...
                                    shifts = (word0 >> 14) & 0xF;
                                    shiftt = (word0 >> 10) & 0xF;
                                    gdl = texHandleType1(gdl, tex, smode, tmode, offset, tex2, shifts, shiftt, min);
                                }
                            }
                            break;
                        ...
                        case 2:
                            ...
                            if (hasOverride) {
                                ...
                                gdl = texHandleType2(gdl, tex, smode, tmode, offset);
                            }
                            break;
                        ...
                        case 3:
                            ...
                            tmode = (word0 >> 20) & 3;
                            offset = (word0 >> 18) & 3;
                            gdl = texHandleType3(gdl, tex, smode, tmode, offset);
                            break;
                        ...
                        case 4:
                            ...
                            tmode = (word0 >> 20) & 3;
                            offset = (word0 >> 18) & 3;
                            gdl = texHandleType4(gdl, tex, smode, tmode, offset);
                            break;
                    ...
                    if (texturenum == 1508) {
                        gdl = sub_GAME_7F09343C(gdl, 1);
                        sub_GAME_7F0CC4C8();
                    }
                    if (texturenum == 1511) {
                        gdl = sub_GAME_7F09365C(gdl, 1);
                        sub_GAME_7F0CC4C8();
                    }

                    if (check_if_imageID_is_light(texturenum)) {
                        isLight = 1;
                        add_entry_to_init_lightfixture_table(gdl);
                    }
                ...
                case 0xBA:
                    if (hasOverride == 0) {
                        ...
                        mwIdx = *((s8 *)src + 2);
                        if (mwIdx == 0x11 || mwIdx == 0x14 || mwIdx == 0x10) {
                            src++;
                        } else {
                            gdl->words.w0 = src->words.w0;
                            gdl->words.w1 = src->words.w1;
    ...
    return (s32)((uintptr_t)gdl - (uintptr_t)dst);
}
#else
GLOBAL_ASM(
.late_rodata
