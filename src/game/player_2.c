...
#endif
#ifdef NATIVE_PORT
    /* On 64-bit, struct hand is larger than the N64 u32 array due to pointer
     * field expansion (audioHandle, rocket, field_B68..B74 → uintptr_t).
     * The raw u32 data at D_8003FDA0 uses N64 struct layout with 4-byte pointers,
     * so casting to struct hand* reads past the array bounds.
     * Zero-init and set key non-zero defaults manually. */
    memset(&default_hand, 0, sizeof(default_hand));
    default_hand.weaponnum_watchmenu = (ITEM_IDS)(-1);
    default_hand.field_87E = 1;
    /* Identity matrix for field_8EC (weapon matrix) */
    default_hand.field_8EC.m[0][0] = 1.0f;
    default_hand.field_8EC.m[1][1] = 1.0f;
    default_hand.field_8EC.m[2][2] = 1.0f;
    default_hand.field_8EC.m[3][3] = 1.0f;
    /* Blend defaults */
    default_hand.blendpos[0].z = -1.0f;
    default_hand.blendlook[0].y = 1.0f;
    /* Negated identity axes for blend arrays */
    default_hand.blendpos[1].x = -1.0f;
    default_hand.blendpos[2].y = -1.0f;
    default_hand.blendpos[3].z = -1.0f;
    default_hand.blendlook[1].x = 1.0f;
    default_hand.blendlook[2].y = 1.0f;
    default_hand.blendlook[3].z = 1.0f;
    default_hand.blendup[0].y = 1.0f;
    default_hand.blendscale = 1.0f;
    default_hand.blendscale1 = 1.0f;
    /* field_944 = 1000.0f (weapon range?) */
    default_hand.field_944 = 1000.0f;
    /* field_948 = 0xFF000000 (color mask?) */
    default_hand.field_948 = 0xFF000000;
    /* Identity matrix for throw_item_pos_related */
    default_hand.throw_item_pos_related.m[0][0] = 1.0f;
    default_hand.throw_item_pos_related.m[1][1] = 1.0f;
    default_hand.throw_item_pos_related.m[2][2] = 1.0f;
    default_hand.throw_item_pos_related.m[3][3] = 1.0f;
#else
    default_hand = *((struct hand*)D_8003FDA0);
#endif
#ifdef NATIVE_PORT
    /* On 64-bit, the player struct is larger than N64's 0x2A80 bytes due to
     * pointer fields expanding from 4 to 8 bytes. Use sizeof. */
...
#ifdef NATIVE_PORT
    g_playerPointers[player_num]->watch_model = NULL;
    g_playerPointers[player_num]->watch_rwdata = NULL;
#endif
