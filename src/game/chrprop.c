...
#include <string.h>
#include <stdarg.h>
/* Forward declarations to fix -Wimplicit-function-declaration on 64-bit */
extern bool bgTestLineIntersectsBbox(coord3d *arg0, coord3d *arg1, s32 *arg2, s32 *arg3);
extern s32 check_if_imageID_is_light(s32 imageID);
extern s32 currentPlayerGetCrouchPos(void);
extern bool doorTestForInteract(PropRecord *prop);
extern int getCurrentWeaponOrItem(void);
extern s32 getMaxNumRooms(void);
extern void handle_alarm_gas_timer_calldamage(void);
extern void loop_set_sound_effect_all_slots(void);
extern s32 object_collectability_routines(PropRecord *prop);
extern s32 object_interaction(struct PropRecord *arg0);
extern bool objTestForInteract(PropRecord *prop);
extern s32 playerTickBeams(PropRecord *prop);
extern s32 propdoorInteract(PropRecord *doorprop);
extern s32 redirect_object_collectability_routines(PropRecord *arg0);
extern void sub_GAME_7F050DE8(Model *model);
extern void sub_GAME_7F062B00(ChrRecord_f180 *arg0);
extern s32 sub_GAME_7F0B8E98(s32 *arg0, s32 arg1);
extern void sub_GAME_7F0BA2D4(void *arg0, void *arg1, s32 *arg2, s32 *arg3, s32 arg4);
extern f32 level_scale;
extern f32 getShortest2dDispToInfTileEdge(StandTile *tile, s32 index, f32 p_x, f32 p_z);
extern f32 distToTilePnt2D(StandTile *tile, int pntI, f32 p_x, f32 p_z);
extern s32 sub_GAME_7F0B0198(StandTile *tile, s32 index, f32 px, f32 pz);
extern int g_frame_count_diag;
extern int g_tri_count_diag;
extern int g_deterministic;
extern s32 portTraceBeginShot(void);
extern void portTraceEndShot(s32 shot_id);
extern void portTraceShotEvent(const char *phase,
                               s32 hand,
                               s32 weapon,
                               s32 slot,
...
extern void portTraceForcedGuardHitEvent(s32 chrnum, s32 hitpart, const char *source,
                                         s32 collision, s32 texture, s32 mtx_index,
                                         s32 queue_node, s32 hit_node, s32 model,
                                         const coord3d *origin, const coord3d *point,
                                         const coord3d *dir, const coord3d *world_origin,
                                         const coord3d *world_point, const coord3d *world_dir);
...
//CODE.bss:80069C30
s16 * ptr_list_object_lookup_indices = NULL;

//CODE.bss:80069C34
u32 num_obj_position_data_entries = 0;

/**
 * Address 0x80069C38.
*/
PropRecord pos_data_entry[POS_DATA_ENTRY_LEN] = {0};

//CODE.bss:80071618
s16 *RoomPropListChunkIndexes = NULL;

//CODE.bss:8007161C
struct roomproplistchunk *RoomPropListChunks = NULL;
 ...
 * Address 0x80071620.
*/
PropRecord *g_OnScreenPropList[ONSCREEN_PROP_LIST_LEN] = {0};

/**
 * Pointer to last onscreen prop.
 * Address 0x80071DF0.
*/
PropRecord **g_LastOnScreenProp = NULL;

#ifdef NATIVE_PORT
static int g_ChrPropTraceEnabled = -1;
static int g_ChrPropTraceBudget = -1;
static int g_InteractTraceEnabled = -1;
static int g_InteractTraceBudget = -1;
static int g_PropTriTraceEnabled = -1;
static int g_PropTriTraceBudget = -1;
static int g_PropTriTraceAfterFrame = -2;
static int g_ShotBgTraceEnabled = -1;
 ...
 * visible and must stay overridable. */
static int chrpropAutogunBeamTickEnabled(void)
{
    const char *value;

    if (g_AutogunBeamTickEnabled >= 0) {
        return g_AutogunBeamTickEnabled;
    }

    value = getenv("GE007_AUTOGUN_BEAM_TICK");
    g_AutogunBeamTickEnabled = (value != NULL && value[0] == '0') ? 0 : 1;

    return g_AutogunBeamTickEnabled;
}

static int chrpropTraceEnabled(void)
{
    const char *value;

    if (g_ChrPropTraceEnabled >= 0) {
        return g_ChrPropTraceEnabled;
    }

    value = getenv("GE007_DOOR_TRACE");
    g_ChrPropTraceEnabled = (value != NULL && value[0] != '\0' && value[0] != '0') ? 1 : 0;

    value = getenv("GE007_DOOR_TRACE_BUDGET");
    g_ChrPropTraceBudget = (value != NULL && value[0] != '\0') ? atoi(value) : 400;

    return g_ChrPropTraceEnabled;
}

static void chrpropTracePrintf(const char *fmt, ...)
{
    va_list ap;

    if (!chrpropTraceEnabled()) {
        return;
    }

    if (g_ChrPropTraceBudget == 0) {
        return;
    }

    if (g_ChrPropTraceBudget > 0) {
        g_ChrPropTraceBudget--;
    }

    va_start(ap, fmt);
    fprintf(stderr, "[CHRPROP_TRACE] ");
...
static int shotBgTraceEnabled(void)
{
    const char *value;

    if (g_ShotBgTraceEnabled >= 0) {
        return g_ShotBgTraceEnabled;
    }

    value = getenv("GE007_TRACE_SHOT_BG");
    g_ShotBgTraceEnabled = (value != NULL && value[0] != '\0' && value[0] != '0') ? 1 : 0;

    value = getenv("GE007_TRACE_SHOT_BG_BUDGET");
    g_ShotBgTraceBudget = (value != NULL && value[0] != '\0') ? atoi(value) : 200;

    return g_ShotBgTraceEnabled;
}

static void shotBgTracePrintf(const char *fmt, ...)
{
    va_list ap;

    if (!shotBgTraceEnabled()) {
        return;
    }

    if (g_ShotBgTraceBudget == 0) {
        return;
    }
...
static int propTriTraceEnabled(void)
{
    const char *value;

    if (g_PropTriTraceEnabled >= 0) {
        return g_PropTriTraceEnabled;
    }

    value = getenv("GE007_TRACE_PROP_TRIS");
    g_PropTriTraceEnabled = (value != NULL && value[0] != '\0' && value[0] != '0') ? 1 : 0;

    value = getenv("GE007_TRACE_PROP_TRIS_BUDGET");
    g_PropTriTraceBudget = (value != NULL && value[0] != '\0') ? atoi(value) : 400;

    value = getenv("GE007_TRACE_PROP_TRIS_AFTER_FRAME");
    g_PropTriTraceAfterFrame = (value != NULL && value[0] != '\0') ? atoi(value) : 0;

    return g_PropTriTraceEnabled;
}

static void propTriTracePrintf(s32 roomid, s32 renderpass, PropRecord *prop, s32 withalpha, s32 delta)
{
    s32 obj_type = -1;
    s32 pad = -1;
    void *model = NULL;

    if (!propTriTraceEnabled()) {
        return;
    }

    if (g_frame_count_diag < g_PropTriTraceAfterFrame) {
        return;
    }

    if (g_PropTriTraceBudget == 0) {
        return;
    }

    if (g_PropTriTraceBudget > 0) {
        g_PropTriTraceBudget--;
    }

    if ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) {
        obj_type = prop->obj->type;
        pad = prop->obj->pad;
        model = prop->obj->model;
    }

    fprintf(stderr,
            "[PROP-TRIS] frame=%d room=%d pass=%d type=%d obj=%d pad=%d alpha=%d "
            "flags=0x%02x delta=%d z=%.2f prop=%p stan_room=%d model=%p\n",
            g_frame_count_diag,
            roomid,
            renderpass,
            (int)prop->type,
...
static int interactTraceEnabled(void)
{
    ...
    if (g_InteractTraceEnabled >= 0) {
        return g_InteractTraceEnabled;
    }

    value = getenv("GE007_INTERACT_TRACE");
    g_InteractTraceEnabled = (value != NULL && value[0] != '\0' && value[0] != '0') ? 1 : 0;

    value = getenv("GE007_INTERACT_TRACE_BUDGET");
    g_InteractTraceBudget = (value != NULL && value[0] != '\0') ? atoi(value) : 300;

    return g_InteractTraceEnabled;
}

static void interactTracePrintf(const char *fmt, ...)
{
    va_list ap;

    if (!interactTraceEnabled()) {
        return;
    }

    if (g_InteractTraceBudget == 0) {
        return;
    }

    if (g_InteractTraceBudget > 0) {
        g_InteractTraceBudget--;
    }

    va_start(ap, fmt);
    fprintf(stderr, "[INTERACT_TRACE] ");
    vfprintf(stderr, fmt, ap);
    fprintf(stderr, "\n");
    va_end(ap);
    fflush(stderr);
    ...
    for (; prop != NULL; prop = prop->prev)
    {
        ...
        if ((prop->flags & (PROPFLAG_ENABLED | PROPFLAG_ONSCREEN)) == (PROPFLAG_ENABLED | PROPFLAG_ONSCREEN))
        {
            ...
            if (prop->type == PROP_TYPE_DOOR) {
                door_count++;
            } else if (prop->type == PROP_TYPE_CHR || prop->type == PROP_TYPE_VIEWER) {
                chr_count++;
...
#ifdef NATIVE_PORT
    chrpropTracePrintf("onscreen total=%d doors=%d objs=%d chrs=%d", count, door_count, obj_count, chr_count);
#endif

    for (i=0; i<count; i++)
    {
        max_idx = -1;
        max_depth = -4.2949673e9f;
        ...
        if (max_idx >= 0)
        {
            ...
            g_OnScreenPropList[i] = g_OnScreenPropList[max_idx];
            g_OnScreenPropList[max_idx] = prop;
        }
    }
}
...
#ifdef NATIVE_PORT
    if (prop && prop->type == PROP_TYPE_DOOR) {
        chrpropTracePrintf("activate door prop=%p cur=%p first=%p flags=0x%02x",
                (void *)prop,
                (void *)ptr_obj_pos_list_current_entry,
                (void *)ptr_obj_pos_list_first_entry,
                (unsigned int)prop->flags);
    }
#endif
    if (cur != NULL)
...
#ifdef NATIVE_PORT
        if (prop && prop->type == PROP_TYPE_DOOR) {
            chrpropTracePrintf("activate-linked door prop=%p prev=%p next=%p current=%p",
                    (void *)prop, (void *)prop->prev, (void *)prop->next,
                    (void *)ptr_obj_pos_list_current_entry);
        }
#endif
        return;
...
#ifdef NATIVE_PORT
    if (prop && prop->type == PROP_TYPE_DOOR) {
        chrpropTracePrintf("activate-first door prop=%p current=%p",
                (void *)prop, (void *)ptr_obj_pos_list_current_entry);
    }
#endif
}
...
#ifdef NATIVE_PORT
    if (prop && prop->type == PROP_TYPE_DOOR) {
        chrpropTracePrintf("delist door prop=%p prev=%p next=%p current=%p first=%p",
                (void *)prop,
                (void *)prop->prev,
                (void *)prop->next,
                (void *)ptr_obj_pos_list_current_entry,
...
Gfx *chrpropRender(Gfx * gdl, PropRecord *prop, s32 withalpha)
{
...
#ifdef NATIVE_PORT
    chrpropTracePrintf("render-dispatch prop=%p type=%d withalpha=%d flags=0x%02x room=%d",
            (void *)prop, type, withalpha, (unsigned int)prop->flags,
            prop->stan ? prop->stan->room : -1);
#endif
    ...
    {
#ifdef NATIVE_PORT
        if (prop->chr != NULL && !chrpropNativeShouldRenderViewerModel()) {
            return gdl;
        }
#endif
    ...
    if ((renderpass == 0) || (renderpass == 2))
    {
        for (pp = g_LastOnScreenProp; --pp >= g_OnScreenPropList; )
        {
            ...
            if (prop != NULL)
            {
                ...
                {
#ifdef NATIVE_PORT
                    s32 tris_before_prop = g_tri_count_diag;
                    s32 obj_type = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->type : -1;
                    s32 pad = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->pad : -1;
                    u32 obj_flags = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags : 0;
                    u32 obj_flags2 = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags2 : 0;
                    void *model = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->model : NULL;
                    if (prop->type == PROP_TYPE_DOOR) {
                        chrpropTracePrintf("render-pass room=%d pass=%d door-prop=%p flags=0x%02x",
                                roomid, renderpass, (void *)prop, (unsigned int)prop->flags);
                    }
                    gfx_set_prop_context(prop, prop->type, obj_type, pad, obj_flags, obj_flags2,
                                         model, prop->stan ? prop->stan->room : -1,
                                         roomid, renderpass, 0);
#endif
                    gdl = chrpropRender(gdl, prop, 0);
#ifdef NATIVE_PORT
                    gfx_clear_prop_context();
                    if (g_tri_count_diag != tris_before_prop) {
                        propTriTracePrintf(roomid, renderpass, prop, 0, g_tri_count_diag - tris_before_prop);
                    }
#endif
                }
                }
        ...
        for (pp = g_OnScreenPropList; pp < g_LastOnScreenProp; pp++)
        {
            ...
            if (prop != NULL)
            {
                ...
                if (flag)
                {
                    ...
                    {
#ifdef NATIVE_PORT
                        s32 tris_before_alpha_prop = g_tri_count_diag;
                        s32 obj_type = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->type : -1;
                        s32 pad = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->pad : -1;
                        u32 obj_flags = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags : 0;
                        u32 obj_flags2 = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags2 : 0;
                        void *model = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->model : NULL;
                        if (prop->type == PROP_TYPE_DOOR) {
                            chrpropTracePrintf("render-pass room=%d pass=%d alpha-door=%p flags=0x%02x",
                                    roomid, renderpass, (void *)prop, (unsigned int)prop->flags);
                        }
                        gfx_set_prop_context(prop, prop->type, obj_type, pad, obj_flags, obj_flags2,
                                             model, prop->stan ? prop->stan->room : -1,
...
#ifdef NATIVE_PORT
                        gfx_clear_prop_context();
                        if (g_tri_count_diag != tris_before_alpha_prop) {
                            propTriTracePrintf(roomid, renderpass, prop, 0, g_tri_count_diag - tris_before_alpha_prop);
                        }
#endif
                    }

#ifdef NATIVE_PORT
                    s32 tris_before_opaque_prop = g_tri_count_diag;
                    s32 obj_type = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->type : -1;
                    s32 pad = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->pad : -1;
                    u32 obj_flags = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags : 0;
                    u32 obj_flags2 = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->flags2 : 0;
                    void *model = ((prop->type == PROP_TYPE_OBJ) || (prop->type == PROP_TYPE_WEAPON) || (prop->type == PROP_TYPE_DOOR)) ? prop->obj->model : NULL;
                    if (prop->type == PROP_TYPE_DOOR) {
                        chrpropTracePrintf("render-pass room=%d pass=%d opaque-door=%p flags=0x%02x",
                                roomid, renderpass, (void *)prop, (unsigned int)prop->flags);
                    }
                    gfx_set_prop_context(prop, prop->type, obj_type, pad, obj_flags, obj_flags2,
                                         model, prop->stan ? prop->stan->room : -1,
...
#ifdef NATIVE_PORT
                    gfx_clear_prop_context();
                    if (g_tri_count_diag != tris_before_opaque_prop) {
                        propTriTracePrintf(roomid, renderpass, prop, 1, g_tri_count_diag - tris_before_opaque_prop);
                    }
#endif
                }
            }
...
#ifdef NONMATCHING
void sub_GAME_7F03AA44(s32 room_arg, coord3d *pos1, coord3d *pos2, u8 *occluded_rooms) {
    f32 room_float;
    f32 vis_scale;
    f32 scale_factor;
    coord3d delta;
    ...
    room_float = get_room_data_float1();
    ...
    scale_factor = vis_scale * room_float;

    scaled.x = pos1->x * scale_factor;
    scaled.y = pos1->y * scale_factor;
    scaled.z = pos1->z * scale_factor;

    i = 1;
    if (getMaxNumRooms() >= 2) {
        while (i < getMaxNumRooms()) {
            if (occluded_rooms[i] == 0) {
                if (sub_GAME_7F03A97C(i, &scaled, &delta) == 0) {
                    occluded_rooms[i] = 1;
                }
            }
            i++;
...
#ifdef NONMATCHING
#ifndef NATIVE_PORT
extern s32 bgTestBulletHitBackground(void *arg0, void *arg1, s32 room, void *arg3);
#endif
s32 sub_GAME_7F03AB58(coord3d *src_pos, coord3d *dest_pos, coord3d *direction, coord3d *scaled_pos, u8 *visited,
#ifdef NATIVE_PORT
    BgHitRecord *hit_output,
#else
    u8 *hit_output,
#endif
    s32 best_room) {
    ...
    s32 count;
    s32 i;
    f32 hit_x, hit_y, hit_z;
    f32 src_x, src_y, src_z;
    ...
    count = sub_GAME_7F0B8E98(rooms, 100);

    for (i = 0; i < count; i++) {
        room_id = rooms[i];
        if (visited[room_id] != 0) {
            continue;
        }
        visited[room_id] = 1;

        if (sub_GAME_7F03A97C(room_id, scaled_pos, direction) == 0) {
            continue;
        }

#ifdef NATIVE_PORT
        if (bgTestBulletHitBackground(src_pos, dest_pos, room_id, &hit_buf) == 0) {
#else
        if (bgTestBulletHitBackground(src_pos, dest_pos, room_id, hit_buf) == 0) {
#endif
            continue;
...
#ifdef NATIVE_PORT
        hit_x = hit_buf.pos.x * scale;
        hit_y = hit_buf.pos.y * scale;
        hit_z = hit_buf.pos.z * scale;
        hit_buf.pos.x = hit_x;
        hit_buf.pos.y = hit_y;
        hit_buf.pos.z = hit_z;
#else
        hit_x = *(f32 *)(hit_buf + 0x00) * scale;
        hit_y = *(f32 *)(hit_buf + 0x04) * scale;
        hit_z = *(f32 *)(hit_buf + 0x08) * scale;
        *(f32 *)(hit_buf + 0x00) = hit_x;
        *(f32 *)(hit_buf + 0x04) = hit_y;
        *(f32 *)(hit_buf + 0x08) = hit_z;
#endif
        ...
        if (best_room > 0) {
            ...
            src_z = src_pos->z;
#ifdef NATIVE_PORT
            out_x = hit_output->pos.x;
            out_y = hit_output->pos.y;
            out_z = hit_output->pos.z;
#else
            out_x = *(f32 *)(hit_output + 0x00);
            out_y = *(f32 *)(hit_output + 0x04);
            out_z = *(f32 *)(hit_output + 0x08);
#endif

            if (src_x <= out_x) {
                if (hit_x < src_x || hit_x >= out_x) {
                    continue;
                }
            } else {
                if (hit_x <= out_x || hit_x > src_x) {
                    continue;
                }
            }

            if (src_y <= out_y) {
                if (hit_y < src_y || hit_y >= out_y) {
                    continue;
                }
            } else {
                if (hit_y <= out_y || hit_y > src_y) {
                    continue;
                }
            }

            if (src_z <= out_z) {
                if (hit_z < src_z || hit_z >= out_z) {
                    continue;
                }
            } else {
                if (hit_z <= out_z || hit_z > src_z) {
                    continue;
                }
            }
        }
        ...
        *hit_output = hit_buf;
#else
        src_words = (s32 *)hit_buf;
        dst_words = (s32 *)hit_output;
        for (j = 0; j < 12; j++) {
            dst_words[j] = src_words[j];
        }
#endif
    }
...
extern s32 bgTestBulletHitBackground(void *arg0, void *arg1, s32 room, void *arg3);
#endif
extern s32 sub_GAME_7F0B8EFC(s32 roomId, s32 *outArray, s32 maxCount);
s32 sub_GAME_7F03ADF4(s32 start_room, coord3d *arg1, coord3d *arg2, coord3d *delta_vec, coord3d *scaled_pos, u8 *occluded,
#ifdef NATIVE_PORT
    BgHitRecord *hit_output
#else
    void *hit_output
    ...
    room_list[0] = (u8)start_room;
    count = 1;

    for (iter = 0; iter < count; iter++) {
        current_room = room_list[iter];

        if (occluded[current_room] == 0) {
            occluded[current_room] = 1;

            if (sub_GAME_7F03A97C(current_room, scaled_pos, delta_vec) != 0) {
                if (bgTestBulletHitBackground(arg1, arg2, current_room, hit_output) != 0) {
                    return (s32)current_room;
                }
            }
        }

        num_adj = sub_GAME_7F0B8EFC(current_room, adj_rooms, 100);

        for (j = 0; j < num_adj; j++) {
            for (k = 0; k < count; k++) {
                if (room_list[k] == (u8)adj_rooms[j]) {
                    break;
                }
            }
            if (k == count) {
                room_list[count] = (u8)adj_rooms[j];
                count++;
            }
        }
    }
...
extern s32 bgTestBulletHitBackground(void *arg0, void *arg1, s32 room, void *arg3);
#endif
s32 sub_GAME_7F03AF5C(s32 unused, coord3d *src_pos, coord3d *dest_pos, coord3d *direction, coord3d *scaled_pos, u8 *visited_rooms,
#ifdef NATIVE_PORT
    BgHitRecord *hit_output
#else
    u8 *hit_output
    ...
    best_dist = 4294967296.0f;
    best_room = 0;
    scale = get_room_data_float2();

    if (getMaxNumRooms() >= 2) {
        for (room_id = 1; room_id < getMaxNumRooms(); room_id++) {
            if (visited_rooms[room_id] != 0) {
                continue;
            }
            visited_rooms[room_id] = 1;
            if (sub_GAME_7F03A97C(room_id, scaled_pos, direction) == 0) {
                continue;
            }
#ifdef NATIVE_PORT
...
#ifdef NATIVE_PORT
            dx = hit_buf.pos.x * scale - src_pos->x;
            dy = hit_buf.pos.y * scale - src_pos->y;
            dz = hit_buf.pos.z * scale - src_pos->z;
#else
            dx = *(f32 *)(hit_buf + 0x00) * scale - src_pos->x;
            dy = *(f32 *)(hit_buf + 0x04) * scale - src_pos->y;
            dz = *(f32 *)(hit_buf + 0x08) * scale - src_pos->z;
#endif
            dist_sq = dx * dx + dy * dy + dz * dz;
            adj_dist = dist_sq;

#ifdef NATIVE_PORT
            if (check_if_imageID_is_light(hit_buf.texturenum) != 0) {
#else
            if (check_if_imageID_is_light(*(s16 *)(hit_buf + 0x2A)) != 0) {
#endif
                adj_dist = dist_sq - 4.0f;
            }
            ...
            if (adj_dist < best_dist) {
                ...
                *hit_output = hit_buf;
#else
                *(f32 *)(hit_output + 0x00) = *(f32 *)(hit_buf + 0x00);
                *(f32 *)(hit_output + 0x04) = *(f32 *)(hit_buf + 0x04);
                *(f32 *)(hit_output + 0x08) = *(f32 *)(hit_buf + 0x08);
                *(f32 *)(hit_output + 0x0C) = *(f32 *)(hit_buf + 0x0C);
                *(f32 *)(hit_output + 0x10) = *(f32 *)(hit_buf + 0x10);
                *(f32 *)(hit_output + 0x14) = *(f32 *)(hit_buf + 0x14);
                *(s32 *)(hit_output + 0x18) = *(s32 *)(hit_buf + 0x18);
                *(s32 *)(hit_output + 0x1C) = *(s32 *)(hit_buf + 0x1C);
                *(s32 *)(hit_output + 0x20) = *(s32 *)(hit_buf + 0x20);
                *(s32 *)(hit_output + 0x24) = *(s32 *)(hit_buf + 0x24);
                *(s16 *)(hit_output + 0x28) = *(s16 *)(hit_buf + 0x28);
                *(s16 *)(hit_output + 0x2A) = *(s16 *)(hit_buf + 0x2A);
#endif
            }
        }
    }
...
extern Mtxf *currentPlayerGetMatrix10D4(void);
extern void mtx4TransformVecInPlace(Mtxf *matrix, coord3d *vector);
extern void mtx4RotateVecInPlace(Mtxf *matrix, coord3d *vector);
extern void bullet_path_from_screen_center(coord3d *pos, coord3d *dir, s32 hand);
extern bool handles_shot_actors(ChrRecord *self, s32 hitpart, coord3d *vector, s32 weaponid, bool isPlayer);
extern s32 sub_GAME_7F041074(coord3d *ray_origin, coord3d *ray_dir, coord3d *sphere_center, f32 sphere_radius);
extern void chrobjMaybeDetonateObjectIfFlags(ObjectRecord *obj, f32 damage, coord3d *pos, ITEM_IDS item, s32 owner);
extern f32 bondwalkItemGetDestructionAmount(ITEM_IDS item);
extern s32 get_cur_playernum(void);
#ifndef NATIVE_PORT
extern s32 bgTestBulletHitBackground(void *arg0, void *arg1, s32 room, void *arg3);
#endif
extern PropRecord *get_curplayer_positiondata(void);
extern s32 getTileRoom(StandTile *tile);
extern f32 get_room_data_float2(void);
extern void sub_GAME_7F067B4C(coord3d *pos);
extern void sub_GAME_7F022078(void *chr_ptr, s32 chr_num, coord3d *dir, coord3d *hit_pos);
extern s32 check_if_imageID_is_light(s32 imageID);
extern void sub_GAME_7F0BBE0C(Gfx *gfx, u32 tri_type, s32 room_index);
extern void *sub_GAME_7F0A3E1C(coord3d *arg0, s32 arg1, f32 arg2, s16 arg3);
extern void sub_GAME_7F022648(PropRecord *prop, struct ShotData *shotdata);
extern void sub_GAME_7F022980(struct ShotData *shotdata, ShotHitRecord *hit);
extern void sub_GAME_7F04E9BC(PropRecord *prop, struct ShotData *shotdata);
extern void sub_GAME_7F04EA68(struct ShotData *shotdata, ShotHitRecord *hit);
extern s32 sub_GAME_7F04D9B0(Model *model, ModelNode *nodearg, coord3d *arg2, coord3d *arg3, struct HitThing *hitthing, s32 *dstmtxindex, ModelNode **dstnode);
extern s32 modelFindNodeMtxIndex(ModelNode *node, s32 arg1);
extern s32 sub_GAME_7F06C010(void **head, uintptr_t unused1, uintptr_t unused2, uintptr_t *arg3, uintptr_t *arg4);
extern s32 probably_damage_detail_blood_effect_related(void **head, uintptr_t arg1, uintptr_t arg2, void **model_out, void **node_io);
extern void sub_GAME_7F0523F8(PropRecord *prop, s32 hand, void **dl_out);
extern s32 chrHatHitListEnabled(void); /* §4.4 item 3, defined in chr.c */
extern void sub_GAME_7F03B9C0(struct ShotData *shotdata, PropRecord *prop, f32 neg_z,
                              s32 hit_result, ModelNode *node, struct HitThing *hitthing,
                              s32 mtx_index, ModelNode *hit_node, Model *model,
                              s32 do_damage, s32 has_flag);
extern u8 bondwalkItemGetObjectsShootThrough(ITEM_IDS item);

#ifdef NATIVE_PORT
#define AUTO_GUARD_HIT_EVENTS_MAX 8

typedef struct AutoGuardHitEvent {
    int frame;
    int chrnum;
    int hitpart;
    int hand;
...
static AutoGuardHitEvent s_autoGuardHitEvents[AUTO_GUARD_HIT_EVENTS_MAX];
static int s_autoGuardHitInitialized = 0;
static int s_autoGuardHitCount = 0;

static int chraiParseForcedGuardHitPart(const char *text, char **endptr)
{
    if (text == NULL) {
        if (endptr != NULL) {
            *endptr = (char *)text;
    ...
    return (int)strtol(text, endptr, 0);
}

static void chraiInitForcedGuardHitEvents(void)
{
    const char *script;

    if (s_autoGuardHitInitialized) {
        return;
    }

    s_autoGuardHitInitialized = 1;
    memset(s_autoGuardHitEvents, 0, sizeof(s_autoGuardHitEvents));

    script = getenv("GE007_AUTO_GUARD_HIT_SCRIPT");
    if (script != NULL && *script != '\0' && !(script[0] == '0' && script[1] == '\0')) {
        while (*script != '\0' && s_autoGuardHitCount < AUTO_GUARD_HIT_EVENTS_MAX) {
            char *endptr;
            long frame;
            long chrnum;
            int hitpart;
            ...
            script = endptr + 1;
            chrnum = strtol(script, &endptr, 10);
            if (endptr == script || *endptr != ':') {
                break;
            }

            script = endptr + 1;
            hitpart = chraiParseForcedGuardHitPart(script, &endptr);
            if (endptr == script) {
                break;
            }
            ...
            if (*script == ':') {
                script++;
                hand = (int)strtol(script, &endptr, 10);
                if (endptr != script) {
                    script = endptr;
                }
            }

            s_autoGuardHitEvents[s_autoGuardHitCount].frame = (int)frame;
            s_autoGuardHitEvents[s_autoGuardHitCount].chrnum = (int)chrnum;
            s_autoGuardHitEvents[s_autoGuardHitCount].hitpart = hitpart;
            s_autoGuardHitEvents[s_autoGuardHitCount].hand = hand;
            s_autoGuardHitEvents[s_autoGuardHitCount].applied = 0;
            s_autoGuardHitCount++;
        }
    }

    if (s_autoGuardHitCount == 0) {
        const char *frame_env = getenv("GE007_AUTO_GUARD_HIT_FRAME");
        const char *chr_env = getenv("GE007_AUTO_GUARD_HIT_CHRNUM");
        const char *target_env = getenv("GE007_AUTO_GUARD_HIT_TARGET");
        const char *hand_env = getenv("GE007_AUTO_GUARD_HIT_HAND");
        int frame = -1;
        int chrnum = -1;
        int hitpart = HIT_NULL_PART;
        int hand = -1;
        ...
        if (target_env != NULL && *target_env != '\0') {
            hitpart = chraiParseForcedGuardHitPart(target_env, NULL);
        }

        if (hand_env != NULL && *hand_env != '\0') {
            hand = (int)strtol(hand_env, NULL, 10);
        }

        if (frame >= 0 && chrnum >= 0 && hitpart != HIT_NULL_PART) {
            s_autoGuardHitEvents[0].frame = frame;
            s_autoGuardHitEvents[0].chrnum = chrnum;
            s_autoGuardHitEvents[0].hitpart = hitpart;
            s_autoGuardHitEvents[0].hand = hand;
            s_autoGuardHitEvents[0].applied = 0;
            s_autoGuardHitCount = 1;
        }
    }
}

static s32 chraiForcedGuardHitFloatUsable(f32 value)
{
    return value == value && value > -1000000000000.0f && value < 1000000000000.0f;
}

static s32 chraiForcedGuardHitMtxUsable(Mtxf *mtx)
{
    f32 basis_sum;
    s32 row;
    s32 col;
    ...
    for (row = 0; row < 4; row++) {
        for (col = 0; col < 4; col++) {
            f32 value = mtx->m[row][col];

            if (!chraiForcedGuardHitFloatUsable(value)) {
                return 0;
            }

            if (row < 3 && col < 3) {
                basis_sum += value * value;
            }
        }
    }

    return basis_sum > 0.000001f;
}

static s32 chraiForcedGuardHitSampleIndex(void)
{
    const char *env = getenv("GE007_AUTO_GUARD_HIT_SAMPLE_INDEX");
...
static s32 chraiForcedGuardHitModelMtxUsable(Model *model, ModelNode *node)
{
    s32 mtx_index;

    if (model == NULL || node == NULL || model->render_pos == NULL) {
        return 0;
    }

    mtx_index = modelFindNodeMtxIndex(node, 0);
    if (mtx_index < 0) {
        return 0;
    }

    return chraiForcedGuardHitMtxUsable((Mtxf *)&model->render_pos[mtx_index]);
}

static s32 chraiTryMaterializeAttachedModelMtx(Model *model)
{
    ModelRenderData renderdata;
    Mtxf *attach_mtx;
    s32 matrix_count;

    if (model == NULL || model->obj == NULL || model->attachedto == NULL
            || model->attachedto_objinst == NULL) {
        return 0;
    }

    attach_mtx = modelFindNodeMtx(model->attachedto, model->attachedto_objinst, 0);
    if (!chraiForcedGuardHitMtxUsable(attach_mtx)) {
        return 0;
    }
...
static void chraiInitForcedGuardHitThing(struct HitThing *hitthing)
{
    if (hitthing == NULL) {
        return;
    }

    memset(hitthing, 0, sizeof(*hitthing));
    hitthing->unk0c.z = 1.0f;
    hitthing->texturenum = -1;
}

static s32 chraiBuildForcedGuardHitAimTrace(Model *model,
                                            struct HitThing *hitthing,
                                            s32 mtx_index,
                                            const struct ShotData *shotdata,
                                            coord3d *point,
                                            coord3d *dir,
                                            coord3d *world_point,
                                            coord3d *world_dir)
{
    ...
    if (model == NULL || model->render_pos == NULL || hitthing == NULL
            || shotdata == NULL || point == NULL || dir == NULL || world_point == NULL
            || world_dir == NULL || mtx_index < 0) {
        return 0;
    }

    *point = hitthing->unk00;
    mtx4TransformVecInPlace((Mtxf *)&model->render_pos[mtx_index], point);

    dir->x = point->x - shotdata->screen_pos.x;
    dir->y = point->y - shotdata->screen_pos.y;
    dir->z = point->z - shotdata->screen_pos.z;

    len_sq = dir->x * dir->x + dir->y * dir->y + dir->z * dir->z;
    if (len_sq <= 0.000001f) {
        return 0;
    }
    ...
    *world_point = *point;
    player_mtx = currentPlayerGetMatrix10D4();
    if (player_mtx == NULL) {
        return 0;
    }

    mtx4TransformVecInPlace(player_mtx, world_point);

    world_dir->x = world_point->x - shotdata->attacker_pos.x;
    world_dir->y = world_point->y - shotdata->attacker_pos.y;
    world_dir->z = world_point->z - shotdata->attacker_pos.z;

    len_sq = world_dir->x * world_dir->x
        + world_dir->y * world_dir->y
...
static s32 chraiTryForcedGuardHitSampledAim(Model *model,
                                            ModelNode *queue_node,
                                            coord3d *screen_pos,
                                            s32 sample_index,
                                            struct HitThing *hitthing,
                                            s32 *dstmtxindex,
                                            ModelNode **dsthitnode)
{
    struct HitThing sample_hitthing;
    s32 sample_mtx_index;
    ModelNode *sample_hit_node;
    coord3d aim_point;
    coord3d aim_dir;
    f32 len_sq;

    if (model == NULL || queue_node == NULL || screen_pos == NULL
            || hitthing == NULL || dstmtxindex == NULL || dsthitnode == NULL) {
        return 0;
    }

    sample_mtx_index = 0;
    sample_hit_node = NULL;
    chraiInitForcedGuardHitThing(&sample_hitthing);
    if (!chrobjTrySampleModelCollisionMaterialAtIndex(model,
                                                      queue_node,
                                                      sample_index,
                                                      &sample_hitthing,
                                                      &sample_mtx_index,
    ...
    aim_point = sample_hitthing.unk00;
    mtx4TransformVecInPlace((Mtxf *)&model->render_pos[sample_mtx_index], &aim_point);

    aim_dir.x = aim_point.x - screen_pos->x;
    aim_dir.y = aim_point.y - screen_pos->y;
    aim_dir.z = aim_point.z - screen_pos->z;
    len_sq = aim_dir.x * aim_dir.x + aim_dir.y * aim_dir.y + aim_dir.z * aim_dir.z;

    if (len_sq > 0.000001f) {
        struct HitThing aimed_hitthing;
        s32 aimed_mtx_index;
        ModelNode *aimed_hit_node;
        f32 inv_len;
        ...
        chraiInitForcedGuardHitThing(&aimed_hitthing);
        aimed_mtx_index = 0;
        aimed_hit_node = NULL;
        if (sub_GAME_7F04D9B0(model,
                              queue_node,
                              screen_pos,
                              &aim_dir,
                              &aimed_hitthing,
                              &aimed_mtx_index,
                              &aimed_hit_node)) {
            *hitthing = aimed_hitthing;
            *dstmtxindex = aimed_mtx_index;
            *dsthitnode = aimed_hit_node;
            return 2;
        }
    }

    *hitthing = sample_hitthing;
    *dstmtxindex = sample_mtx_index;
    *dsthitnode = sample_hit_node;
    return 1;
}

static s32 chraiTryForcedGuardHitList(ChrRecord *chr,
                                      struct ShotData *shotdata,
                                      s32 hitpart,
                                      Model **dstmodel,
                                      ModelNode **dstnode,
                                      struct HitThing *dsthitthing,
                                      s32 *dstmtxindex,
                                      ModelNode **dsthitnode)
{
    void *local_head;
    ...
    void *local_model;
    void *local_node;
    ModelNode *surface_node;
    struct HitThing hitthing;
    s32 hit_result;
    s32 mtx_index;
    s32 found;

    if (chr == NULL || chr->prop == NULL || chr->model == NULL || shotdata == NULL
            || dstmodel == NULL || dstnode == NULL || dsthitthing == NULL
            || dstmtxindex == NULL || dsthitnode == NULL) {
        return 0;
    }

    local_head = chr->field_20;
    temp_hit_list = NULL;
    local_model = NULL;
    local_node = NULL;
    found = 0;
    ...
    if (local_head == NULL) {
        temp_hit_list = sub_GAME_7F06B120(NULL, chr->model);
    ...
    hit_result = sub_GAME_7F06C010(&local_head,
                                   (uintptr_t)&shotdata->screen_pos,
                                   (uintptr_t)&shotdata->screen_dir,
                                   (uintptr_t *)&local_model,
                                   (uintptr_t *)&local_node);

    while (hit_result > 0) {
        if (hit_result == hitpart && local_model != NULL && local_node != NULL) {
            chraiInitForcedGuardHitThing(&hitthing);
            mtx_index = 0;
            surface_node = NULL;

            if (sub_GAME_7F04D9B0((Model *)local_model,
                                  (ModelNode *)local_node,
                                  &shotdata->screen_pos,
                                  &shotdata->screen_dir,
                                  &hitthing,
                                  &mtx_index,
                                  &surface_node)) {
                *dstmodel = (Model *)local_model;
                *dstnode = (ModelNode *)local_node;
                *dsthitthing = hitthing;
                *dstmtxindex = mtx_index;
                *dsthitnode = surface_node;
                found = 1;
                break;
            }
        }

        hit_result = probably_damage_detail_blood_effect_related(
            &local_head,
            (uintptr_t)&shotdata->screen_pos,
            (uintptr_t)&shotdata->screen_dir,
            &local_model,
    ...
    if (temp_hit_list != NULL) {
        sub_GAME_7F06B248(temp_hit_list);
    }

    return found;
}

static void chraiMaybeQueueForcedGuardHit(struct ShotData *shotdata, s32 hand)
{
    int i;

    chraiInitForcedGuardHitEvents();

    if (!g_deterministic || s_autoGuardHitCount == 0 || shotdata == NULL) {
        return;
    }

    for (i = 0; i < s_autoGuardHitCount; i++) {
        AutoGuardHitEvent *event = &s_autoGuardHitEvents[i];
        ChrRecord *chr;
        PropRecord *target = NULL;
        Model *model = NULL;
        ModelNode *queue_node = NULL;
        ModelNode *hit_node = NULL;
        s32 mtx_index = 0;
        f32 neg_z;
        struct HitThing hitthing;
        const char *trace_source = "synthetic";
        s32 collision_found = 0;
        coord3d trace_point;
        coord3d trace_dir;
        coord3d trace_world_point;
        coord3d trace_world_dir;
        s32 trace_aim_valid = 0;

        if (event->applied || event->frame < 0 || g_GlobalTimer < event->frame) {
            continue;
        }

        if (event->hand >= 0 && event->hand != hand) {
            continue;
        }
        ...
        if (event->hitpart == HIT_GUN) {
            target = chr->weapons_held[GUNRIGHT] != NULL
                ? chr->weapons_held[GUNRIGHT]
                : chr->weapons_held[GUNLEFT];
        } else if (event->hitpart == HIT_HAT) {
            target = chr->handle_positiondata_hat;
        }

        if (target == NULL || target->obj == NULL || target->obj->model == NULL) {
            continue;
        }

        model = target->obj->model;
        chraiInitForcedGuardHitThing(&hitthing);
        if (chraiTryForcedGuardHitList(chr,
                                       shotdata,
                                       event->hitpart,
                                       &model,
                                       &queue_node,
                                       ...
                                       &hit_node)) {
            collision_found = 1;
            trace_source = "hit_list";
        } else if (model != NULL && model->obj != NULL && model->obj->RootNode != NULL
                   && (!chraiForcedGuardHitModelMtxUsable(model, model->obj->RootNode)
                       ? chraiTryMaterializeAttachedModelMtx(model)
                       : 1)
                   && chraiForcedGuardHitModelMtxUsable(model, model->obj->RootNode)) {

            queue_node = model->obj->RootNode;
            collision_found = sub_GAME_7F04D9B0(model,
                                                queue_node,
                                                &shotdata->screen_pos,
                                                &shotdata->screen_dir,
                                                &hitthing,
            ...
            if (collision_found) {
                trace_source = "direct_ray";
            }

            if (!collision_found && event->hitpart == HIT_HAT) {
                s32 sampled_result = chraiTryForcedGuardHitSampledAim(model,
                                                                      queue_node,
                                                                      &shotdata->screen_pos,
                                                                      chraiForcedGuardHitSampleIndex(),
                                                                      &hitthing,
                                                                      &mtx_index,
                                                                      &hit_node);
                ...
                if (sampled_result == 2) {
                    collision_found = 1;
                    trace_source = "sample_aim";
                } else if (sampled_result == 1) {
                    collision_found = 1;
                    trace_source = "sample";
                }
            }
        ...
        if (collision_found) {
            trace_aim_valid = chraiBuildForcedGuardHitAimTrace(model,
                                                               &hitthing,
                                                               mtx_index,
                                                               shotdata,
                                                               &trace_point,
        ...
        portTraceForcedGuardHitEvent(event->chrnum,
                                     event->hitpart,
                                     trace_source,
                                     collision_found,
                                     hitthing.texturenum,
                                     ...
                                     hit_node != NULL ? 1 : 0,
                                     model != NULL ? 1 : 0,
                                     trace_aim_valid ? &shotdata->screen_pos : NULL,
                                     trace_aim_valid ? &trace_point : NULL,
                                     trace_aim_valid ? &trace_dir : NULL,
                                     trace_aim_valid ? &shotdata->attacker_pos : NULL,
                                     trace_aim_valid ? &trace_world_point : NULL,
                                     trace_aim_valid ? &trace_world_dir : NULL);

        neg_z = chr->prop->zDepth;
        if (neg_z < 10.0f) {
            neg_z = 100.0f;
        }

        sub_GAME_7F03B9C0(shotdata,
                          chr->prop,
                          neg_z,
                          event->hitpart,
                          queue_node,
...
void chraiDefaultWeaponFireHandler(s32 hand) {
    struct ShotData shotdata;
    coord3d gun_pos_raw;
    coord3d bullet_dir_raw;
    coord3d gun_pos;
    coord3d bullet_dir;
    f32 threshold;
    ITEM_IDS weapon_id;
    coord3d far_point;
    StandTile *player_tile;
    coord3d screen_hit_pos;
    f32 neg_z;
    BgHitRecord hit_output;
    s32 texture_index;
    s32 best_room;
...
#ifdef NATIVE_PORT
    trace_shot_id = portTraceBeginShot();
#endif

    bullet_path_from_screen_center(&gun_pos_raw, &bullet_dir_raw, hand);
    threshold = 4294967296.0f;

    gun_pos = gun_pos_raw;
    mtx = currentPlayerGetMatrix10D4();
    mtx4TransformVecInPlace(mtx, &gun_pos);

    bullet_dir = bullet_dir_raw;
    mtx = currentPlayerGetMatrix10D4();
    mtx4RotateVecInPlace(mtx, &bullet_dir);

    shotdata.screen_pos = gun_pos_raw;
    shotdata.screen_dir = bullet_dir_raw;
    shotdata.weapon = weapon_id;
    shotdata.world_dir = bullet_dir;
    shotdata.attacker_pos = gun_pos;

    range = 65536.0f;
    far_point.x = bullet_dir.x * range + gun_pos.x;
    far_point.y = bullet_dir.y * range + gun_pos.y;
    far_point.z = bullet_dir.z * range + gun_pos.z;

    if (walkTilesBetweenPoints_NoCallback(&player_tile, posdata->pos.x, posdata->pos.z,
                                          gun_pos.x, gun_pos.z) == 0) {
        goto check_weapon;
    }

    room_data_f1 = get_room_data_float1();
    visibility = bgGetLevelVisibilityScale() * room_data_f1;

    player_pos = bondviewGetCurrentPlayersPosition();
    sub_GAME_7F0B1CC4();

    if (walkTilesBetweenPoints_NoCallback(&player_tile, gun_pos.x, gun_pos.z,
                                          far_point.x, far_point.z) == 0) {
        chrlvStanLineDirIntersection(&gun_pos, &bullet_dir, &dest_point);
        hit_bg = 1;
    } else {
        dest_point = far_point;
    }

    delta.x = dest_point.x - player_pos->x;
    delta.y = dest_point.y - player_pos->y;
    delta.z = dest_point.z - player_pos->z;

    tile = player_tile;
    scaled_pos.x = player_pos->x * visibility;
    scaled_pos.y = player_pos->y * visibility;
    scaled_pos.z = player_pos->z * visibility;

    player_room = getTileRoom(player_tile);
    memset(visited, 0, sizeof(visited));

    if (bgTestBulletHitBackground(player_pos, &dest_point, player_room, &hit_output) != 0) {
        best_room = player_room;
    }

    visited[player_room] = 1;

    if (best_room <= 0) {
        if (*(s32 *)g_BgPortals != 0) {
            posdata = get_curplayer_positiondata();
            portal_start_room = getTileRoom(posdata->stan);
            best_room = sub_GAME_7F03ADF4(portal_start_room, player_pos, &dest_point, &delta,
                                          &scaled_pos, visited, &hit_output);
        } else {
            posdata = get_curplayer_positiondata();
            portal_start_room = getTileRoom(posdata->stan);
            best_room = sub_GAME_7F03AF5C(portal_start_room, player_pos, &dest_point, &delta,
                                          &scaled_pos, visited, &hit_output);
        }
    }

    if (best_room > 0) {
        scale2 = get_room_data_float2();
        hit_output.pos.x *= scale2;
        hit_output.pos.y *= scale2;
        hit_output.pos.z *= scale2;
    }

    best_room = sub_GAME_7F03AB58(player_pos, &dest_point, &delta, &scaled_pos,
                                  visited, &hit_output, best_room);

    if (best_room > 0) {
        hit_something = 1;
        texturenum = hit_output.texturenum;
        texture_index = texturenum;
        hit_point = hit_output.pos;
        screen_hit_pos = hit_output.pos;
    } else {
        texture_index = -1;
        best_room = player_room;
        screen_hit_pos = far_point;
    }

#ifdef NATIVE_PORT
    if (best_room > 0 && hit_something != 0) {
        shotBgTracePrintf("candidate frame=%d hand=%d weapon=%d player_room=%d best_room=%d "
                          "hit_bg=%d hit_something=%d dest=(%.2f,%.2f,%.2f) "
                          "hit=(%.2f,%.2f,%.2f) normal=(%.3f,%.3f,%.3f) texture=%d tri=%u",
                          g_frame_count_diag,
                          hand,
                          (int)weapon_id,
                          player_room,
                          ...
                          hit_output.normal.z,
                          (int)texture_index,
                          (unsigned int)hit_output.tri_index);
    } else {
        shotBgTracePrintf("candidate frame=%d hand=%d weapon=%d player_room=%d best_room=%d "
                          "hit_bg=%d hit_something=%d dest=(%.2f,%.2f,%.2f) no_background_hit=1",
                          g_frame_count_diag,
                          hand,
                          (int)weapon_id,
                          player_room,
    ...
    if (hit_bg != 0 || hit_something != 0) {
        mtx = camGetWorldToScreenMtxf();
        mtx4TransformVecInPlace(mtx, &screen_hit_pos);
        neg_z = -screen_hit_pos.z;
        threshold = neg_z;
    }

#ifdef NATIVE_PORT
    portTraceShotEvent("bg_candidate",
                       hand,
                       weapon_id,
                       -1,
                       NULL,
                       ...
                       hit_something,
                       0,
                       bondwalkItemGetObjectsShootThrough(weapon_id),
                       -1,
...
check_weapon:
    if (weapon_id == ITEM_WATCHLASER && 300.0f < threshold) {
        threshold = 300.0f;
    }

    shotdata.unk34 = threshold;

    for (pp = g_LastOnScreenProp - 1; pp >= g_OnScreenPropList; pp--) {
        prop = *pp;
        ...
        if (proptype == PROP_TYPE_CHR) {
            sub_GAME_7F022648(prop, &shotdata);
        } else if (proptype == PROP_TYPE_VIEWER) {
            if (prop->chr != NULL) {
                player_index = getPlayerPointerIndex(prop);

                if (get_cur_playernum() != player_index) {
                    sub_GAME_7F022648(prop, &shotdata);
                }
            }
        } else if (proptype == PROP_TYPE_OBJ || proptype == PROP_TYPE_WEAPON || proptype == PROP_TYPE_DOOR) {
            sub_GAME_7F04E9BC(prop, &shotdata);
        }
    }

#ifdef NATIVE_PORT
    chraiMaybeQueueForcedGuardHit(&shotdata, hand);
#endif

    for (i = 0; i < (s32)ARRAYCOUNT(shotdata.hits); i++) {
        ShotHitRecord *hit = &shotdata.hits[i];
...
#ifdef NATIVE_PORT
        {
            ...
            if ((proptype == PROP_TYPE_OBJ || proptype == PROP_TYPE_WEAPON || proptype == PROP_TYPE_DOOR) &&
                hit_prop->obj != NULL) {
                obj_type = hit_prop->obj->type;
                obj_pad = hit_prop->obj->pad;
            } else if (proptype == PROP_TYPE_CHR && hit_prop->chr != NULL) {
                chrnum = hit_prop->chr->chrnum;
            }

            shotBgTracePrintf("hit frame=%d slot=%d prop=%p type=%d chrnum=%d obj_type=%d obj_pad=%d "
                              "do_damage=%d distance=%.2f mtx=%d tex=%d hit=(%.2f,%.2f,%.2f)",
                              g_frame_count_diag,
                              i,
                              (void *)hit_prop,
                              (int)proptype,
                              ...
                              hit->distance,
                              hit->mtx_index,
                              hit->hitthing.texturenum,
                              hit->hitthing.unk00.x,
                              hit->hitthing.unk00.y,
                              hit->hitthing.unk00.z);
            portTraceShotEvent("queued_hit",
                               hand,
                               weapon_id,
                               i,
                               hit_prop,
        ...
        if (proptype == PROP_TYPE_CHR || proptype == PROP_TYPE_VIEWER) {
            sub_GAME_7F022980(&shotdata, hit);
        } else if (proptype == PROP_TYPE_OBJ || proptype == PROP_TYPE_WEAPON || proptype == PROP_TYPE_DOOR) {
            sub_GAME_7F04EA68(&shotdata, hit);
        }

        do_damage = hit->do_damage;
        if (do_damage != 0) {
            hit_count++;
            if (hit_count >= (s32)bondwalkItemGetObjectsShootThrough(weapon_id)) {
                hit_something = 0;
                hit_bg = 0;
#ifdef NATIVE_PORT
                {
                    ...
                    portTraceShotEvent("clear_background",
                                       hand,
                                       weapon_id,
                                       i,
                                       hit_prop,
                ...
                shotBgTracePrintf("clear-background frame=%d slot=%d hit_count=%d shoot_through=%d "
                                  ...
                                  i,
                                  hit_count,
                                  (int)bondwalkItemGetObjectsShootThrough(weapon_id),
                                  (void *)hit_prop,
                                  (int)proptype);
#endif
            }
    ...
    if (hit_something == 0 && hit_bg == 0) {
#ifdef NATIVE_PORT
        portTraceShotEvent("return_no_impact",
                           hand,
                           weapon_id,
                           -1,
                           NULL,
        ...
        shotBgTracePrintf("return frame=%d reason=no-impact-after-props hit_bg=%d hit_something=%d "
                          ...
                          hit_count,
                          threshold);
        portTraceEndShot(trace_shot_id);
#endif
        return;
    }
    ...
    if (weapon_id == ITEM_WATCHLASER && 300.0f < neg_z) {
        do_effects = 0;
    }
    ...
    if (hit_something != 0) {
        ...
        if (do_effects != 0) {
            if (imgsnd->thing2_len > 0 && weapon_id != ITEM_WATCHLASER) {
                rng = randomGetNext();
                impact_type = imgsnd->thing2[rng % imgsnd->thing2_len];
#ifdef NATIVE_PORT
                portTraceShotEvent("create_world",
                                   hand,
                                   weapon_id,
                                   -1,
                                   NULL,
                ...
                explosionCreateBulletImpact(&hit_point, &hit_output.normal,
                                            impact_type, (s16)best_room, NULL, -1, 0);
            }

            if (check_if_imageID_is_light(hit_output.texturenum) != 0) {
                sub_GAME_7F0BBE0C(hit_output.tricmd, hit_output.tri_index, best_room);
            }
        }
        ...
        range = 65536.0f;
        dest_point.x = bullet_dir.x * range + gun_pos.x;
        dest_point.y = bullet_dir.y * range + gun_pos.y;
        dest_point.z = bullet_dir.z * range + gun_pos.z;
        impact_pos = &dest_point;
    }
    ...
    pushback = 26.0f;
    impact_pos->x -= pushback * bullet_dir.x;
    impact_pos->y -= pushback * bullet_dir.y;
    impact_pos->z -= pushback * bullet_dir.z;

    sub_GAME_7F067B4C(impact_pos);

    if (do_effects != 0) {
        sub_GAME_7F0A3E1C(impact_pos, 1, 26.0f, (s16)best_room);
    }
...
extern void *sub_GAME_7F0A3E1C(coord3d *arg0, s32 arg1, f32 arg2, s16 arg3);
extern void sub_GAME_7F022648(PropRecord *prop, struct ShotData *shotdata);
extern void sub_GAME_7F022980(struct ShotData *shotdata, void *hit_record);
extern void sub_GAME_7F04E9BC(PropRecord *prop, struct ShotData *shotdata);
extern void sub_GAME_7F04EA68(struct ShotData *shotdata, void *hit_record);
extern u8 bondwalkItemGetObjectsShootThrough(ITEM_IDS item);
...
void chraiDefaultWeaponFireHandler(s32 hand) {
    ...
    /* Zero out hit entry prop/result/node fields for entries 0-1 */
    ((s32 *)&shotdata)[0x0F] = 0;
    ((s32 *)&shotdata)[0x10] = 0;
    ((s32 *)&shotdata)[0x11] = 0;
    ((s32 *)&shotdata)[0x23] = 0;
    ...
    /* Zero out hit entries 2-9 in a loop */
    {
        u8 *ptr = (u8 *)&shotdata + 0xA0;
        u8 *end = (u8 *)&shotdata + 0x320;
        do {
            ptr += 0x140;
            *(s32 *)(ptr - 0xB4) = 0;
            *(s32 *)(ptr - 0xB0) = 0;
    ...
    /* Transform gun position to world space */
    gun_pos.x = gun_pos_raw.x;
    gun_pos.y = gun_pos_raw.y;
    gun_pos.z = gun_pos_raw.z;

    mtx = currentPlayerGetMatrix10D4();
    mtx4TransformVecInPlace(mtx, &gun_pos);

    /* Rotate bullet direction to world space */
    bullet_dir.x = bullet_dir_raw.x;
    bullet_dir.y = bullet_dir_raw.y;
    bullet_dir.z = bullet_dir_raw.z;

    mtx = currentPlayerGetMatrix10D4();
    mtx4RotateVecInPlace(mtx, &bullet_dir);

    /* Calculate far_point = bullet_dir * 65536 + gun_pos */
    range = 65536.0f;
    far_point.x = bullet_dir.x * range + gun_pos.x;
    far_point.y = bullet_dir.y * range + gun_pos.y;
    far_point.z = bullet_dir.z * range + gun_pos.z;

    /* Walk tiles from player position to gun position */
    if (walkTilesBetweenPoints_NoCallback(&player_tile, posdata->pos.x, posdata->pos.z,
                                           gun_pos.x, gun_pos.z) == 0) {
        goto check_weapon;
    }
    ...
    /* Walk tiles from gun position to far point */
    if (walkTilesBetweenPoints_NoCallback(&player_tile, gun_pos.x, gun_pos.z,
                                           far_point.x, far_point.z) == 0) {
        chrlvStanLineDirIntersection(&gun_pos, &bullet_dir, &dest_point);
        hit_bg = 1;
    } else {
        dest_point.x = far_point.x;
        dest_point.y = far_point.y;
        dest_point.z = far_point.z;
    }

    /* Calculate delta and scaled_pos */
    delta.x = dest_point.x - player_pos->x;
    delta.y = dest_point.y - player_pos->y;
    delta.z = dest_point.z - player_pos->z;
    ...
    /* Clear visited room array */
    for (i = 0; i < 256; i++) {
        visited[i] = 0;
    }

    /* Test bullet hit against background */
    if (bgTestBulletHitBackground(player_pos, &dest_point, player_room, hit_output) != 0) {
        best_room = player_room;
    }
...
scale_hit_output:
    if (best_room > 0) {
        scale2 = get_room_data_float2();
        *(f32 *)(hit_output + 0x00) *= scale2;
        *(f32 *)(hit_output + 0x04) *= scale2;
        *(f32 *)(hit_output + 0x08) *= scale2;
    }
    ...
    if (best_room > 0) {
        hit_something = 1;
        texturenum = *(s16 *)(hit_output + 0x2a);
        texture_index = texturenum;

        hit_point.x = *(f32 *)(hit_output + 0x00);
        screen_hit_pos.x = *(f32 *)(hit_output + 0x00);
        hit_point.y = *(f32 *)(hit_output + 0x04);
        screen_hit_pos.y = *(f32 *)(hit_output + 0x04);
        hit_point.z = *(f32 *)(hit_output + 0x08);
        screen_hit_pos.z = *(f32 *)(hit_output + 0x08);
    } else {
        texture_index = -1;
        best_room = player_room;
        screen_hit_pos.x = far_point.x;
        screen_hit_pos.y = far_point.y;
        screen_hit_pos.z = far_point.z;
    }
...
check_weapon:
    if (weapon_id == ITEM_WATCHLASER) {
        if (300.0f < threshold) {
            threshold = 300.0f;
        }
    }

    /* Iterate through on-screen props in reverse order */
    pp = (PropRecord **)g_LastOnScreenProp;
    pp--;
    if (pp >= &g_OnScreenPropList[0]) {
        prop = *pp;
...
check_obj_types:
    ...
    /* Process shotdata hit entries */
    entry = (u8 *)&shotdata;
    end_entry = (u8 *)&shotdata + 0x320;

    while (1) {
        hit_prop = *(PropRecord **)(entry + 0x3c);

        if (hit_prop != NULL) {
            proptype = hit_prop->type;

            if (proptype == PROP_TYPE_CHR || proptype == PROP_TYPE_VIEWER) {
                sub_GAME_7F022980(&shotdata, (void *)(entry + 0x38));
            } else if (proptype == PROP_TYPE_OBJ || proptype == PROP_TYPE_WEAPON || proptype == PROP_TYPE_DOOR) {
                sub_GAME_7F04EA68(&shotdata, (void *)(entry + 0x38));
            }

            do_damage = *(s32 *)(entry + 0x84);
            if (do_damage != 0) {
                hit_count++;
                if (hit_count >= (s32)bondwalkItemGetObjectsShootThrough(weapon_id)) {
                    hit_something = 0;
    ...
    if (hit_something != 0) {
        ...
        if (do_effects != 0) {
            if (imgsnd->thing2_len > 0 && weapon_id != ITEM_WATCHLASER) {
                rng = randomGetNext();
                impact_type = imgsnd->thing2[rng % imgsnd->thing2_len];
                explosionCreateBulletImpact(&hit_point, (coord3d *)(hit_output + 0x0c),
                                            impact_type, (s16)best_room, NULL, -1, 0);
            }

            if (check_if_imageID_is_light(*(s16 *)(hit_output + 0x2a)) != 0) {
                sub_GAME_7F0BBE0C(*(Gfx **)(hit_output + 0x24),
                                   *(s16 *)(hit_output + 0x28), best_room);
            }
        }
    ...
    /* Move impact_pos back along bullet direction by 26 units */
    pushback = 26.0f;
    impact_pos->x -= pushback * bullet_dir.x;
    impact_pos->y -= pushback * bullet_dir.y;
    impact_pos->z -= pushback * bullet_dir.z;
    ...
    if (do_effects != 0) {
        pushback = 26.0f;
        sub_GAME_7F0A3E1C(impact_pos, 1, pushback, (s16)best_room);
    }
}
 ...
 * Address 0x7F03B9C0.
 *
 * Registers a hit entry into the ShotData hit array (10 entries, each 0x50 bytes,
 * starting at the shotdata base). If do_damage is set, it may evict the farthest
 * existing hit to make room if the penetration limit is reached. If has_flag is set,
 * it evicts any hit farther than neg_z and sets unk34 to neg_z. Finally, the new
 * hit is stored in the first empty slot.
 *
 * Hit entry layout (offsets from entry base = shotdata + i*0x50):
 *   +0x38: f32  distance
 *   +0x3c: s32  prop (non-zero = occupied)
 *   +0x40: s32  hit_result
 *   +0x44: s32  node
 *   +0x48: s32  hit_data[12] (48 bytes copied from hit_pos param)
 *   +0x78: s32  mtx_index
 *   +0x7c: s32  hit_node
 *   +0x80: s32  model
 *   +0x84: s32  do_damage
 */
void sub_GAME_7F03B9C0(struct ShotData *shotdata, PropRecord *prop, f32 neg_z,
                       s32 hit_result, ModelNode *node, struct HitThing *hitthing,
                       s32 mtx_index, ModelNode *hit_node, Model *model,
                       s32 do_damage, s32 has_flag) {
#ifdef NATIVE_PORT
    static int s_shotQueueDbgBudget = 160;
    s32 i;
    s32 num_valid;
    s32 best_index;
    f32 best_dist;
    ...
    if (do_damage != 0) {
        ...
        for (i = 0; i < (s32)ARRAYCOUNT(shotdata->hits); i++) {
            ShotHitRecord *entry = &shotdata->hits[i];

            if (entry->prop == NULL || entry->do_damage == 0) {
                continue;
            }

            num_valid++;
            if (best_dist < entry->distance) {
        ...
        max_penetration = bondwalkItemGetObjectsShootThrough(shotdata->weapon);

        if (num_valid < (s32)max_penetration) {
            if (num_valid + 1 == (s32)max_penetration && neg_z < shotdata->unk34) {
                shotdata->unk34 = neg_z;
            }
        } else {
            if (best_index >= 0) {
                shotdata->hits[best_index].prop = NULL;
            }

            shotdata->unk34 = second_best;
            if (second_best < neg_z) {
                shotdata->unk34 = neg_z;
            }
            ...
            for (i = 0; i < (s32)ARRAYCOUNT(shotdata->hits); i++) {
                ...
                if (entry->prop != NULL && entry->do_damage == 0 && second_best < entry->distance) {
                    entry->prop = NULL;
                }
            }
        }
    ...
    if (has_flag != 0 && shotdata->weapon != ITEM_RUGER && shotdata->weapon != ITEM_SILVERWPPK) {
        for (i = 0; i < (s32)ARRAYCOUNT(shotdata->hits); i++) {
            ShotHitRecord *entry = &shotdata->hits[i];

            if (entry->prop != NULL && neg_z < entry->distance) {
                entry->prop = NULL;
            }
        }
    ...
    for (i = 0; i < (s32)ARRAYCOUNT(shotdata->hits); i++) {
        ...
        entry->distance = neg_z;
        entry->prop = prop;
        entry->hit_result = hit_result;
        entry->node = node;
        entry->hitthing = *hitthing;
        entry->mtx_index = mtx_index;
        entry->hit_node = hit_node;
        entry->model = model;
        entry->do_damage = do_damage;
        if (ge_dbg_enabled() && s_shotQueueDbgBudget-- > 0) {
            GEDBG("shotqueue store prop=%p type=%d hr=%d dist=%.2f do=%d node=%p hit_node=%p model=%p tex=%d",
                  (void *)prop, prop ? prop->type : -1, hit_result, neg_z, do_damage,
                  (void *)node, (void *)hit_node, (void *)model, hitthing->texturenum);
        }
        return;
    }
#else
    ...
    /* Phase 1: Find the entry with the largest distance among valid entries */
    second_best = 0.0f;
    best_index = 0;
    num_valid = 0;
    best_dist = 0.0f;
    ...
    for (i = 0; i < 10; i++) {
        ...
        entry_dist = *(f32 *)(entry + 0x38);
        num_valid++;
        if (best_dist < entry_dist) {
            second_best = best_dist;
    ...
    /* Phase 2: Check penetration count */
    item = shotdata->unk00[6];
    max_penetration = bondwalkItemGetObjectsShootThrough((ITEM_IDS)item);

    if (num_valid < (s32)max_penetration) {
        /* Path B: not enough valid entries to require eviction */
        item = shotdata->unk00[6];
        max_penetration = bondwalkItemGetObjectsShootThrough((ITEM_IDS)item);

        if (num_valid + 1 == (s32)max_penetration) {
    ...
    /* Evict the farthest (best) entry */
    {
        u8 *evict_entry = (u8 *)shotdata + best_index * 0x50;
        *(s32 *)(evict_entry + 0x3c) = 0;
    }

    /* Update threshold: unk34 = max(second_best, neg_z) */
    shotdata->unk34 = second_best;
    if (second_best < neg_z) {
        shotdata->unk34 = neg_z;
    }

    /* Evict other entries where do_damage==0 and second_best < distance */
    base = (u8 *)shotdata;

    /* Entry 0 */
    if (*(s32 *)(base + 0x3c) != 0) {
        if (*(s32 *)(base + 0x84) == 0) {
            if (second_best < *(f32 *)(base + 0x38)) {
                *(s32 *)(base + 0x3c) = 0;
            }
        }
    }
    ...
    if (*(s32 *)(base + 0x8c) != 0) {
        if (*(s32 *)(base + 0xd4) == 0) {
            if (second_best < *(f32 *)(base + 0x88)) {
                *(s32 *)(base + 0x8c) = 0;
            }
        }
    }

    /* Entries 2-9: loop with stride 0x140 (4 entries per iteration, 2 iterations) */
    {
        u8 *ptr = base + 0xa0;
        s32 counter = 0xa0;

        do {
            /* Sub-entry 0 in group */
            if (*(s32 *)(ptr + 0x3c) != 0) {
                if (*(s32 *)(ptr + 0x84) == 0) {
                    if (second_best < *(f32 *)(ptr + 0x38)) {
                        *(s32 *)(ptr + 0x3c) = 0;
                    }
                }
            }

            /* Sub-entry 1 in group */
            if (*(s32 *)(ptr + 0x8c) != 0) {
                if (*(s32 *)(ptr + 0xd4) == 0) {
                    if (second_best < *(f32 *)(ptr + 0x88)) {
                        *(s32 *)(ptr + 0x8c) = 0;
                    }
                }
            }

            /* Sub-entry 2 in group */
            if (*(s32 *)(ptr + 0xdc) != 0) {
                if (*(s32 *)(ptr + 0x124) == 0) {
                    if (second_best < *(f32 *)(ptr + 0xd8)) {
                        *(s32 *)(ptr + 0xdc) = 0;
                    }
                }
            }

            /* Sub-entry 3 in group (last, uses regular branches) */
            if (*(s32 *)(ptr + 0x12c) != 0) {
                if (*(s32 *)(ptr + 0x174) == 0) {
                    if (second_best < *(f32 *)(ptr + 0x128)) {
                        *(s32 *)(ptr + 0x12c) = 0;
                    }
                }
            }
...
check_has_flag:
    ...
    /* Check item type - skip eviction for items 18 and 20 */
    item = shotdata->unk00[6];
    if (item == 18) {
        goto store_entry;
    }
    ...
    /* Evict entries where neg_z < distance (has_flag path, no do_damage check) */
    base = (u8 *)shotdata;

    /* Entry 0 */
    if (*(s32 *)(base + 0x3c) != 0) {
        if (neg_z < *(f32 *)(base + 0x38)) {
            *(s32 *)(base + 0x3c) = 0;
        }
    }
    ...
    /* Entry 1 */
    if (*(s32 *)(base + 0x8c) != 0) {
        if (neg_z < *(f32 *)(base + 0x88)) {
            *(s32 *)(base + 0x8c) = 0;
        }
    }

    /* Entries 2-9: loop with stride 0x140 */
    {
        u8 *ptr = base + 0xa0;
        s32 counter = 0xa0;
        ...
        do {
            ...
            /* Sub-entry 2 */
            if (*(s32 *)(ptr + 0xdc) != 0) {
                if (neg_z < *(f32 *)(ptr + 0xd8)) {
                    *(s32 *)(ptr + 0xdc) = 0;
                }
            }

            /* Sub-entry 3 (last, regular branches) */
            if (*(s32 *)(ptr + 0x12c) != 0) {
                if (neg_z < *(f32 *)(ptr + 0x128)) {
                    *(s32 *)(ptr + 0x12c) = 0;
                }
            }
...
store_entry:
    /* Phase 3: Find the first empty slot and store the new hit */
    base = (u8 *)shotdata;
    for (i = 0; i < 10; i++) {
        u8 *entry = base + i * 0x50;
        ...
        *(f32 *)(entry + 0x38) = neg_z;
        *(s32 *)(entry + 0x3c) = (s32)(uintptr_t)prop;
        *(s32 *)(entry + 0x40) = hit_result;
        *(s32 *)(entry + 0x44) = (s32)(uintptr_t)node;

        /* Copy 48 bytes (0x30) from hitthing to entry+0x48 */
        src = (s32 *)hitthing;
        dst = (s32 *)(entry + 0x48);
        for (j = 0; j < 12; j++) {
            dst[j] = src[j];
        ...
        *(s32 *)(entry + 0x78) = mtx_index;
        *(s32 *)(entry + 0x7c) = (s32)(uintptr_t)hit_node;
        *(s32 *)(entry + 0x80) = (s32)(uintptr_t)model;
        *(s32 *)(entry + 0x84) = do_damage;
        return;
    }
#endif
}
...
void chraiFistAttackHandler(s32 hand, s32 item_id) {
    ...
    hit = 0;
    posdata = get_curplayer_positiondata();
    duckingHeight = bondviewGetPlayerDuckingHeightRelated(g_CurrentPlayer);

    for (pp = g_LastOnScreenProp - 1; pp >= &g_OnScreenPropList[0]; pp--) {
        prop = *pp;
        ...
        if (prop->type == PROP_TYPE_CHR) {
            ...
            playerIdx = getPlayerPointerIndex(prop);
            if (get_cur_playernum() == playerIdx) {
                continue;
            }
        } else {
            continue;
        ...
        sub_GAME_7F03F748(chr->model, &max0, &min0, 0);
        ...
        sub_GAME_7F03F748(chr->model, &max1, &min1, 1);
        ...
        sub_GAME_7F03F748(chr->model, &max2, &min2, 2);
        ...
        if (!stanTestLineUnobstructed(&tile, posdata->pos.x, posdata->pos.z,
                prop->pos.x, prop->pos.z, 19,
                duckingHeight, duckingHeight, 0.0f, 1.0f)) {
            continue;
        }
        ...
        bullet_path_from_screen_center(&pos, &dir, hand);
        mtx = currentPlayerGetMatrix10D4();
        mtx4RotateVecInPlace(mtx, &dir);

        if (handles_shot_actors(chr, bodypart, &dir, item_id, 1)) {
            hit = 1;
    ...
    if (!hit) {
        if (item_id == ITEM_FIST) {
            sndPlaySfx(g_musicSfxBufferPtr, 105, 0);
        }
    }
}
#else
...
void chraiCheckUseHeldItem(s32 hand)
{
    ...
    if (get_hands_firing_status(hand) != 0)
    {
        ...
        if (item_id == ITEM_TRIGGER)
        {
            ...
            || item_id == ITEM_BOMBDEFUSER
            || item_id == ITEM_BUG
            || item_id == ITEM_MICROCAMERA
            || item_id == ITEM_EXPLOSIVEFLOPPY
            || item_id == ITEM_DATTAPE
...
bool bond_interact_object(void)
{
    ...
    {
#ifdef NATIVE_PORT
        interactTracePrintf("candidate prop=%p type=%d room=%d obj=%d flags=0x%02x pos=(%.2f,%.2f,%.2f)",
                            (void *)prop,
                            prop->type,
                            prop->stan ? prop->stan->room : -1,
                            prop->obj ? prop->obj->type : -1,
                            (unsigned int)prop->flags,
                            prop->pos.x,
                            prop->pos.y,
                            prop->pos.z);
...
#ifdef NATIVE_PORT
        interactTracePrintf("dispatch prop=%p type=%d op=%d",
                            (void *)prop,
                            prop->type,
                            (int)op);
#endif
...
#ifdef NATIVE_PORT
    interactTracePrintf("candidate none");
#endif
    return TRUE;
}
...
void handle_mp_respawn_and_some_things(void) {
    ...
    if (prop != NULL) {
        do {
            var_s2 = 0;
            prev_prop = prop_s1->prev;
            prop_type = prop_s1->type;
            if (prop_type == PROP_TYPE_CHR) {
                /* Beam/muzzle flash tick for character weapon effects */
                ChrRecord *chr = prop_s1->chr;
                if (chr != NULL) {
                    sub_GAME_7F062B00(&chr->unk180[0]);
                    sub_GAME_7F062B00(&chr->unk180[1]);
                }
            } else if ((prop_type == PROP_TYPE_OBJ) || (prop_type == PROP_TYPE_WEAPON) || (prop_type == PROP_TYPE_DOOR)) {
                ObjectRecord *obj = prop_s1->obj;
                s16 prop_time_to_regen = prop_s1->timetoregen;
                if (obj != NULL && prop_time_to_regen > 0) {
                    s32 was_above_threshold = (prop_time_to_regen >= 0x3C) ? 0 : 1;
                    prop_s1->timetoregen = (s16)(prop_time_to_regen - g_ClockTimer);
                    if ((prop_s1->timetoregen < 0x3C) && (was_above_threshold == 0)) {
                        if (sub_GAME_7F03C574(prop_s1) == 0) {
                            prop_s1->timetoregen = (s16)(prop_s1->timetoregen + 0x3C);
                        }
                    }
                    if (prop_s1->timetoregen <= 0) {
                        prop_s1->timetoregen = 0;
                        if (obj->state & 0x10) {
                            obj->runtime_bitflags |= 0x1000;
                        } else {
                            obj->runtime_bitflags &= ~0x1000;
                        }
                    } else if ((prop_s1->timetoregen < 0x3C) && (was_above_threshold == 0)) {
                        if ((obj->maxdamage == 0.0f) && !(obj->state & 0x80)) {
                            /* Simple respawn path (non-reparent) */
                            if (!(obj->flags & 0x8000)) {
                                chrpropEnable(prop_s1);
                                sub_GAME_7F03E134(prop_s1);
                                obj->runtime_bitflags &= ~0x800;
                            }
                            /* Skip 0x8000 reparent path — MP only */
                        } else {
                            if (obj->state & 8) {
                                obj->flags |= 0x100;
                            } else {
                ...
                /* N64 parity (US ASM 7F03C8E4-7F03C904): tick the autogun's
                 * tracer beam. The renderer already draws+arms these beams
                 * (chrobjhandler.c:27611-27622) but nothing was advancing or
                 * expiring them -- frozen/never-visible tracers on drone-gun
                 * levels. Use the struct field (not the N64 0xCC offset poke,
                 * which is wrong on 64-bit -- see chrobjhandler.c:27611-27619
                 * for the same translation). Gated per audit §6.3: default ON
                 * (faithful), GE007_AUTOGUN_BEAM_TICK=0 escape hatch. */
                if (chrpropAutogunBeamTickEnabled() && obj != NULL && obj->type == PROPDEF_AUTOGUN) {
                    AutogunRecord *ag = (AutogunRecord *)obj;
                    if (ag->beam != NULL) {
                        sub_GAME_7F062B00((ChrRecord_f180 *)ag->beam);
                    }
                }
            } else if (prop_type == PROP_TYPE_EXPLOSION) {
                var_s2 = explosionTick(prop_s1);
            } else if (prop_type == PROP_TYPE_SMOKE) {
                var_s2 = explosionSmokeTick(prop_s1);
            }
            /* Skip PROP_TYPE_VIEWER — MP only */
...
#ifdef NATIVE_PORT
    {
        static s32 diag_tick_cnt = 0;
        s32 cnt = 0;
        PropRecord *p = prop;
        while (p) { cnt++; p = p->prev; }
        if (cnt > 0 && ++diag_tick_cnt <= 5) {
            s32 chr_cnt = 0, obj_cnt = 0;
            p = prop;
            while (p) { if (p->type == PROP_TYPE_CHR) chr_cnt++; else obj_cnt++; p = p->prev; }
            osSyncPrintf("[PROPS] prop list: %d total (%d chr, %d other)\n", cnt, chr_cnt, obj_cnt);
        }
    }
#endif
...
f32 sub_GAME_7F03D188(PropRecord *prop, coord3d *arg1, f32 *arg2, f32 *arg3, f32 *arg4)
{
    ...
    transform3Dto2DCoords(arg1, (coord2d *)spa0);
    sp94.x = arg2[0];
    sp94.y = arg1->y;
    sp94.z = arg1->z;
    transform3Dto2DCoords(&sp94, (coord2d *)sp8c);
    sp94.x = arg2[1];
    sp94.y = arg1->y;
    sp94.z = arg1->z;
    transform3Dto2DCoords(&sp94, (coord2d *)sp84);
    sp94.x = arg1->x;
    sp94.y = arg3[1];
    sp94.z = arg1->z;
    transform3Dto2DCoords(&sp94, (coord2d *)sp7c);
    sp94.x = arg1->x;
    sp94.y = arg3[0];
    sp94.z = arg1->z;
    transform3Dto2DCoords(&sp94, (coord2d *)sp74);
...
#ifdef NATIVE_PORT
static void chrpropAddRoomIfNew(s32 *rooms, s32 *num_rooms, s32 max_rooms, s32 room)
{
    s32 i;

    if (room < 0 || room >= g_MaxNumRooms || *num_rooms >= max_rooms) {
        return;
    }
...
static s32 chrpropCollectRoomsNearStan(StandTile *start,
                                       f32 target_x,
                                       f32 target_z,
                                       f32 radius,
                                       s32 *rooms,
                                       ...
                                       s32 max_rooms)
{
    enum { TILE_STACK_MAX = 41 };
    StandTile *tile_stack[TILE_STACK_MAX];
    s32 stack_depth;
    s32 read_index;
    s32 near_edge_count;
    s32 result;
    ...
    if (start == NULL || rooms == NULL || num_rooms == NULL || max_rooms <= 0) {
        return -1;
    }
    ...
    near_edge_count = 0;
    result = -2;
    scaled_x = target_x * level_scale;
    scaled_z = target_z * level_scale;
    scaled_radius = radius * level_scale;
    tile_stack[0] = start;

    while (read_index < stack_depth) {
        StandTile *tile = tile_stack[read_index++];
        s32 point_count;
        ...
        chrpropAddRoomIfNew(rooms, num_rooms, max_rooms, tile->room);

        point_count = (tile->tail.half >> 12) & 0xF;
        for (s32 edge_index = 0; edge_index < point_count; edge_index++) {
            s32 next_edge = (edge_index + 1) % point_count;
            f32 edge_dist = getShortest2dDispToInfTileEdge(tile, edge_index, scaled_x, scaled_z);
            f32 dist_a = distToTilePnt2D(tile, edge_index, scaled_x, scaled_z);
            f32 dist_b = distToTilePnt2D(tile, next_edge, scaled_x, scaled_z);
            u16 link;
            StandTile *linked_tile;
            s32 duplicate;

            if (!(edge_dist < scaled_radius)) {
                continue;
            }

            if (!(dist_a < scaled_radius) && !(dist_b < scaled_radius)
                && !sub_GAME_7F0B0198(tile, edge_index, scaled_x, scaled_z)) {
                continue;
            }

            link = tile->points[edge_index].link;
            if ((link >> 4) == 0 || standTileStart == NULL) {
                near_edge_count++;
                continue;
            }

            linked_tile = &standTileStart[link];
            duplicate = 0;
            for (s32 i = stack_depth - 1; i >= 0; i--) {
                if (tile_stack[i] == linked_tile) {
                    duplicate = 1;
                    break;
                }
            }
            ...
            if (!duplicate) {
                if (stack_depth >= TILE_STACK_MAX) {
                    return 5;
                }
                tile_stack[stack_depth++] = linked_tile;
            }
        }
    }

    if (near_edge_count > 1) {
        result = 2;
    }
...
#ifdef NATIVE_PORT
void sub_GAME_7F03E27C(PropRecord *prop, coord3d *arg1, coord3d *arg2, f32 arg3) {
    ...
    max_rooms = PROPRECORD_STAN_ROOM_LEN - 1;
    num_rooms = 0;
    ...
    if (prop->type != PROP_TYPE_CHR) {
        if (prop->stan != NULL) {
            rooms[0] = prop->stan->room;
            num_rooms = 1;
        }
    } else if (prop->flags & PROPFLAG_00000008) {
        room_src = prop->rooms;
        for (i = 0; i < max_rooms && room_src[i] != 0xFF; i++) {
            rooms[i] = room_src[i];
        }
        ...
        if (stan != NULL) {
            chrpropCollectRoomsNearStan(stan,
                                        prop->pos.x,
                                        prop->pos.z,
                                        arg3,
                                        rooms,
    ...
    if (num_rooms <= 0 && prop->stan != NULL) {
        rooms[0] = prop->stan->room;
        num_rooms = 1;
    }

    sub_GAME_7F0BA2D4(arg1, arg2, rooms, &num_rooms, max_rooms);
...
*/
void sub_GAME_7F03E27C(PropRecord *prop, s32 arg1, s32 arg2, f32 arg3) {
    ObjectRecord *obj;
    s32 rooms[8];
    s32 num_rooms;
    StandTile *stan;
    ...
    if (prop->flags & 8) {
        ...
        if (obj != NULL) {
            if (obj->runtime_bitflags & 0x80) {
                struct Projectile *proj = obj->projectile;
                if (*(s32 *)proj & 8) {
                    room_src = (u8 *)proj + 0xCC;
                }
            }
        }

        i = 0;
        while (room_src[i] != 0xFF) {
            rooms[i] = room_src[i];
            i++;
        }
        num_rooms = i;
        ...
        stan = prop->stan;
        sub_GAME_7F0B21B0(&stan, prop->pos.x, prop->pos.z, arg3, (s32)rooms, &num_rooms, 7);
    }

    i = 0;
    sub_GAME_7F0BA2D4(arg1, arg2, rooms, &num_rooms, 7);
...
void removed_debug_roomblocks_feature(void)
{
    /* gleened from debug
        ...
        inv_length = 1.0f / sqrtf(((axis_x * axis_x) + (axis_y*axis_y)) + (axis_z*axis_z));
        axis_x *= inv_length;
        axis_y *= inv_length;
        axis_z *= inv_length;

        angle *= 0.017453292; //Deg2Rad?

        one_minus_cos = 1.0 - cosf(angle);
        xy_term       = axis_x * axis_y * one_minus_cos;
        yz_term       = axis_y * axis_z * one_minus_cos;
        zx_term       = axis_z * axis_x * one_minus_cos;

        matrix_4x4_set_identity(matrix);

        *matrix    = axis_x * axis_x + cosf(angle) * (1.0 - axis_x * axis_x);
        matrix[9]  = yz_term - axis_x * sinf(angle);
        matrix[6]  = yz_term + axis_x * sinf(angle);
        matrix[5]  = axis_y * axis_y + cosf(angle) * (1.0 - axis_y * axis_y);
        matrix[8]  = zx_term + axis_y * sinf(angle);
        matrix[2]  = zx_term - axis_y * sinf(angle);
        matrix[10] = axis_z * axis_z + cosf(angle) * (1.0 - axis_z * axis_z);
        matrix[4]  = xy_term - axis_z * sinf(angle);
        matrix[1]  = xy_term + axis_z * sinf(angle);
        */
}
...
f32 chrpropSumMatrixPosX(struct ModelRoData_BoundingBoxRecord *bbox, Mtxf *arg1)
{
    ...
    if (arg1->m[0][0] >= 0.0f)
    {
        sum += (bbox->Bounds.xmin * arg1->m[0][0]);
    }
    else
    {
        sum += (bbox->Bounds.xmax * arg1->m[0][0]);
    }

    if (arg1->m[1][0] >= 0.0f)
    {
        sum += (bbox->Bounds.ymin * arg1->m[1][0]);
    }
    else
    {
        sum += (bbox->Bounds.ymax * arg1->m[1][0]);
    }

    if (arg1->m[2][0] >= 0.0f)
    {
        sum += (bbox->Bounds.zmin * arg1->m[2][0]);
    }
    else
    {
        sum += (bbox->Bounds.zmax * arg1->m[2][0]);
    }
...
f32 chrpropSumMatrixPosY(struct ModelRoData_BoundingBoxRecord *bbox, Mtxf *arg1)
{
    ...
    if (arg1->m[0][1] >= 0.0f)
    {
        sum += (bbox->Bounds.xmin * arg1->m[0][1]);
    }
    else
    {
        sum += (bbox->Bounds.xmax * arg1->m[0][1]);
    }

    if (arg1->m[1][1] >= 0.0f)
    {
        sum += (bbox->Bounds.ymin * arg1->m[1][1]);
    }
    else
    {
        sum += (bbox->Bounds.ymax * arg1->m[1][1]);
    }

    if (arg1->m[2][1] >= 0.0f)
    {
        sum += (bbox->Bounds.zmin * arg1->m[2][1]);
    }
    else
    {
        sum += (bbox->Bounds.zmax * arg1->m[2][1]);
    }
...
f32 chrpropSumMatrixPosZ(struct ModelRoData_BoundingBoxRecord *bbox, Mtxf *arg1)
{
    ...
    if (arg1->m[0][2] >= 0.0f)
    {
        sum += (bbox->Bounds.xmin * arg1->m[0][2]);
    }
    else
    {
        sum += (bbox->Bounds.xmax * arg1->m[0][2]);
    }

    if (arg1->m[1][2] >= 0.0f)
    {
        sum += (bbox->Bounds.ymin * arg1->m[1][2]);
    }
    else
    {
        sum += (bbox->Bounds.ymax * arg1->m[1][2]);
    }

    if (arg1->m[2][2] >= 0.0f)
    {
        sum += (bbox->Bounds.zmin * arg1->m[2][2]);
    }
    else
    {
        sum += (bbox->Bounds.zmax * arg1->m[2][2]);
    }
...
s32 sub_GAME_7F03F598(coord3d* pos, f32 arg1, BoundPadRecord *boundpads) {
  ...
  do {
    while( true ) {
      ...
      if (((*node & 0xff) == 10) && (**(node + 2) == arg1)) {
        return node;
      }
      if (*(node + 10) == 0) break;
      node = *(node + 10);
    }
    for (; node != NULL; node = *(node + 4)) {
      if (*(node + 6) != 0) {
        node = *(node + 6);
        break;
      }
