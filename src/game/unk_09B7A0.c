...
//CODE.bss:8007A0E0
struct unk_09B7A0_struct_child* dword_CODE_bss_8007A0E0; // array backing pool 0
//CODE.bss:8007A0E4
struct unk_09B7A0_struct_child* dword_CODE_bss_8007A0E4; // array backing pool 1
//CODE.bss:8007A0E8
...
#ifdef NONMATCHING
// NONMATCHING: Likely register allocation diffs from ido_recomp in tree walk.
// Walks all PROP_TYPE_OBJ props in the current position list.
// For each object's model tree, finds the first node with opcode 0x18
// (DisplayList_Collision). If that node's rw data Vertices pointer matches
// arg0, replaces it with arg1. Used when vertex buffers are reallocated.
void sub_GAME_7F09BAC4(struct unk_09B7A0_struct_child *arg0, struct unk_09B7A0_struct_child *arg1) {
    PropRecord *prop;
    Model *model;
    ModelNode *node;
    ModelNode *child;
    ...
    do {
        if (prop->type == PROP_TYPE_OBJ) {
            ...
            if (node != NULL) {
                do {
                    if ((node->Opcode & 0xFF) == 0x18) {
                        rwdata = modelGetNodeRwData(model, node);
                        if ((struct unk_09B7A0_struct_child *)rwdata->DisplayListCollisions.Vertices == arg0) {
                            rwdata->DisplayListCollisions.Vertices = (Vertex *)arg1;
                        }
                        break;
                    }
...
void sub_GAME_7F09BBBC(void) {
    struct unk_09B7A0_struct_parent *entries;
    struct unk_09B7A0_struct_parent *entry_i;
    struct unk_09B7A0_struct_parent *entry_j;
    s32 merged;
    s32 num_entries;
    s32 limit;
    s16 i;
    ...
    if (word_CODE_bss_8007A0F2 < (s16)(dword_CODE_bss_8007A0D8 >> 2)) {
        num_entries = dword_CODE_bss_8007A0DC;
        i = 0;
        limit = num_entries - 1;
        entries = dword_CODE_bss_8007A0EC;
                ...
                if (entry_i->unk0E > 0) {
                    ...
                    if (j < num_entries) {
                        do {
                            ...
                            if (entry_j->unk0E <= 0) {
                                /* skip */
                            } else if (entry_i->unk04 != entry_j->unk04) {
                                /* skip */
                            } else if (entry_i->unk08 != entry_j->unk08) {
                                /* skip */
                            } else {
                                sub_GAME_7F09BAC4(entry_j->unk00, entry_i->unk00);
                                merged = 1;
                                ...
                                entry_i->unk0E += entry_j->unk0E;

                                entries = dword_CODE_bss_8007A0EC;
                                entries[j].unk0E = 0;

                                entries = dword_CODE_bss_8007A0EC;
                                word_CODE_bss_8007A0F2 += entries[j].unk0C;
    ...
    if (merged) {
        ...
        do {
            entries = dword_CODE_bss_8007A0EC;
            entry_i = &entries[i];
            next_idx = entry_i->unk10;

            if (next_idx < 0) {
                done = 1;
            } else if (entry_i->unk0E != 0) {
                i = (s16)next_idx;
            } else {
                entry_j = &entries[next_idx];

                if (entry_j->unk0E != 0) {
                    i = (s16)next_idx;
                } else {
                    entry_i->unk0C += entry_j->unk0C;

                    entries = dword_CODE_bss_8007A0EC;
                    entries[next_idx].unk0E = -1;

                    entries = dword_CODE_bss_8007A0EC;
                    next_idx = entries[next_idx].unk10;
                    entries[i].unk10 = (s16)next_idx;

                    if (next_idx >= 0) {
                        entries = dword_CODE_bss_8007A0EC;
                        entries[next_idx].unk12 = (s16)i;
                    }
                }
            }
        } while (!done);
...
#ifdef NONMATCHING
void *sub_GAME_7F09BE4C(s32 count, s32 pool_id, s32 arg2, s32 arg3) {
    struct unk_09B7A0_struct_parent *entries;
    struct unk_09B7A0_struct_parent *entry;
    s16 *avail_ptr;
    s16 max_entries;
    s32 idx;
    s32 found;
    ...
    if (pool_id == 0xCCCC) {
        entries = dword_CODE_bss_8007A0E8;
        avail_ptr = &word_CODE_bss_8007A0F0;
        max_entries = (s16)dword_CODE_bss_8007A0D4;
    } else if (pool_id == 0x0B0B) {
        entries = dword_CODE_bss_8007A0EC;
        avail_ptr = &word_CODE_bss_8007A0F2;
        max_entries = (s16)dword_CODE_bss_8007A0DC;
    } else {
        return NULL;
    }
    ...
    if (old_count != count) {
        if (max_entries > 0) {
            for (j = 0; j < max_entries; j++) {
                if (entries[j].unk0E == -1) {
                    entry->unk0C = (s16)count;
                    entries[j].unk00 = (struct unk_09B7A0_struct_child *)
                        ((u8 *)entry->unk00 + count * 16);
                    entries[j].unk0C = (s16)(old_count - count);
                    entries[j].unk0E = 0;
                    entries[j].unk12 = (s16)idx;
                    entries[j].unk10 = entry->unk10;
                    split_done = 1;

                    old_next = entry->unk10;
                    if (old_next >= 0) {
                        entries[old_next].unk12 = (s16)j;
                    }
                    entry->unk10 = (s16)j;
                    break;
                }
...
void sub_GAME_7F09C044(Vertex *arg0) {
    struct unk_09B7A0_struct_parent *entries;
    struct unk_09B7A0_struct_parent *entry;
    struct unk_09B7A0_struct_parent *other;
    s16 *avail_ptr;
    s32 idx;
    s32 found;
    s32 next_idx;
    ...
    if ((uintptr_t)arg0 >= (uintptr_t)dword_CODE_bss_8007A0E0 &&
        (uintptr_t)arg0 <= (uintptr_t)(dword_CODE_bss_8007A0E0 + (dword_CODE_bss_8007A0D0 - 1))) {
        avail_ptr = &word_CODE_bss_8007A0F0;
        entries = dword_CODE_bss_8007A0E8;
    } else if ((uintptr_t)arg0 >= (uintptr_t)dword_CODE_bss_8007A0E4 &&
               (uintptr_t)arg0 <= (uintptr_t)(dword_CODE_bss_8007A0E4 + (dword_CODE_bss_8007A0D8 - 1))) {
        avail_ptr = &word_CODE_bss_8007A0F2;
        entries = dword_CODE_bss_8007A0EC;
    } else {
        sub_GAME_7F09B7A8();
    ...
    do {
        ...
        if ((uintptr_t)entry->unk00 != (uintptr_t)arg0) {
            idx = entry->unk10;
            if (idx == -1) {
                found = 1;
            }
            ...
            if (entry->unk0E == 0) {
                *avail_ptr += entry->unk0C;

                next_idx = entry->unk10;
                if (next_idx >= 0) {
                    other = &entries[next_idx];
                    if (other->unk0E == 0) {
                        entry->unk0C += other->unk0C;
                        entry->unk10 = other->unk10;
                        other->unk0E = -1;
                        if (entry->unk10 >= 0) {
                            entries[entry->unk10].unk12 = (s16)idx;
                        }
                    }
                }

                prev_idx = entry->unk12;
                if (prev_idx >= 0) {
                    other = &entries[prev_idx];
                    if (other->unk0E == 0) {
                        other->unk0C += entry->unk0C;
                        other->unk10 = entry->unk10;
                        entry->unk0E = -1;
                        if (other->unk10 >= 0) {
                            entries[other->unk10].unk12 = (s16)prev_idx;
                        }
                    }
                }
            }
