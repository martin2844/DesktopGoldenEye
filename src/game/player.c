...
Gfx *skyRender(Gfx *gdl)
{
     ...
     * WorldToScreenMtxf.  If any is NULL, fall back to solid fill. */
    {
        static int sky_enabled = -1;
        if (sky_enabled < 0) {
            sky_enabled = (getenv("GE007_NO_SKY") == NULL); /* default ON */
        }
        if (!sky_enabled ||
            !currentPlayerGetMatrix10D4() ||
            !currentPlayerGetProjectionMatrixF() ||
...
#ifdef NATIVE_PORT
    /* Guard: skywaterimages is set during image_bank initialization.
     * If sky code runs before the image bank is ready, the pointer is NULL.
     * Skip the cloud texture layer entirely in that case — the solid-color
     * fill from the first phase already gives a visible sky. */
    if (!skywaterimages) {
        return gdl;
    }
#endif
