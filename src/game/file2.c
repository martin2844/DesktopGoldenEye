...
extern void fileGenerateCRC(u8 *addressA, u8 *addressB, s32 *retval);
...
//CODE.bss:80069B60
#ifdef NATIVE_PORT
void *dword_CODE_bss_80069B60;
s32 port_save_copy_count = 0;
s32 port_save_copy_source = -1;
s32 port_save_copy_target = -1;
s32 port_save_copy_result = 0;
s32 port_save_delete_count = 0;
s32 port_save_delete_folder = -1;
s32 port_save_delete_result = 0;
#else
...
void fileWriteSmallSave(smallSave *save)
{
    if (fileGamePakProbe())
    {
        fileGenerateCRC(&save->unk[0], &save->unk[24], &save->chksum1);
...
void fileWriteSave(save_data *save)
{
    if (save >= &saves[SAVESLOT1] && save < &saves[SAVESLOTRAMROM])
    {
        if ( fileGamePakProbe())
        {
            fileGenerateCRC(&save->completion_bitflags, (u8 *)(save + 1), &save->chksum1);
...
void fileOverwriteSaveSlotWithNewSave(save_data *save1, save_data *save2)
{
    ...
    {
        save_data *target = save1;

        if (target == NULL)
        {
            target = fileGetSaveForFoldernum(fileGetSaveFolder(save2));
        }

        if (target != NULL)
        {
            *target = *save2;
            fileWriteSave(target);
        }
    }
#endif
}
...
void fileGetHighestStageDifficultyCompletedForFolder(s32 foldernum, LEVEL_SOLO_SEQUENCE *levelid, DIFFICULTY *difficulty)
{
    ...
    if (folder)
    {
        for (difficultyid = DIFFICULTY_007; difficultyid >= DIFFICULTY_AGENT; difficultyid--)
        {
            ...
            {
                if (fileGetSaveStageCompletedForDifficulty(folder, (LEVEL_SOLO_SEQUENCE)stageid, (DIFFICULTY)difficultyid))
                {
                    *levelid = (LEVEL_SOLO_SEQUENCE)stageid;
...
LEVEL_SOLO_SEQUENCE fileGetHighestStageUnlockedForFolder(s32 foldernum)
{
    ...
    if ( fileGetSaveForFoldernum(foldernum) != NULL)
    {
        for (levelid = SP_LEVEL_EGYPT; levelid >= SP_LEVEL_DAM; levelid--)
        {
            ...
            {
                if (fileIsStageUnlockedAtDifficulty(foldernum, (LEVEL_SOLO_SEQUENCE)levelid, (DIFFICULTY)difficulty))
                {
                    return (LEVEL_SOLO_SEQUENCE)levelid;
                }
            }
        }
    }
...
void fileDeleteSaveForFolder(s32 foldernum)
{
    ...
    port_save_delete_folder = foldernum;
    port_save_delete_result = 0;
#endif
...
void fileCopyFolderToFirstFree(s32 foldernum)
{
    ...
    port_save_copy_source = foldernum;
    port_save_copy_target = -1;
    port_save_copy_result = 0;
#endif
            ...
            if (levelid >= SP_LEVEL_DAM)
            {
                if (difficulty >= DIFFICULTY_AGENT) {
                    ...
                    if ((s32)other < MAX_FOLDER_COUNT)
                    {
                        ...
                        {
                            *temp_s2 = new_save;
                            fileSetSaveFlagDoReset(temp_s2, FALSE);
                            fileWriteSave(temp_s2);
                            port_save_copy_result = 1;
                        }
                        else
#endif
                        {
...
void fileLoadSettingsForFolder(u32 folder)
{
    ...
    if (save)
    {
        musicVol = (save->music_vol << 7) | (save->music_vol >> 1);
        sfxVol = (save->sfx_vol << 7) | (save->sfx_vol >> 1);

        options = save->options;

#ifdef NATIVE_PORT
        /* Fresh EEPROM has all zeros. On N64, the game menu initializes
         * defaults before first play. The PC --mission path bypasses that,
         * so a blank save gives all options and volumes OFF. Apply N64
         * defaults when the save looks uninitialized (options=0, volumes=0). */
        if (options == 0 && save->music_vol == 0 && save->sfx_vol == 0) {
            musicVol = 0x7FFF;
            sfxVol = 0x7FFF;
            options = OPTION_AUTOAIM | OPTION_SIGHTONSCREEN
                    | OPTION_LOOKAHEAD | OPTION_DISPLAYAMMO;
        }
#endif

        portAudioTraceSfxJson(
            "{\"event\":\"load_settings\",\"folder\":%u,\"raw_music\":%u,\"raw_sfx\":%u,\"music_vol\":%u,\"sfx_vol\":%u,\"options\":%u}",
            (unsigned int)folder,
            (unsigned int)save->music_vol,
            (unsigned int)save->sfx_vol,
            (unsigned int)musicVol,
            (unsigned int)sfxVol,
            (unsigned int)options);

        set_mTrack2Vol(musicVol);
        sub_GAME_7F0A91A0(sfxVol);
