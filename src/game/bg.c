...
s32 sub_GAME_7F0B7F84(s32 gdl, s32 arg1, s32 arg2, s32 arg3, s32 *arg4);
...
/* Forward declarations for external functions missing from headers */
...
extern void initializeRoomData(void);
...
extern void texCopyGdls(Gfx *arg0, Gfx *arg1, s32 arg2);
extern s32 texLoadFromGdl(Gfx *src, s32 size, Gfx *dst, void *pool);
...
#ifdef NATIVE_PORT
/* Line-triangle intersection output struct (defined in unk_092890.c) */
struct unk_bg_struct {
    f32 unk00; f32 unk04; f32 unk08;  /* hit point xyz */
    f32 unk0c; f32 unk10; f32 unk14;  /* normal xyz */
    f32 unk18; f32 unk1c; f32 unk20;
    f32 unk24;
    s16 unk28; s16 unk2a; s16 unk2c; s16 unk2e;
};
extern s32 intersectLineTriangle(Vtx *arg0, Vtx *arg1, Vtx *arg2,
    struct coord3d *arg3, struct coord3d *arg4, struct coord3d *arg5,
    struct coord3d *arg6, struct unk_bg_struct *arg7);
...
static void portParseRoomRenderOverrideList(const char *env, int *rooms, int *count)
{
    *count = 0;
    ...
    if (env == NULL || *env == '\0' || (env[0] == '0' && env[1] == '\0')) {
        return;
    }

    while (*env != '\0' && *count < PORT_ROOM_RENDER_OVERRIDE_MAX) {
        char *endptr;
        long room;
        ...
        room = strtol(env, &endptr, 0);
        if (endptr == env) {
            break;
        }

        rooms[*count] = (int)room;
        (*count)++;
        env = endptr;
    }
}

static void portInitRoomRenderOverrides(void)
{
    if (g_PortRoomRenderOverrideInitialized) {
        return;
    }

    g_PortRoomRenderOverrideInitialized = 1;
    portParseRoomRenderOverrideList(getenv("GE007_FORCE_RENDERED_ROOMS"),
                                    g_PortForceRenderedRooms,
                                    &g_PortForceRenderedRoomCount);
...
static void portApplyRoomRenderOverrides(void)
{
    ...
    for (i = 0; i < g_PortForceRenderedRoomCount; i++) {
        s32 room = g_PortForceRenderedRooms[i];
        if (room > 0 && room < g_MaxNumRooms) {
            g_BgRoomInfo[room].room_rendered = 1;
        }
    }

    for (i = 0; i < g_PortForceUnrenderedRoomCount; i++) {
        s32 room = g_PortForceUnrenderedRooms[i];
        if (room > 0 && room < g_MaxNumRooms) {
...
#ifdef NATIVE_PORT
static void bgPopulateHitRecord(BgHitRecord *dst, const struct unk_bg_struct *src,
    Vertex *vtx0, Vertex *vtx1, Vertex *vtx2, Gfx *tricmd, s16 tri_index, s16 texturenum)
{
    dst->pos.x = src->unk00;
    dst->pos.y = src->unk04;
    dst->pos.z = src->unk08;
    dst->normal.x = src->unk0c;
    dst->normal.y = src->unk10;
    dst->normal.z = src->unk14;
    dst->vtx0 = vtx0;
    dst->vtx1 = vtx1;
    dst->vtx2 = vtx2;
    dst->tricmd = tricmd;
    dst->tri_index = tri_index;
    dst->texturenum = texturenum;
    dst->texture_s = -1;
    dst->texture_t = -1;
}

#define PORT_BG_G_SETTEX 0xC0
#define PORT_BG_IMAGESEG_TOKEN_PREFIX 0xABCD0000U
#define PORT_BG_IMAGESEG_TOKEN_MASK 0xFFFF0000U

static u32 bgReadBe32(const u8 *p)
{
    return ((u32)p[0] << 24) | ((u32)p[1] << 16) | ((u32)p[2] << 8) | (u32)p[3];
}

static s16 bgTextureNumFromToken(u32 token)
{
    u32 texturenum;

    if ((token & PORT_BG_IMAGESEG_TOKEN_MASK) == PORT_BG_IMAGESEG_TOKEN_PREFIX) {
        texturenum = token & 0xFFFFU;
    } else {
        texturenum = token;
    }

    if (texturenum < (u32)MAX_TEXTURES) {
        return (s16)texturenum;
    }

    return -1;
}

static s16 bgFindTextureForTriangleCommand(u8 *dl_base, u8 *cmd)
{
    u8 *search;

    if (dl_base == NULL || cmd == NULL || cmd < dl_base) {
        return -1;
    }

    search = cmd;
    while (search >= dl_base) {
        u8 opcode = search[0];
        u32 w0 = bgReadBe32(search);
        u32 w1 = bgReadBe32(search + 4);

        if (opcode == PORT_BG_G_SETTEX && (w0 != 0 || w1 != 0)) {
            u32 texturenum = w1 & 0xFFFU;
        ...
        if (opcode == (u8)G_SETTIMG) {
            s16 texturenum = bgTextureNumFromToken(w1);
...
extern void lf_verify_sweep_room(s32 room_index);  /* GE007_LF_VERIFY debug sweep (W4.E1.T2); self-gates */
...
void lf_populate_room_lightfixtures(s32 room)
{
    ...
    /* W4.E1.T2: after populating, optionally verify every fixture triangle's
     * vertex base vs the hit-record route (self-gates on GE007_LF_VERIFY). */
    lf_verify_sweep_room(room);
}
...
static int bgHitTraceEnabled(void)
{
    ...
    if (enabled < 0) {
        const char *value = getenv("GE007_TRACE_BG_HIT");
...
static void bgHitTracePrintf(const char *fmt, ...)
{
    static int budget = -1;
    va_list ap;

    if (!bgHitTraceEnabled()) {
        return;
    }

    if (budget < 0) {
        const char *value = getenv("GE007_TRACE_BG_HIT_BUDGET");
        budget = (value != NULL && value[0] != '\0') ? atoi(value) : 200;
    }
...
static int bgTraceTrisEnabled(void)
{
    static int enabled = -1;
    if (enabled < 0) {
        enabled = (getenv("GE007_TRACE_BG_TRIS") != NULL);
    }
    return enabled;
}

static int bgTraceTrisAfterFrame(void)
{
    static int after_frame = -2;
    if (after_frame < -1) {
        const char *env = getenv("GE007_TRACE_BG_AFTER_FRAME");
        after_frame = env ? atoi(env) : 0;
    }
    return after_frame;
}

#define BG_TRANSPARENT_ROOM_SORT_MAX 256

static int bgTransparentRoomSortEnabled(void)
{
    static int enabled = -1;

    if (enabled < 0) {
        const char *disable_all = getenv("GE007_DISABLE_ROOM_XLU_SORT");
        const char *sort_all = getenv("GE007_SORT_ROOM_XLU");
        const char *sort_rooms = getenv("GE007_SORT_ROOM_XLU_ROOMS");

        enabled = 1;

        if ((disable_all != NULL && disable_all[0] != '\0' && disable_all[0] != '0')
            || (sort_all != NULL && sort_all[0] == '0')
            || (sort_rooms != NULL && sort_rooms[0] == '0')) {
            enabled = 0;
        }
    }
...
static int bgTraceRoomXluSortEnabled(void)
{
    static int enabled = -1;

    if (enabled < 0) {
        enabled = (getenv("GE007_TRACE_ROOM_XLU_SORT") != NULL);
    }

    return enabled;
}

static int bgTraceRoomXluSortAfterFrame(void)
{
    static int after_frame = -2;
    ...
    if (after_frame < -1) {
        ...
        if (env == NULL) {
            env = getenv("GE007_TRACE_BG_AFTER_FRAME");
        }
        after_frame = env ? atoi(env) : 0;
    }
...
static f32 bgTransparentRoomDepthKey(s32 roomid)
{
    Mtxf *mtx;
    s_room_info *room;
    f32 minx;
    ...
    minx = room->minbounds.x * room_data_float2;
    miny = room->minbounds.y * room_data_float2;
    minz = room->minbounds.z * room_data_float2;
    maxx = room->maxbounds.x * room_data_float2;
    maxy = room->maxbounds.y * room_data_float2;
    maxz = room->maxbounds.z * room_data_float2;
    ...
    for (xi = 0; xi < 2; xi++) {
        for (yi = 0; yi < 2; yi++) {
            for (zi = 0; zi < 2; zi++) {
                ...
                corner.x = xi ? maxx : minx;
                corner.y = yi ? maxy : miny;
                corner.z = zi ? maxz : minz;
                mtx4TransformVecInPlace(mtx, &corner);
...
static void bgSortTransparentRoomOrderForBucket(s32 *order, f32 *keys, s32 count, s32 sort_bucket)
{
    s32 i;
    s32 reordered = 0;

    if (!bgTransparentRoomSortEnabled() || count <= 1) {
        return;
    }

    for (i = 0; i < count; i++) {
        keys[i] = bgTransparentRoomDepthKey(dword_CODE_bss_8007FFA0[order[i]].roomid);
    }

    for (i = 1; i < count; i++) {
        s32 order_value = order[i];
        f32 key_value = keys[i];
        s32 j = i - 1;
        ...
        order[j + 1] = order_value;
        keys[j + 1] = key_value;
    }

    if (bgTraceRoomXluSortEnabled() && g_frame_count_diag >= bgTraceRoomXluSortAfterFrame()) {
        static int trace_frame = -1;
        static int trace_count = 0;

        if (trace_frame != g_frame_count_diag) {
            trace_frame = g_frame_count_diag;
            trace_count = 0;
        }

        if (trace_count < 32) {
            fprintf(stderr,
                    "[BG-TRANS-SORT] frame=%d bucket=%d count=%d reordered=%d first_room=%d first_key=%.1f last_room=%d last_key=%.1f\n",
                    g_frame_count_diag,
                    sort_bucket,
                    count,
                    reordered,
                    dword_CODE_bss_8007FFA0[order[0]].roomid,
                    keys[0],
                    dword_CODE_bss_8007FFA0[order[count - 1]].roomid,
                    keys[count - 1]);
            trace_count++;
        }
    }
...
static inline struct bbox2d *bgGetCurrentPlayerScreenBbox(void)
{
    return (struct bbox2d *)&g_CurrentPlayer->screenxminf;
}

static inline void bgCopyCurrentPlayerScreenBboxToFloats(f32 *dst)
{
    dst[0] = g_CurrentPlayer->screenxminf;
    dst[1] = g_CurrentPlayer->screenyminf;
    dst[2] = g_CurrentPlayer->screenxmaxf;
    dst[3] = g_CurrentPlayer->screenymaxf;
}
...
static int bgVisTraceBudget(void)
{
    static int budget = -1;
    if (budget < 0) {
        const char *env = getenv("BG_VIS_TRACE_MAX");
        budget = env ? atoi(env) : 160;
    }
    return budget;
}

typedef struct BgRoomAdmitTraceContext {
    const char *reason;
    s32 source_room;
    s32 portal_idx;
    s32 depth;
...
static int bgPortalTraceEnabled(void)
{
    static int enabled = -1;
    if (enabled < 0) {
        enabled = (getenv("GE007_TRACE_PORTALS") != NULL);
    }
    return enabled;
}

static int bgPortalTraceBudget(void)
{
    static int budget = -1;
    if (budget < 0) {
        const char *env = getenv("GE007_TRACE_PORTAL_BUDGET");
        budget = env ? atoi(env) : 256;
    }
    return budget;
}

static int bgPortalTraceAfterFrame(void)
{
    static int after_frame = -2;
    if (after_frame < -1) {
        const char *env = getenv("GE007_TRACE_PORTAL_AFTER_FRAME");
        after_frame = env ? atoi(env) : 0;
    }
    return after_frame;
}

static int bgPortalTraceFilterValue(const char *name)
{
...
static int bgShouldTracePortal(s32 source_room, s32 portal_idx, s32 dest_room)
{
    ...
    if (g_frame_count_diag < bgPortalTraceAfterFrame()) {
        return 0;
    }
    ...
    if (trace_count >= bgPortalTraceBudget()) {
        return 0;
    }
...
static int bgRoomAdmitTraceEnabled(void)
{
    static int enabled = -1;
    if (enabled < 0) {
        enabled = (getenv("GE007_TRACE_ROOM_ADMIT") != NULL);
    }
    return enabled;
}

static int bgRoomAdmitTraceBudget(void)
{
    static int budget = -1;
    if (budget < 0) {
        const char *env = getenv("GE007_TRACE_ROOM_ADMIT_BUDGET");
        budget = env ? atoi(env) : 256;
    }
    return budget;
}
...
static int bgPortalBackfaceBypassApplies(s32 source_room, s32 portal_idx, s32 dest_room)
{
    ...
    if (bypass_all < 0) {
        bypass_all = (getenv("GE007_BYPASS_PORTAL_BACKFACE") != NULL);
    }
    if (!bypass_all) {
        return 0;
    }
    ...
    if (filter_src == -2) {
        filter_src = bgPortalTraceFilterValue("GE007_BYPASS_PORTAL_BACKFACE_SRC");
    }
    if (filter_dest == -2) {
        filter_dest = bgPortalTraceFilterValue("GE007_BYPASS_PORTAL_BACKFACE_DEST");
    }
    if (filter_idx == -2) {
        filter_idx = bgPortalTraceFilterValue("GE007_BYPASS_PORTAL_BACKFACE_IDX");
    }
...
static int bgPortalBackfaceProjectFallbackEnabled(void)
{
    static int enabled = -1;
...
static f32 bgPortalParentClipMinSpan(void)
{
    static f32 value = -1.0f;
    if (value < 0.0f) {
        const char *env = getenv("GE007_PORTAL_PARENT_CLIP_MIN_SPAN");
        value = env ? (f32)atof(env) : 0.0f;
    }
    return value;
}

static f32 bgPortalAcceptedMinSpan(void)
{
    static f32 value = -1.0f;
    if (value < 0.0f) {
        const char *env = getenv("GE007_PORTAL_ACCEPTED_MIN_SPAN");
        value = env ? (f32)atof(env) : 0.0f;
    }
    return value;
}
...
static int bgPortalRetryScreenClipEnabled(void)
{
    ...
    if (enabled < 0) {
        const char *env = getenv("GE007_PORTAL_RETRY_SCREEN_CLIP");
        enabled = env ? atoi(env) : 0;
    }
...
static int bgPortalLegacyProjectClampEnabled(void)
{
    static int enabled = -1;

    if (enabled < 0) {
        const char *env = getenv("GE007_PORTAL_LEGACY_PROJECT_CLAMP");
        enabled = env ? atoi(env) : 0;
    }
...
static int bgPortalEdgeRescueEnabled(void)
{
    ...
    if (enabled == -2) {
        ...
        if (env) {
            ...
            /* §4.1: no explicit env -- default OFF under --faithful (a
             * `--faithful` port divergence widener), ON otherwise. An
             * explicit GE007_PORTAL_EDGE_RESCUE/GE007_AUTO_NEIGHBOR_ROOMS
             * value always wins in either direction (checked above). */
            extern int g_pcFaithfulSim;
            enabled = legacy_env ? (atoi(legacy_env) != 0) : (g_pcFaithfulSim ? 0 : 1);
        }
    }
...
static f32 bgPortalBboxHeight(const f32 *bbox)
{
...
{
    f32 width = bgPortalBboxWidth(bbox);
    f32 height = bgPortalBboxHeight(bbox);
    f32 cx = (bbox[0] + bbox[2]) * 0.5f;
    f32 cy = (bbox[1] + bbox[3]) * 0.5f;
    f32 half;
                                      ...
                                      s32 *reject_reason)
{
    ...
    out_bbox[0] = projected_bbox[0];
    out_bbox[1] = projected_bbox[1];
    out_bbox[2] = projected_bbox[2];
    out_bbox[3] = projected_bbox[3];

    if (bgPortalBboxWidth(parent_bbox) >= bgPortalParentClipMinSpan()
        && bgPortalBboxHeight(parent_bbox) >= bgPortalParentClipMinSpan()) {
        bgRectIntersect((struct bbox2d *)out_bbox, (struct bbox2d *)parent_bbox);
    } else if (trace_portal) {
        fprintf(stderr,
                "[BG-PORTAL] relax=skip_parent_clip src=%d portal=%d phase=%s parent=(%.1f,%.1f,%.1f,%.1f)\n",
                source_room, portal_idx, phase ? phase : "?",
                parent_bbox[0], parent_bbox[1], parent_bbox[2], parent_bbox[3]);
    }

    bgRectIntersect((struct bbox2d *)out_bbox,
                    (struct bbox2d *)&g_CurrentPlayer->screenxminf);

    if (bgPortalRetryScreenClipEnabled()
        && (bgPortalBboxWidth(out_bbox) <= 0.0f || bgPortalBboxHeight(out_bbox) <= 0.0f)) {
        out_bbox[0] = projected_bbox[0];
        out_bbox[1] = projected_bbox[1];
        out_bbox[2] = projected_bbox[2];
        out_bbox[3] = projected_bbox[3];
...
static int bgPortalProjectFrustumFallbackEnabled(void)
{
    ...
    if (enabled < 0) {
        ...
        /* §4.1: default OFF under --faithful, ON otherwise; explicit env
...
/* I-1 fix: bgApplyVisibilitySupplement's adjacency gate needs a snapshot of
 * portal-neighbor-to-rendered that reflects room_rendered AFTER the
 * edge-rescue/frustum-fallback promotions run later in this frame, not just
 * the state at the top of the block (see call site below). The naive fix
 * re-invoked bgMarkPortalNeighborsToRendered() a second time, but that
 * mutates the GLOBAL room_neighbor_to_rendered flags, which also feed
 * getROOMID_isNeighborToRendered() — a second, sim-visible consumer
 * (NATIVE_PORT prop-visibility widener in chrobjhandler.c) that has no
 * business seeing a wider result than the single canonical per-frame mark.
 * This walks the identical portal graph but writes into a caller-owned
 * LOCAL array instead of the global struct field, so the supplement can see
 * an up-to-date adjacency test without perturbing the global flags' normal
 * once-per-frame lifecycle. */
static void bgComputePortalNeighborSnapshotLocal(bg_portal_data_entry *portals, u8 *snapshot)
{
    bg_portal_data_entry *cur = portals;
...
static void bgSetRoomAdmitTraceContext(const char *reason, s32 source_room, s32 portal_idx, s32 depth)
{
    g_BgRoomAdmitTraceContext.reason = reason;
    g_BgRoomAdmitTraceContext.source_room = source_room;
    g_BgRoomAdmitTraceContext.portal_idx = portal_idx;
    g_BgRoomAdmitTraceContext.depth = depth;
}

static void bgClearRoomAdmitTraceContext(void)
{
    g_BgRoomAdmitTraceContext.reason = NULL;
    g_BgRoomAdmitTraceContext.source_room = -1;
    g_BgRoomAdmitTraceContext.portal_idx = -1;
...
/* §4.1 boot-log accessor: resolved widener states for this file, read by
 * main_pc.c's `--faithful` one-line boot log. Calling these also performs
 * their first-use env-read/cache, same as any other call site. */
void bgGetFaithfulWidenerStates(s32 *edgeRescue, s32 *frustumFallback, s32 *visSupplement)
{
    if (edgeRescue) {
        *edgeRescue = bgPortalEdgeRescueEnabled();
    }
    if (frustumFallback) {
        *frustumFallback = bgPortalProjectFrustumFallbackEnabled();
    }
    if (visSupplement) {
        *visSupplement = bgVisibilitySupplementEnabled();
    }
}
...
static int bgVisibilitySupplementAnyRoomEnabled(void)
{
    static int enabled = -1;

    if (enabled < 0) {
        const char *env = getenv("GE007_VIS_SUPPLEMENT_ANY_ROOM");
        enabled = env ? (atoi(env) != 0) : 0;
    }
...
static void bgApplyVisibilitySupplement(bbox2d *player_bbox, const u8 *neighbor_snapshot)
{
    ...
    for (room = 1; room < g_MaxNumRooms; room++) {
        ...
        /* Only rescue rooms that are portal-connected to the rendered set.
         * `neighbor_snapshot` is a LOCAL recomputation (see
         * bgComputePortalNeighborSnapshotLocal) taken after this frame's
         * edge-rescue/frustum-fallback promotions, not the global
         * room_neighbor_to_rendered field — that field has a second,
         * sim-visible consumer (getROOMID_isNeighborToRendered) and must not
         * be widened just to un-stale this gate. The supplement exists to
         * patch portal-BFS false NEGATIVES — a room the stock traversal
         * should have reached but dropped. Without this gate the AABB-gap
         * heuristic also admits rooms the portal graph correctly rejects: on
         * Dam, room 14 (giant-AABB far-valley terrain, AABB-gap 0 to half
         * the level) was drawn every frame from spawn and its fully fogged
         * far geometry sliced through the road as sky-blue shards at
         * FovY >= ~55 (--faithful pins 60; the default 50 just kept it
         * off-screen). GE007_VIS_SUPPLEMENT_ANY_ROOM=1 restores the old
         * unconditional behavior for A/B. */
        if (!neighbor_snapshot[room]
            && !bgVisibilitySupplementAnyRoomEnabled()) {
            continue;
        }
...
static void bgTraceRoomSummaryIfRequested(s32 room)
{
    ...
    {
        bg_room_data *rd = &ptr_bgdata_room_fileposition_list[room];
        s_room_info *ri = &g_BgRoomInfo[room];

        fprintf(stderr,
                "[ROOM-SUMMARY] room=%d pos=(%.1f,%.1f,%.1f) "
                "aabb=(%.1f,%.1f,%.1f)-(%.1f,%.1f,%.1f) "
                "point_off=0x%X pri_off=0x%X sec_off=0x%X "
                "point_size=0x%X pri_size=0x%X sec_size=0x%X\n",
                room,
                rd->pos.x, rd->pos.y, rd->pos.z,
                ri->minbounds.x, ri->minbounds.y, ri->minbounds.z,
                ri->maxbounds.x, ri->maxbounds.y, ri->maxbounds.z,
                (unsigned)rd->pPointTableBin,
                (unsigned)rd->pPriMappingBin,
                (unsigned)rd->pSecMappingBin,
                (unsigned)ri->csize_point_index_binary,
                (unsigned)ri->csize_primary_DL_binary,
                (unsigned)ri->csize_secondary_DL_binary);
        fflush(stderr);
    }
}

static void bgTraceRoomDlIfRequested(s32 room, const u8 *dl_base, const u8 *vtx_base)
{
    static int requested_room = -2;

    if (requested_room == -2) {
        const char *env = getenv("GE007_TRACE_ROOM_DL");
        requested_room = env ? atoi(env) : -1;
    }

    if (requested_room < 0 || requested_room != room || dl_base == NULL || vtx_base == NULL) {
        return;
    }

    fprintf(stderr, "[ROOM-DL] room=%d begin\n", room);

    for (int cmd_index = 0; cmd_index < 24; cmd_index++) {
        const u8 *cmd = dl_base + cmd_index * 8;
        u32 w0 = read_be32u(cmd);
        u32 w1 = read_be32u(cmd + 4);
        u8 opcode = cmd[0];

        fprintf(stderr,
                "[ROOM-DL]   cmd#%d op=0x%02X w0=0x%08X w1=0x%08X\n",
                cmd_index + 1, opcode, w0, w1);

        if (opcode == G_VTX) {
            int base_num = ((cmd[1] >> 4) & 0xF) + 1;
            int base_dest = cmd[1] & 0xF;
            int f3dex_num = (int)((w0 & 0xFFFF) >> 10);
            int f3dex_dest = (int)((w0 >> 16) & 0xFF) / 2;
            u32 vtx_off = w1 & 0x00FFFFFF;
            const u8 *vd = vtx_base + vtx_off;
            s16 x = (s16)((vd[0] << 8) | vd[1]);
            s16 y = (s16)((vd[2] << 8) | vd[3]);
            s16 z = (s16)((vd[4] << 8) | vd[5]);

            fprintf(stderr,
                    "[ROOM-DL]     VTX base(n=%d,dst=%d) f3dex(n=%d,dst=%d) "
                    "seg=%u off=0x%06X first=(%d,%d,%d)\n",
                    base_num, base_dest, f3dex_num, f3dex_dest,
                    (unsigned)((w1 >> 24) & 0xFF), (unsigned)vtx_off,
                    x, y, z);
        } else if (opcode == 0xB1) {
            fprintf(stderr,
                    "[ROOM-DL]     TRI4 (%d,%d,%d) (%d,%d,%d) (%d,%d,%d) (%d,%d,%d)\n",
                    (int)((w1 >> 0) & 0xF), (int)((w1 >> 4) & 0xF), (int)((w0 >> 0) & 0xF),
                    (int)((w1 >> 8) & 0xF), (int)((w1 >> 12) & 0xF), (int)((w0 >> 4) & 0xF),
                    (int)((w1 >> 16) & 0xF), (int)((w1 >> 20) & 0xF), (int)((w0 >> 8) & 0xF),
                    (int)((w1 >> 24) & 0xF), (int)((w1 >> 28) & 0xF), (int)((w0 >> 12) & 0xF));
        }
    ...
    s32 max_rooms)
{
    ...
    if (g_BgPortals == NULL || g_BgPortals->offset_portal == 0) {
        return 0;
    }

    for (s32 pi = 0; g_BgPortals[pi].offset_portal != 0 && out_count < max_rooms; pi++) {
        bg_portal_data_entry *portal = &g_BgPortals[pi];
        s32 dest_room = -1;
        struct PortalMetric metric;
        f32 dot;
        f32 scale;

        if (current_room == portal->connectedRoom1) {
            dest_room = portal->connectedRoom2;
        } else if (current_room == portal->connectedRoom2) {
            dest_room = portal->connectedRoom1;
        } else {
            continue;
        }

        sub_GAME_7F0B96CC(pi, &metric);
        dot = (position->x * metric.normal.x
            + position->y * metric.normal.y
            + position->z * metric.normal.z) * room_data_float1;
        scale = sub_GAME_7F0B9990(pi);

        if ((current_room == portal->connectedRoom1 && metric.max <= (dot - scale))
            || (current_room == portal->connectedRoom2 && (dot + scale) <= metric.min)) {
            s32 duplicate = 0;

            for (s32 out_idx = 0; out_idx < out_count; out_idx++) {
                if (out_rooms[out_idx] == dest_room) {
                    duplicate = 1;
                    break;
                }
            }
...
{
    s32 current_room = seed_room;
    u8 visited[256];

    if (position == NULL
        || current_room <= 0
        || current_room >= g_MaxNumRooms
        || g_BgPortals == NULL
        || g_BgPortals->offset_portal == 0) {
        return seed_room;
    }

    memset(visited, 0, sizeof(visited));

    for (s32 iter = 0; iter < 16; iter++) {
        s32 next_room = -1;

        if (current_room < 0 || current_room >= (s32)sizeof(visited)) {
            break;
        }
        ...
        for (s32 pi = 0; g_BgPortals[pi].offset_portal != 0; pi++) {
            ...
            if (current_room != portal->connectedRoom1
                && current_room != portal->connectedRoom2) {
                continue;
            }
            ...
            if (current_room == portal->connectedRoom1) {
                if (metric.max <= (dot - scale)) {
                    next_room = portal->connectedRoom2;
                    break;
                }
            } else if ((dot + scale) <= metric.min) {
                next_room = portal->connectedRoom1;
                break;
            }
        }
                                              ...
                                              s32 skip_portal, s32 depth)
{
    ...
    if (portals == NULL || portals->offset_portal == 0) {
        return;
    }
...
//CODE.bss:8007FF90
#ifdef NATIVE_PORT
intptr_t dword_CODE_bss_8007FF90;
#else
...
#ifdef NATIVE_PORT
int g_viscmd_diag_frame = 0;

static int bgUseFloatField10E0(void)
{
    return bondviewField10E0IsFloat();
}
#endif
...
#ifdef NATIVE_PORT
/* Per-room buffer tracking for on-demand room loads (sub_GAME_7F0B6368).
 * Preloader buffers are tracked separately in lvl.c's pc_room_allocs[]. */
static void *s_pc_room_vtx_bufs[MAXROOMCOUNT];
static void *s_pc_room_dl_bufs[MAXROOMCOUNT];
static void *s_pc_room_sec_bufs[MAXROOMCOUNT];
#endif
...
//D:80044824
#ifdef NATIVE_PORT
/* On N64, the s_specialportal struct uses a flexible array member (u8 portallist[]),
 * so entries are tightly packed. The parser (sub_GAME_7F0B37EC) walks this as raw
 * bytes. On PC, s_specialportal has portallist[16] (fixed), which adds padding.
 * Use a flat byte array to match the exact N64 memory layout. */
...
/* W1.E2.T1: audited accessor for the world-space sun direction. Normalizes
 * GlobalLight's fixed gdSPDefLights1 direction (77,77,46). Render code reads this
 * (render-reads-sim is allowed, rail R1); the renderer never reaches into the
 * light struct layout directly. Reproduces the prior hand-rounded literal
 * {0.651376,0.651376,0.389134} to within float precision. */
void bgGetGlobalLightDir(f32 out[3]) {
    f32 dx = (f32)GlobalLight.l[0].l.dir[0];
    f32 dy = (f32)GlobalLight.l[0].l.dir[1];
    f32 dz = (f32)GlobalLight.l[0].l.dir[2];
...
void sub_GAME_7F0B37EC(void) {
...
#ifdef NATIVE_PORT
    /* On N64, g_BgCurrentRoom was immediately after specialportalarray in .data,
     * so &g_BgCurrentRoom worked as the end pointer. On PC, globals may have
     * redzones or different layout. Use the actual array size. */
    ...
    do {
        if (levelentry_index == *ptr++) {
            rangeStart = *ptr;
            do {
                index = rangeStart & 0xFF;
                index2 = index;
                if (!((s32)ptr[1] < index)) {
                    do {
                        entry = &g_BgPortals[index2];
                        index++;
                        ctrl = entry->controlbytes1;
                        index2 = index & 0xFF;
                        index = index2;
                        entry->controlbytes1 = ctrl | 2;
                    } while (!((s32)ptr[1] < index2));
                }
                rangeStart = ptr[2];
                ptr += 2;
            } while (rangeStart != 0xFF);
        } else {
            do {
                tempByte = ptr[2];
                ptr += 2;
...
#ifdef NONMATCHING
// Processes a visibility list for room rendering. The list is a sequence of
// null-terminated byte string pairs. Searches the first string of each pair for
// room_id; when found, processes all rooms in both strings of that pair through
// sub_GAME_7F0B5208 (visibility check) and sub_GAME_7F0B39BC (render setup).
// ido_recomp: register allocation differences.
void sub_GAME_7F0B38B4(s32 room_id, u8 *vis_list) {
    u8 *ptr;
    u8 byte;
    u8 target;
    bbox2d *player_bbox;
    ...
    for (;;) {
        /* Search current string for target room */
        while (byte != target) {
            byte = *(++ptr);
            if (byte == 0) {
                /* Not found - skip second string */
                while (*(++ptr) != 0) {}
                ptr++;
                /* Try next pair */
                byte = *ptr;
        ...
        /* Found match - process first string */
        do {
            player_bbox = bgGetCurrentPlayerScreenBbox();
            if (sub_GAME_7F0B5208(byte, player_bbox)) {
                sub_GAME_7F0B39BC(byte, 0, player_bbox, 1);
            }
            byte = *(++ptr);
        } while (byte != 0);

        /* Advance past null terminator to second string */
        ptr++;
...
s32 sub_GAME_7F0B39BC(int curroom,int unk1, bbox2d * screensize, s32 next)
{
...
#ifdef NATIVE_PORT
    bbox2d forced_bbox;
    bbox2d *bbox_src = screensize;
    static int intro_fullscreen_bbox = -1;
    static int trace_room_bbox = -1;
    static int trace_room_admit_count = 0;
    enum CAMERAMODE camera_mode = bondviewGetCameraMode();
    int intro_camera_active = (camera_mode == CAMERAMODE_INTRO
                            || camera_mode == CAMERAMODE_FADESWIRL
                            || camera_mode == CAMERAMODE_SWIRL);

    if (intro_fullscreen_bbox < 0) {
        intro_fullscreen_bbox = (getenv("GE007_INTRO_FULLSCREEN_BBOX") != NULL);
    }
    if (trace_room_bbox < 0) {
        trace_room_bbox = (getenv("GE007_TRACE_ROOM_BBOX") != NULL);
    }

    if (intro_fullscreen_bbox && intro_camera_active)
    {
        forced_bbox.min.x = g_CurrentPlayer->screenxminf;
        forced_bbox.min.y = g_CurrentPlayer->screenyminf;
        forced_bbox.max.x = g_CurrentPlayer->screenxmaxf;
        forced_bbox.max.y = g_CurrentPlayer->screenymaxf;
        bbox_src = &forced_bbox;
    }
#endif
    ...
    if (g_BgRoomInfo[curroom].room_loaded_mask != '\0') {
        ...
        if (bgRoomAdmitTraceShouldLog(curroom) && trace_room_admit_count < bgRoomAdmitTraceBudget()) {
            fprintf(stderr,
                    ...
                    bbox_src->min.x, bbox_src->min.y,
                    bbox_src->max.x, bbox_src->max.y);
            trace_room_admit_count++;
        }
#endif
        return 0;
    }
    ...
    for (i = 0; i < g_BgNumberOfRoomsDrawn; i++)
    {
        if (curroom == dword_CODE_bss_8007FFA0[i].roomid) {
            ...
            sub_GAME_7F0B5CC0(bbox_src,&dword_CODE_bss_8007FFA0[i].bbox);
            temp = (intptr_t)dword_CODE_bss_8007FFA0[i].next;
            dword_CODE_bss_8007FFA0[i].bbox.min.x = bbox_src->min.x;
            dword_CODE_bss_8007FFA0[i].bbox.min.y = bbox_src->min.y;
            dword_CODE_bss_8007FFA0[i].bbox.max.x = bbox_src->max.x;
            dword_CODE_bss_8007FFA0[i].bbox.max.y = bbox_src->max.y;
            ...
            if (trace_room_bbox && intro_camera_active) {
                static int trace_room_bbox_count = 0;
                if (trace_room_bbox_count < 48) {
                    fprintf(stderr,
                            "[ROOM-BBOX] merge room=%d depth=%d bbox=(%.0f,%.0f,%.0f,%.0f) fullscreen=%d next=%d\n",
                            curroom, unk1,
                            bbox_src->min.x, bbox_src->min.y,
                            bbox_src->max.x, bbox_src->max.y,
                            intro_fullscreen_bbox,
                            next);
                    trace_room_bbox_count++;
                }
            }
#endif
            return (s32)temp;
    ...
    dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].bbox.min.x = bbox_src->min.x;
    dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].bbox.min.y = bbox_src->min.y;
    dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].bbox.max.x = bbox_src->max.x;
    ...
    if (trace_room_bbox && intro_camera_active) {
        ...
        if (trace_room_bbox_count < 48) {
            fprintf(stderr,
                    "[ROOM-BBOX] add room=%d depth=%d bbox=(%.0f,%.0f,%.0f,%.0f) fullscreen=%d next=%d\n",
                    curroom, unk1,
                    bbox_src->min.x, bbox_src->min.y,
                    bbox_src->max.x, bbox_src->max.y,
                    intro_fullscreen_bbox,
...
#ifdef NONMATCHING
// NONMATCHING: ido_recomp schedules unrolled sb stores in reverse order.
// Loop body puts -0x13d store in bne delay slot instead of -0x4d.
void sub_GAME_7F0B3BC4(void)
{
  s32 i;
...
Gfx *sub_GAME_7F0B3C8C(Gfx *gdl)
{
    ...
    s32 b_min;
#ifdef NATIVE_PORT
    static int exact_room_scissor = -1;
#endif
    b_min = 99999999;
    b_max = 0;

#ifdef NATIVE_PORT
    if (exact_room_scissor < 0) {
        const char *env = getenv("GE007_EXACT_ROOM_SCISSOR");
        /* Default OFF on PC.
         *
         * The N64 path uses per-room projected scissor boxes as a fill-rate
         * optimization, but the projected bounds can under-cover interior room
         * seams on the PC path and expose blue cracks at sharp transitions.
         *
         * Keep an escape hatch to restore the original precise room scissor
         * behavior for comparison or performance testing. */
        exact_room_scissor = env ? atoi(env) : 0;
    }
#endif
...
notdone = g_BgNumberOfRoomsDrawn;
        ...
        for (j=0; j<g_BgNumberOfRoomsDrawn; j++)
        {
            ...
            {
#ifdef NATIVE_PORT
                s32 tris_before_bg = g_tri_count_diag;
#endif
                ...
                gSPMatrix(gdl++, osVirtualToPhysical((void*)(uintptr_t)get_BONDdata_field_10E0()),
                    (G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_PROJECTION
#ifdef NATIVE_PORT
                    | (bgUseFloatField10E0() ? G_MTX_FLOAT_PORT : 0)
#endif
                    ));
                gdl =
#ifdef NATIVE_PORT
                    fogSetRenderFogColor(
                        exact_room_scissor
                            ...
                            : bgScissorCurrentPlayerViewDefault(gdl++),
                        0);
#else
                    fogSetRenderFogColor(
                        bgScissorCurrentPlayerViewF(
...
#ifdef NATIVE_PORT
                if (bgTraceTrisEnabled() && g_frame_count_diag >= bgTraceTrisAfterFrame()) {
                    static int bg_tri_log_count = 0;
                    static int bg_tri_log_frame = -1;
                    if (bg_tri_log_frame != g_frame_count_diag) {
                        bg_tri_log_frame = g_frame_count_diag;
                        bg_tri_log_count = 0;
                    }
                    if (bg_tri_log_count < 64) {
                        fprintf(stderr,
                                "[BG-TRIS] pass=opaque room=%d sort=%d delta=%d total=%d bbox=(%.0f,%.0f,%.0f,%.0f)\n",
                                dword_CODE_bss_8007FFA0[j].roomid,
                                dword_CODE_bss_8007FFA0[j].unk1,
                                g_tri_count_diag - tris_before_bg,
                                g_tri_count_diag,
                                dword_CODE_bss_8007FFA0[j].bbox.f[0][0],
                                dword_CODE_bss_8007FFA0[j].bbox.f[0][1],
                                dword_CODE_bss_8007FFA0[j].bbox.f[1][0],
...
#ifdef NATIVE_PORT
        s32 trans_order[BG_TRANSPARENT_ROOM_SORT_MAX];
        f32 trans_keys[BG_TRANSPARENT_ROOM_SORT_MAX];
        s32 trans_count = 0;
        s32 trans_iter;

        for (j=0; j<g_BgNumberOfRoomsDrawn; j++)
        {
            if (i == dword_CODE_bss_8007FFA0[j].unk1 && trans_count < BG_TRANSPARENT_ROOM_SORT_MAX)
            {
                trans_order[trans_count] = j;
                trans_count++;
            }
#ifdef DEBUG
notdone --;
        ...
        bgSortTransparentRoomOrderForBucket(trans_order, trans_keys, trans_count, i);

        for (trans_iter=0; trans_iter<trans_count; trans_iter++)
        {
            j = trans_order[trans_iter];
            {
#else
        for (j=0; j<g_BgNumberOfRoomsDrawn; j++)
        {
...
#ifdef NATIVE_PORT
                if (bgTraceTrisEnabled() && g_frame_count_diag >= bgTraceTrisAfterFrame()) {
                    static int bg_tri_trans_log_count = 0;
                    static int bg_tri_trans_log_frame = -1;
                    if (bg_tri_trans_log_frame != g_frame_count_diag) {
                        bg_tri_trans_log_frame = g_frame_count_diag;
                        bg_tri_trans_log_count = 0;
                    }
                    if (bg_tri_trans_log_count < 64) {
                        fprintf(stderr,
                                "[BG-TRIS] pass=trans room=%d sort=%d delta=%d total=%d bbox=(%.0f,%.0f,%.0f,%.0f)\n",
                                dword_CODE_bss_8007FFA0[j].roomid,
                                dword_CODE_bss_8007FFA0[j].unk1,
                                g_tri_count_diag - tris_before_bg,
                                g_tri_count_diag,
...
void load_bg_file(LEVEL_INDEX stagenum)
{
    ...
    for (i = 0; i < MAXROOMCOUNT; i++) {
        g_BgRoomInfo[i].ptr_unique_collision_points = NULL;
    }

    // Search levelinfotable for matching stagenum
    for (i = 0; i < 0x26; i++) {
        if (stagenum == levelinfotable[i].levelID) {
            levelentry_index = i;
        }
    }
    ...
    // Load first 64 bytes of BG file header into stack buffer
    memset(buf, 0, sizeof(buf));
    ptr_bg_data = (intptr_t)buf;
    obLoadBGFileBytesAtOffset(
        (u8 *)levelinfotable[levelentry_index].bg_seg_filename,
        (u8 *)ptr_bg_data, 0, 64);

#ifdef NATIVE_PORT
    // Guard: check if the BG header is valid (non-stub).
    // Stub levels (SHO, ELD, etc.) have bg_*_seg[1] = {0} — 1-byte arrays.
    // Reading 64 bytes from them produces garbage. Check the first 8 bytes.
    {
        u32 w0 = read_be32((void *)ptr_bg_data);
        u32 w1 = read_be32((void *)(ptr_bg_data + 4));
        if (w0 == 0 && w1 == 0) {
            fprintf(stderr, "[BG] load_bg_file: stage %d has stub/empty BG data, skipping\n", stagenum);
            ptr_bg_data = 0;
    ...
    // Set up bgdata offsets pointer
    ptr_bgdata_offsets = ptr_bg_data;

    // Compute room fileposition list from header offset
    {
        s32 hdr_offset = read_be32((void *)(ptr_bg_data + 4));
        ptr_bgdata_room_fileposition_list =
            (bg_room_data *)(hdr_offset + ptr_bg_data + (intptr_t)(s32)0xF1000000);
    }

    // Get total compressed size from first room entry's pos field
    // The size is stored in the upper bytes; mask with 0x00FFFFFF, subtract 1, align to 16
#ifdef NATIVE_PORT
    s0_size = (s32)bswap32(ptr_bgdata_room_fileposition_list[1].pPointTableBin);
#else
    s0_size = (s32)ptr_bgdata_room_fileposition_list[1].pPointTableBin;
#endif
    s0_size = (s0_size & 0x00FFFFFF) - 1;
    s0_size = (s0_size | 0xF) + 1;

    // Allocate memory for full BG data and reload
    ptr_bg_data = (intptr_t)mempAllocBytesInBank(s0_size, 4);
    obLoadBGFileBytesAtOffset(
        (u8 *)levelinfotable[levelentry_index].bg_seg_filename,
    ...
    // Check if BG data has rooms (first word nonzero = already loaded/initialized)
    dword_CODE_bss_8007FF88 = 1;
    dword_CODE_bss_8007BF98 = read_be32((void *)ptr_bg_data);

    {
        u32 bg_first_word = read_be32((void *)ptr_bg_data);
        if (bg_first_word != 0) {
            goto after_room_setup;
        }
    }

    dword_CODE_bss_8007FF88 = 2;

    // Re-derive offsets pointer and room data from the allocated data
    ptr_bgdata_offsets = ptr_bg_data;
    ptr_bgdata_room_fileposition_list =
        (bg_room_data *)(read_be32((void *)(ptr_bg_data + 4)) + ptr_bg_data + (intptr_t)(s32)0xF1000000);

    // Count rooms: iterate room entries until pPriMappingBin is 0
    g_MaxNumRooms = 0;
    {
        bg_room_data *rdata = &ptr_bgdata_room_fileposition_list[1];
        if (rdata->pPriMappingBin != 0) {
            do {
                rdata++;
                g_MaxNumRooms++;
            } while (rdata->pPriMappingBin != 0);
        }
    }

#ifdef NATIVE_PORT
    /* Byte-swap big-endian bg_room_data fields in-place.
     * The u32 offset fields and coord3d floats are stored in ROM byte order.
     * Comparison to 0 above works regardless of endianness, but all
     * arithmetic (size computation, file offset calculation) requires
     * native-endian values. */
    {
        bg_room_data *rd = ptr_bgdata_room_fileposition_list;
        /* Swap entries 0..g_MaxNumRooms inclusive (entry 0 may have data,
         * plus we need the sentinel for size computation). */
        for (int r = 0; r <= g_MaxNumRooms + 1; r++) {
            u32 *p = (u32 *)&rd[r];
            /* First 3 u32 fields: pPointTableBin, pPriMappingBin, pSecMappingBin */
            p[0] = bswap32(p[0]);
            p[1] = bswap32(p[1]);
            p[2] = bswap32(p[2]);
            /* coord3d pos: 3 floats at offsets 3,4,5 as u32 */
            p[3] = bswap32(p[3]);
            p[4] = bswap32(p[4]);
            p[5] = bswap32(p[5]);
        }
    ...
    // Set up portal data pointer
    g_BgPortals = (bg_portal_data_entry *)(read_be32((void *)(ptr_bg_data + 8)) + ptr_bg_data + (intptr_t)(s32)0xF1000000);

    // Set up special portal / visibility data
    {
        s32 offset_0c = read_be32((void *)(ptr_bg_data + 0xC));
        if (offset_0c != 0) {
            dword_CODE_bss_8007FF90 = (intptr_t)(offset_0c + ptr_bg_data + (intptr_t)(s32)0xF1000000);

            {
                s32 offset_10 = read_be32((void *)(ptr_bg_data + 0x10));
                if (offset_10 != 0) {
                    dword_CODE_bss_8007FF94 = (f32 *)(offset_10 + ptr_bg_data + (intptr_t)(s32)0xF1000000);
                } else {
...
#ifdef NATIVE_PORT
    /* PC: Byte-swap the per-room float array (dword_CODE_bss_8007FF94).
     * This is an array of big-endian f32 indexed by room number (+1).
     * Used by sub_GAME_7F0B4F9C for room-specific distance/scale values. */
    if (dword_CODE_bss_8007FF94 != NULL) {
        u32 *fp = (u32 *)dword_CODE_bss_8007FF94;
        for (s32 r = 0; r <= g_MaxNumRooms + 1; r++) {
            fp[r] = bswap32(fp[r]);
        }
    }
#endif
...
#ifdef NATIVE_PORT
    /* PC: Build portal pointer lookup table. The ROM data has 8-byte entries
     * with big-endian 32-bit offsets that can't hold 64-bit pointers.
     * We byte-swap the offsets in-place and build a separate pointer array. */
    {
        s32 portalCount = 0;
        ...
        /* Populate pointer lookup table */
        for (s32 pi = 0; pi < portalCount; pi++) {
            u32 rawOff = read_be32u(rawPortals + pi * 8);
            g_BgPortalPtrs[pi] = (bg_portal_entry *)((intptr_t)rawOff + ptr_bg_data + (intptr_t)(s32)0xF1000000);
        }

        /* Also byte-swap the offset_portal fields in-place for code that checks != 0 */
        for (s32 pi = 0; pi < portalCount; pi++) {
            u32 rawOff;
            __builtin_memcpy(&rawOff, rawPortals + pi * 8, 4);
            rawOff = bswap32(rawOff);
            __builtin_memcpy(rawPortals + pi * 8, &rawOff, 4);
        }

        /* Byte-swap portal entry vertex data (coord3d floats are big-endian in ROM).
         * Each portal entry: u8 numPoints, u8 pad[3], then numPoints × coord3d (3 floats). */
        for (s32 pi = 0; pi < portalCount; pi++) {
            bg_portal_entry *pe = g_BgPortalPtrs[pi];
            if (pe) {
                s32 npts = pe->numPoints;
                u32 *floats = (u32 *)((u8 *)pe + 4);  /* Start of vertex data */
                for (s32 fi = 0; fi < npts * 3; fi++) {
                    floats[fi] = bswap32(floats[fi]);
                }
            }
        }
    ...
    (void)offset;
#else
    // Fix up portal offset_portal pointers (convert segment-relative to real addresses)
    portals = g_BgPortals;
    offset = 0;
    if (portals->offset_portal != NULL) {
        bg_portal_entry *off = portals->offset_portal;
        do {
            offset += 8;
            portals->offset_portal = (bg_portal_entry *)((intptr_t)off + ptr_bg_data + (intptr_t)(s32)0xF1000000);
            portals = (bg_portal_data_entry *)((intptr_t)g_BgPortals + offset);
            off = portals->offset_portal;
        } while (off != NULL);
    }
#endif

    // Fix up special portal data offsets (entries with type 100 / 0x64)
    if (dword_CODE_bss_8007FF90 != 0) {
        u8 type;
        entry = (u8 *)dword_CODE_bss_8007FF90;
        offset = 0;
        type = *entry;
        if (type != 0) {
            u8 val = type;
            do {
                if (val == 100) {
#ifdef NATIVE_PORT
                    s32 off = read_be32(entry + 4);
                    /* Compute byte offset into portal array, then convert to index */
                    intptr_t portalAddr = (intptr_t)off + ptr_bg_data + (intptr_t)(s32)0xF1000000;
                    /* portalAddr points into the raw 8-byte portal entries.
                     * getIndexOfPORTALID expects a portal data pointer — on PC,
                     * compute index from byte offset instead. */
                    s32 byteOff = (s32)(portalAddr - (intptr_t)g_BgPortals);
                    s32 idx = byteOff / 8;  /* 8 bytes per portal entry in ROM data */
                    /* Keep the command stream in big-endian form. The vis list
                     * parser reads every s32 field through vis_read_s32(),
                     * which byteswaps ROM data on PC. Writing the patched
                     * portal index in native endianness corrupts later
                     * opcode-100 portal references, which in turn drops
                     * supplemental rooms from the global visibility packets. */
                    {
                        u32 be_idx = bswap32((u32)idx);
                        memcpy(entry + 4, &be_idx, sizeof(be_idx));
                    }
#else
                    s32 off = *(s32 *)(entry + 4);
                    *(s32 *)(entry + 4) = getIndexOfPORTALID(
                        (bg_portal_data_entry *)(off + ptr_bg_data + (s32)0xF1000000));
#endif
                    entry = (u8 *)(dword_CODE_bss_8007FF90 + offset);
                }
                offset += 8;
                entry += 8;
                val = *entry;
    ...
    // Set up per-room compressed sizes
    if (g_MaxNumRooms >= 2) {
        s_room_info *rinfo = &g_BgRoomInfo[1];
        bg_room_data *rdata = &ptr_bgdata_room_fileposition_list[1];
        s32 room = 1;
        s32 nextroom2;
        ...
        do {
            nextroom2 = room + 1;
            rinfo->model_bin_loaded = 0;
            rinfo->field_35 = 0;

            // csize_primary_DL_binary (pPriMappingBin size)
            if (rdata->pPriMappingBin == 0) {
                rinfo->csize_primary_DL_binary = 0;
            } else {
                nextroom2 = room + 1;
                sp68 = sub_GAME_7F0B4034(nextroom2);
                nextroom = sub_GAME_7F0B4084(room);

                if (nextroom < sp68) {
                    // nextroom < sp68: use pSecMappingBin[nextroom] - pPriMappingBin[current]
                    rinfo->csize_primary_DL_binary =
                        (s32)ptr_bgdata_room_fileposition_list[nextroom].pSecMappingBin -
                        (s32)rdata->pPriMappingBin;
                } else {
                    // nextroom >= sp68: use pPriMappingBin[sp68] - pPriMappingBin[current]
                    rinfo->csize_primary_DL_binary =
                        (s32)ptr_bgdata_room_fileposition_list[sp68].pPriMappingBin -
                        (s32)rdata->pPriMappingBin;
                }
            }

            // csize_secondary_DL_binary (pSecMappingBin size)
            if (rdata->pSecMappingBin == 0) {
                rinfo->csize_secondary_DL_binary = 0;
            } else {
                s32 pri_next = sub_GAME_7F0B4034(nextroom2);
                s32 sec_next = sub_GAME_7F0B4084(nextroom2);

                if (sec_next < pri_next) {
                    // sec_next < pri_next
                    rinfo->csize_secondary_DL_binary =
                        (s32)ptr_bgdata_room_fileposition_list[sec_next].pSecMappingBin -
                        (s32)rdata->pSecMappingBin;
                } else {
                    // sec_next >= pri_next
                    rinfo->csize_secondary_DL_binary =
                        (s32)ptr_bgdata_room_fileposition_list[pri_next].pPriMappingBin -
                        (s32)rdata->pSecMappingBin;
                }
            }

            // csize_point_index_binary (pPointTableBin size)
            if (rdata->pPointTableBin == 0) {
                rinfo->csize_point_index_binary = 0;
            } else {
                s32 pt_next = sub_GAME_7F0B40D4(nextroom2);
                rinfo->csize_point_index_binary =
                    (s32)ptr_bgdata_room_fileposition_list[pt_next].pPointTableBin -
                    (s32)rdata->pPointTableBin;
            }

            rinfo->cur_room_totalsize = -1;
            room = nextroom2;
            rinfo++;
            rdata++;
        } while (nextroom2 < g_MaxNumRooms);
    ...
    // Call sub_GAME_7F0B9338 for each room
    if (g_MaxNumRooms >= 2) {
    ...
    // Process portal data - call sub_GAME_7F0B993C for each portal, store result in D_800443C4
    portals = g_BgPortals;
    outbuf = (char *)D_800443C4;
    i = 0;
    offset = 0;
    if (portals->offset_portal != 0) {
        do {
            *outbuf = sub_GAME_7F0B993C(i);
            portals = g_BgPortals;
            offset += 8;
            outbuf++;
            i++;
#ifdef NATIVE_PORT
        } while (*(s32 *)((intptr_t)portals + offset) != 0);
#else
        } while (*(s32 *)((s32)portals + offset) != 0);
#endif
        i = 0;
        portals = g_BgPortals;
    }

    // Call sub_GAME_7F0B9B94 for each portal
    if (portals->offset_portal != 0) {
        offset = 0;
        do {
            sub_GAME_7F0B9B94(i);
    ...
    // Call sub_GAME_7F0B95D8 for each room
    if (g_MaxNumRooms > 0) {
    ...
    // Clear low bit of controlbytes1 for each portal
    offset = 0;
    if (portals->offset_portal != 0) {
        bg_portal_data_entry *p = portals;
        do {
            offset += 8;
            p->controlbytes1 &= 0xFE;
#ifdef NATIVE_PORT
            p = (bg_portal_data_entry *)((intptr_t)g_BgPortals + offset);
#else
            p = (bg_portal_data_entry *)((s32)g_BgPortals + offset);
#endif
        } while (p->offset_portal != 0);
    }

    sub_GAME_7F0B37EC();

after_room_setup:
    fogRemoved7F0BAA5C(stagenum);
    D_800442F8 = 200;
}
#else
...
void bgRoomVisibilityRelated(void) {
    ...
    current_room = bondviewGetCurrentPlayersRoom();
    g_BgCurrentRoom = current_room;
    position = bondviewGetCurrentPlayersPosition();
    position3 = (coord3d *)bondviewGetCurrentPlayersPosition3();

    portals = g_BgPortals;

    for (outer_iter = 0; outer_iter < 11; outer_iter++) {
        u8 *flags = (u8 *)D_800443C4;

        if (portals[0].offset_portal == 0) {
            break;
        }

        portal_idx = 0;
        while (portals[portal_idx].offset_portal != 0) {
            if (flags[portal_idx] == 0 && portal_idx != last_portal) {
                if (current_room == portals[portal_idx].connectedRoom1 ||
                    current_room == portals[portal_idx].connectedRoom2) {
                    if (sub_GAME_7F0B9F14(portal_idx, position, position3) != 0) {
                        last_portal = portal_idx;
                        portals = g_BgPortals;
                        current_room = portals[portal_idx].connectedRoom2 ^
                                       (current_room ^ portals[portal_idx].connectedRoom1);
                        break;
                    }
                }
            }
        ...
        if (portals[portal_idx].offset_portal == 0) {
            break;
        }
    }
...
#ifdef NATIVE_PORT
    /* Startup portal ambiguity is handled later by seeding additional
     * visibility rooms, not by mutating g_BgCurrentRoom here.
     *
     * The camera matrices for this frame already use the player's authored
     * gameplay room basis. Rewriting the BG room root at this point can send
     * visibility down an unrelated portal chain while the render origin stays
     * anchored to the original room, which is exactly what blanked Facility. */
#endif

    sub_GAME_7F0B8A6C();

#ifdef NATIVE_PORT
    /* Optional debug override: replace portal visibility with a coarse
     * "nearby rooms fullscreen" list. This is useful for diagnostics, but
     * it must never run by default because it bypasses portal clipping and
     * causes large off-room shards to cover the camera. */
    {
        static int force_nearby_rooms = -1;
        if (force_nearby_rooms < 0) {
            force_nearby_rooms = (getenv("GE007_FORCE_NEARBY_ROOMS") != NULL);
        }

        if (force_nearby_rooms) {
            bbox2d fullscreen;
            fullscreen.min.x = 0; fullscreen.min.y = 0;
            fullscreen.max.x = 320; fullscreen.max.y = 240;
            float cx = g_CurrentPlayer->current_model_pos.x / room_data_float2;
            float cz = g_CurrentPlayer->current_model_pos.z / room_data_float2;
            g_BgNumberOfRoomsDrawn = 0;
            for (int rm = 1; rm < g_MaxNumRooms && g_BgNumberOfRoomsDrawn < 30; rm++) {
                if (g_BgRoomInfo[rm].ptr_expanded_mapping_info && g_BgRoomInfo[rm].ptr_point_index) {
                    float dx = ptr_bgdata_room_fileposition_list[rm].pos.x - cx;
                    float dz = ptr_bgdata_room_fileposition_list[rm].pos.z - cz;
                    if (dx * dx + dz * dz < 4000000.0f) { /* within ~2000 stan-units */
                        dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].roomid = rm;
                        dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].unk1 = 1;
                        dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].bbox = fullscreen;
                        dword_CODE_bss_8007FFA0[g_BgNumberOfRoomsDrawn].next = 0;
                        g_BgNumberOfRoomsDrawn++;
...
#ifdef NONMATCHING
extern s32 sub_GAME_7F0B9F14(s32 portalnum, coord3d *arg1, coord3d *arg2);
void sub_GAME_7F0B4AB4(coord3d *pos1, coord3d *pos2, u8 *inputRoomSet, u8 *outputRoomSet, s32 *portalOutput, s32 *portalCountOut, s32 maxPortals) {
    bg_portal_data_entry *portals;
    u8 portalResults[200];
    s32 i;
    s32 portalCounter;
    u8 roomSet[16];
    /* addToByteSetMaxSize15() (this file, ~line 6555) can write up to index 15
     * (max 15 entries + a 0xFF terminator byte), so its target buffer must be
     * 16 bytes. portalRoomSet accumulates the exact same connectedRoom values
     * as newRoomSet below (every call site below adds to both in lockstep),
     * so it needs the same 16-byte capacity -- an 8-byte buffer here is a real
     * stack out-of-bounds write once more than 7 unique rooms accumulate, not
     * just a compiler pedantry (gcc -O3 -Warray-bounds flags this). */
    u8 portalRoomSet[16];
    u8 newRoomSet[16];
    u8 *roomPtr;
    u8 *src;
    ...
    if (portals[0].offset_portal != 0) {
        portalCounter = 0;
        do {
            portalResults[portalCounter] = (u8)sub_GAME_7F0B9F14(portalCounter, pos1, pos2);
            portalCounter++;
        } while (portals[portalCounter].offset_portal != 0);
    }

    /* copy inputRoomSet to roomSet (8 bytes) */
    src = inputRoomSet;
    for (j = 0; j < 8; j++) {
        roomSet[j] = *src++;
    }

    /* copy inputRoomSet to portalRoomSet (8 bytes) */
    src = inputRoomSet;
    for (j = 0; j < 8; j++) {
        portalRoomSet[j] = *src++;
    }
...
outer_loop:
    do {
        ...
        if (portals[0].offset_portal != 0) {
            do {
                if (roomSet[0] != 0xFF) {
                    roomPtr = roomSet;
                    result = portalResults[portalCounter];

                    do {
                        if (result == 1) {
                            if (portals[portalCounter].connectedRoom1 == *roomPtr) {
                                addToByteSetMaxSize15(newRoomSet, portals[portalCounter].connectedRoom2);
                                addToByteSetMaxSize15(portalRoomSet, portals[portalCounter].connectedRoom2);
                                portals = g_BgPortals;
                                portalResults[portalCounter] = 0;
                                result = 0;
                            }
                        }

                        if (result == 2) {
                            if (portals[portalCounter].connectedRoom2 == *roomPtr) {
                                addToByteSetMaxSize15(newRoomSet, portals[portalCounter].connectedRoom1);
                                addToByteSetMaxSize15(portalRoomSet, portals[portalCounter].connectedRoom1);
                                portals = g_BgPortals;
                                portalResults[portalCounter] = 0;
                            }
                        }
        ...
        /* check if newRoomSet has entries */
        if (newRoomSet[0] == 0xFF) {
            break;
        }

        /* copy newRoomSet to roomSet (16 bytes) */
        for (j = 0; j < 16; j++) {
            roomSet[j] = newRoomSet[j];
        }

        i = 0;
        if (newRoomSet[0] != 0xFF) {
            for (j = 0; j < 15; j++) {
                if (newRoomSet[j + 1] == 0xFF) {
                    break;
                }
            }
        }
        i = 0;
    } while (newRoomSet[0] != 0xFF);

    /* copy roomSet to outputRoomSet, max 7 entries */
    i = 0;
    if (roomSet[0] != 0xFF) {
        dst = outputRoomSet;
        src = &roomSet[0];
    ...
    /* terminate output room set */
    outputRoomSet[i] = 0xFF;

    /* copy portalRoomSet to portalOutput as s32 values */
    i = 0;
    if (maxPortals > 0) {
        if (portalRoomSet[0] != 0xFF) {
            src = portalRoomSet;
            *portalOutput = (s32)*src;
...
{
#ifdef NATIVE_PORT
    static int bg_diag_count = 0;
    if (bg_diag_count < 5 && getenv("BG_RENDER_DIAG")) {
        printf("[BG-RENDER] dword_CODE_bss_8007FF88=%d rooms_drawn=%d current_room=%d g_MaxNumRooms=%d\n",
               dword_CODE_bss_8007FF88, g_BgNumberOfRoomsDrawn, g_BgCurrentRoom, g_MaxNumRooms);
        bg_diag_count++;
    }

    /* A3: On N64, the video system sets baseline geometry mode before bgLevelRender.
     * On PC, we need to set it explicitly. GE rooms use baked vertex colors,
     * NOT hardware lighting — G_LIGHTING must be cleared. */
    gSPSetGeometryMode(arg0++,
        G_SHADE | G_SHADING_SMOOTH | G_CULL_BACK |
        (g_FogSkyIsEnabled ? G_FOG : 0));
    gSPClearGeometryMode(arg0++, G_TEXTURE_GEN | G_LIGHTING);
    gSPTexture(arg0++, 0xFFFF, 0xFFFF, 0, G_TX_RENDERTILE, G_ON);
    /* Set GL clear color to match fog/sky for distance fadeout */
    {
        extern void gfx_set_clear_color(int r, int g, int b);
        struct CurrentEnvironmentRecord *envp = fogGetCurrentEnvironmentp();
        if (envp) gfx_set_clear_color(envp->Red, envp->Green, envp->Blue);
    }
#endif
    gSPSetLights1(arg0++, GlobalLight);
    gSPLookAt(arg0++, (void *)sub_GAME_7F078474());
    gSPSegment(arg0++, SPSEGMENT_BG_DL, ptr_bg_data);
#ifdef NATIVE_PORT
    /* Room N64 binary DLs use segment 3 (SPSEGMENT_MODEL_MTX) for matrix
     * references at offsets like 0x03000140. These are offsets into the
     * roomMatrices[] array populated by setupRoomTransformationMatrix().
     * Without this, all G_MTX commands in room DLs fail to resolve. */
    {
        extern Mtx roomMatrices[];
        gSPSegment(arg0++, SPSEGMENT_MODEL_MTX, roomMatrices);
    }
#endif
...
/* PC: Frustum cull — test 8 AABB corners against 6 frustum planes.
 * Returns 1 if the room's AABB is at least partially visible, 0 if fully outside.
 * arg1 is a pointer to 4 floats: {screenxmin, screenymin, screenxmax, screenymax}. */
s32 sub_GAME_7F0B5208(s32 arg0, void *arg1) {
    f32 zrange[2];
    f32 maxZ;
    s_room_info *room;
    ...
    room = &g_BgRoomInfo[arg0];

    for (corner_idx = 0; corner_idx < 8; corner_idx++) {
        /* Generate 8 corners of the room AABB using bit pattern:
         * bit 0: x = minbounds.x (1) or maxbounds.x (0)
         * bit 1: y = minbounds.y (1) or maxbounds.y (0)
         * bit 2: z = minbounds.z (1) or maxbounds.z (0) */
        corner.x = (corner_idx & 1) ? room->minbounds.x : room->maxbounds.x;
        corner.y = (corner_idx & 2) ? room->minbounds.y : room->maxbounds.y;
        corner.z = (corner_idx & 4) ? room->minbounds.z : room->maxbounds.z;

        /* Transform corner to screen space.
         * sub_GAME_7F0B5488 returns 0 if the point is in front of the camera,
         * 1 if behind. projected.x/y are screen coords, projected.z is depth. */
        ret = sub_GAME_7F0B5488(&corner, &projected);

        if (ret == 0) {
            /* Point is in front of camera — normal frustum test */
            if (maxZ <= -projected.z) far_count++;
            if (bbox[0] <= projected.x) left_count++;    /* point right of left edge */
            if (projected.x <= bbox[2]) right_count++;    /* point left of right edge */
            if (bbox[1] <= projected.y) top_count++;      /* point below top edge */
            if (projected.y <= bbox[3]) bottom_count++;   /* point above bottom edge */
            behind_count++;
        } else {
            /* Point is behind camera — inverted comparisons
             * (behind-camera points project to inverted screen space) */
            if (maxZ <= -projected.z) far_count++;
            if (projected.x <= bbox[0]) {
                left_count++;
            } else if (bbox[2] <= projected.x) {
                right_count++;
            }
            if (projected.y <= bbox[1]) {
                top_count++;
            } else if (bbox[3] <= projected.y) {
                bottom_count++;
            }
        }
    }

    /* If ALL 8 corners are outside any single frustum plane, room is invisible */
    if (behind_count == 8 || far_count == 8 ||
        left_count == 8 || right_count == 8 ||
...
void sub_GAME_7F0B5208(s32 arg0, void *arg1) {
    ...
    s32 behind_accum;
    s32 far_count_inner;
    s32 left_count_inner;
    s32 right_count_inner;
    s32 top_accum;
    s32 bottom_accum;
    s32 far_accum;
    s32 left_accum;
    ...
    corner_idx = 0;
    room_info = ((arg0 * 0x50) + &g_BgRoomInfo);
    behind_accum = 0;
    top_accum = 0;
    bottom_accum = 0;
    far_accum = 0;
...
loop_1:
    ...
    {
        // Node 2
        sp5C = (f32) room_info->unk38;
    }
    else
    {
        // Node 3
        sp5C = (f32) room_info->unk44;
    }
    // Node 4
    if ((corner_idx & 2) != 0)
    {
        // Node 5
        sp60 = (f32) room_info->unk3C;
    }
    else
    {
        // Node 6
        sp60 = (f32) room_info->unk48;
    }
    // Node 7
    if ((corner_idx & 4) != 0)
    {
        // Node 8
        sp64 = (f32) room_info->unk40;
    }
    else
    {
        // Node 9
        sp64 = (f32) room_info->unk4C;
    }
    // Node 10
    sp74 = (s32) corner_idx;
    temp_ret = sub_GAME_7F0B5488(&sp5C, &sp68, room_info);
    if (temp_ret == 0)
    {
        // Node 11
        far_count = far_accum;
        ...
        {
            // Node 12
            far_count = (far_accum + 1);
        }
        // Node 13
        left_count = left_accum;
        if (*arg1 <= sp68)
        {
            // Node 14
            left_count = (left_accum + 1);
        }
        // Node 15
        right_count = right_accum;
        if (sp68 <= arg1->unk8)
        {
            // Node 16
            right_count = (right_accum + 1);
        }
        // Node 17
        top_count = top_accum;
        if (arg1->unk4 <= sp6C)
        {
            // Node 18
            top_count = (top_accum + 1);
        }
        // Node 19
        bottom_count = bottom_accum;
        if (sp6C <= arg1->unkC)
        {
            // Node 20
            bottom_count = (bottom_accum + 1);
        }
        // Node 21
        behind_count = (behind_accum + 1);
    }
    else
    {
        // Node 22
        far_count_inner = far_accum;
        if (sp40 <= -sp70)
        {
            // Node 23
            far_count_inner = (far_accum + 1);
        }
        // Node 24
        if (sp68 <= *arg1)
        {
            // Node 25
            left_count_inner = (left_accum + 1);
            right_count_inner = right_accum;
        }
        else
        {
            // Node 26
            left_count_inner = left_accum;
            right_count_inner = right_accum;
            if (arg1->unk8 <= sp68)
            {
                // Node 27
                left_count_inner = left_accum;
                right_count_inner = (right_accum + 1);
            }
        }
        // Node 28
        if (sp6C <= arg1->unk4)
        {
            // Node 29
            behind_count = behind_accum;
            far_count = far_count_inner;
            left_count = left_count_inner;
            right_count = right_count_inner;
            top_count = (top_accum + 1);
            bottom_count = bottom_accum;
        }
        else
    ...
    // Node 32
    temp_v1 = (corner_idx + 1);
    corner_idx = temp_v1;
    room_info = sp38;
    behind_accum = behind_count;
    top_accum = top_count;
    bottom_accum = bottom_count;
    far_accum = far_count;
    left_accum = left_count;
    right_accum = right_count;
    if (temp_v1 != 8)
    {
        goto loop_1;
    }
    // Node 33
    if ((((((behind_count == 8) || (far_count == 8)) || (left_count == 8)) || (right_count == 8)) || (top_count == 8)) || (bottom_count == 8))
    {
        // Node 39
    }
    else
...
extern void sub_GAME_7F0B96CC(s32 arg0, struct PortalMetric *arg1);
extern void mtx4TransformVecInPlace(Mtxf *matrix, struct coord3d *vector);

s32 sub_GAME_7F0B5528(s32 portalIndex, f32 offset, coord3d *outVerts) {
    Mtxf *mtx;
    f32 zrange[2];
    f32 maxZ;
    s32 allBehind;
    ...
    if (numPoints <= 0) {
        ...
        for (i = 0; i < numPoints; i++) {
            ...
            if (offset != 0.0f) {
                ...
                cur->x = cur->x + metric.normal.x * offset;
                cur->y = cur->y + metric.normal.y * offset;
                cur->z = cur->z + metric.normal.z * offset;
            }

            cur->x = cur->x * room_data_float2;
            cur->y = cur->y * room_data_float2;
            cur->z = cur->z * room_data_float2;
            mtx4TransformVecInPlace(mtx, cur);
    ...
    cur = outVerts;
    for (i = 0; i < numPoints; i++) {
        nextWrapped = (i + 1) % numPoints;
        curZ = cur->z;

        if (0.0f < curZ) {
            next = &outVerts[nextWrapped];
            nextZ = next->z;
            if (nextZ <= 0.0f) {
                goto do_clip;
            }
    ...
    do_clip_entry:
        curX = cur->x;
        t = -curZ / (nextZ - curZ);
        outVerts[outputCount].x = (next->x - curX) * t + curX;
        curY = cur->y;
...
#ifdef NONMATCHING
#if defined(NATIVE_PORT) || !defined(PORT_FIXME_STUBS)
extern f32 sub_GAME_7F0B9990(s32 arg0);
extern s32 sub_GAME_7F0B5528(s32 arg0, f32 arg1, coord3d *arg2);
extern void transform3Dto2DWithZScaling(coord3d *in, coord3d *out);
extern struct player *g_CurrentPlayer;

#define BG_PORTAL_NEAR_Z_EPSILON 1.0f

static int bgPortalNearClipEnabled(void)
{
    static int enabled = -1;

    if (enabled < 0) {
        const char *env = getenv("GE007_PORTAL_NEAR_CLIP");
...
static s32 bgClipPortalPolygonToNearPlane(coord3d *dst, const coord3d *src,
                                          s32 count, f32 near_z)
{
    s32 out_count = 0;
    s32 i;
    ...
    for (i = 0; i < count; i++) {
        const coord3d *a = &src[i];
        const coord3d *b = &src[(i + 1) % count];
        s32 a_inside = (a->z <= near_z);
        s32 b_inside = (b->z <= near_z);
...
s32 sub_GAME_7F0B5864(s32 arg0, f32 *arg1) {
    struct unk_portalstruct *entry;
    f32 scale;
    coord3d vertices[20];
    coord3d clipped_vertices[40];
    coord3d *project_vertices;
    coord3d projected;
    s32 totalverts;
    s32 clipped_totalverts;
    s32 project_totalverts;
    s32 count2;
    s32 i;
    s32 viscount;
    f32 left;
    ...
    f32 screenx;
    f32 screeny;
    static int trace_portal_verts = -1;
    static int trace_portal_verts_after_frame = -1;
    ...
    if (trace_portal_verts < 0) {
        trace_portal_verts = (getenv("GE007_TRACE_PORTAL_VERTS") != NULL);
    }
    if (trace_portal_verts_after_frame < 0) {
        const char *env = getenv("GE007_TRACE_PORTAL_VERTS_AFTER_FRAME");
        trace_portal_verts_after_frame = env ? atoi(env) : 0;
    }
    if (trace_portal_verts_idx == -2) {
        const char *env = getenv("GE007_TRACE_PORTAL_VERTS_IDX");
        trace_portal_verts_idx = env ? atoi(env) : -1;
    }

    if (entry->unk0 >= 0) {
        arg1[0] = *(f32 *)&entry->unk4;
        arg1[1] = *(f32 *)&entry->unk8;
        arg1[2] = *(f32 *)&entry->unkC;
        arg1[3] = *(f32 *)&entry->unk10;
        return entry->unk0;
    }

    scale = sub_GAME_7F0B9990(arg0);
    totalverts = sub_GAME_7F0B5528(arg0, scale, vertices);

    if (0.0f < scale) {
        count2 = sub_GAME_7F0B5528(arg0, -scale, &vertices[totalverts]);
        totalverts += count2;
    }

    near_clip_enabled = bgPortalNearClipEnabled();

    if (near_clip_enabled) {
        clipped_totalverts = bgClipPortalPolygonToNearPlane(
            clipped_vertices, vertices, totalverts, -BG_PORTAL_NEAR_Z_EPSILON);
        project_vertices = clipped_vertices;
        project_totalverts = clipped_totalverts;
    } else {
        clipped_totalverts = totalverts;
        project_vertices = vertices;
        project_totalverts = totalverts;
    }

    if (trace_portal_verts
        && g_frame_count_diag >= trace_portal_verts_after_frame
        && (trace_portal_verts_idx < 0 || trace_portal_verts_idx == arg0)
        && trace_portal_vert_count < 32) {
        fprintf(stderr,
                "[PORTAL-VERTS] frame=%d portal=%d total=%d clipped=%d near_clip=%d\n",
                g_frame_count_diag, arg0, totalverts, clipped_totalverts,
                near_clip_enabled);
        for (i = 0; i < project_totalverts && i < 12; i++) {
            fprintf(stderr,
                    "  v[%d]=(%.1f,%.1f,%.1f)\n",
                    i,
                    project_vertices[i].x,
                    project_vertices[i].y,
                    project_vertices[i].z);
        }
        trace_portal_vert_count++;
    }

    viscount = 0;

    if (project_totalverts > 0) {
        for (i = 0; i < project_totalverts; i++) {
            z = project_vertices[i].z;

            /* GE portal projection treats camera-space Z <= 0 as the visible
             * half-space. sub_GAME_7F0B5528 already clips portal polygons
             * against that plane, but values arbitrarily close to 0 still
             * explode when transform3Dto2DWithZScaling divides by Z. Clip
             * to a small negative epsilon before projecting so portal bboxes
             * stay finite for intro cameras grazing the near plane. */
            if (z > 0.0f) {
                continue;
            }
            if (near_clip_enabled && -BG_PORTAL_NEAR_Z_EPSILON < z) {
                continue;
            }

            transform3Dto2DWithZScaling(&project_vertices[i], &projected);
            screenx = projected.x;
            screeny = projected.y;

            if (!__builtin_isfinite(screenx) || !__builtin_isfinite(screeny)) {
                continue;
            }
    ...
    if (viscount == 0) {
        ...
        /* Stock stores the raw projected portal bounds here. Callers intersect
         * those bounds with parent/screen rectangles and reject empty windows.
         * Clamping here can turn a wholly off-screen portal into a degenerate
         * box, which the stock-compatible fallback below expands to full screen. */
        if (bgPortalLegacyProjectClampEnabled()) {
            if (left < g_CurrentPlayer->screenxminf) {
                left = g_CurrentPlayer->screenxminf;
            }
            if (top < g_CurrentPlayer->screenyminf) {
                top = g_CurrentPlayer->screenyminf;
            }
            if (right > g_CurrentPlayer->screenxmaxf) {
                right = g_CurrentPlayer->screenxmaxf;
            }
            if (bottom > g_CurrentPlayer->screenymaxf) {
                bottom = g_CurrentPlayer->screenymaxf;
            }
        }
    ...
    arg1[2] = right;
    arg1[3] = bottom;
    *(f32 *)&entry->unk4 = arg1[0];
    *(f32 *)&entry->unk8 = arg1[1];
    *(f32 *)&entry->unkC = arg1[2];
    entry->unk0 = viscount;
    *(f32 *)&entry->unk10 = arg1[3];

    return viscount;
}
#endif /* NATIVE_PORT || !PORT_FIXME_STUBS */
#else
#if defined(LEFTOVERDEBUG)
GLOBAL_ASM(
.text
...
#ifdef NONMATCHING
// NONMATCHING: ido_recomp consolidates bgDebPortalOutLineNum address into base pointer ($v1)
// instead of using separate lui+%lo patterns for load and store.
char *bgDebPrintPORTALID(s32 portID)
{
    s32 temp_hi = (bgDebPortalOutLineNum + 1) % 0xA;

    bgDebPortalOutLineNum = temp_hi;
    snprintf(bgDebPortalOutBuffer[temp_hi], sizeof(bgDebPortalOutBuffer[temp_hi]), "PORT%d", portID);
...
#ifdef NONMATCHING
// NONMATCHING: ido_recomp consolidates bgDebRoomOutLineNum address into register v1
// and reuses it for both load and store. Original IDO uses separate lui+%lo pairs.
// 6/27 instructions differ (global address consolidation + stack offset changes).
char *bgDebPrintROOMID(s32 roomID)
{
    s32 temp_hi = (bgDebRoomOutLineNum + 1) % 0xA;

    bgDebRoomOutLineNum = temp_hi;
    snprintf(bgDebRoomOutBuffer[temp_hi], sizeof(bgDebRoomOutBuffer[temp_hi]), "ROOM%d", roomID);
...
s32 getIndexOfPORTALID(s32 portalID)
{
    ...
    for(i = 0; g_BgPortals[i].offset_portal != 0; i++)
    {
...
#ifdef NONMATCHING
// https://decomp.me/scratch/TODO 96.7%
// NONMATCHING: ido_recomp load scheduling differs at epilogue.
// 58/60 instructions match. At offset 0x80-0x84, original IDO schedules
// lw t5 (data reload) BEFORE sw v0 (usize store), grouping loads before stores.
// ido_recomp stores v0 first, then loads t5 (interleaved load-store pattern).
s32 sub_GAME_7F0B5FAC(s32 roomID, u8 *data, s32 size)
{
    s_room_info *roominfo = &g_BgRoomInfo[roomID];
    s32 aligned = (roominfo->csize_point_index_binary + 0xf) & ~0xf;
    s32 file_offset;
    u8 *source;

    if (size < aligned + 0x20) {
        return -1;
    }

    file_offset = (s32)ptr_bgdata_room_fileposition_list[roomID].pPointTableBin + ptr_bg_data - ptr_bg_data + (s32)0xF1000000;
    source = data + (size - aligned);

    obLoadBGFileBytesAtOffset(
        levelinfotable[levelentry_index].bg_seg_filename,
        source,
        file_offset,
        aligned
    );
...
#else
    texEnd = (size - dest) + data;
    texCopyGdls(data,texEnd,dest);
    clear_light_fixturetable_in_room(roomID);
    texEnd = texLoadFromGdl(texEnd,dest,data,NULL);
    if (dest < texEnd) {
      dest = texEnd;
    }
    g_BgRoomInfo[roomID].ptr_expanded_mapping_info = data;
...
u32 sub_GAME_7F0B61DC(s32 roomID, u32 *data, s32 size)
{
    ...
    {
        source = (u8 *)data + (size - len);
#ifdef NATIVE_PORT
        obLoadBGFileBytesAtOffset(levelinfotable[levelentry_index].bg_seg_filename,source, (u32)(uintptr_t)ptr_bgdata_room_fileposition_list[roomID].pSecMappingBin + (u32)0xF1000000,len);
#else
        obLoadBGFileBytesAtOffset(levelinfotable[levelentry_index].bg_seg_filename,source, (s32)ptr_bgdata_room_fileposition_list[roomID].pSecMappingBin + (s32)0xF1000000,len);
#endif
        dest = bgDecompress(source,(u8 *)data);
#ifdef NATIVE_PORT
        if ((s32)dest < 0) {
...
#else
        texEnd = (uintptr_t)data + (size - dest);
        texCopyGdls((Gfx *)data,(Gfx *)texEnd,dest);
        texEnd = texLoadFromGdl((Gfx *)texEnd,dest,(Gfx *)data,NULL);
        if (dest < (u32)texEnd) {
            dest = (u32)texEnd;
        }
        g_BgRoomInfo[roomID].ptr_secondary_expanded_mapping_info = data;
...
#ifdef NATIVE_PORT
/*
 * PC implementation of sub_GAME_7F0B6368 — room model loader.
 * Allocates separate buffers for vertex and DL data, loads them via
 * the existing decompression helpers, and registers as N64 DL regions
 * so the GBI translator handles big-endian commands correctly.
 */
#define PC_ROOM_LOAD_BUF_SIZE (256 * 1024)
void sub_GAME_7F0B6368(s32 room) {
    ...
    /* Load vertex data if this room has any */
    if (g_BgRoomInfo[room].csize_point_index_binary > 0) {
        u8 *vtx_buf = (u8 *)malloc(PC_ROOM_LOAD_BUF_SIZE);
        if (!vtx_buf) return;
        vtx_result = sub_GAME_7F0B5FAC(room, vtx_buf, PC_ROOM_LOAD_BUF_SIZE);
        if (vtx_result < 0) {
            free(vtx_buf);
            return;
        }
        gfx_register_n64_dl_region(g_BgRoomInfo[room].ptr_point_index, vtx_result);
        s_pc_room_vtx_bufs[room] = vtx_buf;
    } else {
        g_BgRoomInfo[room].ptr_point_index = NULL;
        g_BgRoomInfo[room].usize_point_index_binary = 0;
    }

    /* Load primary display list data if this room has any */
    if (g_BgRoomInfo[room].csize_primary_DL_binary > 0) {
        u8 *dl_buf = (u8 *)malloc(PC_ROOM_LOAD_BUF_SIZE);
        if (!dl_buf) {
            /* Unwind vertex state on allocation failure */
            g_BgRoomInfo[room].ptr_point_index = NULL;
            g_BgRoomInfo[room].usize_point_index_binary = 0;
            return;
        }
        dl_result = sub_GAME_7F0B609C(room, (int *)dl_buf, PC_ROOM_LOAD_BUF_SIZE);
        if ((s32)dl_result < 0) {
            free(dl_buf);
            /* Unwind vertex state on DL decompression failure */
            g_BgRoomInfo[room].ptr_point_index = NULL;
            g_BgRoomInfo[room].usize_point_index_binary = 0;
            return;
        }
        gfx_register_n64_dl_region(g_BgRoomInfo[room].ptr_expanded_mapping_info, dl_result);
        s_pc_room_dl_bufs[room] = dl_buf;
    } else {
        g_BgRoomInfo[room].ptr_expanded_mapping_info = NULL;
        g_BgRoomInfo[room].usize_primary_DL_binary = 0;
    }

    /* Load secondary display list data (transparent geometry: glass, water, etc.) */
    if (g_BgRoomInfo[room].csize_secondary_DL_binary > 0) {
        u8 *sec_buf = (u8 *)malloc(PC_ROOM_LOAD_BUF_SIZE);
        if (sec_buf) {
            u32 sec_result = sub_GAME_7F0B61DC(room, (u32 *)sec_buf, PC_ROOM_LOAD_BUF_SIZE);
    ...
    g_BgRoomInfo[room].model_bin_loaded = 1;

    {
        static int trace_room_ranges = -1;
        if (trace_room_ranges < 0) {
            trace_room_ranges = (getenv("GE007_TRACE_ROOM_RANGES") != NULL);
        }
        if (trace_room_ranges && room >= 1 && room < g_MaxNumRooms) {
            s_room_info *ri = &g_BgRoomInfo[room];
            fprintf(stderr,
                    "[ROOM-RANGE] room=%d vtx=%p+0x%X dl=%p+0x%X sec=%p+0x%X\n",
                    room,
                    ri->ptr_point_index, (unsigned)ri->usize_point_index_binary,
                    ri->ptr_expanded_mapping_info, (unsigned)ri->usize_primary_DL_binary,
                    ri->ptr_secondary_expanded_mapping_info, (unsigned)ri->usize_secondary_DL_binary);

            if (ri->ptr_point_index != NULL && ri->usize_point_index_binary > 0) {
                uintptr_t start = (uintptr_t)ri->ptr_point_index;
                uintptr_t end = start + (uintptr_t)ri->usize_point_index_binary;
                for (int other = 1; other < room; other++) {
                    s_room_info *ori = &g_BgRoomInfo[other];
                    if (ori->ptr_point_index != NULL && ori->usize_point_index_binary > 0) {
                        uintptr_t other_start = (uintptr_t)ori->ptr_point_index;
                        uintptr_t other_end = other_start + (uintptr_t)ori->usize_point_index_binary;
                        if (start < other_end && other_start < end) {
                            fprintf(stderr,
                                    "[ROOM-RANGE-OVERLAP] room=%d vtx=%p+0x%X overlaps room=%d vtx=%p+0x%X\n",
                                    room,
                                    ri->ptr_point_index, (unsigned)ri->usize_point_index_binary,
                                    other,
                                    ori->ptr_point_index, (unsigned)ori->usize_point_index_binary);
                        }
                    }
                }
            }
    ...
    /* Native replacement for the texLoadFromGdl light-fixture population the PC
     * on-demand room loader (this fn) skips. Without it, a room loaded AFTER level
     * start gets an empty light_fixture_table (shooting its lights silently no-ops)
     * and stale entries could alias a freed room's DL span (wrong-room darkening).
     * Default-off: lf_populate_room_lightfixtures early-returns when the flag is
     * off, so the build stays byte-identical. Must run before redarken so the
     * table is fresh for this room. See W4.E1.T1. */
    lf_populate_room_lightfixtures(room);
    redarken_lights_in_room(room);

    /* CCRM LUT transforms: patch render modes / combine modes in room DL data.
     * On PC, bgLoadFromDynamicCCRMLUT works with big-endian N64 DL bytes. */
    {
        extern s32 g_FogSkyIsEnabled;
        s_room_info *ri = &g_BgRoomInfo[room];
#ifdef NATIVE_PORT
        /* On PC, rooms load on-demand during rendering, but fog state is
         * initialized later in bondviewSetCameraMode.  If fog isn't enabled
         * yet when the first room loads, the LUT rewrite below will use the
         * non-fog LUT, producing render modes without G_RM_FOG_SHADE_A.
         * Fix: ensure fog is loaded before any room LUT rewrite. */
        if (!g_FogSkyIsEnabled) {
            fogLoadLevelEnvironment(bossGetStageNum(), 0);
        }
#endif
        if (g_FogSkyIsEnabled) {
            bgLoadFromDynamicCCRMLUT(ri->ptr_expanded_mapping_info,
                (Gfx *)((u8 *)ri->ptr_expanded_mapping_info + ri->usize_primary_DL_binary), 1);
            if (ri->ptr_secondary_expanded_mapping_info) {
                bgLoadFromDynamicCCRMLUT(ri->ptr_secondary_expanded_mapping_info,
                    (Gfx *)((u8 *)ri->ptr_secondary_expanded_mapping_info + ri->usize_secondary_DL_binary), 5);
            }
        } else {
            bgLoadFromDynamicCCRMLUT(ri->ptr_expanded_mapping_info,
                (Gfx *)((u8 *)ri->ptr_expanded_mapping_info + ri->usize_primary_DL_binary), 6);
            if (ri->ptr_secondary_expanded_mapping_info) {
                bgLoadFromDynamicCCRMLUT(ri->ptr_secondary_expanded_mapping_info,
                    (Gfx *)((u8 *)ri->ptr_secondary_expanded_mapping_info + ri->usize_secondary_DL_binary), 7);
            }
        }
    }

    /* Collision quad precomputation (implemented in NATIVE_PORT path below) */
...
extern void memaFree(void *addr, s32 size);

#ifdef NONMATCHING
// regalloc: ido_recomp loads cur_room_totalsize directly into a1 instead of v0,
// eliminating the move a1,v0 the original IDO generates (1 instruction difference)
...
#ifdef NATIVE_PORT
        /* On PC, collision data was malloc'd by sub_GAME_7F0B6994 */
        free(room->ptr_unique_collision_points);
#else
        memaFree(room->ptr_unique_collision_points, ((*(s16 *)&room->bitflags3 * 0x1C) + 0xF) & ~0xF);
#endif
...
#ifdef NATIVE_PORT
    /* Shoot-out-the-lights: drop this room's light_fixture_table entries before its
     * DL buffers are freed — no table entry may outlive its DL buffer, or a later
     * malloc could alias a stale [ptr_start,ptr_end) span (wrong-room darkening).
     * Gated => byte-identical when the flag is off. See W4.E1.T1. */
    if (ge007_shoot_out_lights_enabled()) { clear_light_fixturetable_in_room(roomID); }
    /* Free on-demand allocated buffers tracked in s_pc_room_*_bufs[].
     * Preloader buffers (tracked in lvl.c pc_room_allocs[]) are also safe
     * to free here — they're set to NULL in lvl.c on stage transitions. */
    if (s_pc_room_vtx_bufs[roomID]) { free(s_pc_room_vtx_bufs[roomID]); s_pc_room_vtx_bufs[roomID] = NULL; }
    if (s_pc_room_dl_bufs[roomID]) { free(s_pc_room_dl_bufs[roomID]); s_pc_room_dl_bufs[roomID] = NULL; }
    if (s_pc_room_sec_bufs[roomID]) { free(s_pc_room_sec_bufs[roomID]); s_pc_room_sec_bufs[roomID] = NULL; }
    room->ptr_point_index = NULL;
    room->ptr_expanded_mapping_info = NULL;
    room->ptr_secondary_expanded_mapping_info = NULL;
#else
    if (size > 0) {
        if (room->ptr_point_index != NULL) {
            memaFree(room->ptr_point_index, size);
        } else {
...
{
#ifdef NATIVE_PORT
    /* PC: all rooms stay loaded — no N64-style memory pressure.
     * Skip room aging/unloading to keep all preloaded geometry alive. */
    return;
#else
    s32 i;
...
{
#ifdef NATIVE_PORT
    static int skip_room = -2;
    static int trace_bg_room_ptrs = -1;
    if (skip_room == -2) {
        const char *env = getenv("GE007_SKIP_BG_ROOM");
        skip_room = env ? atoi(env) : -1;
    }
    ...
    if (trace_bg_room_ptrs && room_index >= 1 && room_index < g_MaxNumRooms) {
        s_room_info *ri = &g_BgRoomInfo[room_index];
        fprintf(stderr,
                "[BG-ROOM-PTR] kind=primary frame=%d room=%d vtx=%p+0x%X dl=%p+0x%X model=%d\n",
                g_frame_count_diag,
                room_index,
                ri->ptr_point_index,
                (unsigned)ri->usize_point_index_binary,
                ri->ptr_expanded_mapping_info,
                (unsigned)ri->usize_primary_DL_binary,
                ri->model_bin_loaded);
        fflush(stderr);
    }
#endif
...
Gfx *sub_GAME_7F0B6898(Gfx *arg0, s32 room_index)
{
    ...
    if (trace_bg_room_ptrs && room_index >= 1 && room_index < g_MaxNumRooms) {
        s_room_info *ri = &g_BgRoomInfo[room_index];
        fprintf(stderr,
                "[BG-ROOM-PTR] kind=secondary frame=%d room=%d vtx=%p+0x%X sec=%p+0x%X model=%d\n",
                g_frame_count_diag,
                room_index,
                ri->ptr_point_index,
                (unsigned)ri->usize_point_index_binary,
                ri->ptr_secondary_expanded_mapping_info,
                (unsigned)ri->usize_secondary_DL_binary,
                ri->model_bin_loaded);
        fflush(stderr);
    }
#endif
    ...
    {
#ifdef NATIVE_PORT
        /* Preloaded rooms have model_bin_loaded=1 but no secondary DL.
         * Try loading the secondary DL on demand if the room has one. */
        if (g_BgRoomInfo[room_index].model_bin_loaded != 0 &&
            g_BgRoomInfo[room_index].csize_secondary_DL_binary > 0) {
            sub_GAME_7F0B6368(room_index);
            /* If loading succeeded, fall through to render it */
            if (g_BgRoomInfo[room_index].ptr_secondary_expanded_mapping_info != 0) {
                goto render_secondary;
            }
        }
#endif
        return arg0;
...
#ifdef NATIVE_PORT
/*
 * sub_GAME_7F0B6994 — Build per-room collision quad data from DL commands.
 *
 * Walks the room's primary DL command stream, counts G_VTX commands (type 4),
 * allocates a collision record array (0x1C bytes per quad), and for each quad
 * computes an axis-aligned bounding box from the referenced vertices.
 *
 * The collision record layout (0x1C bytes):
 *   0x00: s16  cmdIndex — sequential DL command index of this G_VTX
 *   0x02: s16  (padding)
 *   0x04: s32  bbox_min_x
 *   0x08: s32  bbox_min_y
 *   0x0C: s32  bbox_min_z
 ...
 *   0x18: s32  bbox_max_z
 *
 * On NATIVE_PORT, DL commands and vertex data are big-endian (loaded from ROM).
 */
void sub_GAME_7F0B6994(s32 room) {
    s_room_info *ri = &g_BgRoomInfo[room];
    u8 *dl_base;
    u8 *vtx_base;
    u8 *cmd;
    s32 quadCount, cmdIndex_counter;
    s32 i;
    u8 *collision_buf;
    s32 alloc_size;

    if (ri->ptr_unique_collision_points != NULL) {
        return;
    }

    vtx_base = (u8 *)ri->ptr_point_index;
    dl_base = (u8 *)ri->ptr_expanded_mapping_info;

    if (dl_base == NULL || vtx_base == NULL) {
        return;
    }

    /* Pass 1: count G_VTX commands (type 4) */
    quadCount = 0;
    cmd = dl_base;
    for (;;) {
        s8 cmdType = (s8)cmd[0];
        if (cmdType == (s8)0xB8) break;  /* G_ENDDL */
        if (cmdType == 4) {               /* G_VTX */
            quadCount++;
        }
        cmd += 8;
    ...
    /* Allocate collision array: 0x1C bytes per quad, 16-byte aligned */
    alloc_size = (((quadCount * 7) * 4) + 0xF) & ~0xF;
    collision_buf = (u8 *)malloc(alloc_size);
    if (collision_buf == NULL) {
        return;
    }
    memset(collision_buf, 0, alloc_size);

    ri->ptr_unique_collision_points = collision_buf;
    *(s16 *)&ri->bitflags3 = (s16)quadCount;

    /* Pass 2: fill collision records */
    cmd = dl_base;
    cmdIndex_counter = 0;
    i = 0;
    for (;;) {
        ...
        if (cmdType == 4) {  /* G_VTX */
            u8 *rec = collision_buf + (i * 0x1C);
            s32 numVerts, vtxOffset, v;
            u8 *vertPtr;
            s32 min_x = 0x7FFF, min_y = 0x7FFF, min_z = 0x7FFF;
            s32 max_x = (s32)0xFFFF8000, max_y = (s32)0xFFFF8000, max_z = (s32)0xFFFF8000;

            /* Store command index as s16 */
            *(s16 *)rec = (s16)cmdIndex_counter;

            /* Parse G_VTX command (big-endian 8 bytes):
             * byte 0: opcode (4)
             * byte 1: ((numVerts-1) << 4) | vtx_pool_offset
             * bytes 4-7: vertex offset (lower 24 bits) relative to vtx_base */
            numVerts = ((cmd[1] >> 4) & 0xF) + 1;
            vtxOffset = ((u32)cmd[4] << 24) | ((u32)cmd[5] << 16) | ((u32)cmd[6] << 8) | (u32)cmd[7];
            vtxOffset &= 0x00FFFFFF;
            vertPtr = vtx_base + vtxOffset;

            /* Walk vertices (each 0x10 bytes), extract s16 x/y/z from big-endian */
            for (v = 0; v < numVerts; v++) {
                s16 vx = (s16)((vertPtr[0] << 8) | vertPtr[1]);
                s16 vy = (s16)((vertPtr[2] << 8) | vertPtr[3]);
                s16 vz = (s16)((vertPtr[4] << 8) | vertPtr[5]);
            ...
            if (min_z == max_z) max_z++;

            /* Add room position offset */
            {
                bg_room_data *rd = &ptr_bgdata_room_fileposition_list[room];
                s32 rx = (s32)rd->pos.f[0];
                s32 ry = (s32)rd->pos.f[1];
                s32 rz = (s32)rd->pos.f[2];
                min_x += rx; min_y += ry; min_z += rz;
                max_x += rx; max_y += ry; max_z += rz;
            }

            /* Write bounds to collision record */
            *(s32 *)(rec + 0x04) = min_x;
            *(s32 *)(rec + 0x08) = min_y;
            *(s32 *)(rec + 0x0C) = min_z;
            *(s32 *)(rec + 0x10) = max_x;
...
loop_3:
            ...
            {
                // Node 4
                quadNext = (quadAccum + 1);
            }
            // Node 5
            temp_v1_8 = cmdPtr->unk8;
            cmdWord = temp_v1_8;
            cmdPtr = (cmdPtr + 8);
            quadCount = quadNext;
            quadAccum = quadNext;
        ...
        // Node 6
        temp_t8 = (((((quadCount * 8) - quadCount) * 4) + 0xf) & -0x10);
        sp24 = temp_t8;
        sp3C = (s32) quadCount;
...
loop_9:
                ...
                {
                    // Node 10
                    temp_a0 = (temp_ret + (((quadIndex * 8) - quadIndex) * 4));
                    *temp_a0 = (s16) cmdIndex;
                    temp_t2 = (arg0 * 0x18);
                    boundsPtr = (temp_ret + (((quadIndex * 8) - quadIndex) * 4));
                    axisIndex = 0;
loop_11:
                    // Node 11
                    temp_a2 = (axisIndex + 1);
                    ...
                    *temp_v1 = 0x7fff;
                    ...
                    // Node 12
                    temp_a3 = ((((u32) cmdPtr2->unk1 >> 4) & 0xf) + 1);
                    if (temp_a3 > 0)
                    {
                        // Node 13
                        vertPtr = ((cmdPtr2->unk4 & 0xffffff) + sp44);
                        vertIndex = 0;
loop_14:
                        // Node 14
                        temp_v1_2 = *vertPtr;
                        temp_a2_2 = (vertIndex + 1);
...
#ifdef NATIVE_PORT
/*
 * bgTestLineIntersectionInRoom — Test a line segment against room collision triangles.
 *
 * Walks the room's DL commands from the collision entry index, extracting triangle
 * vertex indices from G_TRI1 (-0x41/0xBF) and Rare's G_TRI4 (0xB1) commands. For each
 * triangle, builds a bounding box, does a quick bbox intersection test, then a precise
 * line-triangle intersection test.
 *
 * On NATIVE_PORT, DL commands and vertices are big-endian ROM data.
 *
 * Returns: 1 if any intersection found, 0 otherwise.
 * On NATIVE_PORT, output is a native BgHitRecord so pointer fields cannot
 * overlap the 16-bit triangle/texture metadata from the N64 byte layout.
 */
s32 bgTestLineIntersectionInRoom(coord3d *start, coord3d *dest, coord3d *delta, void *arg3, s32 room, BgHitRecord *output) {
    u8 *collision_entry = (u8 *)arg3;
    s_room_info *ri = &g_BgRoomInfo[room];
    u8 *dl_base = (u8 *)ri->ptr_expanded_mapping_info;
    u8 *vtx_base = (u8 *)ri->ptr_point_index;
    bg_room_data *rd = &ptr_bgdata_room_fileposition_list[room];

    /* Collision entries are generated by sub_GAME_7F0B6994 on native builds,
     * so their command index and bounds are already host-endian. */
    s16 coll_index = *(s16 *)collision_entry;

    u8 *coll_dl;       /* pointer to the DL command for this collision group */
    u8 *cmd;
    s32 vtx_base_offset;  /* temp_t2: vertex pool nibble offset */
    u8 *vtx_pool_base;    /* temp_s1: computed vertex array base */
    s32 result = 0;
    s32 best_dist_sq = 0x7FFFFFFF;
    s8 cmdType;
    s32 tri1_count = 0;
    s32 tri2_cmd_count = 0;
    s32 tri2_tri_count = 0;
    s32 tri_bbox_hits = 0;
    s32 tri_intersects = 0;
    s32 invalid_indices = 0;
    s32 out_of_range_vertices = 0;

    if (dl_base == NULL || vtx_base == NULL) {
        bgHitTracePrintf("room-line frame=%d room=%d reject=missing_buffers dl=%p vtx=%p",
                         g_frame_count_diag, room, (void *)dl_base, (void *)vtx_base);
        return 0;
    }

    s32 vtx_size = ri->usize_point_index_binary;
    if (vtx_size <= 0 || vtx_size > 256 * 1024) vtx_size = 256 * 1024;
    u8 *vtx_end = vtx_base + vtx_size;

    /* Bounds check: stay within the allocated DL buffer.  Use the larger of
     * the decompressed size and the fixed allocation size, but cap at the
     * allocation (256KB) to prevent overreads on stale collision indices.
     * On N64 the DL lived in a contiguous segment so overreads were harmless;
     * on PC the buffer is a fixed-size malloc. */
    s32 dl_size = ri->usize_primary_DL_binary;
    if (dl_size <= 0 || dl_size > 256 * 1024) {
        dl_size = 256 * 1024;  /* fallback to allocation size */
    }
    u8 *dl_end = dl_base + dl_size;

    /* coll_dl = dl_base + (coll_index * 8) */
    coll_dl = dl_base + ((s32)coll_index * 8);

    /* Bounds check: need 16 readable bytes (8-byte header + 8-byte first cmd).
     * Use >= so that coll_dl+15 is the last readable byte < dl_end. */
    if (coll_dl < dl_base || coll_dl + 16 >= dl_end) {
        bgHitTracePrintf("room-line frame=%d room=%d reject=coll_bounds coll=%d dl_size=%d",
                         g_frame_count_diag, room, (int)coll_index, (int)dl_size);
        return 0;
    }

    /* Parse the 8-byte collision DL header:
     * byte 1 & 0xF: vertex pool base offset (temp_t2)
     * bytes 4-7 & 0xFFFFFF: vertex data offset relative to vtx_base */
    vtx_base_offset = coll_dl[1] & 0xF;
    {
        u32 word4 = ((u32)coll_dl[4] << 24) | ((u32)coll_dl[5] << 16) |
                    ((u32)coll_dl[6] << 8) | (u32)coll_dl[7];
        vtx_pool_base = vtx_base + (word4 & 0x00FFFFFF);
    }

    /* Check first command type (byte 8 of the DL header block) */
    cmdType = (s8)coll_dl[8];
    if (cmdType == 4 || cmdType == (s8)0xB8) {
        bgHitTracePrintf("room-line frame=%d room=%d coll=%d reject=empty first_op=0x%02x",
                         g_frame_count_diag, room, (int)coll_index, (unsigned int)(u8)cmdType);
        return 0;  /* G_VTX or G_ENDDL at start — no triangles */
    }

    /* Start walking DL commands from offset +8 */
    cmd = coll_dl + 8;

    while (cmdType != (s8)0xB8 && cmdType != 4) {
        if (cmdType == (s8)0xBF) {
            tri1_count++;
            /* G_TRI1: single triangle */
            /* Vertex indices at bytes 5,6,7, each divided by 10, minus vtx_base_offset */
            s32 idx0 = (s32)(cmd[5] / 10) - vtx_base_offset;
            s32 idx1 = (s32)(cmd[6] / 10) - vtx_base_offset;
            s32 idx2 = (s32)(cmd[7] / 10) - vtx_base_offset;

            /* Build bbox from 3 vertices (BE s16 at vtx_pool_base + idx*0x10) */
            s32 bbox_min[3], bbox_max[3];
            s32 indices[3];
            s32 k;
            struct unk_bg_struct hit_out;
            Vtx temp_verts[3];
            ...
            bbox_min[0] = bbox_min[1] = bbox_min[2] = 0x7FFF;
            bbox_max[0] = bbox_max[1] = bbox_max[2] = (s32)0xFFFF8000;

            for (k = 0; k < 3; k++) {
                u8 *vp = vtx_pool_base + indices[k] * 0x10;
                if (vp < vtx_base || vp + 6 > vtx_end) {
                    out_of_range_vertices++;
                    goto next_cmd;
                }
                s16 vx = (s16)((vp[0] << 8) | vp[1]);
                s16 vy = (s16)((vp[2] << 8) | vp[3]);
                s16 vz = (s16)((vp[4] << 8) | vp[5]);
                if (vx < bbox_min[0]) bbox_min[0] = vx;
                if (vy < bbox_min[1]) bbox_min[1] = vy;
                if (vz < bbox_min[2]) bbox_min[2] = vz;
                if (vx > bbox_max[0]) bbox_max[0] = vx;
                if (vy > bbox_max[1]) bbox_max[1] = vy;
                if (vz > bbox_max[2]) bbox_max[2] = vz;
            }

            /* Add room position offset */
            bbox_min[0] += (s32)rd->pos.f[0];
            bbox_min[1] += (s32)rd->pos.f[1];
            bbox_min[2] += (s32)rd->pos.f[2];
            bbox_max[0] += (s32)rd->pos.f[0];
            bbox_max[1] += (s32)rd->pos.f[1];
            bbox_max[2] += (s32)rd->pos.f[2];

            if (bgTestLineIntersectsBbox(start, delta, bbox_min, bbox_max) != 0) {
                tri_bbox_hits++;
                /* Build native-endian temp vertices for intersectLineTriangle */
                for (k = 0; k < 3; k++) {
                    u8 *vp = vtx_pool_base + indices[k] * 0x10;
                    temp_verts[k].v.ob[0] = (s16)((vp[0] << 8) | vp[1]);
                    temp_verts[k].v.ob[1] = (s16)((vp[2] << 8) | vp[3]);
                    temp_verts[k].v.ob[2] = (s16)((vp[4] << 8) | vp[5]);
                }

                if (intersectLineTriangle(&temp_verts[0], &temp_verts[1], &temp_verts[2],
                                           &rd->pos, start, dest, delta,
                                           &hit_out) != 0) {
                    s16 texturenum;
                    tri_intersects++;
                    /* Compute distance from start to hit point */
                    s32 dx = (s32)(hit_out.unk00 - start->f[0]);
                    s32 dy = (s32)(hit_out.unk04 - start->f[1]);
                    s32 dz = (s32)(hit_out.unk08 - start->f[2]);
                    s32 dist_sq = dx*dx + dy*dy + dz*dz;

                    if (dist_sq < best_dist_sq) {
                        texturenum = bgFindTextureForTriangleCommand(dl_base, cmd);
                        bgHitTracePrintf("tri-hit frame=%d room=%d coll=%d cmd=%ld op=0xbf tri=0 "
                                         "idx=(%d,%d,%d) tex=%d dist=%d "
                        ...
                        bgPopulateHitRecord(output, &hit_out,
                            (Vertex *)(vtx_pool_base + idx0 * 0x10),
                            (Vertex *)(vtx_pool_base + idx1 * 0x10),
                            (Vertex *)(vtx_pool_base + idx2 * 0x10),
                            (Gfx *)cmd, 0, texturenum);
                    }
                }
            }

        } else if (cmdType == (s8)0xB1) {
            tri2_cmd_count++;
            /* Rare's G_TRI4: four triangles packed as 4-bit x/y/z indices.
             * Keep this in lockstep with include/gbi_extension.h and the
             * native Fast3D interpreter. */
            s32 triIndex;
            u32 w0_be = bgReadBe32(cmd + 0);
            u32 w1_be = bgReadBe32(cmd + 4);
            for (triIndex = 0; triIndex < 4; triIndex++) {
                tri2_tri_count++;
                s32 idx0, idx1, idx2;
                s32 bbox_min[3], bbox_max[3];
                s32 indices_tri[3];
                ...
                idx0 = (s32)((w1_be >> (triIndex * 8)) & 0xF) - vtx_base_offset;
                idx1 = (s32)((w1_be >> (triIndex * 8 + 4)) & 0xF) - vtx_base_offset;
                idx2 = (s32)((w0_be >> (triIndex * 4)) & 0xF) - vtx_base_offset;
                ...
                for (k = 0; k < 3; k++) {
                    u8 *vp = vtx_pool_base + indices_tri[k] * 0x10;
                    if (vp < vtx_base || vp + 6 > vtx_end) {
                        out_of_range_vertices++;
                        goto next_tri2;
                    }
                ...
                if (bgTestLineIntersectsBbox(start, delta, bbox_min, bbox_max) != 0) {
                    ...
                    if (intersectLineTriangle(&temp_verts[0], &temp_verts[1], &temp_verts[2],
                        ...
                        if (dist_sq < best_dist_sq) {
                            texturenum = bgFindTextureForTriangleCommand(dl_base, cmd);
                            bgHitTracePrintf("tri-hit frame=%d room=%d coll=%d cmd=%ld op=0xb1 tri=%d "
                                             "idx=(%d,%d,%d) tex=%d dist=%d "
                                             "v0=(%d,%d,%d) v1=(%d,%d,%d) v2=(%d,%d,%d) "
                                             "hit=(%.2f,%.2f,%.2f)",
                                             g_frame_count_diag,
                            ...
                            bgPopulateHitRecord(output, &hit_out,
                                (Vertex *)(vtx_pool_base + indices_tri[0] * 0x10),
                                (Vertex *)(vtx_pool_base + indices_tri[1] * 0x10),
                                (Vertex *)(vtx_pool_base + indices_tri[2] * 0x10),
                                (Gfx *)cmd, (s16)(triIndex + 1), texturenum);
                        }
                    }
                }
        ...
        next_cmd:
        /* Advance to next DL command.  Check bounds before reading the
         * opcode byte — need at least 8 readable bytes for a full command. */
        cmd += 8;
        if (cmd < dl_base || cmd + 8 > dl_end) {
            break;
        }
        cmdType = (s8)cmd[0];
    }

    bgHitTracePrintf("room-line frame=%d room=%d coll=%d result=%d tri1=%d tri2cmd=%d tri2tri=%d "
                     "tri_bbox=%d tri_hit=%d invalid=%d out_vtx=%d best=%d start=(%.2f,%.2f,%.2f) "
                     "dest=(%.2f,%.2f,%.2f)",
                     g_frame_count_diag,
                     room,
                     (int)coll_index,
...
#else
s32 bgTestLineIntersectionInRoom(void *arg0, void *arg1, void *arg2, void *arg3, s32 arg4, void *arg5) {
...
loop_3:
            ...
            if (cmdType == -0x41)
            {
                ...
                spE4 = (s32) (((s32) dlPtr->unk5 / 0xa) - temp_t2);
                spE8 = (s32) (((s32) dlPtr->unk6 / 0xa) - temp_t2);
                spEC = (s32) (((s32) dlPtr->unk7 / 0xa) - temp_t2);
                sp54 = (s32) (arg4 * 0x18);
                indexPtr = &spE4;
loop_5:
                // Node 5
                temp_a0 = (indexPtr + 4);
                temp_v1 = (temp_s1 + (*indexPtr * 0x10));
                if (*temp_v1 < spD4)
                {
                    // Node 6
                    spD4 = (s32) *temp_v1;
                ...
                if (bgTestLineIntersectsBbox(arg0, arg2, &spD4, &spC8) != 0)
                {
                    ...
                    if (intersectLineTriangle((temp_s1 + (spE4 * 0x10)), (temp_s1 + (spE8 * 0x10)), (temp_s1 + (spEC * 0x10)), ((sp54 + ptr_bgdata_room_fileposition_list) + 0xc), arg0, arg1, arg2, &spF8) != 0)
                    {
                        ...
                        if (0xfd != *dlPtr)
                        {
                            ...
                            searchPtr = temp_v0_4;
                            texCmdPtr = temp_v0_4;
                            if ((u32) sp64->unk8 < (u32) dlPtr)
                            {
loop_22:
                                // Node 22
                                temp_v0_5 = (searchPtr + -8);
                                texCmdPtr = temp_v0_5;
                                if (0xfd != searchPtr->unk-8)
                                {
                                    // Node 23
                                    searchPtr = temp_v0_5;
                                    texCmdPtr = temp_v0_5;
                        ...
                        {
                            // Node 26
                            imageId = *((texCmdPtr->unk4 + -8) | 0x80000000);
                        }
                        // Node 27
                        if (check_if_imageID_is_light(imageId, ((s32) sp100 - (s32) arg0->unk8), arg0) != 0)
                        {
                            // Node 28
                        }
                        // Node 29
                        ...
                        if (0x4fd != 0)
                        {
                            ...
                            if (imageId != 0x4fd)
                            {
                                ...
                                *arg5 = spF8;
                                ...
                                arg5->unk2A = (s16) imageId;
                                arg5->unk24 = (void *) dlPtr;
...
loop_34:
                    ...
                    {
                        // Node 35
                        sp9C = (s32) ((dlPtr->unk4 & 0xf) - temp_t2);
                        spA0 = (s32) (((u32) dlPtr->unk7 >> 4) - temp_t2);
                        spA4 = (s32) ((*dlPtr & 0xf) - temp_t2);
                    }
                    else
                    {
                        // Node 36
                        ...
                        {
                            // Node 37
                            sp9C = (s32) ((dlPtr->unk6 & 0xf) - temp_t2);
                            spA0 = (s32) (((u32) dlPtr->unk6 >> 0xc) - temp_t2);
                            spA4 = (s32) (((u32) dlPtr->unk3 >> 4) - temp_t2);
                        }
                        else
                        {
                            // Node 38
                            if (triIndex == 2)
                            {
                                // Node 39
                                sp9C = (s32) ((dlPtr->unk4 & 0xf) - temp_t2);
                                spA0 = (s32) (((u32) dlPtr->unk5 >> 4) - temp_t2);
                                spA4 = (s32) ((dlPtr->unk2 & 0xf) - temp_t2);
                            }
                            else
                            {
                                // Node 40
                                sp9C = (s32) ((dlPtr->unk4 & 0xf) - temp_t2);
                                spA0 = (s32) (((u32) dlPtr->unk4 >> 0x1c) - temp_t2);
                                spA4 = (s32) (((u32) dlPtr->unk2 >> 0xc) - temp_t2);
                            }
                        }
                    }
                    // Node 41
...
loop_42:
                    // Node 42
                    temp_a0_2 = (indexPtr2 + 4);
                    temp_v1_2 = (temp_s1 + (*indexPtr2 * 0x10));
                    if (*temp_v1_2 < sp8C)
                    {
                        // Node 43
                        sp8C = (s32) *temp_v1_2;
                    ...
                    if (bgTestLineIntersectsBbox(arg0, arg2, &sp8C, &sp80) != 0)
                    {
                        ...
                        if (intersectLineTriangle((temp_s1 + (sp9C * 0x10)), (temp_s1 + (spA0 * 0x10)), (temp_s1 + (spA4 * 0x10)), ((sp54 + ptr_bgdata_room_fileposition_list) + 0xc), arg0, arg1, arg2, &spF8) != 0)
                        {
                            ...
                            temp_v0_6 = dlPtr;
                            sp12C = 1;
                            texCmdPtr2 = temp_v0_6;
                            if (0xfd != *dlPtr)
                            {
                                // Node 58
                                searchPtr2 = temp_v0_6;
                                texCmdPtr2 = temp_v0_6;
                                if ((u32) sp64->unk8 < (u32) dlPtr)
                                {
loop_59:
                                    // Node 59
                                    temp_v0_7 = (searchPtr2 + -8);
                                    texCmdPtr2 = temp_v0_7;
                                    if (0xfd != searchPtr2->unk-8)
                                    {
                                        // Node 60
                                        searchPtr2 = temp_v0_7;
                                        texCmdPtr2 = temp_v0_7;
                            ...
                            {
                                // Node 63
                                imageId2 = *((texCmdPtr2->unk4 + -8) | 0x80000000);
                            }
                            // Node 64
                            if (check_if_imageID_is_light(imageId2, ((s32) sp100 - (s32) arg0->unk8), arg0) != 0)
                            {
                                // Node 65
                            }
                            // Node 66
                            if (0x4fd != 0)
                            {
                                ...
                                if (imageId2 != 0x4fd)
                                {
                                    ...
                                    *arg5 = spF8;
                                    ...
                                    arg5->unk2A = (s16) imageId2;
                                    arg5->unk24 = (void *) dlPtr;
                                    arg5->unk20 = (s32) ((spA4 * 0x10) + temp_s1);
                                    arg5->unk28 = (s16) (triIndex + 1);
                                }
                            }
                        }
                    }
...
s32 bgTestBulletHitBackground(coord3d *start, coord3d *dest, s32 room, BgHitRecord *output) {
    ...
    s16 count;
    s32 i;
    coord3d start_scaled;
    coord3d dest_scaled;
    coord3d delta;
    BgHitRecord hit_results;
    f32 scale;
    s32 dx;
    s32 dy;
    s32 dz;
    ...
    found = 0;
    best_dist_sq = 0x7FFFFFFF;
    group_bbox_hits = 0;
    room_line_hits = 0;

    scale = room_data_float1;

    start_scaled.x = start->x * scale;
    start_scaled.y = start->y * scale;
    start_scaled.z = start->z * scale;

    dest_scaled.x = dest->x * scale;
    dest_scaled.y = dest->y * scale;
    dest_scaled.z = dest->z * scale;

    delta.x = dest_scaled.x - start_scaled.x;
    delta.y = dest_scaled.y - start_scaled.y;
    delta.z = dest_scaled.z - start_scaled.z;

    room_info = &g_BgRoomInfo[room];
    collision_ptr = (u8 *)room_info->ptr_unique_collision_points;

    if (collision_ptr == NULL) {
        bgHitTracePrintf("bullet frame=%d room=%d reject=no_collision start=(%.2f,%.2f,%.2f) dest=(%.2f,%.2f,%.2f)",
                         g_frame_count_diag,
                         room,
                         start->x,
                         start->y,
    ...
    count = *(s16 *)&room_info->bitflags3;

    if (count <= 0) {
        bgHitTracePrintf("bullet frame=%d room=%d reject=count count=%d start=(%.2f,%.2f,%.2f) dest=(%.2f,%.2f,%.2f)",
                         g_frame_count_diag,
                         room,
                         (int)count,
                         start->x,
    ...
    for (i = 0; i < count; i++) {
        ...
        bbox_min[0] = *(s32 *)(collision_ptr + 0x04);
        bbox_min[1] = *(s32 *)(collision_ptr + 0x08);
        bbox_min[2] = *(s32 *)(collision_ptr + 0x0C);
        bbox_max[0] = *(s32 *)(collision_ptr + 0x10);
        bbox_max[1] = *(s32 *)(collision_ptr + 0x14);
        bbox_max[2] = *(s32 *)(collision_ptr + 0x18);

        if (bgTestLineIntersectsBbox(&start_scaled, &delta, bbox_min, bbox_max) != 0) {
            group_bbox_hits++;
            if (bgTestLineIntersectionInRoom(&start_scaled, &dest_scaled, &delta, collision_ptr, room, &hit_results) != 0) {
                found = 1;
                room_line_hits++;

                dx = (s32)(hit_results.pos.x - start_scaled.x);
                dy = (s32)(hit_results.pos.y - start_scaled.y);
                dz = (s32)(hit_results.pos.z - start_scaled.z);
                dist_sq = (u32)dx * (u32)dx + (u32)dy * (u32)dy + (u32)dz * (u32)dz;
                dist_adj = dist_sq;

                if (check_if_imageID_is_light(hit_results.texturenum) != 0) {
                    dist_adj = dist_sq - 4;
                }

                if ((s32)dist_adj < (s32)best_dist_sq) {
                    best_dist_sq = dist_adj;
                    *output = hit_results;
                }
            }
        }
    ...
    if (found == 0) {
        bgHitTracePrintf("bullet frame=%d room=%d found=0 groups=%d group_bbox=%d line_hits=%d "
                         "scale=%.4f start=(%.2f,%.2f,%.2f) dest=(%.2f,%.2f,%.2f) "
                         "start_scaled=(%.2f,%.2f,%.2f) dest_scaled=(%.2f,%.2f,%.2f)",
                         g_frame_count_diag,
                         room,
                         (int)count,
                         group_bbox_hits,
    ...
    bgHitTracePrintf("bullet frame=%d room=%d found=1 groups=%d group_bbox=%d line_hits=%d best=%d "
                     "hit=(%.2f,%.2f,%.2f) normal=(%.3f,%.3f,%.3f) texture=%d",
                     g_frame_count_diag,
                     room,
                     (int)count,
                     group_bbox_hits,
...
#else
s32 bgTestBulletHitBackground(coord3d *start, coord3d *dest, s32 room, u8 *output) {
    s32 found;
    s32 best_dist_sq;
    s_room_info *room_info;
    u8 *collision_ptr;
    ...
    bbox_min = (s32 *)(collision_ptr + 4);
    bbox_max = (s32 *)(collision_ptr + 0x10);
    ...
    for (i = 0; i < count; i++) {
        if (bgTestLineIntersectsBbox(&start_scaled, &delta, bbox_min, bbox_max) != 0) {
            if (bgTestLineIntersectionInRoom(&start_scaled, &dest_scaled, &delta, collision_ptr, room, hit_results) != 0) {
                ...
                dx = (s32)(*(f32 *)(hit_results + 0x00) - start_scaled.x);
                dy = (s32)(*(f32 *)(hit_results + 0x04) - start_scaled.y);
                dz = (s32)(*(f32 *)(hit_results + 0x08) - start_scaled.z);
                dist_sq = (u32)dx * (u32)dx + (u32)dy * (u32)dy + (u32)dz * (u32)dz;
                dist_adj = dist_sq;

                if (check_if_imageID_is_light(*(s16 *)(hit_results + 0x2A)) != 0) {
                    dist_adj = dist_sq - 4;
                }

                if ((s32)dist_adj < (s32)best_dist_sq) {
                    best_dist_sq = dist_adj;
                    *(f32 *)(output + 0x00) = *(f32 *)(hit_results + 0x00);
                    *(f32 *)(output + 0x04) = *(f32 *)(hit_results + 0x04);
                    *(f32 *)(output + 0x08) = *(f32 *)(hit_results + 0x08);
                    *(f32 *)(output + 0x0C) = *(f32 *)(hit_results + 0x0C);
                    *(f32 *)(output + 0x10) = *(f32 *)(hit_results + 0x10);
                    *(f32 *)(output + 0x14) = *(f32 *)(hit_results + 0x14);
                    *(s32 *)(output + 0x18) = *(s32 *)(hit_results + 0x18);
                    *(s32 *)(output + 0x1C) = *(s32 *)(hit_results + 0x1C);
                    *(s32 *)(output + 0x20) = *(s32 *)(hit_results + 0x20);
                    *(s16 *)(output + 0x2A) = *(s16 *)(hit_results + 0x2A);
                    *(s32 *)(output + 0x24) = *(s32 *)(hit_results + 0x24);
                    *(s16 *)(output + 0x28) = *(s16 *)(hit_results + 0x28);
                }
            }
        }

        collision_ptr += 0x1C;
        bbox_min = (s32 *)((u8 *)bbox_min + 0x1C);
        bbox_max = (s32 *)((u8 *)bbox_max + 0x1C);
    }
...
#ifdef NATIVE_PORT
    /* On PC, bgTestLineIntersectionInRoom stores 0 at offset 0x24 (no DL pointer).
     * Skip the N64-specific DL backward search for texture commands. */
    *(s16 *)(output + 0x2C) = -1;
    *(s16 *)(output + 0x2E) = -1;
#else
    tex_ptr = *(u8 **)(output + 0x24);
    byte0 = tex_ptr[0];

    if (byte0 != 0xF5) {
        base_ptr = (u8 *)room_info->ptr_expanded_mapping_info;

        if ((uintptr_t)base_ptr < (uintptr_t)tex_ptr) {
            byte0 = *(tex_ptr - 8);
            while (byte0 != 0xF5) {
                tex_ptr -= 8;
                if (!((uintptr_t)base_ptr < (uintptr_t)tex_ptr)) {
    ...
    if (tex_ptr == base_ptr) {
        ...
        *(s16 *)(output + 0x2E) = -1;
        ...
        byte1 = tex_ptr[1];
        *(s16 *)(output + 0x2C) = byte1 >> 5;
        word0 = *(s32 *)tex_ptr;
        *(s16 *)(output + 0x2E) = (word0 << 11) >> 30;
    }
#endif
...
#ifdef NONMATCHING
// NONMATCHING: 6 of 64 instructions differ due to ido_recomp register allocation.
// 1. lui t3 vs lui t9 for D_800448A4 preload (different temp register choice)
// 2. lw t9,(t0); addiu a0,t9,1 vs lw a0,(t0); addiu a0,a0,1 (counter load intermediate)
// 3. bnel t3,a0 vs bnel a0,t9 (operand order + different registers for D_800448A4 comparison)
// 4. sw t4 vs sw t2 (cascaded register difference for decrement store)
// All other 58 instructions match byte-identically.
void sub_GAME_7F0B7DE4(s32 arg0, s32 arg1, s32 arg2, s32 arg3, f32 *arg4) {
    s32 *entry;
    s32 new_idx;

    entry = (s32 *)(dword_CODE_bss_8007C100 + (D_800448A0 << 5));

    if (arg3 >= 2) {
        s32 dest_room = (g_BgPortals[arg2].connectedRoom2 ^ g_BgPortals[arg2].connectedRoom1) ^ arg1;
        u8 portal_count = sub_GAME_7F0B7DA8(dest_room);
#ifdef NATIVE_PORT
        if (bgShouldTracePortal(arg1, arg2, dest_room)) {
            fprintf(stderr,
                    "[BG-PORTAL-QUEUE] %s src=%d portal=%d dest=%d depth=%d room_count=%u\n",
                    portal_count >= 9 ? "drop" : "enqueue",
                    arg1, arg2, dest_room, arg3, portal_count);
        }
#endif
        if (portal_count >= 9) {
            return;
...
#ifdef NATIVE_PORT
    } else if (bgShouldTracePortal(arg1, arg2,
                                  (g_BgPortals[arg2].connectedRoom2 ^ g_BgPortals[arg2].connectedRoom1) ^ arg1)) {
        fprintf(stderr,
                "[BG-PORTAL-QUEUE] enqueue src=%d portal=%d dest=%d depth=%d room_count=shallow\n",
                arg1, arg2,
                (g_BgPortals[arg2].connectedRoom2 ^ g_BgPortals[arg2].connectedRoom1) ^ arg1,
                arg3);
#endif
    ...
    *(f32 *)&entry[7] = arg4[3];
    ...
    new_idx = D_800448A0 + 1;
    D_800448A0 = new_idx;
    if (new_idx == 500) {
        D_800448A0 = 0;
        new_idx = 0;
    }
    if (new_idx == D_800448A4) {
        D_800448A0 = new_idx - 1;
    }
}
#else
...
s32 sub_GAME_7F0B7EE4(s32 *arg0) {
    ...
    entry = (s32 *)((u8 *)dword_CODE_bss_8007C100 + (D_800448A4 << 5));
    gdl = sub_GAME_7F0B7F84(gdl, entry[1], entry[2], entry[3], &entry[4]);
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PC: Portal descent — the core BFS traversal function for portal-based visibility.
 * Decompiled from non-LEFTOVERDEBUG GLOBAL_ASM.
 * Parameters match LEFTOVERDEBUG signature: (gdl, source_room, portal_idx, depth, parent_bbox).
 * gdl is passed through and returned (display list pointer, unused in non-LOD). */
s32 sub_GAME_7F0B7F84(s32 gdl, s32 source_room, s32 portal_idx, s32 depth, s32 *parent_bbox) {
    bg_portal_data_entry *portal_entry;
    coord3d *player_pos;
    struct PortalMetric metric;
    f32 dot;
    f32 scale;
    ...
    s32 i;
    s32 trace_portal;
    s32 use_precomputed_portal_bbox;

    /* Increment global portal traversal counter */
    D_80044898++;

    trace_portal = 0;
    use_precomputed_portal_bbox = 0;

    /* Depth limits */
    if (depth >= 101) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=depth>100 src=%d portal=%d depth=%d\n",
                    source_room, portal_idx, depth);
        }
        return gdl;
    }
    if (D_8004489C < depth) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=depth_limit src=%d portal=%d depth=%d limit=%d\n",
                    source_room, portal_idx, depth, D_8004489C);
        }
        return gdl;
    }
    if (depth >= 16) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=depth>=16 src=%d portal=%d depth=%d\n",
                    source_room, portal_idx, depth);
        }
        return gdl;
    }

    /* Get portal data entry */
    portal_entry = &g_BgPortals[portal_idx];
    controlbytes1 = portal_entry->controlbytes1;

    /* Check if portal is disabled (bit 0 of controlbytes1) */
    if (controlbytes1 & 1) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=disabled src=%d portal=%d cb1=0x%02x\n",
                    source_room, portal_idx, controlbytes1);
        }
        return gdl;
    }

    /* Get player position and compute portal normal */
    player_pos = bondviewGetCurrentPlayersPosition();
    sub_GAME_7F0B96CC(portal_idx, &metric);

    /* Compute dot product: normal · player_position * levelscale */
    dot = (metric.normal.x * player_pos->x
         + metric.normal.y * player_pos->y
         + metric.normal.z * player_pos->z) * room_data_float1;

    /* Get portal expansion scale */
    scale = sub_GAME_7F0B9990(portal_idx);

    /* Determine which side of the portal the player is on,
     * and identify the destination room */
    if (source_room == portal_entry->connectedRoom1) {
        dest_room = portal_entry->connectedRoom2;
        trace_portal = bgShouldTracePortal(source_room, portal_idx, dest_room);
        if (trace_portal) {
            fprintf(stderr,
                    "[BG-PORTAL] src=%d portal=%d side=r1 dest=%d dot=%.3f scale=%.3f min=%.3f max=%.3f\n",
                    source_room, portal_idx, dest_room, dot, scale, metric.min, metric.max);
        }
        if (metric.max <= (dot - scale)) {
            if (bgPortalBackfaceBypassApplies(source_room, portal_idx, dest_room)) {
                if (trace_portal) {
                    fprintf(stderr,
                ...
                if (trace_portal) {
                    fprintf(stderr,
                            "[BG-PORTAL] fallback=projected_visible src=%d portal=%d dest=%d side=r1\n",
                            source_room, portal_idx, dest_room);
                }
            } else {
                if (trace_portal) {
                    fprintf(stderr, "[BG-PORTAL] reject=backface_r1 src=%d portal=%d\n",
                            source_room, portal_idx);
                }
                return gdl;
            }
        }
    } else {
        dest_room = portal_entry->connectedRoom1;
        trace_portal = bgShouldTracePortal(source_room, portal_idx, dest_room);
        if (trace_portal) {
            fprintf(stderr,
                    "[BG-PORTAL] src=%d portal=%d side=r2 dest=%d dot=%.3f scale=%.3f min=%.3f max=%.3f\n",
                    source_room, portal_idx, dest_room, dot, scale, metric.min, metric.max);
        }
        if ((dot + scale) <= metric.min) {
            if (bgPortalBackfaceBypassApplies(source_room, portal_idx, dest_room)) {
                if (trace_portal) {
                    fprintf(stderr,
                ...
                if (trace_portal) {
                    fprintf(stderr,
                            "[BG-PORTAL] fallback=projected_visible src=%d portal=%d dest=%d side=r2\n",
                            source_room, portal_idx, dest_room);
                }
            } else {
                if (trace_portal) {
                    fprintf(stderr, "[BG-PORTAL] reject=backface_r2 src=%d portal=%d\n",
                            source_room, portal_idx);
                }
                return gdl;
            }
    ...
    /* "Inside portal zone" check:
     * If player is between (metric.min - scale) and (metric.max + scale),
     * they're inside the portal expansion zone. Use full screen as bbox. */
    if ((metric.min - scale) < dot && dot < (metric.max + scale)) {
        /* Player is inside the portal zone — use full screen bounds */
        local_bbox[0] = g_CurrentPlayer->screenxminf;
        local_bbox[1] = g_CurrentPlayer->screenyminf;
        local_bbox[2] = g_CurrentPlayer->screenxmaxf;
        local_bbox[3] = g_CurrentPlayer->screenymaxf;
    } else {
        /* Player is outside portal zone — project portal to screen space */
        controlbytes1 = portal_entry->controlbytes1;

        if (controlbytes1 & 2) {
            /* Controlbytes1 bit 1 set: use portal projection + frustum cull */
            if (!sub_GAME_7F0B5864(portal_idx, local_bbox)) {
                if (trace_portal) {
                    fprintf(stderr, "[BG-PORTAL] reject=project_bit1 src=%d portal=%d\n",
                            source_room, portal_idx);
                }
                return gdl;
            }

            /* Compute destination room and frustum-test it */
            dest_room = (portal_entry->connectedRoom2 ^ portal_entry->connectedRoom1) ^ source_room;
            if (!sub_GAME_7F0B5208(dest_room, local_bbox)) {
                if (trace_portal) {
                    fprintf(stderr,
                            "[BG-PORTAL] reject=roomclip_bit1 src=%d portal=%d dest=%d bbox=(%.1f,%.1f,%.1f,%.1f)\n",
                            source_room, portal_idx, dest_room,
            ...
            /* Use full screen bounds as the clip region */
            local_bbox[0] = g_CurrentPlayer->screenxminf;
            local_bbox[1] = g_CurrentPlayer->screenyminf;
            local_bbox[2] = g_CurrentPlayer->screenxmaxf;
            local_bbox[3] = g_CurrentPlayer->screenymaxf;
    ...
    /* Check for degenerate bbox (right <= left or bottom <= top) */
    if (local_bbox[2] <= local_bbox[0]) {
        if (trace_portal) {
            fprintf(stderr,
                    "[BG-PORTAL] reject=bbox_x src=%d portal=%d bbox=(%.1f,%.1f,%.1f,%.1f)\n",
                    source_room, portal_idx,
                    local_bbox[0], local_bbox[1], local_bbox[2], local_bbox[3]);
        }
        return gdl;
    }
    if (local_bbox[3] <= local_bbox[1]) {
        if (trace_portal) {
            fprintf(stderr,
                    "[BG-PORTAL] reject=bbox_y src=%d portal=%d bbox=(%.1f,%.1f,%.1f,%.1f)\n",
                    source_room, portal_idx,
                    local_bbox[0], local_bbox[1], local_bbox[2], local_bbox[3]);
        }
        return gdl;
    ...
    /* Stock writes the portal-depth byte only after side/projection/bbox tests
     * have left a drawable portal window. Keep this later write position so
     * rejected portals don't look traversed in diagnostics or later checks. */
    ((u8 *)D_800442FC)[portal_idx] = (u8)depth;

    /* Validate bbox has positive area */
    if (!(local_bbox[0] < local_bbox[2])) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=bbox_x_strict src=%d portal=%d\n",
                    source_room, portal_idx);
        }
        return gdl;
    }
    if (!(local_bbox[1] < local_bbox[3])) {
        if (trace_portal) {
            fprintf(stderr, "[BG-PORTAL] reject=bbox_y_strict src=%d portal=%d\n",
                    source_room, portal_idx);
        }
        return gdl;
    }

    /* Add destination room to render queue */
    {
        u8 cb1 = portal_entry->controlbytes1;
        s32 flag = cb1 & 2;
        s32 already_queued;
        bgSetRoomAdmitTraceContext("portal_bfs", source_room, portal_idx, depth);
        already_queued = sub_GAME_7F0B39BC(dest_room, depth, (bbox2d *)local_bbox, flag);
        bgClearRoomAdmitTraceContext();
        if (trace_portal) {
            fprintf(stderr,
        ...
        if (already_queued) {
            /* Room was already in the render queue — don't traverse further */
            return gdl;
        }
    }

    /* Enqueue connecting portals of the destination room for further BFS traversal.
     * Skip the portal we just came through (portal_idx). */
    {
        bg_portal_data_entry *cur = g_BgPortals;
        s32 byteOffset = 0;
        portalIndex = 0;

        if (cur->offset_portal != 0) {
            do {
                if (portalIndex != portal_idx) {
                    if (dest_room == cur->connectedRoom1 || dest_room == cur->connectedRoom2) {
                        sub_GAME_7F0B7DE4(0, dest_room, portalIndex, depth + 1, local_bbox);
                        cur = (bg_portal_data_entry *)((u8 *)g_BgPortals + byteOffset);
                    }
                }
                byteOffset += 8;
...
/**
 * Read a big-endian s32 from the visibility command list.
 * On NATIVE_PORT (little-endian PC), the command list is ROM data that was
 * never byte-swapped, so every s32 field must be swapped at read time.
 */
#ifdef NATIVE_PORT
static inline s32 vis_read_s32(u8 *pc, int offset) {
    u32 raw;
    memcpy(&raw, pc + offset, 4);
    return (s32)bswap32(raw);
}
#else
#define vis_read_s32(pc, offset) (*(s32 *)((pc) + (offset)))
#endif

/**
 * Parses and executes a global visibility command list.
 *
 * The command list is a bytecode stream. Each instruction has:
 *   - byte 0: opcode
 *   - byte 1: size of this instruction in 8-byte units (used to advance to next instruction)
 *   - word at offset 0x4: push value (for push_to_stack)
 *   - word at offset 0xC: room id / portal id parameter
 *   - word at offset 0x14: end room id (for range commands)
 *
 * Opcodes 0x00-0x27 use vis_command_jpt (stack ops, room vis commands).
 * Opcodes 0x50-0x5C use jpt_80058C80 (if/else/endif control flow).
 *
 * @param pc   Pointer to the start of the command list.
 * @param flag If nonzero, commands are executed. If zero, commands are skipped (read-only traversal).
 * @return     Pointer to the instruction after the terminating command.
 *
 * Address 0x7F0B8414.
 */
void *parse_global_vis_command_list(u8 *pc, s32 flag) {
    s32 canPreload;
    s32 *cur_vis;          // s4 = &current_visibility
    f32 *screen_bbox;      // s6 = (f32 *)dword_CODE_bss_80081600
    struct player **pp;    // s7 constant 1, s5 constant 1, fp = &g_CurrentPlayer
    s32 temp;
    s32 val;
    u8 opcode;
    u8 *ret;
    struct bbox2d local_bbox;   // sp+0x68 (visible_if_seen_through_portal)
    struct bbox2d local_bbox2;  // sp+0x58 (not_visible_if_seen_through_portal)
#ifdef NATIVE_PORT
    static int trace_depth = 0;
    static int trace_count = 0;
    int my_trace_depth = trace_depth++;
    int trace_enabled = bgVisTraceEnabled();
#endif

    canPreload = 1;
    *(s32 *)&dword_CODE_bss_80081600[0x10] = 0;
    ...
    top:
        ...
        if (trace_enabled && trace_count < bgVisTraceBudget()) {
            fprintf(stderr,
                    ...
                    flag,
                    current_visibility,
                    ((bbox2d *)dword_CODE_bss_80081600)->min.x,
                    ((bbox2d *)dword_CODE_bss_80081600)->min.y,
                    ((bbox2d *)dword_CODE_bss_80081600)->max.x,
                    ((bbox2d *)dword_CODE_bss_80081600)->max.y);
            trace_count++;
        }
#endif

    dispatch:
        /* Opcodes >= 0x28: check the high-range jump table (0x50-0x5C) */
        if ((s32)opcode >= 0x28) {
            if ((u32)(opcode - 0x50) >= 0xD) {
            ...
            /* 0x50: if_statement */
            case 0:
                ret = parse_global_vis_command_list(pc + (pc[1] << 3), flag);
                pc = ret + (ret[1] << 3);
                goto top;

            /* 0x51: dont_exec_commands_even_on_return */
            case 1: {
                s32 was_active = (flag != 0);
                flag = 0;
                pc += (pc[1] << 3);
                if (!was_active) {
                    goto top;
                }
                *(s32 *)&dword_CODE_bss_80081600[0x10] = 1;
                goto top;
            }
            ...
            case 0xA: {
                val = bgStackPop();
                ret = parse_global_vis_command_list(pc + (pc[1] << 3), val & flag);
                pc = ret;
                if (*(s32 *)&dword_CODE_bss_80081600[0x10] == 0) {
                    opcode = pc[0];
                    goto dispatch;
                }
                flag = 0;
        ...
        /* Opcodes < 0x28: use the low-range jump table */
        if ((u32)opcode >= 0x28) {
            goto invalid_type_terminate;
        }
        switch (opcode) {
        ...
        /* 0x05: not_merge_last_two_on_stack (actually NOT top of stack) */
        case 0x05:
            if (flag != 0) {
                val = bgStackPop();
                bgStackPush(val == 0);
        ...
        /* 0x06: carrot_merge_last_two_on_stack (XOR) */
        case 0x06:
            if (flag != 0) {
                temp = bgStackPop();
                val = bgStackPop();
        ...
        case 0x14:
            if (flag != 0) {
                s32 room = g_BgCurrentRoom;
                s32 lo = vis_read_s32(pc, 0xC);
                s32 hi = vis_read_s32(pc, 0x14);
                s32 inrange = (room >= lo);
                if (inrange) {
                    inrange = (hi >= room);
                }
        ...
        case 0x20:
            if (flag != 0) {
                if (current_visibility == 0) {
                    ...
                    if (vis_room < 0 || vis_room > g_MaxNumRooms) {
                        pc += (pc[1] << 3);
                        goto top;
                    }
#endif
                    if (sub_GAME_7F0B5208(vis_room, (void *)dword_CODE_bss_80081600)) {
                        bgSetRoomAdmitTraceContext("global_vis_packet", g_BgCurrentRoom, -1, 0);
                        sub_GAME_7F0B39BC(vis_room, 0, (bbox2d *)dword_CODE_bss_80081600, 0);
                        bgClearRoomAdmitTraceContext();
        ...
        case 0x22:
            if (flag != 0) {
                ...
                if (!bgRectIntersect(&local_bbox, bgGetCurrentPlayerScreenBbox())) {
                    goto vis_through_portal_done;
                }
                if (current_visibility != 0) {
                    bbox2dCopy((struct bbox2d *)dword_CODE_bss_80081600, &local_bbox);
                    current_visibility = 0;
                } else {
                    sub_GAME_7F0B5CC0((struct bbox2d *)dword_CODE_bss_80081600, &local_bbox);
                }
            }
        vis_through_portal_done:
            pc += (pc[1] << 3);
            goto top;
        ...
        case 0x23:
            if (flag != 0) {
                ...
                if (!sub_GAME_7F0B5864(vis_read_s32(pc, 0xC), (f32 *)&local_bbox2)) {
                    current_visibility = 1;
                    goto not_vis_through_portal_done;
                }
                if (!bgRectIntersect(&local_bbox2, bgGetCurrentPlayerScreenBbox())) {
                    current_visibility = 1;
                    goto not_vis_through_portal_done;
                }
                if (!bgRectIntersect((struct bbox2d *)dword_CODE_bss_80081600, &local_bbox2)) {
                    current_visibility = 1;
                }
            }
        not_vis_through_portal_done:
            pc += (pc[1] << 3);
            goto top;
        ...
        case 0x27:
            if (flag != 0) {
                ...
                for (; room <= endroom; room++) {
                    if (canPreload != 0) {
                        canPreload = (bgCheckIfRoomModelNeedsLoad(room) == 0);
                        endroom = vis_read_s32(pc, 0x14);
                    }
                }
            }
            pc += (pc[1] << 3);
...
void *sub_GAME_7F0B8A24(s32 *pc) {
#ifdef NATIVE_PORT
    static int viscmd_frame_count = 0;
    extern int g_viscmd_diag_frame;
    g_viscmd_diag_frame = viscmd_frame_count++;

    {
        static int vis_diag_enabled = -1;
        if (vis_diag_enabled < 0) {
            vis_diag_enabled = (getenv("BG_VIS_DIAG") != NULL);
        }

        if (vis_diag_enabled && pc != NULL && g_viscmd_diag_frame < 3) {
            u8 *entry = (u8 *)pc;
            fprintf(stderr,
                    "[BG-VIS-CMD] frame=%d ptr=%p first:",
                    g_viscmd_diag_frame, (void *)pc);
            for (int cmd = 0; cmd < 8 && entry[0] != 0; cmd++, entry += (entry[1] << 3)) {
                fprintf(stderr, " [%02X size=%u]", entry[0], entry[1]);
            }
            fprintf(stderr, "\n");
        }
    }
    ...
    {
        void *ret = parse_global_vis_command_list((u8 *)pc, 1);
#ifdef NATIVE_PORT
        {
            static int vis_diag_enabled = -1;
            if (vis_diag_enabled < 0) {
            ...
            if (vis_diag_enabled && g_viscmd_diag_frame < 3) {
                fprintf(stderr,
                        "[BG-VIS-CMD] frame=%d parsed_rooms=%d current_visibility=%d\n",
                        g_viscmd_diag_frame,
                        num_visible_rooms_in_cur_global_vis_packet,
                        current_visibility);
                if (num_visible_rooms_in_cur_global_vis_packet > 0) {
                    fprintf(stderr, "[BG-VIS-CMD]   rooms:");
                    for (int i = 0;
                         i < num_visible_rooms_in_cur_global_vis_packet && i < 12;
                         i++) {
                        fprintf(stderr, " %d",
                                (u8)list_visible_rooms_in_cur_global_vis_packet[i]);
                    }
                    fprintf(stderr, "\n");
                }
            }
...
void sub_GAME_7F0B8A6C(void) {
    ...
    curplayer = g_CurrentPlayer;
    screen_bbox[0] = curplayer->screenxminf;
    screen_bbox[1] = curplayer->screenyminf;
    screen_bbox[2] = curplayer->screenxmaxf;
    screen_bbox[3] = curplayer->screenymaxf;

    sub_GAME_7F0B3BC4();

    /* Clear room_rendered, room_neighbor_to_rendered, room_loaded_mask for all rooms */
    /* NOTE: Original N64 code used &g_MaxNumRooms as end sentinel (g_MaxNumRooms was
     * placed right after g_BgRoomInfo in BSS). On 64-bit, these are at different addresses.
     * Use g_MaxNumRooms value instead. */
    roomptr = g_BgRoomInfo;
#ifdef NATIVE_PORT
    {
        s32 room_clear_idx;
...
#else
    while (roomptr < (s_room_info *)&g_MaxNumRooms) {
        roomptr->room_rendered = 0;
        roomptr->room_neighbor_to_rendered = 0;
        roomptr->room_loaded_mask = 0;
        roomptr++;
    }
#endif

    /* Clear D_800442FC array (50 entries x 4 bytes) */
    /* NOTE: Original N64 code used D_800443C4 address as end sentinel because
     * D_800443C4 was placed immediately after D_800442FC in BSS.
    ...
    do {
        ...
        byteptr[0] = 0;
        byteptr += 4;
    } while (byteptr != (u8 *)D_800443C4);
#endif

    /* Increment D_80044858 modulo 10 */
    D_80044858 = (D_80044858 + 1) % 10;
    ...
    sub_GAME_7F0B7D94();
    sub_GAME_7F0B5168();
    sub_GAME_7F0B8A24((s32 *)dword_CODE_bss_8007FF90);

    portals = g_BgPortals;
    use_portal_bfs = (levelentry_index != LEVEL_INDEX_CRAD
        && portals != NULL
        && portals->offset_portal != 0);

#ifdef NATIVE_PORT
    {
        static int portal_bfs_enabled = -1;
        static int intro_frustum_all = -1;
        enum CAMERAMODE camera_mode = bondviewGetCameraMode();
        if (portal_bfs_enabled < 0) {
            const char *env = getenv("GE007_PORTAL_BFS");
            /* Default ON on PC.
             *
             * The original room-visibility path is still the correct default,
             * but we keep an escape hatch to force the coarse frustum-all
             * admission path while debugging visibility issues.
             *
             * Set GE007_PORTAL_BFS=0 to disable portal BFS and fall back to
             * the frustum-all visibility path. */
            portal_bfs_enabled = env ? atoi(env) : 1;
        }
        if (intro_frustum_all < 0) {
            const char *env = getenv("GE007_INTRO_FRUSTUM_ALL");
            /* Keep authored-intro cameras on the portal BFS path by default.
             * The frustum-all diagnostic path over-admits unrelated Dam rooms
             * and can draw foreground shards across the intro camera. It stays
             * available as GE007_INTRO_FRUSTUM_ALL=1 for visibility probes. */
            intro_frustum_all = env ? atoi(env) : 0;
        }
        if (intro_frustum_all
            && (camera_mode == CAMERAMODE_INTRO
                || camera_mode == CAMERAMODE_FADESWIRL
                || camera_mode == CAMERAMODE_SWIRL)) {
            use_portal_bfs = 0;
        }
        if (!portal_bfs_enabled) {
            use_portal_bfs = 0;
        }
    }
#endif

    /* Visibility diagnostic (set BG_VIS_DIAG=1 to enable) */
    static int vis_diag_enabled = -1;
    if (vis_diag_enabled < 0) vis_diag_enabled = (getenv("BG_VIS_DIAG") != NULL);
    if (vis_diag_enabled) {
        static int vis_diag_frame = 0;
        if (vis_diag_frame < 5) {
            int portal_count = 0;
            coord3d *diag_position = bondviewGetCurrentPlayersPosition();
            f32 exact_pos[3];
            StandTile *exact_tile;
            f32 nearest_x;
            f32 nearest_y;
            f32 nearest_z;
            StandTile *nearest_tile;
            if (portals) {
                bg_portal_data_entry *pc = portals;
                while (pc->offset_portal != 0) { portal_count++; pc++; }
            }
            fprintf(stderr, "[BG-VIS] frame=%d path=%s g_BgCurrentRoom=%d portals=%d g_MaxNumRooms=%d\n",
                    vis_diag_frame, use_portal_bfs ? "PORTAL_BFS" : "FRUSTUM_ALL",
                    g_BgCurrentRoom, portal_count, g_MaxNumRooms);
            /* Room AABBs for first 3 loaded rooms */
            for (int di = 1; di < g_MaxNumRooms && di < 4; di++) {
                fprintf(stderr, "[BG-VIS]   room[%d] AABB=(%.0f,%.0f,%.0f)-(%.0f,%.0f,%.0f)\n", di,
                        g_BgRoomInfo[di].minbounds.x, g_BgRoomInfo[di].minbounds.y, g_BgRoomInfo[di].minbounds.z,
                        g_BgRoomInfo[di].maxbounds.x, g_BgRoomInfo[di].maxbounds.y, g_BgRoomInfo[di].maxbounds.z);
            }
            /* Camera matrix first row */
            {
                Mtxf *cam = camGetWorldToScreenMtxf();
                if (cam) {
                    fprintf(stderr, "[BG-VIS]   cam_w2s row0=[%.3f %.3f %.3f %.3f] row3=[%.1f %.1f %.1f %.1f]\n",
                            cam->m[0][0], cam->m[0][1], cam->m[0][2], cam->m[0][3],
                            cam->m[3][0], cam->m[3][1], cam->m[3][2], cam->m[3][3]);
                } else {
                    fprintf(stderr, "[BG-VIS]   cam_w2s = NULL!\n");
                }
            }
            /* Screen bbox */
            fprintf(stderr, "[BG-VIS]   screen_bbox=(%.0f,%.0f,%.0f,%.0f)\n",
                    screen_bbox[0], screen_bbox[1], screen_bbox[2], screen_bbox[3]);
            exact_pos[0] = diag_position->f[0];
            exact_pos[1] = diag_position->f[1];
            exact_pos[2] = diag_position->f[2];
            exact_tile = sub_GAME_7F0AF20C(exact_pos, 0, NULL);
            nearest_x = diag_position->f[0];
            nearest_y = diag_position->f[1];
            nearest_z = diag_position->f[2];
            nearest_tile = sub_GAME_7F0AFB78(&nearest_x, &nearest_y, &nearest_z, 0.0f);
            fprintf(stderr,
                    "[BG-VIS]   tiles portal=%d collision=%d exact=%d nearest=%d\n",
                    g_CurrentPlayer->field_488.current_tile_ptr_for_portals
                        ? g_CurrentPlayer->field_488.current_tile_ptr_for_portals->room : -1,
                    g_CurrentPlayer->field_488.current_tile_ptr
                        ? g_CurrentPlayer->field_488.current_tile_ptr->room : -1,
                    exact_tile ? exact_tile->room : -1,
                    nearest_tile ? nearest_tile->room : -1);
            vis_diag_frame++;
        }
    }
    /* Portal-based room visibility BFS */
#ifdef NATIVE_PORT
    {
        static int force_all_rooms = -1;
        if (force_all_rooms < 0) {
            force_all_rooms = (getenv("GE007_FORCE_ALL_ROOMS") != NULL);
        }
        if (force_all_rooms) use_portal_bfs = 0;
    }
#endif
    if (use_portal_bfs) {
        /* Portals exist and not cradle level */
#ifdef NATIVE_PORT
...
#endif
        /* Always add the player's current room */
        bgSetRoomAdmitTraceContext("seed_current_room", -1, -1, 0);
        sub_GAME_7F0B39BC(g_BgCurrentRoom, 0, player_bbox, 1);
        bgClearRoomAdmitTraceContext();

#ifdef NATIVE_PORT
        if (!bgIsDetachedAuthoredCamera(camera_mode)) {
            portal_seed_count = bgCollectPortalSeedRoomsFromPosition(
                g_BgCurrentRoom,
        ...
        if (vis_diag_enabled) {
            static int portal_probe_frame = 0;
            if (portal_probe_frame < 3) {
                bbox2d portal_bbox;
                int probe_portal_index = 0;
                fprintf(stderr, "[BG-VIS]   current_room=%d connected portals:\n", g_BgCurrentRoom);
#ifdef NATIVE_PORT
                if (portal_seed_count > 0) {
                    fprintf(stderr, "[BG-VIS]   supplemental portal seeds:");
                    for (i = 0; i < portal_seed_count; i++) {
                        fprintf(stderr, " %d", portal_seed_rooms[i]);
                    }
                    fprintf(stderr, "\n");
                }
#endif
                for (bg_portal_data_entry *probe = g_BgPortals; probe->offset_portal != 0; probe++, probe_portal_index++) {
                    if (g_BgCurrentRoom == probe->connectedRoom1 || g_BgCurrentRoom == probe->connectedRoom2) {
                        int dest_room = (probe->connectedRoom1 ^ probe->connectedRoom2) ^ g_BgCurrentRoom;
                        memset(&portal_bbox, 0, sizeof(portal_bbox));
                        int projected_ok = sub_GAME_7F0B5864(probe_portal_index, (f32 *)&portal_bbox);
                        int room_ok = projected_ok ? sub_GAME_7F0B5208(dest_room, &portal_bbox) : 0;
                        fprintf(stderr,
                                "[BG-VIS]     portal=%d %d->%d cb=0x%02X proj=%d room=%d bbox=(%.0f,%.0f,%.0f,%.0f)\n",
                                probe_portal_index, g_BgCurrentRoom, dest_room, probe->controlbytes1,
                                projected_ok, room_ok,
                                portal_bbox.min.x, portal_bbox.min.y,
                                portal_bbox.max.x, portal_bbox.max.y);
                    }
                }
                portal_probe_frame++;
            }
        ...
        /* Enqueue the player's room plus any adjacent startup portal seeds. */
        bgQueueConnectedRoomPortals(g_BgCurrentRoom, screen_bbox);
#ifdef NATIVE_PORT
        for (i = 0; i < portal_seed_count; i++) {
        ...
        /* No portals or cradle level — frustum cull all rooms */
#ifdef NATIVE_PORT
        void *player_bbox = (void *)&g_CurrentPlayer->screenxminf;
#else
        void *player_bbox = (void *)((u8 *)g_CurrentPlayer + 0x1118);
        ...
        for (i = 1; i < g_MaxNumRooms; i++) {
            if (sub_GAME_7F0B5208(i, player_bbox)) {
                bgSetRoomAdmitTraceContext("frustum_all", -1, -1, 0);
    ...
    /* Post-visibility diagnostic */
    if (vis_diag_enabled) {
        static int post_vis_frame = 0;
        if (post_vis_frame < 5) {
            ...
            for (i = 0; i < g_BgNumberOfRoomsDrawn && i < 8; i++) {
                fprintf(stderr,
                        ...
                        dword_CODE_bss_8007FFA0[i].bbox.min.x,
     ...
     * trigger a guarded frustum rebuild when that rebuild is a small correction. */
    {
         ...
         * the supplement runs, into a LOCAL snapshot rather than the global
         * room_neighbor_to_rendered flags. bgPromotePortalEdgeRescueCandidates()
         * and bgApplyPortalProjectFrustumFallback() above can flip additional
         * rooms' room_rendered to 1 after the original global mark at the
         * top of this block, leaving the global flags stale for rooms only
         * adjacent to those newly-promoted rooms. bgApplyVisibilitySupplement
         * needs an up-to-date adjacency test, but the global flags are also
         * read by getROOMID_isNeighborToRendered() (a sim-visible consumer:
         * the NATIVE_PORT prop-visibility widener in chrobjhandler.c), so
         * re-marking them here would silently widen prop visibility beyond
         * what the original single per-frame mark established. Computing a
         * throwaway local snapshot instead keeps the global flags on their
         * pre-existing once-per-frame lifecycle. */
        {
            u8 local_neighbor_snapshot[MAXROOMCOUNT] = {0};
            bgComputePortalNeighborSnapshotLocal(g_BgPortals, local_neighbor_snapshot);
...
#ifdef NONMATCHING
// Searches g_BgPortals for all portals connected to a given room ID.
// Stores matching portal indices in outArray, up to maxCount. Returns count found.
// Matches with original IDO. ido_recomp: global address formation differs.
s32 sub_GAME_7F0B8DF4(s32 roomId, s32 *outArray, s32 maxCount) {
    bg_portal_data_entry *portals = g_BgPortals;
    bg_portal_data_entry *cur = portals;
    s32 outCount = 0;
    s32 portalIndex = 0;
    ...
    while (cur->offset_portal != 0) {
        if (roomId == cur->connectedRoom1 || roomId == cur->connectedRoom2) {
            outArray[outCount] = portalIndex;
            outCount++;
...
s32 sub_GAME_7F0B8E98(s32 *arg0, s32 arg1)
{
    ...
    while (count < num_visible_rooms_in_cur_global_vis_packet && count < arg1)
    {
        arg0[count] = (u8)list_visible_rooms_in_cur_global_vis_packet[count];
        count++;
    }
    return count;
}
...
#ifdef NONMATCHING
// Finds all rooms connected to roomId via portals, storing unique room IDs
// in outArray. Returns the number of connected rooms found.
// Similar to sub_GAME_7F0B8DF4 but outputs connected room IDs instead of portal indices,
// and includes duplicate-checking to avoid listing the same room twice.
// ido_recomp: register allocation differences.
s32 sub_GAME_7F0B8EFC(s32 roomId, s32 *outArray, s32 maxCount) {
    bg_portal_data_entry *cur;
    s32 count;
    s32 otherRoom;
    s32 i;
    ...
    do {
        otherRoom = cur->connectedRoom1;

        if (otherRoom == roomId) {
            otherRoom = cur->connectedRoom2;
        } else if (cur->connectedRoom2 != roomId) {
            goto next_portal;
        }

        /* Check for duplicates in output */
        for (i = 0; i < count; i++) {
            if (otherRoom == outArray[i]) {
                goto next_portal;
            }
        }

        outArray[count] = otherRoom;
        count++;
        if (count >= maxCount) {
            return count;
        }
...
#ifdef NONMATCHING
/* Searches portal list for a connection between two rooms in either direction. */
s32 sub_GAME_7F0B8FD0(s32 arg0, u8 arg1)
{
    bg_portal_data_entry *p = g_BgPortals;
    if (!p) return 0;

    while (p->offset_portal != 0)
    {
        s32 room1 = p->connectedRoom1;
        s32 room2 = p->connectedRoom2;

        if (room1 == arg0 && room2 == arg1) return 1;
        if (room2 == arg0 && room1 == arg1) return 1;
        p++;
    }
    return 0;
}
...
#ifdef NONMATCHING
// Computes the center point (midpoint of min and max bounding box) for a room.
// Stores 3 floats (x, y, z) into the output array.
// Matches with original IDO. ido_recomp: loop unrolling/scheduling differs.
void sub_GAME_7F0B92B4(s32 room, f32 *out) {
    f32 *base;
    s32 i;
    f32 val;

    base = (f32 *)&g_BgRoomInfo[room].minbounds;
    i = 0;
    val = (base[3] + base[0]) * 0.5f;
    while (++i != 3) {
        *out++ = val;
        val = (base[i + 3] + base[i]) * 0.5f;
    }
    *out = val;
}
#else
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* bgRoomCalcBB — calculate room bounding box from vertex data or stan tiles.
 * Populates g_BgRoomInfo[room].minbounds/maxbounds. */
extern s32 dword_CODE_bss_8007B9DC;
extern s16 stan_room_bbox[][6];
void sub_GAME_7F0B6368(s32 room);
void delete_room_data(s32 roomID);

void sub_GAME_7F0B9338(s32 room) {
    bg_room_data *rdata = &ptr_bgdata_room_fileposition_list[room];
    s_room_info *ri = &g_BgRoomInfo[room];

    if (rdata->pPointTableBin == 0) {
        /* No geometry file — compute bounds from stan tile data */
        if (room >= dword_CODE_bss_8007B9DC) return;
        s16 *stan_bounds = stan_room_bbox[room];

        for (int axis = 0; axis < 3; axis++) {
            s16 bmin = stan_bounds[axis];       /* min at offset 0,1,2 */
            s16 bmax = stan_bounds[axis + 3];   /* max at offset 3,4,5 */
            ((f32 *)&ri->minbounds)[axis] = (f32)bmin;
            ((f32 *)&ri->maxbounds)[axis] = (f32)bmax;
            /* Store midpoint in room data position */
            s32 mid = (bmin + bmax);
            mid = (mid >= 0) ? mid / 2 : (mid + 1) / 2;
            ((f32 *)&rdata->pos)[axis] = (f32)mid;
        }
    } else {
        /* Has geometry file — scan vertex data for min/max */
        u8 was_loaded = ri->model_bin_loaded;
        if (!was_loaded) {
            /* Temporarily load room data to scan vertices.
             * Use direct decompression to avoid side effects. */
            sub_GAME_7F0B6368(room);
        }

        if (!ri->ptr_point_index || ri->usize_point_index_binary <= 0) {
            /* No vertex data available, set generous bounds */
            ri->minbounds.x = -32767.0f;
            ri->minbounds.y = -32767.0f;
            ri->minbounds.z = -32767.0f;
            ri->maxbounds.x = 32767.0f;
            ri->maxbounds.y = 32767.0f;
            ri->maxbounds.z = 32767.0f;
            if (!was_loaded) delete_room_data(room);
            return;
        }

        u8 *vtx_data = (u8 *)ri->ptr_point_index;
        s32 vtx_size = ri->usize_point_index_binary;
        u8 *vtx_end = vtx_data + vtx_size;

        s16 mins[3] = {32767, 32767, 32767};
        s16 maxs[3] = {-32767, -32767, -32767};

        while (vtx_data < vtx_end) {
            /* Each vertex is 16 bytes: s16 x, s16 y, s16 z, ... */
            for (int axis = 0; axis < 3; axis++) {
                s16 val = (s16)((vtx_data[axis * 2] << 8) | vtx_data[axis * 2 + 1]);
                if (val < mins[axis]) mins[axis] = val;
                if (val > maxs[axis]) maxs[axis] = val;
            }
            vtx_data += 16; /* sizeof(Vtx_t) */
        }

        /* Store bounds: vertex coords + room position offset */
        for (int axis = 0; axis < 3; axis++) {
            f32 room_off = ((f32 *)&rdata->pos)[axis];
            ((f32 *)&ri->minbounds)[axis] = room_off + (f32)mins[axis];
            ((f32 *)&ri->maxbounds)[axis] = room_off + (f32)maxs[axis];
        }

        bgTraceRoomDlIfRequested(room,
            (const u8 *)ri->ptr_expanded_mapping_info,
            (const u8 *)ri->ptr_point_index);
    ...
    bgTraceRoomSummaryIfRequested(room);
}
...
void sub_GAME_7F0B95D8(s32 arg0) {
    ...
    // Node 0
    portalPtr = g_BgPortals;
    if (*g_BgPortals != 0)
    {
loop_1:
        // Node 1
        if ((arg0 == portalPtr->unk4) || (arg0 == portalPtr->unk5))
        {
            // Node 3
            temp_a2 = *portalPtr;
            if (*temp_a2 > 0)
...
loop_5:
                // Node 5
                axisOffset = 0;
                roomInfoPtr = ((arg0 * 0x50) + &g_BgRoomInfo);
loop_6:
                // Node 6
                temp_t8 = ((portalData + (((vertIdx * 4) - vertIdx) * 4)) + axisOffset);
                temp_t0 = (axisOffset + 4);
                if (temp_t8->unk4 < roomInfoPtr->unk38)
                {
                    // Node 7
                    roomInfoPtr->unk38 = (f32) temp_t8->unk4;
                }
                // Node 8
                if (roomInfoPtr->unk44 < temp_t8->unk4)
                {
                    // Node 9
                    roomInfoPtr->unk44 = (f32) temp_t8->unk4;
                }
                // Node 10
                temp_a2_2 = *portalPtr;
                portalData = temp_a2_2;
                axisOffset = temp_t0;
                roomInfoPtr = (roomInfoPtr + 4);
                if (temp_t0 != 0xc)
                {
                    goto loop_6;
                }
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PC: Compute portal plane normal and min/max dot products.
 * Decompiled from GLOBAL_ASM — the N64 decompiled C was just debug printf stubs. */
void sub_GAME_7F0B96CC(s32 arg0, struct PortalMetric *arg1) {
    bg_portal_entry *portal;
    f32 *points;
    s32 numPoints;
    f32 edge1[3], edge2[3];
    f32 nx, ny, nz, mag, invmag;
    f32 dot, minDot, maxDot;
    s32 i;

    portal = bgGetPortalEntryPtr(arg0);
    if (!portal) return;

    /* Portal vertices start at offset 4 (after numPoints + padding).
     * Each vertex is 3 floats (12 bytes). */
    points = (f32 *)((u8 *)portal + 4);

    /* edge1 = point[0] - point[1] */
    edge1[0] = points[0] - points[3];
    edge1[1] = points[1] - points[4];
    edge1[2] = points[2] - points[5];

    /* edge2 = point[2] - point[1] */
    edge2[0] = points[6] - points[3];
    edge2[1] = points[7] - points[4];
    edge2[2] = points[8] - points[5];

    /* normal = edge1 × edge2 */
    nx = edge1[1] * edge2[2] - edge2[1] * edge1[2];
    ny = edge1[2] * edge2[0] - edge2[2] * edge1[0];
    nz = edge1[0] * edge2[1] - edge2[0] * edge1[1];
    ...
    if (mag != 0.0f) {
        ...
        invmag = mag; /* 0 — degenerate portal */
    }
    arg1->normal.x = nx * invmag;
    arg1->normal.y = ny * invmag;
    arg1->normal.z = nz * invmag;

    /* Compute min/max dot product of normal with each portal vertex */
    numPoints = portal->numPoints;
    minDot = 3.4028235e38f;   /* FLT_MAX */
    maxDot = -3.4028235e38f;  /* -FLT_MAX */

    for (i = 0; i < numPoints; i++) {
        dot = arg1->normal.x * points[i * 3 + 0]
            + arg1->normal.y * points[i * 3 + 1]
            + arg1->normal.z * points[i * 3 + 2];
...
#ifdef NONMATCHING
// Extracts a packed scale value from a portal's controlbytes2 byte.
// Low nibble = mantissa, high nibble = exponent. Computes mantissa * 0.25 * 2^exponent.
// The exponent loop uses Duff's device: handles remainder first, then groups of 4 doublings.
// Matches with original IDO. ido_recomp: global access pattern differs.
f32 sub_GAME_7F0B9990(s32 arg0) {
    u8 byte;
    s32 mantissa;
    s32 exponent;
    ...
    byte = g_BgPortals[arg0].controlbytes2;
    mantissa = byte & 0xF;
    exponent = (byte >> 4) & 0xF;
    remainder = -(exponent & 3);
    result = (f32)mantissa * 0.25f;
    ...
    if (remainder != 0) {
        ...
        } while ((remainder + exponent) != count);
        if (count == 0) {
            return result;
        }
    }
...
#ifdef NONMATCHING
// NONMATCHING: ido_recomp allocates byte2 to $a1 instead of $v0.
// lbu a1 vs lbu v0, slti at,a1 vs slti at,v0, addiu v0,a1,1 vs addiu v0,v0,1.
// 3 of 20 instructions differ. All other instructions match byte-identically.
void sub_GAME_7F0B9A2C(s32 p)
{
    u8 byte2;

    byte2 = g_BgPortals[p].controlbytes2;
    if (byte2 >= 0xFF)
    {
        byte2 = 0xFF;
    }
...
f32 sub_GAME_7F0B9B94(s32 arg0) {
    ...
    {
        debugPrintResult  = osSyncPrintf("bg: bgOrderPortal: Portal \'%s\' not planar by %5.2f\n", bgDebPrintPORTALID(arg0), local_50 - local_54);
    }
    #endif
    ...
    sp3C = (f32) normalX;
    sp34 = (f32) normalY;
    temp_f0 = ((sp50 * normalX) + ((normalY * sp48) + (sp38 * sp4C)));
    result = temp_f0;
    if (temp_f0 <= planeDistNeg)
    {
        // Node 3
        result = temp_f0;
        if (swapped != 0)
        {
            // Node 4
            sp24 = (s32) swapped;
            result = bgSwapConnectedRooms(planeDist, planeDistNeg, arg0);
        }
    }
    // Node 5
    return result;
...
#ifdef NONMATCHING
// Searches all portals for one connecting room arg0 and arg1 (in either direction),
// verifying the match via sub_GAME_7F0B9F14. Returns portal index or -1 if not found.
// Debug name: bgGetPortalBetweenRooms.
// ido_recomp: register allocation differences.
s32 sub_GAME_7F0B9CC8(s32 arg0, s32 arg1, struct coord3d *arg2, struct coord3d *arg3) {
    ...
    cur = portals;
    do {
        room1 = cur->connectedRoom1;

        if (arg0 == room1 && arg1 == cur->connectedRoom2) {
            if (sub_GAME_7F0B9F14(portalIndex, arg2, arg3) != 0) {
                result = portalIndex;
            }
        } else if (arg1 == room1 && arg0 == cur->connectedRoom2) {
            if (sub_GAME_7F0B9F14(portalIndex, arg2, arg3) != 0) {
                result = portalIndex;
            }
        }
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PC: Portal-crossing test. Returns 0 if line arg1→arg2 doesn't cross the portal,
 * 1 if crossing from negative→positive side, 2 if positive→negative. */
s32 sub_GAME_7F0B9F14(s32 portalnum, coord3d *arg1, coord3d *arg2) {
    struct PortalMetric metric;
    ...
    s32 numPoints;
    s32 j, next;
    f32 edge_x, edge_y, edge_z;
    f32 cross_x, cross_y, cross_z;
    f32 mag_sq;
    f32 vert_dot, arg1_dot;
    u8 flag_pos = 0, flag_neg = 0;
    ...
    dot1 = (arg1->x * metric.normal.x + arg1->y * metric.normal.y + arg1->z * metric.normal.z) * room_data_float1;
    dot2 = (arg2->x * metric.normal.x + arg2->y * metric.normal.y + arg2->z * metric.normal.z) * room_data_float1;

    /* Both positions on same side of portal plane → no crossing */
    if (dot1 < metric.min) {
        if (dot2 < metric.min) return 0;
    } else if (dot1 > metric.min) {
        if (dot2 > metric.min) return 0;
    }

    D_80044900 = (dot1 + dot2) * 0.5f - metric.min;

    portal = bgGetPortalEntryPtr(portalnum);
    if (!portal) return 0;
    numPoints = portal->numPoints;
    if (numPoints <= 0) return 0;

    /* Test each edge of the portal polygon:
     * compute cross product of (edge × movement_dir), then test
     * whether all edges produce consistent side results. */
    for (j = 0; j < numPoints; j++) {
        f32 *cur_vert, *next_vert;
        next = (j + 1) % numPoints;

        cur_vert = (f32 *)((u8 *)portal + 4 + j * 12);
        next_vert = (f32 *)((u8 *)portal + 4 + next * 12);

        edge_x = next_vert[0] - cur_vert[0];
        edge_y = next_vert[1] - cur_vert[1];
        edge_z = next_vert[2] - cur_vert[2];

        cross_x = edge_y * dir_z - dir_y * edge_z;
        cross_y = edge_z * dir_x - dir_z * edge_x;
        cross_z = edge_x * dir_y - dir_x * edge_y;

        mag_sq = cross_x * cross_x + cross_y * cross_y + cross_z * cross_z;
        if (mag_sq == 0.0f) return 0;

        vert_dot = cross_x * cur_vert[0] + cross_y * cur_vert[1] + cross_z * cur_vert[2];
        arg1_dot = (cross_x * arg1->x + cross_y * arg1->y + arg1->z * cross_z) * room_data_float1;

        if (arg1_dot < vert_dot) {
            if (flag_neg) return 0;
            flag_pos = 1;
        } else {
            flag_neg = 1;
            if (flag_pos) return 0;
        }
    }

    return (dot1 < metric.min) ? 1 : 2;
}
#elif defined(PORT_FIXME_STUBS)
s32 sub_GAME_7F0B9F14(s32 portalnum, coord3d *arg1, coord3d *arg2) {
    return 0;
...
s32 sub_GAME_7F0B9F14(s32 portalnum, coord3d *arg1, coord3d *arg2) {
    ...
    midpoint = (dot1 + dot2) * 0.5f;
    D_80044900 = midpoint - metric.min;

    portal_entry = &g_BgPortals[portalnum];
    portal = portal_entry->offset_portal;
    numPoints = portal->numPoints;
    ...
    if (numPoints > 0) {
        ...
        do {
            f32 *cur_vert;
            f32 *next_vert;
            s32 next_byte_offset;

            next_byte_offset = (next % numPoints) * 12;
            cur_vert = (f32 *)((u8 *)portal + byte_offset + 4);
            next_vert = (f32 *)((u8 *)portal + next_byte_offset + 4);

            edge_x = next_vert[0] - cur_vert[0];
            edge_y = *(f32 *)((u8 *)portal_entry->offset_portal + next_byte_offset + 8) - *(f32 *)((u8 *)portal_entry->offset_portal + byte_offset + 8);
            edge_z = *(f32 *)((u8 *)portal_entry->offset_portal + next_byte_offset + 12) - *(f32 *)((u8 *)portal_entry->offset_portal + byte_offset + 12);
            ...
            cur_vert = (f32 *)((u8 *)portal_entry->offset_portal + byte_offset + 4);
            vert_dot = cross_x * cur_vert[0] + cross_y * cur_vert[1] + cross_z * cur_vert[2];
            arg1_dot = (cross_x * arg1->x + cross_y * arg1->y + arg1->z * cross_z) * room_data_float1;
...
void sub_GAME_7F0BA2D4(void *arg0, void *arg1, s32 *arg2, s32 *arg3, s32 arg4) {
    ...
    propbbmin.x = ((coord3d *)arg0)->x * room_data_float1;
    propbbmin.y = ((coord3d *)arg0)->y * room_data_float1;
    propbbmin.z = ((coord3d *)arg0)->z * room_data_float1;
    propbbmax.x = ((coord3d *)arg1)->x * room_data_float1;
    propbbmax.y = ((coord3d *)arg1)->y * room_data_float1;
    propbbmax.z = ((coord3d *)arg1)->z * room_data_float1;

    do {
        previous_count = count;

        for (s32 room_index = 0; room_index < previous_count && count < arg4; room_index++) {
            s32 source_room = arg2[room_index];

            for (s32 portal_index = 0; portal_index < g_NumBgPortals && count < arg4; portal_index++) {
                bg_portal_data_entry *portal_entry = &g_BgPortals[portal_index];
                bg_portal_entry *portal = bgGetPortalEntryPtr(portal_index);
                coord3d portalbbmin;
                coord3d portalbbmax;
                s32 dest_room;
                s32 duplicate;

                if (portal == NULL || portal->numPoints <= 0) {
                    continue;
                }

                if (portal_entry->controlbytes1 & 1) {
                    continue;
                }

                if (source_room == portal_entry->connectedRoom1) {
                    dest_room = portal_entry->connectedRoom2;
                } else if (source_room == portal_entry->connectedRoom2) {
                    dest_room = portal_entry->connectedRoom1;
                } else {
                    continue;
                }

                if (dest_room < 0 || dest_room >= g_MaxNumRooms) {
                    continue;
                }

                {
                    f32 *points = (f32 *)((u8 *)portal + 4);

                    portalbbmin.x = portalbbmax.x = points[0];
                    portalbbmin.y = portalbbmax.y = points[1];
                    portalbbmin.z = portalbbmax.z = points[2];

                    for (s32 point_index = 1; point_index < portal->numPoints; point_index++) {
                        f32 x = points[point_index * 3 + 0];
                        f32 y = points[point_index * 3 + 1];
                        f32 z = points[point_index * 3 + 2];

                        if (x < portalbbmin.x) portalbbmin.x = x;
                        if (y < portalbbmin.y) portalbbmin.y = y;
                        if (z < portalbbmin.z) portalbbmin.z = z;
                        if (x > portalbbmax.x) portalbbmax.x = x;
                        if (y > portalbbmax.y) portalbbmax.y = y;
                        if (z > portalbbmax.z) portalbbmax.z = z;
                    }
                }
                ...
                duplicate = 0;
                for (s32 existing_index = 0; existing_index < count; existing_index++) {
                    if (arg2[existing_index] == dest_room) {
                        duplicate = 1;
                        break;
                    }
                }
                ...
                if (!duplicate) {
                    arg2[count++] = dest_room;
                }
            }
        }
    } while (count != previous_count && count < arg4);
