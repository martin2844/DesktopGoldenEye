 ...
 * Address 8008D370.
 */
u8 g_gfxDramStack[SP_DRAM_STACK_SIZE8] = {0};

/**
 * Address 8008D770.
 */
u8 g_gfxYieldBuf[OS_YIELD_GFX_DATA_SIZE] = {0};
...
/**
 * Submit one graphics task. Native builds translate the display list directly.
 */
void rspGfxTaskStart(Gfx *firstGdl, Gfx *gdl, s32 arg2, OSMesg rspReplyMsg)
{
#ifdef NATIVE_PORT
    /* PC path: run display list through GBI translator directly,
     * then send completion message. No scheduler/RSP involved. */
...

    {
        ...
        if (dl_size > 512 * 1024) {
            fprintf(stderr, "[GFX] WARNING: DL overflow! size=%zu bytes (max 524288)\n", dl_size);
        }
    }
    gfx_run_dl(firstGdl);
    gfx_end_frame();

    /* Send completion message so bossMainloop can continue */
    osSendMesg(&gfxFrameMsgQ, rspReplyMsg, OS_MESG_NOBLOCK);

    /* Toggle task settings (same as N64 path) */
    g_gfxTaskSettingsList = (struct GfxInfo_s *)((uintptr_t)g_gfxTaskSettingsList ^ (uintptr_t)&g_gfxTaskSettings[0] ^ (uintptr_t)&g_gfxTaskSettings[1]);
#else
    OSScTask *sctask;
    OSTask *task;
