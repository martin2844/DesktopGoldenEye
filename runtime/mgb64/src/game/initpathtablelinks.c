...
{
#ifdef NATIVE_PORT
    /* Skip waypoint/waygroup validation on PC. This function is a debug
     * consistency checker that intentionally hangs (while(1)) on error.
     * The decompiled C version also has incorrect loop termination
     * (while(var_t3 != NULL) should check var_t3->neighbours != NULL per asm).
     * Its side effects (setting groupNum, dist) are non-critical for gameplay. */
    return; /* skip decompilation-buggy validation function */
#else
    waypoint *spA0;
    waypoint *sp9C;
