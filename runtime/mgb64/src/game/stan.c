...
#ifdef NATIVE_PORT
    /* On 64-bit: stanfile stores a pointer (native width), ptr_firstroom is native pointer.
     * The N64 binary layout has these as 32-bit values at offsets 0 and 4, but we only
     * use this struct for runtime state — binary data is parsed separately. */
    void *stanfile;
...
/* Keep the old names as unused placeholders so non-port code still compiles */
...
#ifdef NATIVE_PORT
s32 sub_GAME_7F0B21B0(StandTile **tileStack, f32 target_x, f32 target_z, f32 unknown, intptr_t unk00, s32 *count_rtn, s32 bufMax);
#else
...
s32 sub_GAME_7F0AEF3C(void *arg0) {
    /* Tile debug string: format tile ID into rotating 9-byte buffer pool.
     * Reads raw big-endian tile header bytes and formats as "<flag><room><letter><suffix>". */
    if (!arg0) return 0;
    char *buf = (char *)D_80040F64 + D_80040FAC * 9;
    D_80040FAC = (D_80040FAC + 1) & 7;
    u8 *p = (u8 *)arg0;
    u16 word0 = (u16)((p[0] << 8) | p[1]);
    u8 byte2 = p[2];
    int flag_char = ((word0 >> 15) & 1) + 0x70;
    int room_num = word0 & 0x7FFF;
    int room_letter = ((byte2 >> 3) & 0x1F) + 0x61;
    int suffix = (byte2 & 7);
    suffix = suffix ? suffix + 0x30 : 0;
    ge007_sprintf(buf, "%c%d%c%c", flag_char, room_num, room_letter, suffix);
    return (s32)(intptr_t)buf;
}
#else
s32 sub_GAME_7F0AEF3C(void *arg0) { return 0; }
#endif
#else
...
s32 sub_GAME_7F0AEF3C(void *arg0) {
    ...
    {
        // Node 2
        room_suffix = (temp_a1 + 0x30);
    }
    // Node 3
    ge007_sprintf(sp24, &aCDCC, ((((s32) *arg0 >> 0xf) & 1) + 0x70), (*arg0 & 0x7fff), (s32) ((((s32) arg0->unk2 >> 3) & 0x1f) + 0x61), (s32) room_suffix);
    return sp24;
}
...
void sub_GAME_7F0AEFE0(void) {
    sub_GAME_7F0AEF3C(NULL); //maybe getstanroomID
}
...
void stanInit(void) {
    debTryAdd(&stan_c_debug_notice_list_entry, aStan_c_debug);
}
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PORT: Clean reimplementation of sub_GAME_7F0AF038 (room bounding box init).
 * Walks all tiles from stan_prefix.ptr_firstroom, tracking first-tile-per-room
 * and computing per-room axis-aligned bounding boxes from tile vertex coordinates. */
void sub_GAME_7F0AF038(void) {
    dword_CODE_bss_8007B9DC = 0;
    memset(firststaninroom, 0, sizeof(firststaninroom));
    memset(stan_room_bbox, 0, sizeof(stan_room_bbox));

    StandTile *tile = stan_prefix.ptr_firstroom;
    if (tile == NULL || *(s32 *)tile == 0) {
        return;
    }
    ...
    s32 tile_count = 0;
    while (*(s32 *)tile != 0) {
        s32 room = tile->room & 0xFF;
        if (curroom != room) {
            curroom = room;
            if (curroom >= dword_CODE_bss_8007B9DC) {
                dword_CODE_bss_8007B9DC = curroom + 1;
            }
            if (curroom < STAN_MAX_ROOMS) {
                firststaninroom[curroom] = tile;
                /* Init bounding box to extremes */
                stan_room_bbox[curroom][0] = 0x7FFF;  /* min_x */
                stan_room_bbox[curroom][1] = 0x7FFF;  /* min_y */
                stan_room_bbox[curroom][2] = 0x7FFF;  /* min_z */
                stan_room_bbox[curroom][3] = -0x8000;  /* max_x */
                stan_room_bbox[curroom][4] = -0x8000;  /* max_y */
                stan_room_bbox[curroom][5] = -0x8000;  /* max_z */
            }
        }

        /* Update bounding box with this tile's vertices */
        s32 pointCount = (tile->tail.half >> 12) & 0xF;
        if (curroom < STAN_MAX_ROOMS) {
            for (s32 i = 0; i < pointCount; i++) {
                s16 coords[3];
                coords[0] = tile->points[i].x;
                coords[1] = tile->points[i].y;
                coords[2] = tile->points[i].z;
                for (s32 a = 0; a < 3; a++) {
                    if (coords[a] < stan_room_bbox[curroom][a])
                        stan_room_bbox[curroom][a] = coords[a];
                    if (coords[a] > stan_room_bbox[curroom][a + 3])
                        stan_room_bbox[curroom][a + 3] = coords[a];
                }
            }
        }
...
void sub_GAME_7F0AF038(void) {
    ...
    clear_ptr = &dword_CODE_bss_8007B134;
loop_1:
    // Node 1
    temp_v1 = (clear_ptr + 0x10);
    temp_v1->unk-10 = 0;
    temp_v1->unk-C = 0;
    temp_v1->unk-8 = 0;
    temp_v1->unk-4 = 0;
...
loop_4:
        ...
        if (temp_s0_4 > 0)
        {
            // Node 9
            vertex_index = 0;
            stan_vertex_ptr = firststan;
loop_10:
            // Node 10
            vertex_ptr = stan_vertex_ptr;
            bounds_ptr = (&dword_CODE_bss_8007B358 + (((curroom * 4) - curroom) * 4));
            axis_index = 0;
loop_11:
            // Node 11
            temp_s0_3 = vertex_ptr->unk8;
            temp_s2 = (axis_index + 2);
            coord_value = temp_s0_3;
            if (temp_s0_3 < *bounds_ptr)
            {
                // Node 12
                *bounds_ptr = temp_s0_3;
                coord_value = vertex_ptr->unk8;
            }
            // Node 13
            if (bounds_ptr->unk6 < coord_value)
            {
                // Node 14
                bounds_ptr->unk6 = (s16) coord_value;
            }
            // Node 15
            vertex_ptr = (vertex_ptr + 2);
            bounds_ptr = (bounds_ptr + 2);
            axis_index = temp_s2;
            if (temp_s2 != 6)
            {
                goto loop_11;
            ...
            // Node 16
            temp_v1_2 = (vertex_count + 1);
            temp_s0_5 = (((s32) firststan->unk6 >> 0xc) & 0xf);
            vertex_count = temp_v1_2;
            side_count = temp_s0_5;
        ...
        // Node 17
        temp_a0 = (*(&list_of_tilesizes + side_count) + firststan);
        firststan = temp_a0;
        curroom = curroom;
        if (*temp_a0 != 0)
        {
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PORT: Clean reimplementation of sub_GAME_7F0AF20C (find tile at position).
 * Given a world-space position, finds the highest floor tile that the position
 * is above. Used for floor detection and collision.
 *
 * pos: float[3] = {x, y, z} world position
 * roomset: if non-zero, pointer to s32 array of allowed room indices (-1 terminated)
 * out_dist: if non-null, receives the Y distance from pos to the tile
 *
...
StandTile *sub_GAME_7F0AF20C(f32 *pos, intptr_t roomset, f32 *out_dist) {
    ...
    f32 sx = pos[0] * level_scale;
    f32 sy = pos[1] * level_scale;
    f32 sz = pos[2] * level_scale;

    /* Clamp Y to s16 range */
    ...
    for (s32 room = 0; room < dword_CODE_bss_8007B9DC; room++) {
        ...
        /* Check room bounding box */
        if (sx16 < stan_room_bbox[room][0] || sx16 > stan_room_bbox[room][3]) continue;
        ...
        while (*(s32 *)tile != 0 && (tile->room & 0xFF) == room) {
            ...
            /* Advance to next tile */
            s32 pc = (tile->tail.half >> 12) & 0xF;
            tile = (StandTile *)((u8 *)tile + list_of_tilesizes[pc]);
        }
    }

    if (best_tile != NULL && out_dist != NULL) {
        *out_dist = best_y;
    }
    return best_tile;
}
#elif defined(PORT_FIXME_STUBS)
StandTile *sub_GAME_7F0AF20C(f32 *pos, intptr_t roomset, f32 *out_dist) { return NULL; }
#else
StandTile *sub_GAME_7F0AF20C(f32 *pos, intptr_t roomset, f32 *out_dist)
{
  float tile_y;
  ...
  bVar2 = false;
  local_58 = *pos * level_scale;
  local_54 = pos[1] * level_scale;
  local_50 = pos[2] * level_scale;
  if (32767.0 < local_54) {
    local_54 = 32767.0;
  }
  if (local_54 < -32767.0) {
  ...
  local_38 = 0;
  pfStack00000014 = pos;
  iStack0000001c = roomset;
  pfStack00000024 = out_dist;
  do {
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* PORT: stanLoadFile uses values already computed by stanDetermineEOF.
 * The N64 version reads ptr_firstroom from the binary data at offset 4,
 * but we've already parsed and byte-swapped that in stanDetermineEOF. */
void stanLoadFile(struct StanPrefixRecord *arg0) {
    m_stanRegion = 1;
    stan_prefix.stanfile = (void *)arg0;
    /* standTileStart already set by stanDetermineEOF */
...
#elif defined(PORT_FIXME_STUBS)
/* PORT NOTE: stanLoadFile needs stanDetermineEOF to run first (relocates
 * N64 segment pointers to host addresses) and the StanPrefixRecord struct
 * has 64-bit alignment padding that mismatches the raw binary layout
 * (ptr_firstroom at offset 8 vs binary offset 4). Both issues must be
 * resolved together to unstub this properly. */
void stanLoadFile(struct StanPrefixRecord *arg0) { (void)arg0; }
#else
...
s32 sub_GAME_7F0AF638(s32 arg0, s32 arg1, void *arg2) { return 0; }
#elif defined(NATIVE_PORT)
/**
 * stanFillin — BFS traversal that flips tile polarity bits to match a desired state.
 * Walks neighbors via point link fields, flipping mismatched tiles and counting
 * successful (non-degenerate) flips.
 */
s32 sub_GAME_7F0AF638(StandTile *startTile, s32 arg1, StandTile **stack) {
    s32 desiredPolarity = arg1 & 0xFF;
    s32 flipCount = 0;
    s32 depth;
    s32 i;
    ...
    while (depth > 0) {
        /* Pop tile from stack */
        StandTile *curTile = stack[--depth];

        /* Check polarity (bit 15 of id) */
        s32 curPolarity = (curTile->id >> 15) & 1;
        if (desiredPolarity != curPolarity) {
            s32 numSides;

            /* Flip polarity bit */
            curTile->id ^= (1u << 15);

            /* If cross product is non-degenerate, count as successful flip */
            if (sub_GAME_7F0AF760(curTile) == 0) {
                flipCount++;
            }

            /* Walk edges: push neighbors with mismatched polarity */
            numSides = (curTile->tail.half >> 12) & 0xF;
            for (i = 0; i < numSides; i++) {
                u16 link = curTile->points[i].link;
                if ((link >> 4) != 0) {
                    StandTile *neighbor = &standTileStart[link];
                    s32 neighborPolarity = (neighbor->id >> 15) & 1;
                    if (desiredPolarity != neighborPolarity) {
...
s32 sub_GAME_7F0AF638(s32 arg0, s32 arg1, void *arg2) {
    ...
    s32 inner_depth;
    s32 outer_depth;
    s32 inner_flip_count;
    s32 total_flip_count;
    s32 cur_depth;
...
loop_1:
    // Node 1
    temp_s3 = stack_ptr->unk-4;
    temp_s1 = (outer_depth + -1);
    temp_s2 = (stack_ptr + -4);
    stack_depth = temp_s1;
    flip_count = total_flip_count;
    stack_ptr = temp_s2;
    num_sides = num_sides;
    if (temp_s4 != (((s32) *temp_s3 >> 0xf) & 1))
    {
        ...
        *temp_s3 = (s16) (*temp_s3 ^ 0x8000);
        inner_flip_count = total_flip_count;
        if (sub_GAME_7F0AF760(temp_s3, num_sides) == 0)
        {
            // Node 3
            inner_flip_count = (total_flip_count + 1);
        }
        // Node 4
        temp_a1 = (((s32) temp_s3->unk6 >> 0xc) & 0xf);
        edge_ptr = temp_s3;
        inner_stack_ptr = temp_s2;
        edge_index = 0;
        stack_depth = temp_s1;
        flip_count = inner_flip_count;
        stack_ptr = temp_s2;
        num_sides = temp_a1;
        cur_sides = temp_a1;
        cur_depth = temp_s1;
...
loop_5:
            // Node 5
            temp_v0 = edge_ptr->unkE;
            temp_s0 = (edge_index + 1);
            inner_sides = cur_sides;
            next_stack_ptr = inner_stack_ptr;
            inner_depth = cur_depth;
            if ((temp_v0 >> 4) != 0)
            {
                // Node 6
                ...
                {
                    // Node 7
                    *inner_stack_ptr = temp_v1;
                    inner_sides = (((s32) temp_s3->unk6 >> 0xc) & 0xf);
                    next_stack_ptr = (inner_stack_ptr + 4);
                    inner_depth = (cur_depth + 1);
                }
            }
            // Node 8
            edge_ptr = (edge_ptr + 8);
            inner_stack_ptr = next_stack_ptr;
            edge_index = temp_s0;
            stack_depth = inner_depth;
            flip_count = inner_flip_count;
            stack_ptr = next_stack_ptr;
            num_sides = inner_sides;
            cur_sides = inner_sides;
            cur_depth = inner_depth;
            if (temp_s0 < inner_sides)
            {
                goto loop_5;
            }
        }
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0AF760(StandTile *tile) { return 0; }
#elif defined(NATIVE_PORT)
s32 sub_GAME_7F0AF760(StandTile *tile) {
    s32 iA = (tile->tail.half >> 8) & 0xF;
    s32 iB = (tile->tail.half >> 4) & 0xF;
    s32 iC = tile->tail.half & 0xF;

    s32 AB_x = tile->points[iB].x - tile->points[iA].x;
    s32 AB_z = tile->points[iB].z - tile->points[iA].z;

    s32 AC_x = tile->points[iC].x - tile->points[iA].x;
    s32 AC_z = tile->points[iC].z - tile->points[iA].z;

    s32 crossProduct = (AB_z * AC_x) - (AB_x * AC_z);
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
StandTile *sub_GAME_7F0AF808(f32 f1, f32 f2, f32 f3, f32 f4) { return NULL; }
#elif defined(NATIVE_PORT)
// 'findSomeTile' — iterates all tiles from first room, returns tile containing (f1,f3) below height f2
StandTile *sub_GAME_7F0AF808(f32 f1, f32 f2, f32 f3, f32 f4)
{
    ...
    while (*(u32 *)tile != 0)
    {
        ...
        if (
            ...
            (sub_GAME_7F0B20D0(tileStack, f1, f3, f4) != 0) &&
            (tile == tileStack[0]) &&
            (stanGetPositionYValue(tile, f1, f3) < f2)
        )
        {
            return tile;
        }
...
#ifdef NONMATCHING
#if 0 /* PORT_FIXME_STUBS — unstubbed for PC port */
struct StandTile *sub_GAME_7F0AFB78(f32 *arg_x, f32 *arg_y, f32 *arg_z, f32 arg3) { return NULL; }
#else
StandTile *sub_GAME_7F0AFB78(f32 *arg_x, f32 *arg_y, f32 *arg_z, f32 arg3) {
    StandTile *tile;
    StandTile *result;
    StandTile *tileStack;
    coord3d point;
    coord3d local_coords;
    f32 best_dist;
    f32 dist;
    s32 i;
...
#ifdef NATIVE_PORT
    if (stan_prefix.ptr_firstroom == NULL) return NULL;
#endif

    local_coords.x = *arg_x;
    local_coords.y = *arg_y;
    local_coords.z = *arg_z;
    ...
    while (*(u32 *)tile != 0) {
        if (!((*(u16 *)tile >> 15) & 1)) {
            if (sub_GAME_7F0AF760(tile) == 0) {
                for (i = 0; i < 4; i++) {
                    if (i == 3) {
                        getTileMidPoint(tile, &point);
                    } else {
                        getPointJustInsideOfTileTriple(tile, i, &point);
                    }

                    tileStack = tile;
                    if (sub_GAME_7F0B20D0(&tileStack, point.x, point.z, arg3) < 0) {
                        dist = sub_GAME_7F0AFB1C(&point, &local_coords);
                        if (dist < best_dist) {
                            result = tile;
                            best_dist = dist;
                            *arg_x = point.x;
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
f32 getShortest2dDispToInfTripleEdge(StandTile *tile, s32 start3index, f32 p_x, f32 p_z) { return 0.0f; }
#else
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
f32 sub_GAME_7F0B00C4(StandTile *tile, s32 index, f32 p_x, f32 p_z) { return 0.0f; }
#else
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
f32 sub_GAME_7F0B0140(StandTile *tile, s32 index, f32 p_x, f32 p_z) { return 0.0f; }
#else
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
f32 sub_GAME_7F0B0198(void *arg0, s32 arg1, f32 arg2, f32 arg3) { return 0.0f; }
#elif defined(NATIVE_PORT)
/**
 * Edge projection test: returns 1 if the perpendicular projection of point (px,pz)
 * onto the edge from points[index] to points[nextIndex] falls within the segment.
 * Assembly returns s32 in $v0 (0 or 1), not f32. Caller uses beql $v0,$zero.
 */
s32 sub_GAME_7F0B0198(StandTile *tile, s32 index, f32 px, f32 pz) {
    s32 pointCount = (tile->tail.half >> 12) & 0xF;
    s32 nextIndex = (index + 1) % pointCount;
    f32 curr_x = (f32)tile->points[index].x;
    f32 curr_z = (f32)tile->points[index].z;
    f32 next_x = (f32)tile->points[nextIndex].x;
    f32 next_z = (f32)tile->points[nextIndex].z;
    f32 edge_x = next_x - curr_x;
    f32 edge_z = next_z - curr_z;
    f32 dx = px - curr_x;
    f32 dz = pz - curr_z;
    f32 edge_len_sq = edge_x * edge_x + edge_z * edge_z;
    f32 dot = dx * edge_x + dz * edge_z;

    /* Point projects onto segment if 0 < dot < edge_len_sq */
    if (dot > 0.0f && dot < edge_len_sq) {
        return 1;
    }
    return 0;
}
...
f32 sub_GAME_7F0B0198(void *arg0, s32 arg1, f32 arg2, f32 arg3) {
    ...
    // Node 2
    if ((beyond_edge != 0) && (temp_f0_2 < 0.0f))
    {
        // Node 4
        is_negative = 1;
        // Node 5
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B07BC(f32 arg0, f32 arg1, f32 arg2, f32 arg3, f32 arg4, f32 arg5, f32 arg6, f32 arg7, s32 arg8) { return 0; }
#else
...
#ifdef NATIVE_PORT
/**
 * PORT: Clean 64-bit version of sub_GAME_7F0B0914 (stanLineDo / walkTilesBetweenPoints).
 *
 * Walks collision tiles along a line from (start_x, start_z) to (dest_x, dest_z).
 * Starting from the current tile (*tileStack), checks each edge to see if the line
 * crosses it. When a crossing is found, follows the link to the adjacent tile and
 * continues. Calls `func` callback for each tile visited.
 *
 * Returns TRUE if the line reaches the destination within the tile network.
 * Returns FALSE if it hits a boundary edge (no adjacent tile) or loops too many times.
 * On return, *tileStack is updated to the destination tile.
 */
bool sub_GAME_7F0B0914(StandTile **tileStack, f32 start_x, f32 start_z, f32 dest_x, f32 dest_z,
    standTileWalkCallback_t func, struct StandTileWalkCallbackRecord *funcData)
{
    StandTile *curTile;
    StandTile *prevTile;
    StandTile *prevPrevTile;
    StandTile *nextTile;
    f32 scaled_startX, scaled_startZ, scaled_destX, scaled_destZ;
    f32 zDist, xDist;
    s32 edgeIdx;
    s32 crossCount;
    s32 iterCount;
    ...
    lastCrossEdge = 0;
    prevTile = curTile;
    prevPrevTile = curTile;

    while (1)
    {
        if (func != NULL) {
            func(curTile, prevTile, funcData);
        }

        s32 pointCount = (curTile->tail.half >> 0xC) & 0xF;
        if (pointCount > 0)
        {
            s32 i;
            for (i = edgeIdx; i < pointCount; i++)
            {
                s32 nextPt = (i + 1) % pointCount;
                /* Current edge: point[i] → point[nextPt] */
                s16 pt_x = curTile->points[i].x;
                s16 pt_z = curTile->points[i].z;
                s16 npt_x = curTile->points[nextPt].x;
                s16 npt_z = curTile->points[nextPt].z;
                u16 link = curTile->points[i].link;

                /* Cross product test: does the movement direction face this edge? */
                f32 edge_cross = ((f32)(npt_z - pt_z) * xDist) + (zDist * (f32)(npt_x - pt_x));
                if (edge_cross <= 0.0f)
                {
                    /* Line segment intersection test */
                    if (sub_GAME_7F0B07BC(scaled_startX, scaled_startZ,
                            scaled_destX, scaled_destZ,
                            (f32)pt_x, (f32)pt_z,
                            (f32)npt_x, (f32)npt_z,
                            (link >> 4) != 0) != 0)
                    {
                        ...
                        /* Compute the linked tile address */
                        linkedTile = &standTileStart[link];

                        if (prevTile != linkedTile && prevPrevTile != linkedTile)
                        {
                            lastCrossEdge = i;
                            if ((link >> 4) != 0) {
                                nextTile = linkedTile;
        ...
        /* Advance tile chain (3-tile history prevents backtracking) */
        {
            StandTile *oldCurTile = curTile;
            prevPrevTile = prevTile;
            prevTile = curTile;
            curTile = nextTile;

            /* If tile didn't change (old curTile == nextTile), no real crossing */
            if (oldCurTile == nextTile) {
                crossCount = 0;
            }
        }
        ...
        if (crossCount == 0)
        {
            /* No edge crossing → destination is on the current tile */
            *tileStack = prevTile;
            return TRUE;
        }
        ...
        if (iterCount >= 0x1F5 || nextTile == NULL)
        {
            ...
            stanSavedColl_pointI = lastCrossEdge;
#ifdef NATIVE_PORT
            {
                static int twalk_fail = 0;
                if (ge_dbg_enabled() && twalk_fail < 10) {
                    twalk_fail++;
                    fprintf(stderr, "[TILE_WALK_FAIL] iters=%d nextTile=%s start=(%.1f,%.1f) dest=(%.1f,%.1f) lastTile=%p room=%d\n",
                           iterCount, nextTile == NULL ? "NULL" : "limit",
                           start_x, start_z, dest_x, dest_z,
                           (void*)prevTile, prevTile ? prevTile->room : -1);
                    /* Dump link info for the failing tile */
                    if (prevTile) {
                        s32 pc = (prevTile->tail.half >> 12) & 0xF;
                        fprintf(stderr, "  tile id=0x%06X room=%d points=%d standTileStart=%p tile_offset=%ld\n",
                                prevTile->id, prevTile->room, pc,
                                (void*)standTileStart,
                                (long)((u8*)prevTile - (u8*)standTileStart));
                        for (s32 pi = 0; pi < pc; pi++) {
                            u16 lnk = prevTile->points[pi].link;
                            StandTile *resolved = &standTileStart[lnk];
                            s32 resolved_id = (*(s32 *)resolved) ? resolved->id : -1;
                            s32 resolved_room = (*(s32 *)resolved) ? resolved->room : -1;
                            fprintf(stderr, "  pt[%d] coord=(%d,%d,%d) link=0x%04X link>>4=0x%03X resolved=%p resolved_id=0x%X resolved_room=%d\n",
                                    pi, prevTile->points[pi].x, prevTile->points[pi].y, prevTile->points[pi].z,
                                    lnk, lnk >> 4, (void*)resolved, resolved_id, resolved_room);
                        }
                    }
                    fflush(stderr);
                }
...
#elif defined(PORT_FIXME_STUBS)
bool sub_GAME_7F0B0914(StandTile **tileStack, f32 start_x, f32 start_z, f32 dest_x, f32 dest_z, standTileWalkCallback_t func, struct StandTileWalkCallbackRecord *funcData) { return 0; }
#else
/* Original decomp C omitted — see NATIVE_PORT version above */
bool sub_GAME_7F0B0914(StandTile **tileStack, f32 start_x, f32 start_z, f32 dest_x, f32 dest_z, standTileWalkCallback_t func, struct StandTileWalkCallbackRecord *funcData) { return 0; }
#endif /* NATIVE_PORT / PORT_FIXME_STUBS */
#else
GLOBAL_ASM(
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B0D0C(StandTile *tile, f32 start_x, f32 start_z, StandTile **tilePtr, f32 end_x, f32 end_z, s32 *roomBuf, s32 maxBufSize) { return 0; }
#else
// Not too close
    ...
    f32 end_x, f32 end_z, s32 *roomBuf, s32 maxBufSize)
{
    ...
    if (*tilePtr != 0x0) {
        ...
        if (sub_GAME_7F0B8FD0(roomA & 0xFF, roomB & 0xFF) != 0) {
...
#ifdef NATIVE_PORT
        /* Safety: guard against NULL stanSavedColl_tile (shouldn't happen now
         * that sub_GAME_7F0B0914 is unstubbed, but defensive). */
        if (stanSavedColl_tile == NULL) {
            *pTile = sp154;
            return 1; /* treat line as unobstructed */
        }
#endif
...
#ifdef NATIVE_PORT
        {
            static int line_tile_log = -1;
            static int line_tile_cnt = 0;
            if (line_tile_log < 0)
                line_tile_log = (getenv("GE007_TRACE_COLLISION") != NULL);
            if (line_tile_log && line_tile_cnt < 200) {
                u16 link = stanSavedColl_tile->points[stanSavedColl_pointI].link;
                printf("[LINE_HIT_TILE] room=%d edge=%d/%d "
                       "link=0x%04X(linked=%s) "
                       "line=(%.1f,%.1f)->(%.1f,%.1f) isect=%.3f "
                       "edge_world=(%.1f,%.1f)->(%.1f,%.1f)\n",
                       stanSavedColl_tile->room,
                       stanSavedColl_pointI, (s32)((stanSavedColl_tile->tail.half >> 0xC) & 0xF),
                       link, (link >> 4) != 0 ? "yes" : "NO",
                       p_x, p_z, dest_x, dest_z, sp140,
                       stanSavedColl_pntA.f[0], stanSavedColl_pntA.f[1],
                       stanSavedColl_pntB.f[0], stanSavedColl_pntB.f[1]);
                fflush(stdout);
                line_tile_cnt++;
            }
        }
...
#ifdef NATIVE_PORT
                                    {
                                        static int line_prop_log = -1;
                                        static int line_prop_cnt = 0;
                                        if (line_prop_log < 0)
                                            line_prop_log = (getenv("GE007_TRACE_COLLISION") != NULL);
                                        if (line_prop_log && line_prop_cnt < 200) {
                                            printf("[LINE_HIT_PROP] type=%d prop_pos=(%.1f,%.1f,%.1f) "
                                                   "line=(%.1f,%.1f)->(%.1f,%.1f) isect=%.3f "
                                                   "edge=(%.1f,%.1f)->(%.1f,%.1f)\n",
                                                   prop->type,
                                                   prop->pos.f[0], prop->pos.f[1], prop->pos.f[2],
                                                   p_x, p_z, dest_x, dest_z, temp_f0,
                                                   sp134.f[0], sp134.f[1], sp12C.f[0], sp12C.f[1]);
                                            fflush(stdout);
                                            line_prop_cnt++;
                                        }
                                    }
...
#ifdef NATIVE_PORT
    /* Player-blocked diagnostic: fires every frame the PLAYER's movement
     * is obstructed. Detects "player call" by checking if the player's chr
     * has CHRHIDDEN_MOVING set (set by sub_GAME_7F03D058 before this call).
     * Writes to file specified by GE007_TRACE_BLOCKED=/path. */
    if (retval == 0) {
        static FILE *blocked_fp = NULL;
        static int blocked_init = 0;
        if (!blocked_init) {
            blocked_init = 1;
            const char *env = getenv("GE007_TRACE_BLOCKED");
            if (env) blocked_fp = fopen(env, "w");
        }
        if (blocked_fp) {
            /* Check if this is the player's movement call.
             * bondview.c sets g_playerMovementActive=1 around the player
             * collision check (bracketed by sub_GAME_7F03D058 calls). */
            extern int g_playerMovementActive;
            int is_player = g_playerMovementActive;
            if (is_player) {
                extern int g_frame_count_diag;
                if (stanSavedColl_posData != NULL) {
                    /* Blocked by a PROP (object/door/weapon) */
                    fprintf(blocked_fp,
                            "[BLOCKED] f=%d by=PROP type=%d pos=(%.1f,%.1f,%.1f) "
                            "player=(%.1f,%.1f) dest=(%.1f,%.1f) isect=%.3f "
                            "edge=(%.1f,%.1f)->(%.1f,%.1f) "
                            "prop_y_bounds=(%.1f,%.1f)\n",
                            g_frame_count_diag,
                            stanSavedColl_posData->type,
                            stanSavedColl_posData->pos.f[0],
                            stanSavedColl_posData->pos.f[1],
                            stanSavedColl_posData->pos.f[2],
                            p_x, p_z, sp144.f[0], sp144.f[1],
                            sp140,
                            stanSavedColl_pntA.f[0], stanSavedColl_pntA.f[1],
                            stanSavedColl_pntB.f[0], stanSavedColl_pntB.f[1],
                            spA0, spA4);
                } else {
                    /* Blocked by a TILE EDGE (wall/railing) */
                    u16 link = 0;
                    s32 room = -1;
                    if (stanSavedColl_tile) {
                        link = stanSavedColl_tile->points[stanSavedColl_pointI].link;
                        room = stanSavedColl_tile->room;
                    }
                    fprintf(blocked_fp,
                            "[BLOCKED] f=%d by=TILE room=%d edge=%d link=0x%04X "
                            "player=(%.1f,%.1f) dest=(%.1f,%.1f) isect=%.3f "
                            "edge=(%.1f,%.1f)->(%.1f,%.1f)\n",
                            g_frame_count_diag,
                            room, stanSavedColl_pointI, link,
                            p_x, p_z, sp144.f[0], sp144.f[1],
                            sp140,
                            stanSavedColl_pntA.f[0], stanSavedColl_pntA.f[1],
                            stanSavedColl_pntB.f[0], stanSavedColl_pntB.f[1]);
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
PropRecord *sub_GAME_7F0B1410(StandTile *arg0, f32 arg1, f32 arg2, f32 arg3, f32 arg4, s32 arg5) { return NULL; }
#else
extern void roomGetProps(s32 *roomList);
extern s32 propIsOfCdType(struct PropRecord *prop, s32 cdtypes);
extern void chraiGetCollisionBoundsWithoutY(struct PropRecord *prop, struct rect4f **polygon, s32 *numvertices);
extern s32 sub_GAME_7F0B0688(f32 arg0, f32 arg1, f32 arg2, f32 arg3, f32 arg4, f32 arg5, f32 arg6, f32 arg7);

// stanLineDoor — arg0 is StandTile* (stored to local, &local passed to sub_GAME_7F0B0C24)
PropRecord *sub_GAME_7F0B1410(StandTile *tile, f32 start_x, f32 start_z, f32 dest_x, f32 dest_z, s32 cdtypes)
{
    struct PropRecord *bestProp;
    f32 bestFactor;
    s32 spEC;
    StandTile *tileLocal;
    s32 roomBuffer[0x15];
    ...
    s32 i;
    s32 next;
    struct coord2d lineStart;
    struct coord2d lineDest;
    ...
    sub_GAME_7F0B0C24(&tileLocal, start_x, start_z, dest_x, dest_z, &roomBuffer[0], &spEC, 20);
    ...
    roomBuffer[spEC] = -1;
    roomGetProps(&roomBuffer[0]);

    for (spLookup = ptr_list_object_lookup_indices; *spLookup >= 0; spLookup++)
    {
        prop = &pos_data_entry[*spLookup];

        if (propIsOfCdType(prop, cdtypes) != 0)
        {
            chraiGetCollisionBoundsWithoutY(prop, &polygon, &numvertices);
            ...
            if (numvertices > 0)
            {
                for (i = 0; i < numvertices; i++)
                {
                    ...
                    if (sub_GAME_7F0B0688(start_x, start_z, dest_x, dest_z,
                            polygon->points[i].f[0], polygon->points[i].f[1],
                            polygon->points[next].f[0], polygon->points[next].f[1]) != 0)
                    {
                        edgeStart.f[0] = polygon->points[i].f[0];
                        edgeStart.f[1] = polygon->points[i].f[1];
                        edgeEnd.f[0] = polygon->points[next].f[0];
                        edgeEnd.f[1] = polygon->points[next].f[1];

                        factor = calculateLineIntersectionFactor(&lineStart, &lineDest, &edgeStart, &edgeEnd);
...
#ifdef NATIVE_PORT
    temp_v0 = sub_GAME_7F0B21B0(arg0, arg1, arg2, arg3, (intptr_t)&spA8[0], &spFC, 20);
#else
...
#ifdef NATIVE_PORT
                                {
                                    static int vol_prop_log = -1;
                                    static int vol_prop_cnt = 0;
                                    if (vol_prop_log < 0)
                                        vol_prop_log = (getenv("GE007_TRACE_COLLISION") != NULL);
                                    if (vol_prop_log && vol_prop_cnt < 200) {
                                        printf("[VOL_HIT_PROP] type=%d prop_pos=(%.1f,%.1f,%.1f) "
                                               "test_pos=(%.1f,%.1f) radius=%.1f edge_dist=%.2f "
                                               "edge=(%.1f,%.1f)->(%.1f,%.1f)\n",
                                               prop->type,
                                               prop->pos.f[0], prop->pos.f[1], prop->pos.f[2],
                                               arg1, arg2, arg3, var_f20,
                                               spA4->points[i].f[0], spA4->points[i].f[1],
                                               spA4->points[next].f[0], spA4->points[next].f[1]);
                                        fflush(stdout);
                                        vol_prop_cnt++;
                                    }
                                }
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B1DDC(struct StandTile **arg0, f32 arg1, f32 arg2, f32 arg3, standTileLocusCallback_A_t arg4, standTileLocusCallback_B_t arg5, standTileLocusCallback_C_t arg6, struct StandTileLocusCallbackRecord *arg7) { return -1; }
#elif defined(NATIVE_PORT)
/**
 * Tile radius BFS search. Searches all tiles reachable within radius of (x,z).
 * Calls callback_A on each visited tile, callback_B on each qualifying edge,
 * callback_C when the search completes or a collision is found.
 *
 * Returns: -2 = search exhausted (no collision), 2 = callback_B triggered collision,
 *           5 = stack overflow (>= 41 tiles visited).
 */
s32 sub_GAME_7F0B1DDC(struct StandTile **arg0, f32 arg1, f32 arg2, f32 arg3,
    standTileLocusCallback_A_t callback_A,
    standTileLocusCallback_B_t callback_B,
    standTileLocusCallback_C_t callback_C,
    struct StandTileLocusCallbackRecord *cbRecord)
{
    #define TILE_STACK_MAX 41
    struct StandTile *tileStack[TILE_STACK_MAX];
    s32 stackDepth;
    s32 readIdx;
    struct StandTile *curTile;
    s32 edgeIdx, pointCount;
    f32 scaledX, scaledZ, scaledRadius;
    s32 i;

    scaledX = arg1 * level_scale;
    scaledZ = arg2 * level_scale;
    scaledRadius = arg3 * level_scale;

    tileStack[0] = *arg0;
    stackDepth = 1;
    readIdx = 0;

    while (readIdx < stackDepth) {
        curTile = tileStack[readIdx];
        readIdx++;

        /* Callback A: visit notification */
        if (callback_A != NULL) {
            callback_A(curTile, cbRecord);
        }

        pointCount = (curTile->tail.half >> 12) & 0xF;
        if (pointCount <= 0) goto check_stack;

        for (edgeIdx = 0; edgeIdx < pointCount; edgeIdx++) {
            s32 nextEdge = (edgeIdx + 1) % pointCount;
            f32 edgeDist, distPt0, distPt1;

            edgeDist = getShortest2dDispToInfTileEdge(curTile, edgeIdx, scaledX, scaledZ);
            distPt0 = distToTilePnt2D(curTile, edgeIdx, scaledX, scaledZ);
            distPt1 = distToTilePnt2D(curTile, nextEdge, scaledX, scaledZ);

            /* Skip edge if signed distance >= radius (too far) */
            if (!(edgeDist < scaledRadius)) continue;

            /* Edge qualifies if either endpoint is within radius,
               or if the perpendicular projection falls on the segment */
            if (!(distPt0 < scaledRadius) && !(distPt1 < scaledRadius)) {
                if (!sub_GAME_7F0B0198(curTile, edgeIdx, scaledX, scaledZ)) {
                    continue;
                }
            }

            /* Callback B: edge collision check */
            if (callback_B != NULL) {
                s32 cbResult = callback_B(curTile, edgeIdx, edgeDist, distPt0,
                    *(s32*)&distPt1, cbRecord);
                if (cbResult != 0) {
                    /* Collision detected */
                    stanSavedColl_tile = curTile;
                    stanSavedColl_pointI = edgeIdx;
                    if (callback_C != NULL) {
                        s32 cResult = callback_C(tileStack, stackDepth, cbRecord);
                        if (cResult == 1) {
                            /* callback_C says continue searching */
                            goto next_edge;
                        }
                    }
                    return 2;
            ...
            /* Push linked tile if it has a valid link and isn't already in stack */
            {
                u16 link = curTile->points[edgeIdx].link;
                if ((link >> 4) != 0) {
                    struct StandTile *linkedTile = &standTileStart[link];
                    s32 duplicate = 0;
                    for (i = stackDepth - 1; i >= 0; i--) {
                        if (tileStack[i] == linkedTile) {
                            duplicate = 1;
                            break;
                        }
                    }
                    if (!duplicate && stackDepth < TILE_STACK_MAX) {
                        tileStack[stackDepth] = linkedTile;
                        stackDepth++;
                    }
                } else {
                    /* No linked tile on this edge — save collision state */
                    stanSavedColl_tile = curTile;
                    stanSavedColl_pointI = edgeIdx;
#ifdef NATIVE_PORT
                    {
                        static int locus_edge_log = -1;
                        static int locus_edge_cnt = 0;
                        if (locus_edge_log < 0)
                            locus_edge_log = (getenv("GE007_TRACE_COLLISION") != NULL);
                        if (locus_edge_log && locus_edge_cnt < 200) {
                            s32 pc = (curTile->tail.half >> 0xC) & 0xF;
                            s32 ne = (edgeIdx + 1) % (pc > 0 ? pc : 1);
                            printf("[LOCUS_UNLINKED_EDGE] room=%d tile=%p edge=%d/%d "
                                   "edge_pts=(%d,%d)->(%d,%d) "
                                   "test_pos=(%.1f,%.1f) radius=%.1f edgeDist=%.2f\n",
                                   curTile->room, (void*)curTile, edgeIdx, pc,
                                   curTile->points[edgeIdx].x, curTile->points[edgeIdx].z,
                                   curTile->points[ne].x, curTile->points[ne].z,
                                   arg1, arg2, arg3, edgeDist);
                            fflush(stdout);
                            locus_edge_cnt++;
                        }
                    }
    ...
    check_stack:
        ...
        if (stackDepth >= TILE_STACK_MAX) {
            return 5;
        }
    }

    /* Search exhausted — call callback_C for final notification */
    if (callback_C != NULL) {
        callback_C(tileStack, stackDepth, cbRecord);
    }
    return -2;
    #undef TILE_STACK_MAX
}
#else
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B2110(StandTile *tile, struct StandTileLocusCallbackRecord *data) { return 0; }
#elif defined(NATIVE_PORT)
// 'addTileRoomIfNew' — adds tile->room to the room buffer if not already present
s32 sub_GAME_7F0B2110(StandTile *tile, struct StandTileLocusCallbackRecord* data)
{
    s32 *roomBuf = (s32 *)(data->unk00);
    s32 i;

    for (i = 0; i < data->count; i++)
    {
        if (roomBuf[i] == tile->room) {
            return 0;
        }
    }

    if (data->count < data->bufMax) {
        roomBuf[data->count] = tile->room;
        data->count++;
    }
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B2274(StandTile *arg0, s32 arg1, f32 arg2, f32 arg3, s32 arg4, struct StandTileLocusCallbackRecord *arg5) { return 0; }
#else
/**
 * Address 0x7F0B2274.
 *
...
s32 stanTileDistanceRelated(StandTile **arg0, f32 arg1, f32 arg2, f32 arg3, struct StandTileLocusCallbackRecord *arg4)
{
    ...
    // N64 original zeroed a larger region (16 s32s = 64 bytes) because on MIPS
    // the struct lived in a known stack layout. On PC, this overflows the 16-byte
    // struct and corrupts adjacent stack variables (e.g. the tile pointer).
    // Only zero the actual struct fields.
    arg4->unk00 = 0;
    arg4->count = 0;
    arg4->bufMax = 0;
 ...
 * Address 0x7F0B2420.
 *
 * Given the ladder tile stored in dword_CODE_bss_8007BA0C, iterate its edges
 * looking for a neighbor tile whose D_80040F30 entry has bit 0x40 set
 * (another ladder tile). When found, try up to 12 rotations of the 3 neighbor
 * triangle vertices plus the source tile's vertex to fill arg2 (4 coord3d
 * slots) such that all y-values satisfy the height checks. The first valid
 * rotation is kept; *arg0 receives the source tile and *arg1 the neighbor.
 */
void stanGetMoveBondCollisionTiles(struct StandTile **arg0, struct StandTile **arg1, struct move_bond_collision *arg2)
{
    StandTile *srcTile = dword_CODE_bss_8007BA0C;
    s32 numSides = STAN_TAIL_E(srcTile);
    s32 side;
    coord3d *slots = (coord3d *)arg2;
    ...
    for (side = 0; side < numSides; side++) {
        u16 link = srcTile->points[side].link;
        StandTile *neighborTile = &standTileStart[link];

        if ((link >> 4) == 0) {
            continue;
        }

        if (!(D_80040F30[neighborTile->mid.half >> 0xc] & 0x40)) {
            continue;
        }

        /* Found a neighbor with the 0x40 flag */
        *arg0 = srcTile;
        *arg1 = neighborTile;

        {
            s32 startTriple = (side + 2) % 3;
            s32 rotate;

            for (rotate = 0; rotate < 12; rotate++) {
                s32 base = rotate >> 2;
                s32 i;

                for (i = 0; i < 3; i++) {
                    s32 tripleIndex = (base + i) % 3;
                    s32 slotIndex = (rotate + i) & 3;
                    sub_GAME_7F0B23AC(neighborTile, tripleIndex, &slots[slotIndex]);
                }

                {
                    s32 lastSlot = (rotate + 3) & 3;
                    sub_GAME_7F0B23AC(srcTile, startTriple, &slots[lastSlot]);
                }

                /* Check if this rotation satisfies height conditions */
                if (slots[2].y < slots[0].y) {
                    continue;
                }
                if (slots[2].y < slots[1].y) {
                    continue;
                }
                if (slots[3].y < slots[0].y) {
                    continue;
                }
                if (slots[3].y < slots[1].y) {
                    continue;
                }

                /* All four checks passed — valid configuration found */
                return;
            }

            /* Exhausted all 12 rotations without finding a valid one */
            return;
        }
    }
}
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
s32 sub_GAME_7F0B260C(StandTile *tile, s32 index, f32 p_x, f32 p_z, s32 arg4, struct StandTileLocusCallbackRecord *arg5) { return 0; }
#else
/**
 * arg5 type is wrong, first offset needs to be a float.
 * Caller passes (StandTileLocusCallbackRecord*)&float_data, so unk00 holds float bits.
*/
s32 sub_GAME_7F0B260C(StandTile *tile, s32 index, f32 p_x, f32 p_z, s32 arg4, struct StandTileLocusCallbackRecord *arg5)
{
    /* unk00 holds float bits reinterpreted as s32 */
    f32 height;
    memcpy(&height, &arg5->unk00, sizeof(f32));

    if (height < (f32)tile->points[index].y
        && height < (f32)tile->points[(index + 1) % STAN_TAIL_E(tile)].y)
    {
        return 1;
    }
 ...
 * US address 7F0B26B8.
*/
s32 sub_GAME_7F0B26B8(StandTile **tile, f32 target_x, f32 target_z, f32 b_z, f32 radius)
{
    f32 data;

    data = radius * level_scale;
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
StandTile *sub_GAME_7F0B2718(StandTile *srcTile, tilePredicate_t tilePred) { return NULL; }
#elif defined(NATIVE_PORT)
/* stanFillSearch — BFS tile graph traversal with predicate.
 * Fixed: STAN_TAIL_E for point count, 64-bit pointer arithmetic. */
StandTile *sub_GAME_7F0B2718(StandTile *srcTile, tilePredicate_t tilePred)
{
    StandTile *tile;
    StandTile *linkTile;
    u32 i, pointCount;
    s32 pntI, seenCount, countForIter, linkI;
    u16 link;
    ...
    do {
        for (pntI = 0; pntI < countForIter; pntI++) {
            ...
            for (linkI = 0; (u32)linkI < pointCount; linkI++) {
                link = tile->points[linkI].link;

                if ((link >> 4) != 0) {
                    linkTile = &standTileStart[link];

                    /* Check if already visited */
                    for (i = 0; (s32)i < seenCount; i++) {
...
f32 stanGetPositionYValue(StandTile *tile, f32 p_x, f32 p_z)
{
    ...
    {
        static int stan_log = 0;
        if (getenv("GE007_VERBOSE") && stan_log < 3) {
            s32 td = STAN_TAIL_D(tile);
            s32 tc = STAN_TAIL_C(tile);
            s32 tn = STAN_TAIL_POINT_COUNT(tile);
            printf("[STAN_Y] tile=%p p=(%.1f,%.1f) level_scale=%.6f inv=%.6f pt_d=(%d,%d,%d) pt_c=(%d,%d,%d) pt_n=(%d,%d,%d)\n",
                   (void*)tile, p_x, p_z, level_scale, inv_level_scale,
                   tile->points[td].x, tile->points[td].y, tile->points[td].z,
                   tile->points[tc].x, tile->points[tc].y, tile->points[tc].z,
                   tile->points[tn].x, tile->points[tn].y, tile->points[tn].z);
            fflush(stdout);
            stan_log++;
        }
    }
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
f32 sub_GAME_7F0B2C74(StandTile *tile, f32 *heights) { return 0.0f; }
#else
...
#ifdef NONMATCHING
#if 0 /* PORT_FIXME_STUBS — unstubbed for PC port */
struct StandTile *stanMatchTileName(char *name) { return NULL; }
#else
// Looks up a StandTile by its name string (e.g. "p1234a"). Packs the name
// into a 24-bit id (u16 high + u8 low) via stanPackId, then walks the tile
// list comparing each tile's id fields. Returns matching tile or NULL.
// Matches with original IDO. ido_recomp: global address merging and bnel
// delay slot scheduling differ.
StandTile *stanMatchTileName(char *name) {
    u16 stanIdHi;
    u8 stanIdLo;
    StandTile *tile;
    StandTile *first_match = NULL;
    s32 pointCount;
#ifdef NATIVE_PORT
    const char *trace_name = getenv("GE007_TRACE_TILE_NAME");
    int trace_enabled = trace_name && strcmp(trace_name, name) == 0;
    int trace_match_count = 0;
#endif
    ...
    stanPackId(name, &stanIdHi, &stanIdLo);
    tile = stan_prefix.ptr_firstroom;
    ...
    while (*(u32 *)tile != 0) {
        if (*(u16 *)tile == stanIdHi && *((u8 *)tile + 2) == stanIdLo) {
#ifdef NATIVE_PORT
            if (trace_enabled) {
                fprintf(stderr,
                        "[STAN_MATCH] name=%s match[%d] tile=%p room=%d points=%d\n",
                        name, trace_match_count, (void *)tile, tile->room,
                        (tile->tail.half >> 12) & 0xf);
            }
            trace_match_count++;
#endif
            if (first_match == NULL) {
    ...
    if (trace_enabled) {
        fprintf(stderr,
                "[STAN_MATCH] name=%s total_matches=%d first=%p first_room=%d\n",
                name, trace_match_count, (void *)first_match,
                first_match ? first_match->room : -1);
    }
#endif
    return first_match;
}
...
void sub_GAME_7F0B2F00(char **arg0) {
    *arg0 = (char *)stanMatchTileName(*arg0);
}
...
#ifdef NATIVE_PORT
/**
 * PORT: stanDetermineEOF — byte-swap big-endian stan binary and set up globals.
 *
 * The .stan file is loaded raw from ROM (big-endian). On N64 this was native
 * byte order, but on little-endian PC we must swap all multi-byte fields.
 * Additionally, the N64 version relocated 32-bit VMA pointers in-place, which
 * doesn't work on 64-bit (host pointers don't fit in 32-bit slots).
 *
 * Binary layout:
 *   offset 0: s32 header (unused)
 *   offset 4: u32[] room pointers (file-relative offsets), null-terminated
 *   after null: StandTile data (variable-length tiles, null-terminated)
 *
 * StandTile binary layout (8-byte fixed header):
 *   bytes 0-2: id (24-bit, big-endian byte order)
 *   byte 3: room (u8, no swap needed)
 *   bytes 4-5: mid (s16, big-endian)
 *   bytes 6-7: tail (s16, big-endian)
 *   bytes 8+: StandTilePoint[] (each 8 bytes: s16 x, s16 y, s16 z, u16 link)
 */
static void stanByteSwapBinary(u8 *data) {
    u8 *p = data;
    u8 tmp;

    /* Swap header word at offset 0 */
    tmp = p[0]; p[0] = p[3]; p[3] = tmp;
    tmp = p[1]; p[1] = p[2]; p[2] = tmp;
    p += 4;

    /* Swap room pointer array (u32 file-offsets, null-terminated).
     * 4 zero bytes = null terminator, same in both endiannesses. */
    while (1) {
        u32 val;
        memcpy(&val, p, 4);
        if (val == 0) { p += 4; break; }
    ...
    /* Swap tile data (variable-length records, null-terminated) */
    while (1) {
        u32 first_word;
        memcpy(&first_word, p, 4);
        if (first_word == 0) break; /* end of tiles */

        /* Swap id u16 at bytes 0-1 (stanIdHi BE→LE), byte 2 (stanIdLo) stays, byte 3 (room) stays */
        tmp = p[0]; p[0] = p[1]; p[1] = tmp;

        /* Read tail.half (big-endian s16 at offset 6) to get point count.
         * Must read BEFORE swapping so we know the tile size. */
        s16 tail_val = (s16)((p[6] << 8) | p[7]);
        s32 pointCount = (tail_val >> 12) & 0xf;

        /* Swap mid (s16 at offset 4) */
        tmp = p[4]; p[4] = p[5]; p[5] = tmp;

        /* Swap tail (s16 at offset 6) */
        tmp = p[6]; p[6] = p[7]; p[7] = tmp;

        /* Swap each s16 in the points area */
        s32 tile_size = list_of_tilesizes[pointCount];
        s32 points_bytes = tile_size - 8;
        for (s32 j = 0; j < points_bytes; j += 2) {
            u8 *sh = p + 8 + j;
            tmp = sh[0]; sh[0] = sh[1]; sh[1] = tmp;
        }
...
void *stanDetermineEOF(struct StanPrefixRecord *r, s32 arg1, s32 arg2) {
    u8 *raw = (u8 *)r;
    intptr_t base = (intptr_t)raw;
    StandTile *tile;
    s32 pointCount;

    /* Byte-swap the entire binary from big-endian to native */
    stanByteSwapBinary(raw);

    /* Read first room offset from binary offset 4 (now native-endian u32) */
    u32 first_room_off;
    memcpy(&first_room_off, raw + 4, 4);

    /* Convert file offset to host pointer: file offset + base address */
    stan_prefix.stanfile = (void *)r;
    stan_prefix.ptr_firstroom = (StandTile *)(base + (intptr_t)first_room_off);
    standTileStart = (StandTile *)(base + (intptr_t)first_room_off - 0x80);
    ptr_firstroom_0 = (intptr_t)stan_prefix.ptr_firstroom;

    /* Walk the room pointer array at offset 4 and relocate in place.
     * Each entry is a u32 file offset. We store the relocated host pointer
     * truncated back to u32 — this is only used for counting rooms, the actual
     * tile pointers come from walking the tile data. */
    u32 *room_arr = (u32 *)(raw + 4);
    s32 num_rooms = 0;
    while (room_arr[num_rooms] != 0) {
        num_rooms++;
    }

    /* Walk all tiles past the room pointer array to find EOF (last tile).
     * Tile data starts after the null-terminated room pointer array.
     * Room array: num_rooms entries + 1 null = (num_rooms+1) * 4 bytes at offset 4. */
    tile = (StandTile *)(raw + 4 + (num_rooms + 1) * 4);
    ...
    if (*(s32 *)tile != 0) {
        ...
        do {
            pointCount = (tile->tail.half >> 12) & 0xf;
            if (pointCount > 10) {
                printf("[STAN] ERROR: invalid pointCount %d at tile %d, aborting walk\n",
                       pointCount, tile_count);
                break;
            }
            tile = (StandTile *)((u8 *)tile + list_of_tilesizes[pointCount]);
            tile_count++;
...
#elif defined(PORT_FIXME_STUBS)
void *stanDetermineEOF(struct StanPrefixRecord *r, s32 arg1, s32 arg2) { return NULL; }
#else
// NONMATCHING: Likely register allocation diffs from ido_recomp.
// Relocates all room pointers in the StanPrefixRecord by an offset,
// then walks all tiles to find the last one, storing it in D_80040F60.
void *stanDetermineEOF(struct StanPrefixRecord *r, s32 arg1, s32 arg2) {
    s32 offset;
    StandTile **roomPtr;
    StandTile *tile;
    ...
    stan_prefix.stanfile = (s32)r;

    offset = arg2 - arg1;
    standTileStart = (struct StandTile *)((s32)r->ptr_firstroom + offset - 0x80);
    ptr_firstroom_0 = (s32)r->ptr_firstroom + offset;

    roomPtr = &r->ptr_firstroom;

    if (*roomPtr != NULL) {
        do {
            *roomPtr = (StandTile *)((s32)*roomPtr + offset);
            roomPtr++;
        } while (*roomPtr != NULL);
    }
...
s32 sub_GAME_7F0B3044(Gfx *gdl) {
    s32 sp1C;
    f32 temp_f0;

    sp1C = 0;
    if (((dynGetFreeGfx(gdl) < 0x1000) || (dynGetFreeVtx() < 0x1000)) && (*D_800413D0 == 0)) {
        D_800413C0 = 0.0f;
        D_800413C4 = 0.0f;
...
Gfx * sub_GAME_7F0B312C(Gfx *arg0, s32 arg1)
{
      ...
      extended_param2 = ZEXT48(param_2);
      *ppuStack0000002c = param_2;
      ...
      edge_calc = 0;
      Function_82278968(pqVar2,pqVar1,0,param_4,param_5,param_6,extended_param2);
      do {
        do {
          if (local_30 == NULL) {
            Function_82279088();
        ...
        *psStack0000001c = *psStack0000001c ^ 0x8000;
        ...
        check_result = sub_GAME_7F0B3044(arg0);
        if (check_result != 0) {
          Function_8238AC90(psStack0000001c,uStack00000014,edge_calc,param_4,param_5,param_6,extended_param2);
        }
        for (local_28 = 0; cVar4 = getsides(psStack0000001c), local_28 < cVar4; local_28 = local_28 + 1)
        {
          if (psStack0000001c[local_28 + 1].tail >> 4 == 0) {
            check_result = sub_GAME_7F0B3044(arg0);
            if (check_result != 0) {
              edge_calc = local_28 + 1;
              cVar4 = getsides(psStack0000001c);
              trapWord(6,cVar4,0);
              trapWord(5,cVar4 & ~(((edge_calc & 0x7fffffff) << 1 | (edge_calc << 0x20) >> 0x3f) - 1),0xffff);
              edge_calc = edge_calc - (edge_calc / cVar4) * cVar4;
              param_4 = 0xffffffffffffffc0;
              Function_8238B8F0(psStack0000001c,local_28,edge_calc,0xffffffffffffffc0,param_5,param_6,extended_param2)
              ;
            }
          }
          else if (*(stanTileStart + psStack0000001c[local_28 + 1].tail) >> 0xf != cStack00000027) {
            extended_param2 = ZEXT48(ppuStack0000002c);
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
int sub_GAME_7F0B3138(void *unused, StandTile **pTile, f32 p_x, f32 p_z, f32 dest_x, f32 dest_z, int cdtypes, f32 unkHeight, f32 unkA) { return 0; }
#elif defined(NATIVE_PORT)
