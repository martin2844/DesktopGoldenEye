...
static s32 s_pcRamromAiRngTraceEnabled = -1;
static s32 s_pcRamromAiRngTraceBudget = 0;
static s32 s_pcAiMoveTraceEnabled = -1;

static s32 pcChraiAiMoveTraceEnabled(void)
{
    const char *value;

    if (s_pcAiMoveTraceEnabled < 0) {
        value = getenv("GE007_TRACE_AI_MOVE");
        if (value == NULL || *value == '\0') {
            value = getenv("GE007_VERBOSE");
        }
        s_pcAiMoveTraceEnabled = value != NULL && *value != '\0' ? 1 : 0;
    }

    return s_pcAiMoveTraceEnabled;
}

static s32 pcChraiRamromAiRngTraceEnabled(void)
{
    const char *value;

    if (s_pcRamromAiRngTraceEnabled < 0) {
        value = getenv("GE007_TRACE_RAMROM_AI_RNG");
        if (value == NULL || *value == '\0') {
            value = getenv("GE007_TRACE_RAMROM_CHR_ACTION");
        }
        s_pcRamromAiRngTraceEnabled = value != NULL && *value != '\0' ? 1 : 0;

        if (s_pcRamromAiRngTraceEnabled) {
            value = getenv("GE007_TRACE_RAMROM_AI_RNG_BUDGET");
            if (value == NULL || *value == '\0') {
                value = getenv("GE007_TRACE_RAMROM_CHR_ACTION_BUDGET");
            }
            s_pcRamromAiRngTraceBudget = value != NULL && *value != '\0'
                ? atoi(value)
                : 1024;
        }
    }

    return s_pcRamromAiRngTraceEnabled && s_pcRamromAiRngTraceBudget != 0;
}

static void pcChraiTraceRamromAiRng(const char *event,
                                    const ChrRecord *chr,
                                    s32 offset,
                                    s32 opcode,
                                    u32 random_value)
{
    ...
    if (!pcChraiRamromAiRngTraceEnabled()) {
        return;
    }
    ...
    if (s_pcRamromAiRngTraceBudget > 0) {
        s_pcRamromAiRngTraceBudget--;
    }
    ...
    fprintf(stderr,
            "[RAMROM-AI] event=%s chrnum=%d action=%d sleep=%d hidden=0x%x "
            "chrflags=0x%x aioffset=%d offset=%d opcode=%d random=0x%08x "
            "random_low=%u input_stream_offset=%u cursor_offset=%u block_count=%d "
            "block_speedframes=%d block_randseed=%d block_check=%d "
            "call_count=%llu calls_since_restore=%llu seed_low=%d "
            "pos_x=%.2f pos_y=%.2f pos_z=%.2f\n",
            event != NULL ? event : "",
            chr != NULL ? chr->chrnum : -1,
            chr != NULL ? chr->actiontype : -1,
            chr != NULL ? chr->sleep : -1,
            chr != NULL ? chr->hidden : 0,
            chr != NULL ? chr->chrflags : 0,
            chr != NULL ? chr->aioffset : -1,
            offset,
            opcode,
            random_value,
            random_value & 0xffU,
...
static void pcChraiTraceRamromAiDecision(const char *event,
                                         const ChrRecord *chr,
                                         s32 offset,
                                         s32 opcode,
                                         s32 compare,
                                         ...
                                         s32 next_offset)
{
    ...
    fprintf(stderr,
            "[RAMROM-AI] event=%s chrnum=%d action=%d sleep=%d hidden=0x%x "
            "chrflags=0x%x aioffset=%d offset=%d opcode=%d random=0x%08x "
            "random_low=%u compare=%d label=%d taken=%d next_offset=%d "
            "input_stream_offset=%u cursor_offset=%u block_count=%d "
            "block_speedframes=%d block_randseed=%d block_check=%d "
            "call_count=%llu calls_since_restore=%llu seed_low=%d "
            "pos_x=%.2f pos_y=%.2f pos_z=%.2f\n",
            event != NULL ? event : "",
            ...
            offset,
            opcode,
            chr != NULL ? chr->random : 0,
            chr != NULL ? (chr->random & 0xffU) : 0,
            compare,
            label,
            taken,
            next_offset,
...
#define AI_AT(list, offset, type) ((type *)((u8 *)(list) + (offset)))
...
#ifdef NATIVE_PORT
static void audioClearStaleSlotState(s32 slot, const char *source)
{
    sfxRecord *sfx;
    ...
    if (sfx->state != NULL && !sndStateIsOwnedBySlot(sfx->state, &sfx->state))
    {
        portAudioTraceSfxJson(
            "{\"event\":\"owner_stale_clear\",\"frame\":%d,\"source\":\"%s\","
            "\"state\":\"%p\",\"slot\":%d}",
            g_GlobalTimer,
            source,
            (void *)sfx->state,
...
#ifdef NATIVE_PORT
    audioClearStaleSlotState(slot, "chrai_slot_update");
#endif
    ...
    if ((sfx->state ) && (sndGetPlayingState(sfx->state) ))
    {
        ...
        if (tempvol != sfx->Volume2)
        {
            ...
            if (sfx->pos != NULL)
            {
                ...
                sndSetStatePosition(sfx->state, sfx->pos->x * ls, sfx->pos->y * ls, sfx->pos->z * ls);
            }
            else if (sfx->Obj != NULL)
            {
                f32 ls = get_room_data_float1();
                sndSetStatePosition(sfx->state,
                                    sfx->Obj->runtime_pos.x * ls,
                                    sfx->Obj->runtime_pos.y * ls,
                                    sfx->Obj->runtime_pos.z * ls);
            }
#endif
            portAudioTraceSfxJson(
                "{\"event\":\"post_source\",\"frame\":%d,\"source\":\"chrai_slot\",\"state\":\"%p\",\"value\":%d,"
                "\"slot\":%d}",
                g_GlobalTimer,
                (void *)sfx->state,
                tempvol,
...
void audioPlayFromProp(s32 slot, s16 soundIndex)
{
    ...
    {
#ifdef NATIVE_PORT
        audioClearStaleSlotState(slot, "chrai_slot_play");
#endif
    ...
    {
#ifdef NATIVE_PORT
        sfx->state = sndPlaySfx(g_musicSfxBufferPtr, soundIndex, (ALSoundState *)&sfx->state);
#else
        sfx->state = sndPlaySfx(g_musicSfxBufferPtr, soundIndex, sfx->state);
#endif
    }
    else
    {
        sndPlaySfx(g_musicSfxBufferPtr, soundIndex, NULL);
    }
}
...
void sub_GAME_7F0349BC(s32 slot)
{
    ...
    {
#ifdef NATIVE_PORT
        audioClearStaleSlotState(slot, "chrai_slot_stop");
#endif
        sndDeactivate(sfx_related[slot].state);
        sfx_related[slot].state = NULL;
        sfx_related[slot].Volume2 = 0;
    }
}
 ...
 * @canonical name
 */
s32 chraiitemsize(void *AIList, s32 offset)
{
    u8 *cmdstream = (u8 *)AIList;

    //matches only as u8* despite text evidence of ai->val structs
    switch (cmdstream[offset])
    {
...
#ifdef NATIVE_PORT
#define AI_TRACE_MAX_FILTER_LISTS 32

static int g_AiTraceEnabled = -1;
static int g_AiTraceBudget = 4000;
static s32 g_AiTraceFrameMin = -1;
...
{
    while (cursor != NULL && *cursor != '\0' && g_AiTraceFilterListCount < AI_TRACE_MAX_FILTER_LISTS)
    {
        char *endptr;
        long value;
        ...
        g_AiTraceFilterLists[g_AiTraceFilterListCount++] = (s32)value;
        cursor = endptr;
    }
}

static s32 chraiTraceEnabled(void)
{
    const char *value;

    if (g_AiTraceEnabled >= 0)
    {
        return g_AiTraceEnabled;
    }

    value = getenv("GE007_AI_TRACE");
    g_AiTraceEnabled = (value != NULL && value[0] != '\0' && value[0] != '0') ? 1 : 0;

    value = getenv("GE007_AI_TRACE_BUDGET");
    g_AiTraceBudget = (value != NULL && value[0] != '\0') ? atoi(value) : 4000;

    value = getenv("GE007_AI_TRACE_FRAME_MIN");
    g_AiTraceFrameMin = (value != NULL && value[0] != '\0') ? atoi(value) : -1;

    value = getenv("GE007_AI_TRACE_FRAME_MAX");
    g_AiTraceFrameMax = (value != NULL && value[0] != '\0') ? atoi(value) : -1;

    chraiTraceParseListFilter(getenv("GE007_AI_TRACE_LISTS"));

    return g_AiTraceEnabled;
}

static s32 chraiTraceFrameAllowed(void)
{
    extern int g_frame_count_diag;

    if (g_AiTraceFrameMin >= 0 && g_frame_count_diag < g_AiTraceFrameMin)
    {
        return 0;
    }

    if (g_AiTraceFrameMax >= 0 && g_frame_count_diag > g_AiTraceFrameMax)
    {
        return 0;
    }
...
static s32 chraiTraceListAllowed(s32 listID)
{
    s32 i;

    if (g_AiTraceFilterListCount <= 0)
    {
        return 1;
    }

    for (i = 0; i < g_AiTraceFilterListCount; i++)
    {
        if (g_AiTraceFilterLists[i] == listID)
        {
            return 1;
        }
    }
...
static void chraiTraceCommand(const ChrRecord *chr, AIRecord *AIList, s32 Offset)
{
    extern int g_frame_count_diag;
    ...
    s32 len;
    u8 *raw;
    u8 values[5] = { 0, 0, 0, 0, 0 };
    s32 i;

    if (!chraiTraceEnabled())
    {
        return;
    }

    if (!chraiTraceFrameAllowed())
    {
        return;
    }

    if (g_AiTraceBudget == 0)
    {
        return;
    }

    listID = chraiGetAIListID(AIList, &isGlobalAIList);
    if (!chraiTraceListAllowed(listID))
    {
        return;
    }

    len = chraiitemsize(AIList, Offset);
    raw = (u8 *)AIList + Offset;
    for (i = 0; i < 5 && (i + 1) < len; i++)
    {
        values[i] = raw[i + 1];
    }
    ...
    fprintf(stderr,
            "[AI_TRACE] frame=%d chrnum=%d list=0x%04X global=%d offset=%d cmd=0x%02X len=%d vals=%02X:%02X:%02X:%02X:%02X\n",
            g_frame_count_diag,
            chr != NULL ? chr->chrnum : -1,
            (unsigned int)listID,
            isGlobalAIList ? 1 : 0,
...
static void chraiTraceDoorCommand(AIRecord *AIList, const ChrRecord *chr, const char *op, s32 tag, DoorRecord *door)
{
    extern int g_frame_count_diag;
    bool isGlobalAIList = FALSE;
    s32 listID;
    ...
    fprintf(stderr,
            "[AI_TRACE] frame=%d chrnum=%d list=0x%04X op=%s tag=%d found=%d obj=%d obj_type=%d prop_type=%d runtime=0x%08X state=%d openPos=%.3f\n",
            g_frame_count_diag,
            chr != NULL ? chr->chrnum : -1,
            (unsigned int)listID,
            op,
            tag,
            door != NULL ? 1 : 0,
            door != NULL ? door->obj : -1,
            door != NULL ? door->type : -1,
            (door != NULL && door->prop != NULL) ? door->prop->type : -1,
            door != NULL ? (unsigned int)door->runtime_bitflags : 0,
            door != NULL ? door->openstate : -1,
            door != NULL ? door->openPosition : 0.0f);
    fflush(stderr);
}

static void chraiTraceDoorStateCheck(AIRecord *AIList, const ChrRecord *chr, s32 tag, s32 wanted, bool pass, DoorRecord *door)
{
    extern int g_frame_count_diag;
    bool isGlobalAIList = FALSE;
    s32 listID;
    ...
    fprintf(stderr,
            "[AI_TRACE] frame=%d chrnum=%d list=0x%04X op=door_state tag=%d wanted=0x%02X pass=%d found=%d obj=%d obj_type=%d prop_type=%d runtime=0x%08X state=%d openPos=%.6f speed=%.6f maxFrac=%.6f perimFrac=%.6f maxSpeed=%.6f\n",
            g_frame_count_diag,
            chr != NULL ? chr->chrnum : -1,
            (unsigned int)listID,
            tag,
...
#ifdef NATIVE_PORT
static bool chraiDeterministicCloneFromChr(ChrRecord *source, ChrRecord *chr, AIRecord *ailist, s32 clear_seen_gate)
{
    s32 had_seen;
    int chrnum;
    ...
    if (source == NULL || chr == NULL || ailist == NULL || !(chr->chrflags & CHRFLAG_CLONE)) {
        return FALSE;
    }

    had_seen = (chr->chrflags & CHRFLAG_HAS_BEEN_ON_SCREEN) != 0;
    if (clear_seen_gate) {
        chr->chrflags &= ~CHRFLAG_HAS_BEEN_ON_SCREEN;
    }

    cloneprop = chrSpawnAtChr(source, chr->bodynum, -1, chr->chrnum, ailist, 0);
    ...
    if (srcweaponL && srcweaponR && cloneweaponL && cloneweaponR &&
        srcweaponR == srcweaponL->dualweapon &&
        srcweaponL == srcweaponR->dualweapon) {
...
s32 portDeterministicTryCloneChr(s32 source_chrnum,
                                 s32 target_chrnum,
                                 s32 ailist_id,
                                 s32 clear_seen_gate)
{
    ...
    chr = chrFindByLiteralId(target_chrnum);
    if (chr == NULL) {
        return FALSE;
    }
    ...
    if (ailist_id < 0) {
        ailist_id = GAILIST_STANDARD_CLONE;
    }

    ailist = ailistFindById(ailist_id);

    return chraiDeterministicCloneFromChr(source, chr, ailist, clear_seen_gate);
}
#endif
...
PathRecord *pathFindById(s32 ID)
{
    int i;
        ...
        if (g_CurrentSetup.patrolpaths == NULL)
            return NULL;
...
void ai(PropDefHeaderRecord *Entityp, PROP_TYPE EntityType)
{
    ...
    #include "aicommands.def"
                ...
                case AI_GotoNext:
                {
                    AIRecord1 *ai = (void *)((u8 *)AiListp + Offset);
...
#    endif
                ...
                case AI_SetChrAiList:
                {
                    AIRecord  *ai         = (void *)((u8 *)AiListp + Offset);         /* needed for stack count inflation */
                ...
                case AI_PlayAnimation:
                {
                    ...
                    else if (AircraftEntityp)
                    {
                        ...
                        modelSetAnimation(AircraftEntityp->model, ANIM_TABLE_PTR2(anim_id), zero, startframe, 0.5f, (s32)ai->val[7]);
                        if (endframe >= 0)
                        {
                ...
                { // run from bond
                    AIRecord1 *ai = (void *)((u8 *)AiListp + Offset);
                    bool result = removed_animation_routine_27(ChrEntityp);
#ifdef NATIVE_PORT
                    {
                        static int rfb_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && rfb_log < 50) {
                            rfb_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=TRYRunFromBond(0x27) result=%d pad=%d aioff=%d\n",
                                ChrEntityp->chrnum, result, ChrEntityp->padpreset1, Offset);
                            fflush(stderr);
                        }
                    }
#endif
                ...
                {
                    AIRecord1 *ai = (void *)((u8 *)AiListp + Offset);
                    bool result = chrGoToBond(ChrEntityp, SPEED_RUN);
#ifdef NATIVE_PORT
                    {
                        static int rtb_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && rtb_log < 80) {
                            rtb_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=TRYRunToBond(0x28) speed=RUN result=%d seenCount=%d aioff=%d\n",
                                ChrEntityp->chrnum, result, g_SeenBondRecentlyGuardCount, Offset);
                            fflush(stderr);
                        }
                    }
#endif
                ...
                {
                    AIRecord *ai = (void *)((u8 *)AiListp + Offset);
                    bool result = chrGoToBond(ChrEntityp, SPEED_WALK);
#ifdef NATIVE_PORT
                    {
                        static int wtb_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && wtb_log < 50) {
                            wtb_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=TRYWalkToBond speed=WALK result=%d seenCount=%d aioff=%d\n",
                                ChrEntityp->chrnum, result, g_SeenBondRecentlyGuardCount, Offset);
                            fflush(stderr);
                        }
                    }
                ...
                {
                    AIRecord *ai = (void *)((u8 *)AiListp + Offset);
                    bool result = chrGoToBond(ChrEntityp, SPEED_SPRINT);
#ifdef NATIVE_PORT
                    {
                        static int stb_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && stb_log < 50) {
                            stb_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=TRYSprintToBond speed=SPRINT result=%d seenCount=%d aioff=%d\n",
                                ChrEntityp->chrnum, result, g_SeenBondRecentlyGuardCount, Offset);
                            fflush(stderr);
                        }
                    }
                ...
                { //Find Cover
                    AIRecord *ai = (void *)((u8 *)AiListp + Offset);
                    bool result = removed_animation_routine_2B(ChrEntityp);
#ifdef NATIVE_PORT
                    {
                        static int fc_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && fc_log < 20) {
                            fc_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=TRYFindCover(0x2B) result=%d pad=%d aioff=%d\n",
                                ChrEntityp->chrnum, result, ChrEntityp->padpreset1, Offset);
                            fflush(stderr);
                        }
                    }
...
#ifdef NATIVE_PORT
                    s32 taken = ai->val[0] > ChrEntityp->random;
                    s32 next_offset = taken
                        ? chraiGoToLabel(AiListp, Offset, ai->val[1])
                        : Offset + AI_IFRandomLessThan_LENGTH;

                    pcChraiTraceRamromAiDecision(
                        "ai_if_random_less_than",
                        ChrEntityp,
                        Offset,
                        AI_IFRandomLessThan,
                        ai->val[0],
...
#ifdef NATIVE_PORT
                    {
                        static int rtp_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && rtp_log < 50) {
                            rtp_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=RunToPad pad=%d speed=RUN seenCount=%d\n",
                                ChrEntityp->chrnum, pad, g_SeenBondRecentlyGuardCount);
                            fflush(stderr);
                        }
                    }
#endif
...
#ifdef NATIVE_PORT
                    {
                        static int rtpp_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && rtpp_log < 50) {
                            rtpp_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=RunToPadPreset pad=%d speed=RUN seenCount=%d\n",
                                ChrEntityp->chrnum, ChrEntityp->padpreset1, g_SeenBondRecentlyGuardCount);
                            fflush(stderr);
                        }
                    }
#endif
...
#ifdef NATIVE_PORT
                    {
                        static int wtp_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && wtp_log < 50) {
                            wtp_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=WalkToPad pad=%d speed=WALK seenCount=%d\n",
                                ChrEntityp->chrnum, pad, g_SeenBondRecentlyGuardCount);
                            fflush(stderr);
                        }
                    }
...
#ifdef NATIVE_PORT
                    {
                        static int stp_log = 0;
                        if (pcChraiAiMoveTraceEnabled() && stp_log < 50) {
                            stp_log++;
                            fprintf(stderr,
                                "[AI_MOVE] chrnum=%d op=SprintToPad pad=%d speed=SPRINT seenCount=%d\n",
                                ChrEntityp->chrnum, pad, g_SeenBondRecentlyGuardCount);
                            fflush(stderr);
                        }
                    }
                ...
                case AI_MoveObject: //canonicly aiMoveObj
                {
                    ...
                    if (obj && obj->prop)
                    {
                        ...
                        if (obj->model)
                        {
                            matrix_scalar_multiply(obj->model->scale, (f32 *)&matrix);
                        }
                        sub_GAME_7F04088C(obj, (coord3d *)&pad->pos, &matrix, pad->stan, (coord3d *)&pad->pos);
                ...
                {
                    AIRecord   *ai  = (void *)((u8 *)AiListp + Offset);
                    DoorRecord *obj = (DoorRecord *)objFindByTagId(ai->val[0]);
#ifdef NATIVE_PORT
                    chraiTraceDoorCommand(AiListp, ChrEntityp, "door_open", ai->val[0], obj);
#endif
...
#ifdef NATIVE_PORT
                        chraiTraceDoorCommand(AiListp, ChrEntityp, "door_open_after", ai->val[0], obj);
#endif
                    }
                ...
                case AI_DoorClose:
                {
                    ...
                    DoorRecord *obj = (DoorRecord *)objFindByTagId(ai->val[0]);
#ifdef NATIVE_PORT
                    chraiTraceDoorCommand(AiListp, ChrEntityp, "door_close", ai->val[0], obj);
#endif
                    if (obj && obj->prop && obj->prop->type == PROP_TYPE_DOOR)
                    {
                        //DoorRecord *door = (DoorRecord *)obj;
...
#ifdef NATIVE_PORT
                        chraiTraceDoorCommand(AiListp, ChrEntityp, "door_close_after", ai->val[0], obj);
#endif
                    }
...
#ifdef NATIVE_PORT
                        chraiTraceDoorStateCheck(AiListp, ChrEntityp, ai->val[0], ai->val[1], pass, door);
#endif
                    }
#ifdef NATIVE_PORT
                    else
                    {
                        chraiTraceDoorStateCheck(AiListp, ChrEntityp, ai->val[0], ai->val[1], pass, NULL);
                    }
#endif
                    if (pass)
                    {
                ...
                case AI_SetMySpeedRating:
                {   AIRecord1s *ai = (void *)((u8 *)AiListp + Offset);
#ifdef DEBUG
                    /*
                ...
                {
                    AIRecord *ai   = (void *)((u8 *)AiListp + Offset);
                    char     *text = (char *)langGet(CharArrayTo16(ai->val,0));
#ifdef ENABLE_LOG
...
#        endif
                ...
                case AI_TvChangeScreenBank:
                {
                    ...
                    if (obj && obj->prop)
                    {
                        if (obj->type == PROPDEF_MONITOR)
                        {
                            ...
                            monitorSetImageByNum(&sm->Monitor, ai->val[2]);
                        }
                        else if (obj->type == PROPDEF_MULTI_MONITOR)
                        {
                            ...
                            if (slot < 4)
                            {
                                ...
                                monitorSetImageByNum(&mm->Monitor[slot], ai->val[2]);
                            }
                        }
                    }
                ...
                case AI_CameraSwitch:
                {
                    ...
                    if (tag)
                    {
                        int TagIndex = tagGetCommandIndex((PropDefHeaderRecord *)tag); //get index
                        if (TagIndex >= 0)
                        {
                ...
                {
                    AIRecord   *ai   = (void *)((u8 *)AiListp + Offset);
                    DoorRecord *door = (DoorRecord *)objFindByTagId(ai->val[0]);
                    if (door && door->prop)
                    {
                        //DoorRecord *door   = (DoorRecord *)obj;
                        door->speed        = 0;
