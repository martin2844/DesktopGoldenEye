...
#ifdef NATIVE_PORT
extern s32 texLoadFromGdl(Gfx *src, s32 size, Gfx *dst, struct texpool *pool);
extern void fileSetSize(s32 filenum, u8 *ptr, u32 size, s32 reallocate);
#endif
...
void sub_GAME_7F0762E0(ModelFileHeader *arg0, char *arg1, void *arg2, struct texpool *arg3)
{
    ModelNode *sp74;
    Gfx *gdl; // sp6C
    uintptr_t sp58;
    ...
    sp74 = NULL;
    modelIterateDisplayLists(arg0, &sp74, &gdl);
    ...
    if (gdl)
    {
        ...
        temp_a2 = ((uintptr_t)temp_fp - ((uintptr_t)arg0->Switches + ((uintptr_t)gdl & 0xFFFFFFu))) + (uintptr_t)sp54;
        temp_a1 = ((uintptr_t)temp_s0 + (uintptr_t)sp54) - temp_a2;
        sp58 = temp_a1 - ((uintptr_t)arg0->Switches + ((uintptr_t)gdl & 0xFFFFFFu));
        
        texCopyGdls((Gfx *)((uintptr_t)arg0->Switches + ((uintptr_t)gdl & 0xFFFFFFu)),
                    (Gfx *)temp_a1,
                    (s32)temp_a2);
        ...
        while (sp74 != NULL)
        {
            ...
            {
                var_s2 = 0;
                var_s1 = (uintptr_t)gdl & 0xFFFFFFu;
            }
            else
            {
                var_s1 = 0;
                var_s2 = temp_fp - (s32)var_s1;
            }
            
            modelNodeReplaceGdl(0, sp74, gdl, var_s3);
            
            var_s3 += texLoadFromGdl(
                (Gfx *)((uintptr_t)arg0->Switches + (uintptr_t)var_s1 + sp58),
                var_s2,
                (Gfx *)((uintptr_t)arg0->Switches + ((uintptr_t)var_s3 & 0xFFFFFFu)),
                arg3);
        }
        ...
        fileSetSize(
            sp50,
            (u8*)sp54,
            (((u32)(((uintptr_t)arg0->Switches + ((uintptr_t)var_s3 & 0xFFFFFFu)) - (uintptr_t)sp54)) + 0xf) & ~0xf,
            arg2 == NULL);
    }
}
#else
...
*/
void load_object_fill_header(struct ModelFileHeader *objheader, const char *name, u8* dst, s32 size, struct texpool * buffer)
{
    void *filedata;
    ...
    {
#ifdef NATIVE_PORT
        /* On PC, use DEFAULT mode so mempAddEntryOfSizeToBank shrinks the
         * allocation to the decompressed size.  EXTRAMEM (0) consumes the
         * entire remaining bank in one shot, leaving zero bytes for any
         * subsequent load into the same bank — the second+ file would get
         * bytes=0, causing load_resource to do a raw romCopy without
         * decompression (returning 0x1172 compressed data). */
        filedata = _fileNameLoadToBank(name, FILELOADMETHOD_DEFAULT, 0x100, 4);
#else
...
#ifdef NATIVE_PORT
    /* PC 64-bit: N64 model binary has 32-bit pointers, struct layout doesn't
     * match our 64-bit C structs. Use the model converter to create native
     * structs from the big-endian binary data. */
    {
        ...
        }
        {
            ...
            objheader->debugName = (const char *)name;
            (void)modelGetRenderPosCountForObject(objheader);
            /* modelConvertN64Binary sets RootNode, Switches, Textures, n64_filedata */
            /* Register the file buffer as N64 DL region so the GBI interpreter
             * uses the N64 DL path for model display lists (big-endian data). */
            gfx_register_n64_dl_region(filedata, fsize);
            /* Guard DL region registration is done surgically in chrTickBeams
             * using obj->n64_filedata — only character models, not all props. */
        }
    }
    /* Skip sub_GAME_7F0762E0 (texture/DL copy) — has extensive (s32) pointer
     * casts. Model DL data stays in the original file buffer as big-endian
     * and is processed by our N64 DL interpreter. */
#else
...
{
   load_object_fill_header(header,(const char *)recallstring,targetloc,sizeleft,0);
   return;
}
