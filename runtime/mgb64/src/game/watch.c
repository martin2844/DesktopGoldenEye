...
#ifdef NATIVE_PORT
typedef uintptr_t watch_font_handle_t;
#else
typedef s32 watch_font_handle_t;
#endif

typedef u8 *watch_text_ptr_t;

#ifdef NATIVE_PORT
static void watchComposeScaledMatrix(const Mtx *base, f32 sx, f32 sy, f32 sz, Mtxf *dst)
{
    Mtxf scale;
    ...
    scale.m[1][1] = sy;
    scale.m[2][2] = sz;
    matrix_4x4_multiply((Mtxf *)(uintptr_t)base, &scale, dst);
}

static struct fontchar *watchFontChars(watch_font_handle_t handle)
{
    return (struct fontchar *)(uintptr_t)handle;
}

static struct font *watchFontFile(watch_font_handle_t handle)
{
    return (struct font *)(uintptr_t)handle;
}

static void watchTextMeasure(s32 *textheight, s32 *textwidth, const void *text, watch_font_handle_t chars, watch_font_handle_t file, s32 lineheight)
{
    textMeasure(textheight, textwidth, (char *)text, watchFontChars(chars), watchFontFile(file), lineheight);
}

static Gfx *watchTextRender(Gfx *gdl, s32 *x, s32 *y, const void *text, watch_font_handle_t chars, watch_font_handle_t file, s32 arg6, s16 view_x, s16 view_y, s32 arg9, s32 arga)
{
    return textRender(gdl, x, y, (char *)text, chars, file, arg6, view_x, view_y, arg9, arga);
}

static Gfx *watchTextRenderGlow(Gfx *gdl, s32 *x, s32 *y, const void *text, watch_font_handle_t chars, watch_font_handle_t file, s32 arg6, u32 arg7, s16 view_x, s16 view_y, s32 arga, s32 argb)
{
    return textRenderGlow(gdl, x, y, (s8 *)text, chars, file, arg6, arg7, view_x, view_y, arga, argb);
}

static void watchWrapText(s32 width, const void *src, void *dst, watch_font_handle_t chars, watch_font_handle_t file)
{
    sub_GAME_7F0AEB64(width, (s8 *)src, (s8 *)dst, watchFontChars(chars), watchFontFile(file));
}

#define textMeasure(textheight, textwidth, text, font1, font2, lineheight) \
    watchTextMeasure(textheight, textwidth, text, (watch_font_handle_t)(font1), (watch_font_handle_t)(font2), lineheight)
#define textRender(gdl, x, y, text, second_font_table, first_font_table, arg6, view_x, view_y, arg9, arga) \
    watchTextRender(gdl, x, y, text, (watch_font_handle_t)(second_font_table), (watch_font_handle_t)(first_font_table), arg6, view_x, view_y, arg9, arga)
#define textRenderGlow(gdl, x, y, text, second_font_table, first_font_table, arg6, arg7, view_x, view_y, arga, argb) \
    watchTextRenderGlow(gdl, x, y, text, (watch_font_handle_t)(second_font_table), (watch_font_handle_t)(first_font_table), arg6, arg7, view_x, view_y, arga, argb)
#define sub_GAME_7F0AEB64(arg0, arg1, arg2, arg3, arg4) \
    watchWrapText(arg0, arg1, arg2, (watch_font_handle_t)(arg3), (watch_font_handle_t)(arg4))
#endif

extern int ge007_sprintf(char *dst, const char *fmt, ...);

static size_t watchBoundedLen(const char *text, size_t max_len)
{
    size_t len = 0;
...
static void watchCopyText(char *dst, size_t dst_size, const u8 *src)
{
    size_t i;
...
static s32 watchAppendText(char *dst, size_t dst_size, const u8 *src)
{
    size_t dst_len;
    size_t room;
    size_t copy_len;
    ...
    dst_len = watchBoundedLen(dst, dst_size);
    ...
    copy_len = watchBoundedLen((const char *)src, room);
    memcpy(dst + dst_len, src, copy_len);
    dst[dst_len + copy_len] = '\0';

    return copy_len == room && src[copy_len] != '\0';
}
...
#endif
void sub_GAME_7F0A611C(f32 *arg0, s32 *arg1, s32 arg2, s32 *arg3, s32 *arg4, u32 *arg5, s32 arg6, s32 arg7, s32 arg8);

void set_page_rectangle_colors(s32 watch_screen_index, struct WatchVertex *arg2);
Gfx *draw_watch_mission_status_page(Gfx *gdl, Mtx *mtx);
Gfx *unused_draw_watch_inventory_page(Gfx *gdl, Mtx *mtx);
Gfx *draw_watch_inventory_page(Gfx *gdl, Mtx *mtx);
Gfx *draw_watch_control_options_page(Gfx *gdl, Mtx *mtx);
Gfx *draw_watch_game_options_page(Gfx *gdl, Mtx *mtx);
Gfx *draw_watch_mission_briefing_page(Gfx *gdl, Mtx *mtx);
Gfx *draw_background_health_and_armor_transitioning(Gfx *gdl, Mtx *mtx);
Gfx *draw_background_health_and_armor(Gfx *gdl, Mtx *arg1, s32 zoom_squish);
void sub_GAME_7F0A68D8(u8 *arg0);
void game_option_select_value(u32 *option_ptr, u32 value);

void sub_GAME_7F0A8D40(void *arg0, f32 arg1, s32 arg2);
u16 get_mTrack2Vol(void);
void set_mTrack2Vol(u16 volume);
...
Gfx * sub_GAME_7F0A4B40(Gfx *DL)
{
    ...
    {
        char buffer[0x12];
        watch_font_handle_t pFontFile;
        watch_font_handle_t pFontChars;
        s32 y;
        s32 x;

        pFontFile = ptrFontBankGothic;
        pFontChars = ptrFontBankGothicChars;
        ge007_sprintf(buffer, "%d, %d\n", D_80040B48, D_80040B4C);
...
{
    game_options_entries[PLAYER_OPTION_LOOK].current_value = value;
}
...
void cur_player_set_autoaim(u32 value)
{
    game_options_entries[PLAYER_OPTION_AUTOAIM].current_value = value;
}
...
{
    game_options_entries[PLAYER_OPTION_LOOKAHEAD].current_value = value;
}
...
{
    game_options_entries[PLAYER_OPTION_AIM].current_value = value;
}
...
{
    game_options_entries[PLAYER_OPTION_SIGHT].current_value = value;
}
...
{
    game_options_entries[PLAYER_OPTION_AMMODISPLAY].current_value = value;
}
...
{
    game_options_entries[PLAYER_OPTION_SCREEN].current_value = value;
}
...
void sub_GAME_7F0A5B80(void)
{
    ...
    max_scroll = invItems - 0.5f;
    if (D_800409BC <= invItems - 0.5f)
    {
        max_scroll = D_800409BC;
    }
    D_800409BC = max_scroll;
    if (D_800409BC < -0.5f)
    {
        D_800409BC = -0.5f;
    }
...
void sub_GAME_7F0A611C(f32 *arg0, s32 *arg1, s32 arg2, s32 *arg3, s32 *arg4, u32 *arg5, s32 arg6, s32 arg7, s32 arg8) {
    ...
    /* D-pad/C-button up or stick far up => scroll up by 1 */
    if (joyGetButtonsPressedThisFrame(PLAYER_1, U_CBUTTONS | U_JPAD) || joyGetStickY(PLAYER_1) >= 0x47) {
        val = *arg0;
        if ((s32)val <= 0) {
            goto hold_buttons;
        }
    ...
    /* D-pad/C-button down or stick far down => scroll down by 1 */
    if (joyGetButtonsPressedThisFrame(PLAYER_1, D_CBUTTONS | D_JPAD) || joyGetStickY(PLAYER_1) < -0x46) {
        val = *arg0;
        if ((s32)val >= arg2 - 1) {
            goto hold_buttons;
        }
...
hold_buttons:
    /* Hold up button => slow continuous scroll up */
    if (joyGetButtons(PLAYER_1, U_CBUTTONS | U_JPAD)) {
        val = *arg0;
        if ((s32)val > 0 && arg7) {
            *arg0 = val - 0.1f;
        }
    ...
    /* Hold down button => slow continuous scroll down */
    if (joyGetButtons(PLAYER_1, D_CBUTTONS | D_JPAD)) {
        val = *arg0;
        if ((s32)val < arg2 - 1 && arg7) {
            *arg0 = val + 0.1f;
        }
    }

analog_stick:
    /* Analog stick up: moderate range [0x1f, 0x46) */
    if (joyGetStickY(PLAYER_1) >= 0x1f && joyGetStickY(PLAYER_1) < 0x46) {
        if (*arg1 > 0 && arg7) {
            *arg0 = *arg0 - (f32)joyGetStickY(PLAYER_1) / 300.0f;
            goto helper_scroll;
        }
    }

    /* Analog stick down: moderate range (-0x45, -0x1e] */
    if (joyGetStickY(PLAYER_1) < -0x1e && joyGetStickY(PLAYER_1) >= -0x45) {
        val = *arg0;
        if ((s32)val < arg2 - 1 && arg7) {
            *arg0 = val - (f32)joyGetStickY(PLAYER_1) / 300.0f;
        }
    }

helper_scroll:
    /* Helper function: scroll up */
    if (sub_GAME_7F0A5160()) {
        if (*arg1 > 0 && arg7) {
            *arg0 = *arg0 - 1.0f;
            goto update_ae4;
        }
    ...
    /* Helper function: scroll down */
    if (sub_GAME_7F0A519C()) {
        if (*arg1 < arg2 - 1 && arg7) {
            *arg0 = *arg0 + 1.0f;
        }
    }
...
update_ae4:
    /* Update D_80040AE4 based on stick hold state */
    ...
    /* Clamp *arg0 to valid range [-0.5, numItems - 0.5] */
    upper = (f32)arg2 - 0.5f;
    val = *arg0;
    lower = -0.5f;
    ...
    /* Compute target scroll position and ease toward it */
    ival = (s32)val;
    *arg1 = ival;
    target = (u32)(-ival) * (u32)arg8 + (u32)arg6 * (u32)arg8;
    *arg4 = target;
    scrollval = *arg3;

    if (target < scrollval) {
        *arg3 = scrollval - (scrollval - target) / 3 - 1;
        *arg5 = 0;
    } else if (scrollval < target) {
        *arg3 = scrollval + (target - scrollval) / 3 + 1;
        *arg5 = 0;
    } else {
        *arg5 = 1;
    }

    /* Drift correction: ease *arg0 back toward integer index */
    if (*arg0 > (f32)*arg1 + 0.55f) {
...
#ifdef NONMATCHING
// Initializes watch face tick mark vertices. Computes Y step from the ellipse
// equation sqrt(213444 - D_80040B40^2), then fills 4 entries (2 columns x 2 ticks)
// with positions, zero'd fields, and RGBA color (0, 160, 0, D_80040B08).
...
void sub_GAME_7F0A68D8(u8 *arg0) {
    ...
    ystep = (s32)sqrtf(213444.0f - (f32)(D_80040B40 * D_80040B40));

    for (col = 0; col != 8; col += 4) {
        for (tick = -1; tick != 3; tick += 2) {
            *(s16 *)(arg0 + 0) = (s16)((u32)ystep * (u32)tick);
            *(s16 *)(arg0 + 2) = 0;
            *(s16 *)(arg0 + 4) = (s16)(col + D_80040B40);
            *(s16 *)(arg0 + 6) = 0;
            *(s16 *)(arg0 + 8) = 0;
            *(s16 *)(arg0 + 0xa) = 0;
            arg0[0xc] = 0;
...
void sub_GAME_7F0A6A80(void)
{
    ...
    u32 random_value;
#ifdef NATIVE_PORT
    static s32 auto_watch_page = -2;
    static s32 auto_watch_page_delay = 0;
    static s32 auto_watch_close = -1;
#endif
...
#ifdef NATIVE_PORT
    if (auto_watch_page == -2) {
        const char *page_env = getenv("GE007_AUTO_WATCH_PAGE");
        const char *page_delay_env = getenv("GE007_AUTO_WATCH_PAGE_DELAY");
        const char *close_env = getenv("GE007_AUTO_CLOSE_WATCH");

        auto_watch_page = page_env ? atoi(page_env) : -1;
        auto_watch_page_delay = page_delay_env ? atoi(page_delay_env) : 0;
        auto_watch_close = close_env ? atoi(close_env) : -1;

        if (auto_watch_page < WATCH_INDEX_MISSION_STATUS || auto_watch_page > WATCH_INDEX_MISSION_BRIEFING) {
            auto_watch_page = -1;
        }

        if (auto_watch_page_delay < 0) {
            auto_watch_page_delay = 0;
        }
    }

    if (auto_watch_page >= 0 && g_CurrentPlayer->watch_pause_time >= auto_watch_page_delay) {
        watch_screen_index = (u32)auto_watch_page;
    }

    if (auto_watch_close >= 0 && g_CurrentPlayer->watch_pause_time >= auto_watch_close) {
        set_open_close_solo_watch_menu_to1();
    }
#endif
...
Gfx *draw_background_health_and_armor(Gfx *gdl, Mtx *arg1, s32 zoom_squish)
{
...
#ifdef NATIVE_PORT
    /* 64-bit Gfx commands are 16 bytes each; need ~29 commands max = 464 bytes */
    sp40 = dynAllocate(29 * sizeof(Gfx));
...
#ifdef NATIVE_PORT
    watchComposeScaledMatrix(arg1, 0.25f, 0.25f, 0.25f, barMtx);
    gSPMatrix(gdl++, barMtx, G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW | G_MTX_FLOAT_PORT);
#else
...
#ifdef NATIVE_PORT
        { extern Gfx g_pcArmorBarDL[]; gSPDisplayList(gdl++, g_pcArmorBarDL); }
        { extern Gfx g_pcHealthBarDL[]; gSPDisplayList(gdl++, g_pcHealthBarDL); }
#else
    ...
    */
#ifdef NATIVE_PORT
    watchComposeScaledMatrix((Mtx *)barMtx, 1.0f, 1.0f, scale, pageMtx);
    gSPMatrix(gdl++, pageMtx, G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW | G_MTX_FLOAT_PORT);
#else
    ...
    */
    ...
    gDPSetCombineMode(gdl++, G_CC_SHADE, G_CC_SHADE);
#ifdef NATIVE_PORT
    { extern Gfx g_pcGreenBackdropDL[]; gSPDisplayList(gdl++, g_pcGreenBackdropDL); }
#else
    ...
    */
    ...
    {
        sp48[i].color.rgba[3] = (u8)(s8)g_WatchBackgroundGreen;
        sp44[i].color.rgba[3] = (u8)(s8)g_WatchBackgroundGreen;
    }

    if (g_WatchBackgroundGreen < 0xE0)
    {
        sub_GAME_7F0A68D8((u8 *)&g_CurrentPlayer->buffer_for_watch_static_vertices[0].vtx[0]);

        gDPSetRenderMode(gdl++, G_RM_AA_XLU_SURF, G_RM_AA_XLU_SURF2);
#ifdef NATIVE_PORT
        { extern Gfx g_pcStaticDL[]; gSPDisplayList(gdl++, g_pcStaticDL); }
#else
...
Gfx *draw_background_health_and_armor_transitioning(Gfx *gdl, Mtx *mtx)
{
    return draw_background_health_and_armor(gdl, mtx, 1);
}
...
Gfx *draw_abort_cancel_confirm(Gfx *gdl)
{
    ...
    watch_font_handle_t pFontFile;
    watch_font_handle_t pFontChars;
    watch_text_ptr_t sp54;
    watch_text_ptr_t sp50;
    watch_text_ptr_t sp4C;
...
{
    watch_text_ptr_t txtptr_1;
    watch_text_ptr_t txtptr_2;
    s32 sp64;
    s32 sp60 = 0;
    s32 sp5C = 0;
    s32 sp58;
...
#ifdef NONMATCHING
extern Gfx *sub_GAME_7F0A6EE8(Gfx *gdl);
extern Gfx *sub_GAME_7F06A334(Gfx *gdl);
...
Gfx *draw_current_hand_item_and_ammo(Gfx *gdl) {
    ...
    f32 hpos;
    f32 depth;
    f32 different45angle;
    f32 xrot;
    f32 yrot;
    watch_text_ptr_t firstTitle;
    watch_text_ptr_t secondTitle;
    ...
    curEquippedItem = bondinvGetCurEquippedItem();
    weapontext = bondinvGetTextbyInvIndex(curEquippedItem);
    vpos = bondinvGetVposWatchForIndex(curEquippedItem);
    hpos = bondinvGetHposWatchForIndex(curEquippedItem);
    depth = bondinvGetDepthWatchForIndex(curEquippedItem);
    different45angle = bondinvGetDifferent45AngleForIndex(curEquippedItem);
    xrot = bondinvGetXrotWatchForIndex(curEquippedItem);
    yrot = bondinvGetYrotWatchForIndex(curEquippedItem);
    firstTitle = bondinvGetFirstTitlebyIndex(curEquippedItem);
    secondTitle = bondinvGetSecondTitlebyIndex(curEquippedItem);

    if (get_debug_gunwatchpos_flag()) {
        gitem = &gitem_structs[getCurrentPlayerWeaponId(0)];

        if (joyGetButtons(0, 2)) {
            gitem->watch_pos_y -= 2.0f;
        }
        if (joyGetButtons(0, 1)) {
            gitem->watch_pos_y += 2.0f;
        }
        if (joyGetButtons(0, 4)) {
            gitem->watch_pos_x += 2.0f;
        }
        if (joyGetButtons(0, 8)) {
            gitem->watch_pos_x -= 2.0f;
        }
        if (joyGetButtons(0, 32)) {
            gitem->watch_pos_z *= 0.98000002f;
        }
        if (joyGetButtons(0, 16)) {
    ...
    matrix_4x4_set_rotation_around_y(yrot * 6.2831855f / 360.0f, &mtxf_combined);

    matrix_4x4_set_rotation_around_z(6.2831855f - (xrot * 6.2831855f / 360.0f), &mtxf_temp);
    matrix_4x4_multiply_in_place(&mtxf_temp, &mtxf_combined);

    matrix_4x4_7F059694(&mtxf_temp, depth, vpos, hpos, 0.0f, vpos, hpos, 0.0f, 1.0f, 0.0f);
    matrix_4x4_multiply_in_place(&mtxf_temp, &mtxf_combined);

    gdl = sub_GAME_7F0A6EE8(gdl);

    if (g_WatchBackgroundGreen < 0xE0) {
        gdl = set_enviro_fog_for_items_in_solo_watch_menu(gdl, weapontext, &mtxf_combined, g_WatchBackgroundGreen + 1, 0x64DC6428);
    } else {
    ...
    xpos = 96;
    ypos = 188;
    textMeasure(&textheight, &textwidth, firstTitle, pFontChars, pFontFile, 0);
    gdl = textRender(gdl, &xpos, &ypos, firstTitle, pFontChars, pFontFile, 0xFF00B0, textwidth, textheight, 0, 0);

    ypos = 198;
    textMeasure(&textheight, &textwidth, secondTitle, pFontChars, pFontFile, 0);
    gdl = textRender(gdl, &xpos, &ypos, secondTitle, pFontChars, pFontFile, 0xFF00B0, textwidth, textheight, 0, 0);
...
Gfx *draw_watch_mission_status_page(Gfx *gdl, Mtx *mtx)
{
    gdl = draw_background_health_and_armor(gdl, mtx, 0);
...
#ifdef NONMATCHING
/* String constant normally defined in #else asm block */
const char aDDDF[] = "%d, %d\n%d %f\n";
#if 0 /* PORT_FIXME_STUBS — replaced: enable real inventory page for Dam parity */
Gfx *draw_watch_inventory_page(Gfx *gdl, Mtx *proj_mtx) { return gdl; }
#else
Gfx *draw_watch_inventory_page(Gfx *gdl, Mtx *proj_mtx) {
    Mtx *mtx;
    u16 perspNorm;
    Mtxf mtxf_combined;
    Mtxf mtxf_temp;
    ...
    watch_font_handle_t pFontChars;
    char text_buf[0x7D0];
    char sprintf_buf[0x24];
    watch_font_handle_t pFontChars2;
    s32 textwidth2;
    s32 textheight2;
    watch_text_ptr_t curItemName;
    s32 temp_1;
    s32 yoffset;
    s32 linespacing;
    s32 lineheight;
    s32 inventory_text_truncated;
    GunModelFileRecord *gitem;
    s32 i;

    gdl = draw_background_health_and_armor(gdl, proj_mtx, 0);
    ...
    mtx = dynAllocateMatrix();
    different45angle = bondinvGet45AngleForIndex(g_curWatchItemIndex);
    hpos = bondinvGetHoffsetForIndex(g_curWatchItemIndex);
    vpos = bondinvGetVoffsetForIndex(g_curWatchItemIndex);
    depth = bondinvGetDepthForIndex(g_curWatchItemIndex);
    weapontext = bondinvGetTextbyInvIndex(g_curWatchItemIndex);
    xrot = bondinvGetXrotWatchForIndex(g_curWatchItemIndex);
    yrot = bondinvGetYrotWatchForIndex(g_curWatchItemIndex);
    ...
    cos_depth = cosf(D_80040B14) * depth;
    matrix_4x4_7F059694(&mtxf_temp, cos_depth, vpos, sinf(D_80040B14) * depth + hpos, 0.0f, vpos, hpos, 0.0f, 1.0f, 0.0f);
    matrix_4x4_multiply_in_place(&mtxf_temp, &mtxf_combined);

    gdl = sub_GAME_7F0A6EE8(gdl);
    gdl = set_enviro_fog_for_items_in_solo_watch_menu(gdl, weapontext, &mtxf_combined, 64, 0xA0FFA03C);
    ...
    textwidth = 0;
    text_buf[0] = 0;
    inventory_text_truncated = 0;

    for (i = 0; i < bondinvCountTotalItemsInInv(); i++) {
        if (watchAppendText(text_buf, sizeof(text_buf), (u8 *)bondinvGetNameByIndex(i))) {
            inventory_text_truncated = 1;
            break;
        }
    }

#ifdef NATIVE_PORT
    if (inventory_text_truncated) {
        static s32 warned = 0;

        if (!warned) {
            fprintf(stderr, "[WATCH] Inventory text truncated at item %d\n", i);
            fflush(stderr);
            warned = 1;
        }
    }
    ...
    xpos = 0x4E;
    ypos = 0x8C;
    yoffset = (j_text_trigger ? 14 : 12) * 2 + 0x8D;

    gdl = microcode_constructor(gdl);

    textMeasure(&textheight, &textwidth, text_buf, pFontChars, pFontFile, j_text_trigger ? 14 : 12);

    gdl = microcode_constructor_related_to_menus(gdl, 0x4E, 0x8C, textwidth + 0x4E, (j_text_trigger ? 14 : 12) * 5 + 0x8C, 0);

    linespacing = j_text_trigger ? 14 : 12;
    lineheight = j_text_trigger ? 14 : 12;

    gdl = textRender(gdl, &xpos, &ypos, text_buf, pFontChars, pFontFile, 0xAA00B0, textwidth + 1, linespacing * 5, D_800409B0, lineheight);

    gdl = microcode_constructor_related_to_menus(gdl, 0x4B, yoffset, textwidth + 0x52, (j_text_trigger ? 14 : 12) + yoffset - 2, 0x800050);

    pFontFile = ptrFontBankGothic;
    pFontChars2 = ptrFontBankGothicChars;
    curItemName = bondinvGetNameByIndex(g_curWatchItemIndex);

    ge007_sprintf(sprintf_buf, aDDDF,
        D_800409B0, D_800409B4, g_curWatchItemIndex, (f64)D_800409BC);

    gdl = microcode_constructor(gdl);

    textMeasure(&textheight2, &textwidth2, sprintf_buf, pFontChars2, pFontFile, 0);

    if (D_800409C0 != 0) {
        textMeasure(&textheight2, &textwidth2, curItemName, pFontChars2, pFontFile, j_text_trigger ? 14 : 12);

        xpos = 0x4E;
        ypos = (j_text_trigger ? 14 : 12) * 2 + 0x8C;

        if (D_800409C4 == 0) {
            gdl = textRender(gdl, &xpos, &ypos, curItemName, pFontChars2, pFontFile, 0xA0FFA0F0, textwidth2, 0x64, 0, j_text_trigger ? 14 : 12);
        } else {
            gdl = textRenderGlow(gdl, &xpos, &ypos, curItemName, pFontChars2, pFontFile, -1, 0x7000A0, textwidth2 + 1, 0x64, 0, j_text_trigger ? 14 : 12);
        }
        sub_GAME_7F0A8378();
    }
...
#ifdef NONMATCHING
// Computes vertex positions and colors for watch menu animation frame.
// arg0 is a Vtx-like array (16 bytes per entry, 12 entries).
// arg1 is animation progress (0.0 to 1.0), arg2 is integer offset param.
// Entries 0-3: set RGBA to {32, 64, 32, 224}.
// Entries 4-5: interpolated color only.
// Entries 6-9: interpolated color + x position clamped to min.
// Entries 10-11: interpolated x position clamped to max.
// ido_recomp: register allocation / loop unrolling differences.
void sub_GAME_7F0A8D40(void *arg0, f32 arg1, s32 arg2) {
    u8 *base;
    u8 *ptr;
    s32 i;
    ...
    base = (u8 *)arg0;
    some_offset = (s32)((f32)arg2 * (1.2f - arg1));
    diff = *(s16 *)(base + 0x20) - *(s16 *)(base + 0x40);

    /* Fill RGBA of entries 0-3 */
    ptr = base;
    for (i = 0; i < 4; i++) {
        ptr += 16;
        ptr[-4] = 32;
    ...
    /* Process entries 4-11 */
    ptr = base + 0x40;
    for (i = 4; i != 12; i++) {
        if (i < 10) {
            color_lo = (s32)(48.0f * arg1) + 64;
            color_hi = (s32)(96.0f * arg1) + 128;
            ptr[0xC] = (u8)color_lo;
            ptr[0xD] = (u8)color_hi;
            ptr[0xE] = (u8)color_lo;

            if (i >= 6) {
                interp = (f32)*(s16 *)(base + 0x40)
                       + ((f32)diff + (f32)some_offset) * arg1
                       - (f32)some_offset;
                x = (s16)(s32)interp;
                *(s16 *)ptr = x;
                if (*(s16 *)ptr < *(s16 *)(base + 0x40)) {
                    *(s16 *)ptr = *(s16 *)(base + 0x40);
                }
            }
        } else {
            interp = (f32)*(s16 *)(base + 0x40)
                   + (f32)(diff + some_offset) * arg1
                   + (f32)some_offset;
            x = (s16)(s32)interp;
            *(s16 *)ptr = x;
            if (*(s16 *)ptr > *(s16 *)(base + 0x20)) {
                *(s16 *)ptr = *(s16 *)(base + 0x20);
            }
        }
        ptr += 16;
    }

    *(s16 *)(base + 0x10) = *(s16 *)(base + 0xA0);
    *(s16 *)base = *(s16 *)(base + 0xA0);
}
#else
GLOBAL_ASM(
.late_rodata
...
#ifdef NONMATCHING
// ido_recomp ori scheduling: ori pair at 0x7F0A9080 is crosshatch in target, serial in recomp
Gfx *sub_GAME_7F0A8FEC(Gfx *gdl) {
    u16 vol;
    f32 volFloat;
    struct WatchVertex *vtx;
    struct WatchVertex *rect;
    Gfx *_g;
    ...
    if (watch_item_is_actively_selected != 0 && game_options_index == 1) {
        sub_GAME_7F0A8ED0(&vol);
    }

    volFloat = (f32)vol / 32767.0f;
    sndApplyVolumeAllSfxSlot(vol);

    _g = gdl++;
    _g->words.w0 = 0xB900031D;
    _g->words.w1 = 0x00504240;

    gdl = sub_GAME_7F0A3B40(gdl, (s32 *)OS_K0_TO_PHYSICAL(vtx));

    rect = setup_watch_rectangles(vtx, 0, 0, 600, 20, -299, -205);
    gdl = sub_GAME_7F0A3B40(gdl, (s32 *)OS_K0_TO_PHYSICAL(rect));

    rect = setup_watch_rectangles(rect, 0, 0, 600, 20, -299, -205);
    gdl = sub_GAME_7F0A3B40(gdl, (s32 *)OS_K0_TO_PHYSICAL(rect));

    rect = setup_watch_rectangles(rect, 0, 0, 600, 20, -299, -205);
    sub_GAME_7F0A8D40(vtx, volFloat, 30);
...
#ifdef NONMATCHING
// ido_recomp ori scheduling: ori pair at 0x7F0A9258 is crosshatch in target, serial in recomp
Gfx *sub_GAME_7F0A91C8(Gfx *gdl) {
    ...
    rect = setup_watch_rectangles(vtx, 0, 0, 600, 20, -299, -275);
    gdl = sub_GAME_7F0A3B40(gdl, (s32 *)OS_K0_TO_PHYSICAL(rect));

    rect = setup_watch_rectangles(rect, 0, 0, 600, 20, -299, -275);
    gdl = sub_GAME_7F0A3B40(gdl, (s32 *)OS_K0_TO_PHYSICAL(rect));
...
#ifdef NONMATCHING
Gfx *sub_GAME_7F0A9398(Gfx *gdl, s32 x, s32 y, const void *text, s32 color, s32 glow_yoffset, s32 glow_color, s32 center_flag, s32 bg_flag, s32 bg_color, s32 right_flag) {
    Gfx *_g;
    s32 text_height;
    s32 text_width;
    s32 adjusted_x;
    ...
    textMeasure(&text_height, &text_width, text, (struct fontchar *)ptrFontBankGothicChars, (struct font *)ptrFontBankGothic, 10);
    ...
    x2 = adjusted_x + text_width;
    y2 = y + text_height;

    if (g_WatchBackgroundGreen < 0xE0) {
        _g = gdl++;
        _g->words.w0 = 0xB900031D;
        _g->words.w1 = 0x0050004B;
    } else {
        _g = gdl++;
        _g->words.w0 = 0xB900031D;
        _g->words.w1 = 0x005041C8;
    }

    if (bg_flag != 0) {
        gdl = microcode_constructor_related_to_menus(gdl, adjusted_x - 1, y + glow_yoffset + 1, x2 + 1, y2 + 1, bg_color);
    }
    ...
    if (glow_yoffset == 0) {
        gdl = textRender(gdl, &adjusted_x, &y, text, ptrFontBankGothicChars, ptrFontBankGothic, color, (s16)text_width, (s16)text_height, 0, 10);
    }

    if (glow_yoffset != 0) {
        gdl = textRenderGlow(gdl, &adjusted_x, &y, (s8 *)text, ptrFontBankGothicChars, ptrFontBankGothic, color, (u32)glow_color, (s16)(text_width + 1), (s16)text_height, 0, 10);
    }
...
f32 sub_GAME_7F0A95C4(f32 current, f32 target, f32 divisor)
{
    if (current < target) {
        current += (target - current) / divisor;
    } else if (target < current) {
        current -= (current - target) / divisor;
    }
...
#ifdef NONMATCHING
// Watch menu rotation handler: reads joystick input to rotate menu items.
// Increments a frame counter; once it reaches 100, uses speed-based rotation.
// When actively selected with controller option 1, uses joystick angle directly.
// ido_recomp: register allocation differences.
void sub_GAME_7F0A9684(s32 player_num, s32 *counter, f32 *angle_out, f32 *speed_ptr) {
    s32 stickX;
    s32 count;
    s32 active_input;
    f32 delta;

    stickX = joyGetStickX((s8)player_num);
    active_input = 0;
    if (stickX >= 10) {
        active_input = 1;
    } else {
    ...
    if (active_input && watch_item_is_actively_selected != 0 && controller_options_index == 1) {
        *counter = 0;
        count = 0;
    } else {
        count = *counter;
    ...
    if (count >= 100) {
        delta = -*speed_ptr / 10.0f;
        *angle_out = sub_GAME_7F0A95C4(*angle_out, delta, 4.0f);
    } else {
        if (watch_item_is_actively_selected != 0 && controller_options_index == 1) {
            stickX = joyGetStickX((s8)player_num);
            delta = -(f32)stickX * 0.2f * 6.2831855f / 360.0f;
            *angle_out = sub_GAME_7F0A95C4(*angle_out, delta, 4.0f);
        }
    }
}
...
Gfx *sub_GAME_7F0A97D0(Gfx *gdl) {
    ...
    for (i = 0; i < D_800409D8; i++) {
        watchAppendText(buffer, sizeof(buffer), langGet(D_800409DC[i * 10]));
    }

    if (watch_item_is_actively_selected != 0 && controller_options_index == 0) {
        lineheight = j_text_trigger ? 14 : 10;
        sub_GAME_7F0A611C(
            &g_CurrentPlayer->cur_player_control_type_2,
            &g_CurrentPlayer->cur_player_control_type_0,
            D_800409D8,
            &g_CurrentPlayer->neg_vspacing_for_control_type_entry,
            &g_CurrentPlayer->cur_player_control_type_1,
            &g_CurrentPlayer->has_set_control_type_data,
            0,
            1,
            lineheight
        );
    ...
    textMeasure(&textheight, &textwidth, buffer, (struct fontchar *)pFontChars, (struct font *)pFontFile, lineheight);

    lineheight = j_text_trigger ? 14 : 10;
    lineheight2 = j_text_trigger ? 14 : 10;

    gdl = textRender(gdl, &x, &y, buffer, pFontChars, pFontFile, 0x00AA00B0, textwidth, lineheight, g_CurrentPlayer->neg_vspacing_for_control_type_entry, lineheight2);

    if (g_CurrentPlayer->has_set_control_type_data != 0) {
        selectedText = (char *)langGet(D_800409DC[g_CurrentPlayer->cur_player_control_type_0 * 10]);

        lineheight = j_text_trigger ? 14 : 10;

        textMeasure(&textheight, &textwidth, selectedText, (struct fontchar *)pFontChars, (struct font *)pFontFile, lineheight);
        ...
        gdl = textRender(gdl, &x, &y, selectedText, pFontChars, pFontFile, 0xA0FFA0F0, textwidth, 100, 0, lineheight);
    }
...
#ifdef NONMATCHING
/* String constants normally defined in #else asm block */
const char aA[] = "(A)\n";
const char aB[] = "(B)\n";
const char aZ[] = "(Z)\n";
const char aL[] = "(L)\n";
...
const char aS_0[] = "(S)\n";
const char a3d[] = "(3D)\n";
#if 0 /* PORT_FIXME_STUBS — replaced: enable real controls page for Dam parity */
Gfx *sub_GAME_7F0A9AB8(Gfx *gdl) { return gdl; }
#else
Gfx *sub_GAME_7F0A9AB8(Gfx *gdl) {
    ...
    s32 move_sight_flag;
    s32 y_offset;
    watch_text_ptr_t textptr_dpad;
    watch_text_ptr_t textptr_cbutton;
    watch_text_ptr_t text_up;
    watch_text_ptr_t text_down;
    s32 ctrl_type;

    move_sight_flag = 0;

    if (game_options_entries[0].current_value == 1) {
        text_up = langGet(getStringID(LOPTIONS, OPTION_STR_2D_UP_LF));
        text_down = langGet(getStringID(LOPTIONS, OPTION_STR_2C_DOWN_LF));
    } else {
        text_up = langGet(getStringID(LOPTIONS, OPTION_STR_2C_DOWN_LF));
        text_down = langGet(getStringID(LOPTIONS, OPTION_STR_2D_UP_LF));
    }
    ...
    *(s32 *)buf_c = *(s32 *)aC;
    buf_c[4] = aC[4];
    *(s32 *)buf_plus = *(s32 *)D_800577A8;
    buf_plus[4] = D_800577A8[4];
    *(s32 *)buf_s = *(s32 *)aS_0;
    buf_s[4] = aS_0[4];
    *(s32 *)buf_3d = *(s32 *)a3d;
    *(u16 *)(buf_3d + 4) = *(u16 *)(a3d + 4);

    gdl = microcode_constructor(gdl);

    /* L_TRIG - D_800409DC column 4 */
    if (joyGetButtons(PLAYER_1, L_TRIG)) {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 50, 82, langGet(D_800409DC[ctrl_type * 10 + 4]),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 0);
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        if (D_800409DC[ctrl_type * 10 + 4] == getStringID(LOPTIONS, OPTION_STR_01_AIM_LF)) {
            move_sight_flag = 1;
        }
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 50, 82, langGet(D_800409DC[ctrl_type * 10 + 4]),
            0xAA00B0, 0, -1, 0, 0, 0x3000B0, 0);
    }

    /* D-pad / D_800409DC column 7 */
    y_offset = 107;

    if (controller_options_index == CONTROLLER_OPTIONS_INDEX_INPUTS &&
        watch_item_is_actively_selected != 0 &&
        joyGetButtons(PLAYER_1, U_JPAD | D_JPAD | L_JPAD | R_JPAD)) {

        /* Check individual d-pad directions */
        if (joyGetButtons(PLAYER_1, U_JPAD)) {
            ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
            textptr_dpad = text_up;
            if (D_800409DC[ctrl_type * 10 + 7] == getStringID(LOPTIONS, OPTION_STR_05_MOVE_LF)) {
                textptr_dpad = langGet(getStringID(LOPTIONS, OPTION_STR_30_FORWARD_LF));
            }
        } else if (joyGetButtons(PLAYER_1, D_JPAD)) {
            ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
            textptr_dpad = text_down;
            if (D_800409DC[ctrl_type * 10 + 7] == getStringID(LOPTIONS, OPTION_STR_05_MOVE_LF)) {
                textptr_dpad = langGet(getStringID(LOPTIONS, OPTION_STR_31_BACK_LF));
            }
        } else if (joyGetButtons(PLAYER_1, L_JPAD)) {
            textptr_dpad = langGet(getStringID(LOPTIONS, OPTION_STR_2F_SIDESTEP_LF));
        } else if (joyGetButtons(PLAYER_1, R_JPAD)) {
            textptr_dpad = langGet(getStringID(LOPTIONS, OPTION_STR_2E_SIDESTEP_LF));
        }

        gdl = sub_GAME_7F0A9398(gdl, 50, y_offset, (char *)textptr_dpad,
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 0);
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 50, y_offset, langGet(D_800409DC[ctrl_type * 10 + 7]),
            0xAA00B0, 0, -1, 0, 0, 0x3000B0, 0);
    }

    /* D_800409DC column 8 - pause */
    y_offset += 25;
    ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
    gdl = sub_GAME_7F0A9398(gdl, 50, y_offset, langGet(D_800409DC[ctrl_type * 10 + 8]),
        0xAA00B0, 0, -1, 0, 0, 0x3000B0, 0);

    /* Z_TRIG - D_800409DC column 3 */
    y_offset += 25;
    if (joyGetButtons(PLAYER_1, Z_TRIG)) {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 50, y_offset, langGet(D_800409DC[ctrl_type * 10 + 3]),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 0);
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        if (D_800409DC[ctrl_type * 10 + 3] == getStringID(LOPTIONS, OPTION_STR_01_AIM_LF)) {
            move_sight_flag = 1;
        }
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
    ...
    /* R_TRIG - D_800409DC column 5 */
    y_offset = y_offset - 75;
    if (joyGetButtons(PLAYER_1, R_TRIG)) {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 5]),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 1);
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        if (D_800409DC[ctrl_type * 10 + 5] == getStringID(LOPTIONS, OPTION_STR_01_AIM_LF)) {
            move_sight_flag = 1;
        }
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 5]),
            0xAA00B0, 0, -1, 0, 0, 0x3000B0, 1);
    }

    /* C buttons - D_800409DC column 6 */
    y_offset += 25;
    if (controller_options_index == CONTROLLER_OPTIONS_INDEX_INPUTS &&
        watch_item_is_actively_selected != 0 &&
        joyGetButtons(PLAYER_1, U_CBUTTONS | D_CBUTTONS | L_CBUTTONS | R_CBUTTONS)) {

        {
            s32 cbuttons;
            cbuttons = joyGetButtons(PLAYER_1, U_CBUTTONS | D_CBUTTONS | L_CBUTTONS | R_CBUTTONS);
            if (cbuttons & (cbuttons - 1)) {
                /* Multiple C buttons pressed - show dimmed */
                goto cbutton_multi;
            }
        }

        /* Check individual C buttons */
        if (joyGetButtons(PLAYER_1, U_CBUTTONS)) {
            ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
            textptr_cbutton = text_up;
            if (D_800409DC[ctrl_type * 10 + 6] == getStringID(LOPTIONS, OPTION_STR_05_MOVE_LF)) {
                textptr_cbutton = langGet(getStringID(LOPTIONS, OPTION_STR_30_FORWARD_LF));
            }
        } else if (joyGetButtons(PLAYER_1, D_CBUTTONS)) {
            ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
            textptr_cbutton = text_down;
            if (D_800409DC[ctrl_type * 10 + 6] == getStringID(LOPTIONS, OPTION_STR_05_MOVE_LF)) {
                textptr_cbutton = langGet(getStringID(LOPTIONS, OPTION_STR_31_BACK_LF));
            }
        } else if (joyGetButtons(PLAYER_1, L_CBUTTONS)) {
            textptr_cbutton = langGet(getStringID(LOPTIONS, OPTION_STR_2F_SIDESTEP_LF));
        } else if (joyGetButtons(PLAYER_1, R_CBUTTONS)) {
            textptr_cbutton = langGet(getStringID(LOPTIONS, OPTION_STR_2E_SIDESTEP_LF));
        }

        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, (char *)textptr_cbutton,
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 1);
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 6]),
            0xAA00B0, 0, -1, 0, 0, 0x3000B0, 1);
    }
    goto after_cbutton;
cbutton_multi:
...
after_cbutton:
    ...
    /* B_BUTTON - D_800409DC column 2 (action) */
    y_offset += 25;
    if (joyGetButtons(PLAYER_1, B_BUTTON)) {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 2]),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 1);
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 2]),
    ...
    /* A_BUTTON - D_800409DC column 1 (weapon) */
    y_offset += 25;
    if (joyGetButtons(PLAYER_1, A_BUTTON)) {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 1]),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 1);
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 270, y_offset, langGet(D_800409DC[ctrl_type * 10 + 1]),
    ...
    /* Move sight / D_800409DC column 9 */
    if (move_sight_flag != 0) {
        gdl = sub_GAME_7F0A9398(gdl, 250, 195, langGet(getStringID(LOPTIONS, OPTION_STR_08_MOVESIGHT_LF)),
            -1, 1, 0x7000A0, 0, 0, 0x3000B0, 1);
    } else {
        ctrl_type = g_CurrentPlayer->cur_player_control_type_0;
        gdl = sub_GAME_7F0A9398(gdl, 250, 195, langGet(D_800409DC[ctrl_type * 10 + 9]),
            0xAA00B0, 0, -1, 0, 0, 0x3000B0, 1);
    }
...
#ifdef NONMATCHING
extern Gfx *sub_GAME_7F0A9AB8(Gfx *gdl);

Gfx *sub_GAME_7F0AADC0(Gfx *gdl) {
    Mtx *mtx;
    Mtxf mtxf_identity;
    Mtxf mtxf_rot_z;
    Mtxf mtxf_rot_x;
    ...
    /* On N64, D_80040B5C/B60/B64 are consecutive in BSS so a 12-byte
     * struct copy works.  On PC they may not be adjacent (ASan redzones). */
    position.f[0] = D_80040B5C;
    position.f[1] = D_80040B60;
    position.f[2] = D_80040B64;

    flag2 = 0;
    flag1 = 1;

    for (i = 0; i < 55; i++) {
        vertices1[i] = D_80040B68[i];
    }
    for (i = 0; i < 55; i++) {
        vertices2[i] = D_80040C44[i];
    }
    for (i = 0; i < 55; i++) {
        vertices3[i] = D_80040D20[i];
    }

    sub_GAME_7F0A9684(0, &D_80040B2C, &D_80040B28, &D_80040B24);

    matrix_4x4_set_identity(&mtxf_identity);

    D_80040B24 = D_80040B24 + (D_80040B28 * (f32)speedgraphframes * 0.5f);
    D_80040B24 = watchWrapAroundPI(D_80040B24);
    matrix_4x4_set_rotation_around_z(D_80040B24, &mtxf_rot_z);

    if (watch_item_is_actively_selected && controller_options_index == 1) {
        stick_y = joyGetStickY(0);
        D_80040B20 = sub_GAME_7F0A95C4(D_80040B20, (f32)stick_y * 6.2831855f / 360.0f, 4.0f);
    } else {
        D_80040B20 = sub_GAME_7F0A95C4(D_80040B20, 0.0f, 4.0f);
    }

    matrix_4x4_set_rotation_around_x(-D_80040B20 - 0.78539819f, &mtxf_rot_x);
    matrix_4x4_multiply(&mtxf_identity, &mtxf_rot_z, &mtxf_temp1);
    matrix_4x4_multiply(&mtxf_temp1, &mtxf_rot_x, &mtxf_temp2);
    matrix_4x4_set_identity_and_position(&position, &mtxf_temp1);
    matrix_4x4_multiply(&mtxf_temp1, &mtxf_temp2, &mtxf_combined);

    if (controllerCheckDualControllerTypesAllowed()) {
        vertex_data_ptr = vertices2;
        matrix_4x4_7F059694(&mtxf_cam1, 495.0f, 2500.0f, 32.0f, 495.0f, 0.0f, 32.0f, 0.0f, 0.0f, -1.0f);
    } else {
        vertex_data_ptr = vertices1;
        matrix_4x4_7F059694(&mtxf_cam1, -5.0f, 2000.0f, -168.0f, -5.0f, 0.0f, -168.0f, 0.0f, 0.0f, -1.0f);
    }

    matrix_4x4_multiply(&mtxf_cam1, &mtxf_combined, &mtxf_final);

    guPerspective(mtx, &perspNorm, 50.5f, 1.3333334f, 1000.0f, 3000.0f, 1.0f);
    ...
    if (g_WatchBackgroundGreen < 0xE0) {
        gdl = sub_GAME_7F06359C(gdl, &mtxf_final, g_WatchBackgroundGreen - 6, 1, vertex_data_ptr, &flag2);
    } else {
        gdl = sub_GAME_7F064364(gdl, &mtxf_final, 1, vertex_data_ptr, &flag2);
    }
    ...
    if (controllerCheckDualControllerTypesAllowed()) {
        ...
        D_80040B34 = D_80040B34 + (D_80040B38 * (f32)speedgraphframes * 0.5f);
        D_80040B34 = watchWrapAroundPI(D_80040B34);
        matrix_4x4_set_rotation_around_z(D_80040B34, &mtxf_rot_z);

        if (watch_item_is_actively_selected && controller_options_index == 1) {
            stick_y = joyGetStickY(1);
            D_80040B30 = sub_GAME_7F0A95C4(D_80040B30, (f32)stick_y * 6.2831855f / 360.0f, 4.0f);
        } else {
            D_80040B30 = sub_GAME_7F0A95C4(D_80040B30, 0.0f, 4.0f);
        }

        matrix_4x4_set_rotation_around_x(-D_80040B30 - 0.78539819f, &mtxf_rot_x);
        matrix_4x4_multiply(&mtxf_identity, &mtxf_rot_z, &mtxf_temp1);
        matrix_4x4_multiply(&mtxf_temp1, &mtxf_rot_x, &mtxf_temp2);
        matrix_4x4_set_identity_and_position(&position, &mtxf_temp1);
        matrix_4x4_multiply(&mtxf_temp1, &mtxf_temp2, &mtxf_combined);

        matrix_4x4_7F059694(&mtxf_cam2, -505.0f, 2500.0f, 32.0f, -505.0f, 0.0f, 32.0f, 0.0f, 0.0f, -1.0f);

        matrix_4x4_multiply(&mtxf_cam2, &mtxf_combined, &mtxf_final);
        ...
        if (g_WatchBackgroundGreen < 0xE0) {
            gdl = sub_GAME_7F06359C(gdl, &mtxf_final, g_WatchBackgroundGreen - 6, 1, vertices3, &flag1);
        } else {
            gdl = sub_GAME_7F064364(gdl, &mtxf_final, 1, vertices3, &flag1);
        }
    }
...
Gfx *draw_watch_control_options_page(Gfx *gdl, Mtx *mtx) {
    ...
    if (check_watch_page_transistion_running() != 1) {
        ...
        if ((watch_item_is_actively_selected != 0) && (controller_options_index == 0))
        {
            gdl = textRenderGlow(gdl, &sp5C, &sp58, textptr, pFontChars, pFontFile, text_color, 0x7000A0, sp54 + 1, sp50, 0, 0);
        } else
        {
            gdl = textRender(gdl, &sp5C, &sp58, textptr, pFontChars, pFontFile, text_color, sp54, sp50, 0, 0);
        }
...
void game_option_select_value(u32 *option, u32 value)
{
    *option = value;
    set_controlstick_lr_disabled();
...
#ifdef NONMATCHING
#if 0 /* PORT_FIXME_STUBS — replaced: enable real options renderer for Dam parity */
Gfx *sub_GAME_7F0AB908(Gfx *gdl, u32 y_pos, int option_index, s32 select_state) { return gdl; }
#else
Gfx *sub_GAME_7F0AB908(Gfx *gdl, u32 y_pos, int option_index, s32 select_state)
{
  watch_text_ptr_t text_ptr;
  Gfx *gdl_after;
  u32 x_pos;
  ...
  color_opt3 = 0x800080;
    ...
    if (select_state != 0) {
        ...
        if (select_state != 1) {
            ...
            if (select_state != 2) {
                ...
                if (game_options_entries[option_index].current_value == 0) {
                    color_opt1 = 0xa0ffa0f0;
                }
                else {
                    if (game_options_entries[option_index].current_value == 1) {
                        color_opt2 = 0xa0ffa0f0;
                    }
                    else {
                        if (game_options_entries[option_index].current_value == 2) {
                            color_opt3 = 0xa0ffa0f0;
                        }
                    }
                }
            }
    ...
    if (game_options_entries[option_index].text[3] == 0)
    {
        x_pos = 0xC8;
        if (j_text_trigger != 0)
        {
  ...
  text_ptr = langGet(game_options_entries[option_index].text[1]);
  gdl_after = sub_GAME_7F0A9398(gdl, x_pos, y_pos, text_ptr, color_opt1, 0, 0xffffffff,1,0, 0x3000B0,0);

  text_ptr = langGet(game_options_entries[option_index].text[2]);
  gdl_result = sub_GAME_7F0A9398(gdl_after, option2_x_pos, y_pos, text_ptr, color_opt2,0,0xffffffff,1,0, 0x3000B0, 0);

  if (game_options_entries[option_index].text[3] != 0) {
    text_ptr = langGet(game_options_entries[option_index].text[3]);
    gdl_result = sub_GAME_7F0A9398(gdl_result, 0x10e, y_pos, text_ptr, color_opt3,0,0xffffffff,1,0, 0x3000B0, 0);
  }
...
#if defined(LEFTOVERDEBUG)
//D:800577C4
const u32 D_800577C4[521];
//D:80057FC0
const u32 D_80057FC0[296];
#endif
//D:80058440
...
Gfx *draw_watch_mission_briefing_page(Gfx *gdl, Mtx *mtx) {
    ...
    s32 local_E38[2];
    s32 local_E40[15];
    s8 content_buffer[0xBB8];
    char title_buffer[0x20];
    u8 *strComplete;
    u8 *strIncomplete;
    u8 *strFailed;
    u8 *briefing_text;
    ...
    if (check_watch_page_transistion_running() != 1)
    {
        // Copy global animation tables to locals
        memcpy(local_DFC, D_80040DFC, sizeof(local_DFC));
        local_E38[0] = D_80040E38;
        local_E38[1] = D_80040E3C;
        memcpy(local_E40, D_80040E40, sizeof(local_E40));
        ...
        /* N64 debug path: memcpy 0xBB8 bytes from D_800577C0 into adjacent globals.
         * On PC, D_800577C0 is only 2 bytes — this would overflow. Use strcpy. */
        memcpy(content_buffer, D_800577C0, 0xBB8);
#else
        watchCopyText((char *)content_buffer, sizeof(content_buffer), (const u8 *)D_800577C0);
#endif

        strComplete = langGet(44072);
        strIncomplete = langGet(44073);
        strFailed = langGet(44087);

        briefing_text = get_ptr_text_for_watch_breifing_page(BRIEFING_TITLE);

        gdl = microcode_constructor(gdl);

        textMeasure(&textheight, &textwidth, briefing_text, pFontChars, pFontFile, 0);

        // Center the title text
        temp = (170 - textwidth) / 2;
        x = temp + 0x4B;

        y = 30;
        box_x2 = x + textwidth;
        box_y2 = textheight + 0x1E;

        gdl = draw_blackbox_to_screen(gdl, x, y, box_x2, box_y2);

        gdl = textRender(gdl, &x, &y, briefing_text, pFontChars, pFontFile, 0xA0FFA0F0, textwidth, textheight, 0, 0);
        ...
        switch (mission_brief_index)
        {
            case BRIEF_INDEX_BACKGROUND:
                watchCopyText(title_buffer, sizeof(title_buffer), langGet(44088));
                briefing_text = get_ptr_text_for_watch_breifing_page(BRIEFING_OVERVIEW);
                sub_GAME_7F0AEB64(210, briefing_text, content_buffer, pFontChars, pFontFile);
                mission_brief_background_navigation();
                break;

            case BRIEF_INDEX_M:
                watchCopyText(title_buffer, sizeof(title_buffer), langGet(44089));
                briefing_text = get_ptr_text_for_watch_breifing_page(BRIEFING_M);
                sub_GAME_7F0AEB64(210, briefing_text, content_buffer, pFontChars, pFontFile);
                mission_brief_m_briefing_navigation();
                break;

            case BRIEF_INDEX_Q:
                watchCopyText(title_buffer, sizeof(title_buffer), langGet(44090));
                briefing_text = get_ptr_text_for_watch_breifing_page(BRIEFING_Q);
                sub_GAME_7F0AEB64(210, briefing_text, content_buffer, pFontChars, pFontFile);
                mission_brief_q_branch_navigation();
                break;

            case BRIEF_INDEX_MONEYPENNY:
                watchCopyText(title_buffer, sizeof(title_buffer), langGet(44091));
                briefing_text = get_ptr_text_for_watch_breifing_page(BRIEFING_MONEYPENNY);
                sub_GAME_7F0AEB64(210, briefing_text, content_buffer, pFontChars, pFontFile);
            ...
            case BRIEF_INDEX_OBJECTIVES:
#if defined(LEFTOVERDEBUG)
                // Copy objective format buffer
                memcpy(obj_buffer, (u8 *)D_80057FC0 + 0x3B8, 0xC8);
#endif
                s2 = 1;
                s3 = 0;
                letter_index = 0;
                ...
                watchCopyText(title_buffer, sizeof(title_buffer), langGet(44092));

                for (i = 0; i < objectiveGetCount(); i++)
                {
                    obj_difficulty = get_difficulty_for_objective(i);

                    if (lvlGetSelectedDifficulty() < obj_difficulty)
                    {
                        continue;
                    }

                    obj_text = get_text_for_objective(i);
                    s0 = 0;

                    obj_buffer[0] = 0;

                    // Prepend spacing lines (s2 count of " \n")
                    for (s0 = 0; s0 < s2; s0++)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), (const u8 *)D_80058440);
                    }

                    // Prepend spacing lines (s3 count of " \n\n")
                    for (s0 = 0; s0 < s3; s0++)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), (const u8 *)D_80058444);
                    }

                    // Append objective letter prefix
                    obj_buf_ptr = (u8 *)obj_buffer + watchBoundedLen((char *)obj_buffer, sizeof(obj_buffer));
                    {
                        char prefix[8];
                        ge007_sprintf(prefix, aC_2, letter_index + 'a');
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), (const u8 *)prefix);
                    }

                    // Append objective text
                    watchAppendText((char *)obj_buffer, sizeof(obj_buffer), obj_text);

                    // Determine y position for objective text
                    if (j_text_trigger)
                    {
                        obj_y = 1 + y;
                    }
                    ...
                    gdl = textRender(gdl, &obj_x, &obj_y, obj_buffer, pFontChars, pFontFile, 0x00FF00B0, screen_x, viGetY(), 0, 10);

                    s2 += sub_GAME_7F0AC0E8(obj_buf_ptr);

                    briefing_text = sub_GAME_7F0AC120(obj_buf_ptr);

                    // Measure the remaining text to check line overflow
                    textMeasure(&textheight, &textwidth, briefing_text, pFontChars, pFontFile, 10);

                    half_x = viGetX() / 2;

                    if ((textwidth + 0x3C) < half_x)
                    {
                        s2--;
                    }
                    ...
                    for (s0 = 0; s0 < s2; s0++)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), (const u8 *)D_80058450);
                    }

                    for (s0 = 0; s0 < s3; s0++)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), (const u8 *)D_80058454);
                    }

                    status = get_status_of_objective(i);

                    if (status == OBJECTIVESTATUS_INCOMPLETE)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), strIncomplete);
                        obj_color = (D_80040AFC << 16) | 0x400040FF;
                    }
                    else if (status == OBJECTIVESTATUS_COMPLETE)
                    {
                        obj_color = 0xA0FFA0F0;
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), strComplete);
                    }
                    else if (status == OBJECTIVESTATUS_FAILED)
                    {
                        watchAppendText((char *)obj_buffer, sizeof(obj_buffer), strFailed);
                        if (j_text_trigger)
                        {
                            obj_color = 0xA0FFA0F0;
                        }
                        ...
                        {
                            obj_color = 0x00FF00B0;
                        }
                    }

                    textMeasure(&textheight, &textwidth, obj_buffer, pFontChars, pFontFile, 10);
                    ...
                    gdl = textRender(gdl, &obj_x, &obj_y, obj_buffer, pFontChars, pFontFile, obj_color, 210, viGetY(), 0, 10);
        ...
        // Render the bottom title/page label
        textMeasure(&textheight, &textwidth, title_buffer, pFontChars, pFontFile, 10);

        gdl = textRender(gdl, &x, &y, title_buffer, pFontChars, pFontFile, 0xA0FFA0F0, 210, 130, 0, 10);

        // Render the full content buffer as scrollable text
        y += 5;
        x = 60;

        textMeasure(&textheight, &textwidth, content_buffer, pFontChars, pFontFile, 10);

        screen_x = viGetX();

        gdl = textRender(gdl, &x, &y, content_buffer, pFontChars, pFontFile, 0x00FF00B0, screen_x, viGetY(), 0, 10);
    }
