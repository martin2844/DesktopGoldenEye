...
void load_resource(u8 *ptrdata, s32 bytes,  fileentry *srcfile,  resource_lookup_data_entry *lookupdata)
{
    u8 *source;
#ifdef NATIVE_PORT
    /* struct huft is 16 bytes on 64-bit vs 8 on N64 — double the buffer */
    struct huft buffer[0x4200 / sizeof(struct huft)];
#else
    struct huft buffer[0x2100 / sizeof(struct huft)];
#endif
    s32 unused;
...
#ifdef NATIVE_PORT
    if ((intptr_t)(source - ptrdata) < 8)
#else
    if ((u32) (source - ptrdata) < 8U)
#endif
    {
#ifdef NATIVE_PORT
        printf("[LOAD-RES] WARNING: source-ptrdata < 8 (bytes=%d rom_size=%u) → poolRemaining=0\n",
               bytes, lookupdata->rom_size);
#endif
    ...
    else
    {
        ...
        if (result == 0)
        {
            ge007_sprintf(sp54, "DMA-Crash %s %d Ram: %02x%02x%02x%02x%02x%02x%02x%02x", "ob.c", 204, scratch[0], scratch[1], scratch[2], scratch[3], scratch[4], scratch[5], scratch[6], scratch[7]);
...
void resource_load_from_indy(u8 *ptrdata, s32 bytes,  fileentry *srcfile,  resource_lookup_data_entry *lookupdata)
{
    u8 *pPayload;
#ifdef NATIVE_PORT
    struct huft buffer[16896 / sizeof(struct huft)];
#else
    struct huft buffer[8448 / sizeof(struct huft)];
#endif
    s32 size;
    ...
    }
    {
        ...
        indycommHostCheckFileExists(srcfile->filename, &pc_size);
        lookupdata->pc_size = (u32)pc_size;
    }
...
void obLoadBGFileBytesAtOffset(const char *bgname, u8 *target, s32 offset, s32 len)
{
  s32 index;
   fileentry *fileentry;

  index = fileGetIndex((const char *)bgname);
  ...
  if (resource_lookup_data_array[index].rom_size != 0)
  {
    ...
    {
#ifdef NATIVE_PORT
      fprintf(stderr, "[OB] obLoadBGFileBytesAtOffset: file too small (rom_size=%d, offset=%d, len=%d), skipping\n",
              resource_lookup_data_array[index].rom_size, offset, len);
      return;
#else
      while (1){};
#endif
...
{
    return fileIndexLoadToBank(index, loadMethod, size, bank);
}
#endif
...
{
    return fileIndexLoadToAddr(index, loadMethod, ptrdata, size);
}
#endif
...
void obLoadBGFileBytesAtOffset(const char *bgname, u8 *target, s32 offset, s32 len)
{
  ...
  if (resource_lookup_data_array[index].rom_size != 0)
  {
    ...
    {
#ifdef NATIVE_PORT
      fprintf(stderr, "[OB] obLoadBGFileBytesAtOffset(debug): file too small (rom_size=%d, offset=%d, len=%d), skipping\n",
              resource_lookup_data_array[index].rom_size, offset, len);
      return;
#else
      while (1){};
...
#ifdef NATIVE_PORT
    /* On NATIVE_PORT, files with hw_address==0 and rom_size==0 are dev-kit-only
     * (loaded via Indy host communication, unavailable on PC). Skip them. */
    if (file_resource_table[index].hw_address == 0 && info->rom_size == 0) {
        return NULL;
    }
#endif
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
/* Fixed version: original had semicolon after if condition (line 443) */
int fileGetIndex(const char *resname)
{
    int i;
    int size;
    /* if file exists, return its index */
    for (i = 1; i < file_entry_max; i++)
    {
...
s32 fileGetIndex(char *resname) { (void)resname; return 0; }
#else
//needs work
...
{
    resource_lookup_data_entry *entry = &resource_lookup_data_array[1];

    if (file_entry_max > 1)
    {
        do {
            entry++;
        } while (entry < &resource_lookup_data_array[file_entry_max]);
    }
}
