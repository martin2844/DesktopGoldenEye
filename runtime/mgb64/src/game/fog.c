...
void fogLoadCurrentEnvironment(EnvironmentRecord *arg0)
{
    ...
    found_fogless = (256.0f * (0.5f - g_DifferenceFromFarFogIntensity)) / (g_FarFogIntensity - g_DifferenceFromFarFogIntensity);

    g_CurFogDetails.far_fog_dist_scaled = ((pk4 * -sp20 * (pk0 + 1.0f)) / (pk4 - pk0)) / 255.0f;
    g_CurFogDetails.near_fog_dist_scaled = ((sp20 * (pk4 + 1.0f) / (pk4 - pk0)) + found_fogless) / 255.0f;
...
{
    EnvironmentRecord *entry;
    EnvironmentFoglessRecord *fogless_entry;
    EnvironmentFoglessRecord *found_fogless;
    s32 num_players;
    ...
    if (arg1)
    {
        for (entry = &fog_tables[0]; entry->Id != 0; entry++)
        {
            if (entry->Id == (level_id + 900))
            {
                g_EnvironmentFoundp = entry;
                g_EnvironmentMainp = entry;
                g_EnvironmentAltp = entry + 1;
    ...
    for (entry = &fog_tables[0]; entry->Id != 0; entry++)
    {
        if (entry->Id == (level_id + (num_players * 100)))
        {
            g_EnvironmentFoundp = entry;
            g_EnvironmentMainp = entry;
            g_EnvironmentAltp = entry + 1;
    ...
    for (fogless_entry = fog_tables2; fogless_entry->Id != 0; fogless_entry++)
    {
        if (fogless_entry->Id == level_id)
        {
            found_fogless = fogless_entry;
    ...
    if (found_fogless == NULL)
    {
        found_fogless = &fog_tables2[0];
    }

    fogLoadFoglessCurrentEnvironment(found_fogless);
