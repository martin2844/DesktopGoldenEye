...
void sub_GAME_7F092E50(void) {
...
#ifdef VERSION_EU
    timerDelta = g_GlobalTimerDelta;
#else
    timerDelta = (f32)g_ClockTimer;
#endif

    /* Update scroll accumulator 80: wraps within [0, 256) */
    flt_CODE_bss_80079E80 += timerDelta * 0.25f;
    scroll80 = flt_CODE_bss_80079E80;
    if (scroll80 >= wrap256) {
        flt_CODE_bss_80079E80 = scroll80 - wrap256;
        scroll80 = flt_CODE_bss_80079E80;
    }
    if (scroll80 < zero) {
        flt_CODE_bss_80079E80 = scroll80 + wrap256;
    ...
    /* Update scroll accumulator 84: wraps within [0, 256) */
    flt_CODE_bss_80079E84 += timerDelta * 0.1f;
    scroll84 = flt_CODE_bss_80079E84;
    if (scroll84 >= wrap256) {
        flt_CODE_bss_80079E84 = scroll84 - wrap256;
        scroll84 = flt_CODE_bss_80079E84;
    }
    if (scroll84 < zero) {
        flt_CODE_bss_80079E84 = scroll84 + wrap256;
    ...
    /* Update scroll accumulator 88: wraps within [0, 2*pi) */
    {
        f32 twopi = 6.2831802f;
        flt_CODE_bss_80079E88 += timerDelta * 0.04f;
        scroll88 = flt_CODE_bss_80079E88;
        if (scroll88 >= twopi) {
            flt_CODE_bss_80079E88 = scroll88 - twopi;
            scroll88 = flt_CODE_bss_80079E88;
        }
        if (scroll88 < zero) {
            flt_CODE_bss_80079E88 = scroll88 + twopi;
    ...
    /* --- First pass: update MipMap2C_Something_Setup tile scrolling --- */
    gfx1 = MipMap2C_Something_Setup;

    /* Tile 0 SetTileSize: update uls (bits 12-23 of w0) */
    scrollU = (u32)scroll80;
    word = gfx1[2].words.w0;
    gfx1[2].words.w0 = word ^ ((((scrollU ^ (word >> 12)) << 20) >> 8));

    /* Tile 0 SetTileSize: update ult (bits 0-11 of w0 via lower halfword) */
    scrollV = (u32)scroll84;
    half = *(u16 *)((u8 *)gfx1 + 0x12);
    *(u16 *)((u8 *)gfx1 + 0x12) = (half & 0xF000) | (scrollV & 0xFFF);

    /* Tile 1 SetTileSize: update uls via XOR trick */
    word = gfx1[3].words.w0;
    truncU = (s32)scroll80;
    truncV = (s32)scroll84;
    gfx1[3].words.w0 = word ^ (((((truncU + 0x5A) & 0xFF) ^ (word >> 12)) << 20) >> 8);

    /* Tile 1 SetTileSize: update ult via lower halfword */
    half = *(u16 *)((u8 *)gfx1 + 0x1A);
    *(u16 *)((u8 *)gfx1 + 0x1A) = (half & 0xF000) | ((truncV + 0x96) & 0xFF);

    /* Compute prim color LOD fraction from sin of scroll88 */
    alpha = (u32)(sinf(flt_CODE_bss_80079E88) * 127.0f + 128.0f);

    gfx1 = MipMap2C_Something_Setup;
    gfx2 = MipMap2C_Something2_Setup;

    /* Update prim color LOD fraction for MipMap2C_Something_Setup */
    gfx1[4].words.w0 = (gfx1[4].words.w0 & 0xFFFFFF00) | alpha;

    /* --- Second pass: update MipMap2C_Something2_Setup tile scrolling --- */
    scroll80 = flt_CODE_bss_80079E80;

    scrollU = (u32)scroll80;
    word = gfx2[2].words.w0;
    gfx2[2].words.w0 = word ^ ((((scrollU ^ (word >> 12)) << 20) >> 8));

    scroll84 = flt_CODE_bss_80079E84;
    scrollV = (u32)scroll84;
    half = *(u16 *)((u8 *)gfx2 + 0x12);
    *(u16 *)((u8 *)gfx2 + 0x12) = (half & 0xF000) | (scrollV & 0xFFF);

    word = gfx2[3].words.w0;
    truncU = (s32)scroll80;
    truncV = (s32)scroll84;
    gfx2[3].words.w0 = word ^ (((((truncU + 0x5A) & 0xFF) ^ (word >> 12)) << 20) >> 8);

    half = *(u16 *)((u8 *)gfx2 + 0x1A);
    *(u16 *)((u8 *)gfx2 + 0x1A) = (half & 0xF000) | ((truncV + 0x96) & 0xFF);

    /* Second sinf call for MipMap2C_Something2_Setup prim color */
    alpha = (u32)(sinf(flt_CODE_bss_80079E88) * 127.0f + 128.0f);

    /* Update prim color LOD fraction for MipMap2C_Something2_Setup */
    /* Note: reads from MipMap2C_Something_Setup[4].w0 as the mask base */
    gfx2[4].words.w0 = (gfx1[4].words.w0 & 0xFFFFFF00) | alpha;
}
#else
#ifndef VERSION_EU
GLOBAL_ASM(
...
Gfx *sub_GAME_7F09343C(Gfx *gdl, s32 arg1) {
    ...
    if (arg1 != 0) {
        ...
        gdl->words.w0 = 0xF5100800;
        gdl->words.w1 = 0x00014050;
        gdl++;
        gdl->words.w0 = 0xF5100800;
        gdl->words.w1 = 0x01014050;
        gdl++;
        gdl->words.w0 = 0xF2000000;
        gdl->words.w1 = 0x00000000;
        gdl++;
        gdl->words.w0 = 0xF205A096;
        gdl->words.w1 = 0x01000000;
        gdl++;

        alpha = (u32)(sinf(flt_CODE_bss_80079E88) * 127.0f + 128.0f);

        gdl->words.w0 = 0xFA000000 | (alpha & 0xFF);
        gdl->words.w1 = 0xFFFFFFFF;
        gdl++;
        gdl->words.w0 = 0xBA001102;
        gdl->words.w1 = 0x00000000;
        gdl++;
        gdl->words.w0 = 0xBA000C02;
        gdl->words.w1 = 0x00002000;
        gdl++;
        gdl->words.w0 = 0xFC272C04;
        gdl->words.w1 = 0x1F1093FF;
        gdl++;
        gdl->words.w0 = 0xB900031D;
        gdl->words.w1 = 0x0C192078;
        gdl++;
