...
#ifdef NATIVE_PORT
extern void debmenuSetFgColour(s32 r, s32 g, s32 b, s32 a);
extern void debmenuSetPos(s32 x, s32 y);
extern void debmenuPrintString(const unsigned char *str);
#endif
...
Gfx *debugmenuRender(Gfx *gdl)
{
	...
	for (i = 0; i < g_DMenuNumOptions; i++) {
		...
		debmenuPrintString((const unsigned char *)g_DMenuCurLabels[i]);
	}
    #endif
	return gdl;
}
