...
s32 D_80040EA8 = 0;
#ifdef NATIVE_PORT
uintptr_t ptrFontBankGothic = 0;
uintptr_t ptrFontBankGothicChars = 0;
uintptr_t ptrFontZurichBold = 0;
uintptr_t ptrFontZurichBoldChars = 0;
uintptr_t ptrFontBankGothicRaw = 0;
uintptr_t ptrFontZurichBoldRaw = 0;
#else
...
static FILE *g_text_audit_file = NULL;
static int g_text_audit_inited = 0;
static int g_text_audit_remaining = 0;
static int g_text_audit_frame_min = 0;
static int g_text_audit_frame_max = INT_MAX;
static int g_text_audit_glyphs = -1;
static const char *g_text_glyph_dump_dir = NULL;
static int g_text_glyph_dump_inited = 0;

static void text_audit_close(void)
{
    if (g_text_audit_file != NULL) {
        fclose(g_text_audit_file);
        g_text_audit_file = NULL;
    }
}

static int text_env_int(const char *name, int fallback)
{
...
static void text_audit_init(void)
{
    const char *path;

    if (g_text_audit_inited) {
        return;
    }

    g_text_audit_inited = 1;
    path = getenv("GE007_TEXT_AUDIT");

    if (path == NULL || path[0] == '\0') {
        return;
    }

    g_text_audit_file = fopen(path, "w");
    if (g_text_audit_file == NULL) {
        fprintf(stderr, "[TEXT-AUDIT] failed to open %s\n", path);
        return;
    }

    g_text_audit_remaining = text_env_int("GE007_TEXT_AUDIT_BUDGET", 12000);
    if (g_text_audit_remaining <= 0) {
        g_text_audit_remaining = INT_MAX;
    }

    g_text_audit_frame_min = text_env_int("GE007_TEXT_AUDIT_FRAME_MIN", 0);
    g_text_audit_frame_max = text_env_int("GE007_TEXT_AUDIT_FRAME_MAX", INT_MAX);
    atexit(text_audit_close);

    fprintf(g_text_audit_file,
        "{\"scope\":\"text\",\"event\":\"meta\",\"frame\":%d,\"budget\":%d}\n",
        g_frame_count_diag,
        g_text_audit_remaining);
    fflush(g_text_audit_file);
}

static int text_audit_enabled(void)
{
    text_audit_init();

    return g_text_audit_file != NULL &&
        g_text_audit_remaining != 0 &&
        g_frame_count_diag >= g_text_audit_frame_min &&
        g_frame_count_diag <= g_text_audit_frame_max;
}

static int text_audit_glyphs_enabled(void)
{
    if (g_text_audit_glyphs < 0) {
        g_text_audit_glyphs = getenv("GE007_TEXT_GLYPH_AUDIT") != NULL ? 1 : 0;
    }

    return g_text_audit_glyphs != 0;
}

static const char *text_audit_font_name(uintptr_t second_font_table, uintptr_t first_font_table)
{
    (void)first_font_table;

    if (second_font_table == ptrFontBankGothicChars) {
        return "gothic";
    }

    if (second_font_table == ptrFontZurichBoldChars) {
        return "zurich_bold";
    }
...
static void text_audit_write_json_string(FILE *fp, const char *text)
{
    const unsigned char *ptr = (const unsigned char *)text;
    size_t emitted = 0;
    const size_t max_emit = 512;

    fputc('"', fp);

    if (ptr != NULL) {
        while (*ptr != '\0' && emitted < max_emit) {
...
static void text_audit_log_render(const char *kind, const char *text, s32 x, s32 y,
    uintptr_t second_font_table, uintptr_t first_font_table, s32 color,
    u32 glow_color, s16 view_x, s16 view_y, s32 arg9, s32 lineheight)
{
    ...
    fprintf(g_text_audit_file,
        "{\"scope\":\"text\",\"event\":\"render\",\"kind\":\"%s\",\"frame\":%d,"
        "\"x\":%d,\"y\":%d,\"view_x\":%d,\"view_y\":%d,"
        "\"clip_x\":%d,\"lineheight\":%d,\"color\":\"0x%08X\","
        "\"glow\":\"0x%08X\",\"font\":\"%s\",\"text\":",
        kind,
        g_frame_count_diag,
        x,
        y,
        ...
        (u32)color,
        glow_color,
        text_audit_font_name(second_font_table, first_font_table));
    text_audit_write_json_string(g_text_audit_file, text);
    fputs("}\n", g_text_audit_file);
    fflush(g_text_audit_file);
    g_text_audit_remaining--;
}

static void text_audit_log_glyph(const char *kind, const struct fontchar *fontchar,
    s32 x, s32 y)
{
    if (!text_audit_glyphs_enabled() || !text_audit_enabled() || fontchar == NULL) {
        return;
    }

    fprintf(g_text_audit_file,
        "{\"scope\":\"text\",\"event\":\"glyph\",\"kind\":\"%s\",\"frame\":%d,"
        "\"x\":%d,\"y\":%d,\"index\":%u,\"baseline\":%d,"
        "\"width\":%u,\"height\":%u,\"kerningindex\":%d,\"pixeldata\":\"%p\"}\n",
        kind,
        g_frame_count_diag,
        x,
        y,
        ...
        fontchar->width,
        fontchar->height,
        fontchar->kerningindex,
        (void *)fontchar->pixeldata);
    fflush(g_text_audit_file);
    g_text_audit_remaining--;
}

static void text_glyph_dump_init(void)
{
    if (g_text_glyph_dump_inited) {
        return;
    }

    g_text_glyph_dump_inited = 1;
    g_text_glyph_dump_dir = getenv("GE007_DUMP_TEXT_GLYPHS");

    if (g_text_glyph_dump_dir != NULL && g_text_glyph_dump_dir[0] != '\0') {
#ifdef _WIN32
        _mkdir(g_text_glyph_dump_dir);
#else
        mkdir(g_text_glyph_dump_dir, 0755);
#endif
    }
}

static int text_glyph_dump_enabled(void)
{
    text_glyph_dump_init();
    return g_text_glyph_dump_dir != NULL && g_text_glyph_dump_dir[0] != '\0';
}

static void text_dump_font_glyph_pgm(const char *font_name, const struct fontchar *fontchars, int ch)
{
    char path[512];
    FILE *fp;
    const struct fontchar *fontchar;
    u32 width;
    u32 height;
    u32 aligned_width;
    u32 stride;

    if (!text_glyph_dump_enabled() || fontchars == NULL || ch < 0x21 || ch > 0x7e) {
        return;
    }

    fontchar = &fontchars[ch - 0x21];
    width = fontchar->width;
    height = fontchar->height;
    aligned_width = (width + 7) & ~7u;
    stride = aligned_width;

    if (fontchar->pixeldata == NULL || width == 0 || height == 0 || width > 128 || height > 128 || stride == 0) {
        return;
    }

    snprintf(path, sizeof(path), "%s/%s_%02X.pgm", g_text_glyph_dump_dir, font_name, ch);
    fp = fopen(path, "wb");
    if (fp == NULL) {
        return;
    }
    ...
    for (u32 y = 0; y < height; y++) {
        const u8 *row = fontchar->pixeldata + y * stride;
...
static void text_dump_font_glyphs(const char *font_name, const struct fontchar *fontchars)
{
    const char *chars;

    if (!text_glyph_dump_enabled()) {
        return;
    }

    chars = getenv("GE007_DUMP_TEXT_GLYPH_CHARS");
    if (chars != NULL && chars[0] != '\0') {
        while (*chars != '\0') {
            text_dump_font_glyph_pgm(font_name, fontchars, (unsigned char)*chars);
            chars++;
        }
        return;
    }
    ...
    for (int ch = 0x21; ch <= 0x7e; ch++) {
        text_dump_font_glyph_pgm(font_name, fontchars, ch);
    }
}
#endif
 ...
 * (8-byte pointer + alignment padding). We load the raw ROM data, then
 * convert each entry into a proper PC struct, fixing up pointer fields.
 *
 * Font binary layout:
 *   Offset 0x000: s32 kerning[13*13] (676 bytes, big-endian)
 *   Offset 0x2A4: N64 fontchar entries (94 × 24 bytes, big-endian)
 *   Remaining:    pixel data (bytes, no swap needed)
 *
 * N64 fontchar entry (24 bytes):
 *   u32 index, s32 baseline, u32 height, u32 width,
 *   s32 kerningindex, u32 pixeldata_offset
 */

extern u32 _fontbankgothicSegmentRomStart;
extern u32 _fontzurichboldSegmentRomStart;

static void pc_convert_font(u8 *raw_data, s32 raw_size, struct font *out_font, struct fontchar **out_chars) {
    /* Byte-swap kerning table (169 s32 values at offset 0) */
    u32 *raw_kerning = (u32 *)raw_data;
    for (int i = 0; i < 13 * 13; i++) {
        out_font->kerning[i] = (s32)bswap32(raw_kerning[i]);
    }

    /* Convert 94 fontchar entries from 24-byte N64 format to PC struct */
    u8 *raw_chars = raw_data + 0x2A4;
    for (int i = 0; i < 94; i++) {
        u32 *entry = (u32 *)(raw_chars + i * 24);
        out_font->chars[i].index        = bswap32(entry[0]);
        out_font->chars[i].baseline     = (s32)bswap32(entry[1]);
        out_font->chars[i].height       = bswap32(entry[2]);
        out_font->chars[i].width        = bswap32(entry[3]);
        out_font->chars[i].kerningindex = (s32)bswap32(entry[4]);
        u32 pixel_offset = bswap32(entry[5]);
        out_font->chars[i].pixeldata    = raw_data + pixel_offset;

        /* The ROM glyphs are stored on 8-byte row boundaries, but the padding
         * bytes to the right of the visible glyph are not guaranteed to be
         * blank. The N64's texrect path samples the authored width exactly;
         * the PC backend is slightly less forgiving, so sanitize the padding
         * in the converted stage copy. */
        {
            u32 width = out_font->chars[i].width;
            u32 height = out_font->chars[i].height;
            u32 alignw = (width + 7) & ~7u;
            if (alignw > width) {
                for (u32 row = 0; row < height; row++) {
                    memset(raw_data + pixel_offset + row * alignw + width, 0, alignw - width);
                }
            }
        }
...
void load_font_tables(void) {
    ...
    pc_convert_font(gothic_raw, gothic_size, gothic, &gothic_chars);

    ptrFontBankGothic      = (uintptr_t)gothic;
    ptrFontBankGothicChars = (uintptr_t)gothic_chars;
    ptrFontBankGothicRaw   = (uintptr_t)gothic_raw;
    text_dump_font_glyphs("gothic", gothic_chars);
    ...
    pc_convert_font(zurich_raw, zurich_size, zurich, &zurich_chars);

    ptrFontZurichBold      = (uintptr_t)zurich;
    ptrFontZurichBoldChars = (uintptr_t)zurich_chars;
    ptrFontZurichBoldRaw   = (uintptr_t)zurich_raw;
    text_dump_font_glyphs("zurich_bold", zurich_chars);
}
...
loop_1:
    ...
    offset = 0x30;
loop_3:
    temp_v0_2 = ptrFontZurichBoldChars + offset;
    temp_v0_2->unk14 = (s32) (temp_v0_2->unk14 + ptrFontZurichBold);
    temp_v0_3 = ptrFontZurichBoldChars + offset;
    temp_v0_3->unk2C = (s32) (temp_v0_3->unk2C + ptrFontZurichBold);
    temp_v0_4 = ptrFontZurichBoldChars + offset;
    temp_v0_4->unk44 = (s32) (temp_v0_4->unk44 + ptrFontZurichBold);
    temp_v0_5 = ptrFontZurichBoldChars + offset;
    temp_v1_2 = offset + 0x60;
...
Gfx *microcode_constructor_related_to_menus(Gfx *gdl, s32 arg1, s32 arg2, s32 arg3, s32 arg4, s32 color) {
    (void)arg1; (void)arg2; (void)arg3; (void)arg4; (void)color;
    return gdl;
}
#elif defined(NATIVE_PORT)
/**
 * Draws a colored rectangle overlay using RDP commands.
 * color is packed RGBA (R in high byte, A in low byte).
 */
Gfx *microcode_constructor_related_to_menus(Gfx *gdl, s32 arg1, s32 arg2, s32 arg3, s32 arg4, s32 color) {
    ...
    /* SetCombineLERP: TEXEL0*PRIM blend mode for subsequent text rendering */
    gdl->words.w0 = 0xFCFF97FF;
    gdl->words.w1 = 0xFF2DFEFF;
    gdl++;
    return gdl;
}
#else
...
#ifdef NONMATCHING
#ifdef NATIVE_PORT
Gfx * construct_fontchar_microcode(Gfx *DL, s32 *pxpos, s32 *pypos, void *ptr_tbl2entry, void *ptr_unk, void *ptr_tbl1, int ulx, int uly, int lrx, int lry, int unk) {
#else
Gfx * construct_fontchar_microcode(Gfx *DL, int xpos, int ypos, void *ptr_tbl2entry, void *ptr_unk, void *ptr_tbl1, int ulx, int uly, int lrx, int lry, int unk) {
#endif
    struct fontchar *fc = (struct fontchar *)ptr_tbl2entry;
    struct fontchar *fc_prev = (struct fontchar *)ptr_unk;
    s32 *kerntbl = (s32 *)ptr_tbl1;
#ifndef NATIVE_PORT
    s32 *pxpos = (s32 *)xpos;
    s32 *pypos = (s32 *)ypos;
#endif
    Gfx *_g;
    s32 ybase;     /* $t1: *pypos + unk */
    s32 xend;      /* sp+0x24: ulx + lrx */
    s32 yend;      /* $t3: uly + lry */
    s32 xright;    /* $t0: *pxpos + fc->width */
    s32 ytop;      /* $v1: fc->baseline + ybase */
    s32 ybot;      /* $v0: ytop + fc->height */
    s32 kerning;
    s32 newx;
    s32 charw;
    s32 tmul;
    ...
    /* Compute ybase = *pypos + unk ($t1) */
    ybase = *pypos + unk;

    /* Kerning lookup: ptr_tbl1[fc_prev->kerningindex * 13 + fc->kerningindex] */
    kerning = kerntbl[fc_prev->kerningindex * 13 + fc->kerningindex];

    /* Update *pxpos: subtract kerning + spacing, +1 */
    newx = *pxpos - (kerning + text_spacing) + 1;
    *pxpos = newx;
    ...
    /* Normal orientation: screen-level culling */
    if (newx <= 0) {
        goto done;
    }
...
bounds_setup:
    ...
    if (ytop + (s32)fc->height < uly) {
        goto done;
    }

#ifdef NATIVE_PORT
    text_audit_log_glyph("plain", fc, *pxpos, ybase);
#endif
    ...
    if (fc->index < 0x80) {
        ...
        _g->words.w1 = osVirtualToPhysical(fc->pixeldata);
        ...
        /* G_LOADBLOCK */
        {
            ...
            }
            {
                ...
                dxt = (dwords + 0x7FF) / dwords2;
            }
            _g->words.w1 = ((texels & 0xFFF) << 12) | (dxt & 0xFFF) | 0x07000000;
        }
        ...
        /* G_SETTILE: render tile (I 4b) */
        {
            s32 line = ((((fc->width + 7) & ~7) + 7) >> 3) & 0x1FF;
            _g = DL++;
            _g->words.w0 = 0xF5880000 | (line << 9);
            _g->words.w1 = 0x00000000;
        }

        /* G_SETTILESIZE */
        {
            s32 wm1 = ((fc->width + 7) & ~7) - 1;
            s32 hm1 = fc->height - 1;
            _g = DL++;
            _g->words.w0 = 0xF2000000;
            _g->words.w1 = (((wm1 * 4) & 0xFFF) << 12) | ((hm1 * 4) & 0xFFF);
        }
        ...
        _g = DL++;
        _g->words.w0 = 0xBA000E02;
        _g->words.w1 = 0x0000C000;

        if (D_80040EA8 != 0) {
            D_80040EA8 = 0;

            /* Load TLUT from D_80040EBC */
            _g = DL++;
            _g->words.w0 = 0xFD100000;
            _g->words.w1 = osVirtualToPhysical(D_80040EBC);
            ...
            /* Load TLUT from D_80040EDC */
            _g = DL++;
            _g->words.w0 = 0xFD100000;
            _g->words.w1 = osVirtualToPhysical(&D_80040EDC);
        ...
        /* G_LOADBLOCK */
        {
            ...
            _g = DL++;
            _g->words.w0 = 0xF3000000;
            _g->words.w1 = ((lbval & 0xFFF) << 12) | 0x07000000 | 0x0800;
        }
        ...
        /* G_SETTILE: render tile (CI 8b) */
        _g = DL++;
        _g->words.w0 = 0xF5400200;
        _g->words.w1 = ((fc->index & 1) << 20) | 0x00080000 | 0x0200;

        /* G_SETTILESIZE */
        _g = DL++;
        _g->words.w0 = 0xF2000000;
        _g->words.w1 = (((fc->height - 1) * 4) & 0xFFF) | 0x0003C000;
    }

    /* ===== Post-texture bounds checking and rectangle emission ===== */

    xright = *pxpos + fc->width;
    ...
    if (ytop < uly) {
        /* Top is clipped */
        ybot = ytop + (s32)fc->height;
        if (ybot < uly) {
            /* Entirely above viewport */
            goto skip_render;
        }

        /* Emit clipped-top TextureRectangle */
        {
            _g = DL++;
            _g->words.w0 = 0xE4000000 | (((xright * 4 + text_x) & 0xFFF) << 12) | ((ybot * 4 + text_y) & 0xFFF);
            _g->words.w1 = (((*pxpos * 4 + text_x) & 0xFFF) << 12) | ((uly * 4 + text_y) & 0xFFF);

            _g = DL++;
            _g->words.w0 = 0xB4000000;
            _g->words.w1 = (text_s << 16) | (((uly - ybase - fc->baseline) * 32 + text_t) & 0xFFFF);
    ...
    if (yend < ybot) {
        /* Bottom is clipped */
        if (yend < ytop) {
            /* Entirely below start - shouldn't happen but assembly checks it */
            goto skip_render;
        }

        /* Emit clipped-bottom TextureRectangle */
        {
            _g = DL++;
            _g->words.w0 = 0xE4000000 | (((xright * 4 + text_x) & 0xFFF) << 12) | ((yend + text_y) & 0xFFF);
            _g->words.w1 = (((*pxpos * 4 + text_x) & 0xFFF) << 12) | (((ybase + fc->baseline) * 4 + text_y) & 0xFFF);

            _g = DL++;
            _g->words.w0 = 0xB4000000;
            _g->words.w1 = (text_s << 16) | (text_t & 0xFFFF);
    ...
    if (text_orientation != 0) {
        /* Rotated text: TextureRectangleFlip */
        {
            _g = DL++;
            _g->words.w0 = 0xE5000000
                | ((xright * 4 + text_x) & 0xFFF)
                | ((((ybase - fc->baseline) * 4 + text_y) & 0xFFF) << 12);
            _g->words.w1 = ((*pxpos * 4 + text_x) & 0xFFF)
                | (((((s32)(ybase - fc->baseline - fc->height)) * 4 + text_y) & 0xFFF) << 12);

            _g = DL++;
            _g->words.w0 = 0xB4000000;
            _g->words.w1 = (text_t << 16) | ((((s32)(fc->height - 1)) * 32 + text_s) & 0xFFFF);

            _g = DL++;
            _g->words.w0 = 0xB3000000;
            _g->words.w1 = 0x0400FC00;
        }
    } else {
        /* Normal text: TextureRectangle */
        {
            _g = DL++;
            _g->words.w0 = 0xE4000000
                | (((xright * 4 + text_x) & 0xFFF) << 12)
                | ((ybot * 4 + text_y) & 0xFFF);
            _g->words.w1 = (((*pxpos * 4 + text_x) & 0xFFF) << 12)
                | (((ybase + fc->baseline) * 4 + text_y) & 0xFFF);
...
// Renders text with G_SETPRIMCOLOR. Processes ASCII and Japanese characters,
// calling construct_fontchar_microcode for each character's display list.
#ifdef NATIVE_PORT
...
Gfx *textRender(Gfx *gdl, s32 *x, s32 *y, char *text, s32 second_font_table, s32 first_font_table, s32 arg6, s16 view_x, s16 view_y, s32 arg9, s32 arga) {
    ...
    s32 initial_y;
    s32 prev_char;
    struct fontchar jpn_char;
    char *ptr;
    u8 ch;
    s32 jis_code;

#ifdef NATIVE_PORT
    if (text == NULL || second_font_table == 0 || first_font_table == 0) {
        return gdl;
    }
    ...
    if (arga == 0) {
#ifdef NATIVE_PORT
        /* Entry 91 = character '|' (124 = 91+33) — used as height reference */
        struct fontchar *fc_ref = &((struct fontchar *)second_font_table)[91];
        arga = fc_ref->baseline + fc_ref->height;
#else
        arga = *(s32 *)((u8 *)second_font_table + 0x88c) + *(s32 *)((u8 *)second_font_table + 0x890);
#endif
    }

    if (j_text_trigger != 0 && arga < 14) {
        arga = 14;
    }

#ifdef NATIVE_PORT
    text_audit_log_render("plain", text, *x, *y, second_font_table, first_font_table,
        arg6, 0, view_x, view_y, arg9, arga);
#endif
    ...
    while (ch != 0) {
        if (ch == 0x20) {
            *x += 5;
            ...
            prev_char = 72;
            ptr++;
            if (overlap_correction >= 0 && *x == initial_x) {
            ...
            *x = initial_x;
            ...
            gdl = construct_fontchar_microcode(
#ifdef NATIVE_PORT
                gdl, x, y,
                (void *)&((struct fontchar *)second_font_table)[ch - 33],
                (void *)&((struct fontchar *)second_font_table)[prev_char - 33],
#else
                gdl, (s32)x, (s32)y,
                (void *)(second_font_table + ch * 24 - 0x318),
                (void *)(second_font_table + prev_char * 24 - 0x318),
#endif
                (void *)first_font_table,
                initial_x, initial_y,
                view_x, view_y,
            ...
            jpn_char = *(struct fontchar *)&D_80040EFC;

            jis_code = (((ch & 0x7F) << 7) | (ptr[1] & 0x7F)) & 0xFFFF;
            ...
            if ((jis_code & 0x1FFF) >= 0x3C7) {
                jis_code = 2;
            }

            jpn_char.index = jis_code + 0x80;
            jpn_char.pixeldata = (u8 *)langGetJpnCharPixels(jis_code);
...
#endif
extern Gfx *sub_GAME_7F0AE45C(Gfx *gdl, s32 x, s32 y, struct fontchar *chardata, s32 vp_x, s32 vp_y, s32 vp_w, s32 vp_h);

Gfx *sub_GAME_7F0ADDAC(Gfx *gdl, s32 *x, s32 *y, struct fontchar *fontchar,
    struct fontchar *prev_char, s32 *kerning_table, s32 vp_x, s32 vp_y,
    u32 color, u32 glow_color, s32 vp_w, s32 vp_h, s32 lineheight)
{
    Gfx *_g;
    s32 y_total;
    s32 kerning_val;
    ...
    y_total = *y + lineheight;
    kerning_val = kerning_table[prev_char->kerningindex * 13 + fontchar->kerningindex];
    tmp = kerning_val + text_spacing;
    *x = *x - tmp + 1;
    ...
    y_bottom = y_total + fontchar->baseline;
    ...
    if (y_bottom + (s32)fontchar->height < vp_y) {
        goto epilogue;
    }

#ifdef NATIVE_PORT
    text_audit_log_glyph("glow", fontchar, xval, y_total);
#endif

    if ((s32)fontchar->index < 0x80) {
        /* 4-bit texture path */
        _g = gdl++;
        _g->words.w0 = 0xBA000E02;
        _g->words.w1 = 0;
        ...
        width_aligned = (fontchar->width + 7) & 0xF8;
        height = fontchar->height;
        num_dwords = (((width_aligned * height) + 1) >> 1) - 1;
        ...
        _g->words.w1 = (((dxt_lines + 0x7FF) / num_lines & 0xFFF) | 0x07000000) | ((num_dwords & 0xFFF) << 12);
        ...
        _g = gdl++;
        _g->words.w0 = 0xF5880000 | ((((width_aligned + 7) / 8) & 0x1FF) << 9);
        _g->words.w1 = 0;

        _g = gdl++;
        _g->words.w0 = 0xF2000000;
        _g->words.w1 = (((((fontchar->width + 7) & 0xF8) - 1) * 4 & 0xFFF) << 12) | (((fontchar->height - 1) * 4) & 0xFFF);
    } else {
        /* 8-bit texture path (index >= 0x80) */
        _g = gdl++;
        _g->words.w0 = 0xE7000000;
        _g->words.w1 = 0;
        ...
        if (D_80040EA8 != 0) {
            ...
            /* Load dither texture 0 (D_80040EBC) */
            _g = gdl++;
            _g->words.w0 = 0xFD100000;
            ...
            /* Load dither texture 1 (D_80040EDC) */
            _g = gdl++;
            _g->words.w0 = 0xFD100000;
        ...
        /* Load main texture (fontchar->pixeldata) */
        _g = gdl++;
        _g->words.w0 = 0xFD500000;
        _g->words.w1 = (uintptr_t)fontchar->pixeldata;
        GFX_DL_REGISTER_PTR(_g->words.w1); /* wasm32: record host ptr for DL resolve */
        ...
        _g->words.w1 = ((num_dwords & 0xFFF) << 12) | 0x07000800;
        ...
        _g = gdl++;
        _g->words.w0 = 0xF5400200;
        _g->words.w1 = ((fontchar->index & 1) << 20) | 0x00080200;

        _g = gdl++;
        _g->words.w0 = 0xF2000000;
        _g->words.w1 = (((fontchar->height - 1) * 4) & 0xFFF) | 0x0003C000;
    }

    /* Set glow prim color */
    _g = gdl++;
    _g->words.w0 = 0xFA000000;
    _g->words.w1 = ((glow_color >> 24) << 24) | (((glow_color >> 16) & 0xFF) << 16)
                  | (((glow_color >> 8) & 0xFF) << 8) | (glow_color & 0xFF);

    xval = *x;

    /* Glow outline loop: render at 8 surrounding offsets */
    xoff = -1;
    do {
        yoff = -1;
        do {
            ...
            gdl = sub_GAME_7F0AE45C(gdl, xval + xoff, y_total + yoff, fontchar, vp_x, vp_y, vp_w, vp_h);
            xval = *x;
            yoff++;
        } while (yoff != 2);
        xoff++;
    ...
    /* Set main prim color */
    gdl->words.w0 = 0xFA000000;
    gdl->words.w1 = ((color >> 24) << 24) | (((color >> 16) & 0xFF) << 16)
                    | (((color >> 8) & 0xFF) << 8) | (color & 0xFF);

    gdl = sub_GAME_7F0AE45C(gdl + 1, xval, y_total, fontchar, vp_x, vp_y, vp_w, vp_h);
...
#ifdef NONMATCHING
#if defined(PORT_FIXME_STUBS) && !defined(NATIVE_PORT)
Gfx *sub_GAME_7F0AE45C(Gfx *gdl, s32 x, s32 y, struct fontchar *chardata, s32 vp_x, s32 vp_y, s32 vp_w, s32 vp_h) {
    (void)x; (void)y; (void)chardata; (void)vp_x; (void)vp_y; (void)vp_w; (void)vp_h;
    return gdl;
}
#else
Gfx *sub_GAME_7F0AE45C(Gfx *gdl, s32 x, s32 y, struct fontchar *chardata_arg, s32 vp_x, s32 vp_y, s32 vp_w, s32 vp_h) {
    u8 *char_data = (u8 *)chardata_arg;
    Gfx *_g;
    s32 width;
    s32 y_offset;
    s32 tex_height;
    ...
    width = *(s32 *)(char_data + 0x0C);
    right_edge = x + width;
    vp_right = vp_x + vp_w;

    if (vp_right < right_edge) {
        return gdl;
    }

    y_offset = *(s32 *)(char_data + 0x04);
    top = y + y_offset;

    if (top < vp_y) {
        tex_height = *(s32 *)(char_data + 0x08);
        ...
        _g = gdl++;
        _g->words.w0 = 0xE4000000 | (((right_edge * 4) & 0xFFF) << 12) | ((bottom * 4) & 0xFFF);
        _g->words.w1 = (((x * 4) & 0xFFF) << 12) | ((vp_y * 4) & 0xFFF);

        _g = gdl++;
        _g->words.w0 = 0xB4000000;
        _g->words.w1 = ((vp_y - y_offset - y) << 5) & 0xFFFF;
    ...
    if (vp_bottom < bottom) {
        ...
        _g = gdl++;
        _g->words.w0 = 0xE4000000 | (((right_edge * 4) & 0xFFF) << 12) | ((vp_bottom * 4) & 0xFFF);
        _g->words.w1 = (((x * 4) & 0xFFF) << 12) | ((top * 4) & 0xFFF);
    ...
    if (text_orientation != 0) {
        _g = gdl++;
        _g->words.w0 = 0xE5000000 | ((((y - y_offset) * 4) & 0xFFF) << 12) | ((right_edge * 4) & 0xFFF);
        _g->words.w1 = ((((y - y_offset - tex_height) * 4) & 0xFFF) << 12) | ((x * 4) & 0xFFF);

        _g = gdl++;
        _g->words.w0 = 0xB4000000;
        _g->words.w1 = ((tex_height - 1) << 5) & 0xFFFF;
...
#ifdef NONMATCHING
// NONMATCHING: Likely register allocation diffs from ido_recomp.
// Renders text with glow effect. Processes ASCII and Japanese characters,
// calling sub_GAME_7F0ADDAC for each character's display list construction.
extern struct jpncharpixels *langGetJpnCharPixels(s32 codepoint);
...
#ifdef NATIVE_PORT
        struct fontchar *fc_ref = &((struct fontchar *)second_font_table)[91];
        argb = fc_ref->baseline + fc_ref->height;
#else
        argb = *(s32 *)((u8 *)second_font_table + 0x88c) + *(s32 *)((u8 *)second_font_table + 0x890);
#endif
    }

    if (j_text_trigger != 0 && argb < 14) {
        argb = 14;
    }

#ifdef NATIVE_PORT
    text_audit_log_render("glow", (const char *)text, *x, *y, second_font_table, first_font_table,
        arg6, arg7, view_x, view_y, arga, argb);
#endif
    ...
    while (ch != 0) {
        if (ch == 0x20) {
            ...
            *y = *y + argb;
            ...
            gdl = sub_GAME_7F0ADDAC(
                gdl, x, y,
#ifdef NATIVE_PORT
                &((struct fontchar *)second_font_table)[ch - 33],
                &((struct fontchar *)second_font_table)[prev_char - 33],
                (s32 *)first_font_table,
#else
                (struct fontchar *)(second_font_table + ch * 24 - 0x318),
                (struct fontchar *)(second_font_table + prev_char * 24 - 0x318),
                (s32 *)first_font_table,
#endif
                initial_x, initial_y,
                arg6, arg7,
            ...
            jpn_char = *(struct fontchar *)&D_80040F14;
...
void textMeasure(s32 *textheight, s32 *textwidth, char *text, struct fontchar *font1, struct font *font2, s32 lineheight)
{
	...
	*textwidth = 0;
	...
	if (text)
	{
		while (*text != '\0')
		{
			unsigned char cur = (unsigned char)*text;
...
#ifdef NONMATCHING
void sub_GAME_7F0AEB64(s32 maxwidth, s8 *src, s8 *dst, struct fontchar *font1, struct font *font2)
{
	s32 measuredheight;
	s32 measuredwidth;
	s32 accumulated_width;
	s32 continue_flag;
	s32 wordlen;
	s32 wordwidth;
	s32 fits_on_line;
	...
	do {
			...
			*wp = curchar;
			...
			do {
				...
				if ((u8)*rp >= 0x80) {
					...
					*wp = ch2;
					wp++;
					wordwidth += font1[ch2 - 0x21].width;
				}
		...
		textMeasure(&measuredheight, &measuredwidth, wordbuf, font1, font2, 0);

		accumulated_width += measuredwidth;

		if (maxwidth < accumulated_width) {
			fits_on_line = 0;
		} else {
			fits_on_line = 1;
		}
			...
			*dst = '\n';
			...
			if (!fits_on_line) {
				...
				*dst = '\n';
				...
				j = 0;
				accumulated_width = text_wordwrap * 5 + measuredwidth;
			}
...
u32 sub_GAME_7F0AEF20(u32 arg0, u32 arg1){
  return arg0;
}
