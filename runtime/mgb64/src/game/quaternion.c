...
void quaternion_7F05BC68(quatf q, f32 t, quatf result)
{
    ...
    if (cos_angle < -0.99998999f) {
        result[0] = (q[0] * t) - ((1.0f - t) * sign);
        result[1] = (q[1] * t);
        result[2] = (q[2] * t);
        result[3] = (q[3] * t);
    } else if (cos_angle <= 0.99998999f) {
        angle = acosf(cos_angle);
        t_angle = t * angle;
        inv_t_angle = (1.0f - t) * angle;
        sin_angle = sinf(angle);
        coeff_q = sinf(t_angle) / sin_angle;
        coeff_identity = sinf(inv_t_angle) / sin_angle;
        result[0] = (q[0] * coeff_q) + (coeff_identity * sign);
        result[1] = (q[1] * coeff_q);
        result[2] = (q[2] * coeff_q);
        result[3] = (q[3] * coeff_q);
    } else {
        result[0] = (q[0] * t) + ((1.0f - t) * sign);
        result[1] = (q[1] * t);
        result[2] = (q[2] * t);
