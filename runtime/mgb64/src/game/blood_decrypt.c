...
void sub_GAME_7F01D16C(u8 *arg0, s32 arg1, s32 arg2, u8 *arg3)
{
    ...
    var_v1 = arg0;
    var_t0 = arg3;
    t1 = (uintptr_t)arg0 + temp_lo;
    var_t2 = (uintptr_t)arg0 + arg1;
    ...
    do
    {
        ...
        } while ((uintptr_t) var_v1 < temp_a0);

        var_t0 = (var_t0 - (temp_lo)) + 1;

    } while ((uintptr_t) var_v1 < (uintptr_t) t1);
}
...
// RLE decompression for bleeding animation mask data.
// Reads packed byte stream, outputs rows of width bytes (0xFF or 0x00).
// ido_recomp: loop/register differences.
u8 *sub_GAME_7F01D1C0(u8 *src, s32 width, s32 rows, u8 *dst)
{
    u8 first_byte;
    u8 byte;
    u8 fill_byte;
    ...
    do {
        ...
        if (byte != 0xFF) {
            /* RLE mode: low 5 bits + first_byte = offset, high 3 bits + 1 = repeat count */
            run_offset = (byte & 0x1F) + first_byte;
            repeat = (byte >> 5) + 1;
            rows -= repeat;
            ...
            /* Escape sequence: alternating run lengths of 0xFF/0x00 */
            byte = *src++;
            total = 0;
