...
#else
/* Non-SGI: all uses in this codebase are zero-arg, so just expand to zero-init */
...
#pragma region Stand Tiles
    ...
    */
    ...
    {
#ifdef NATIVE_PORT
        intptr_t unk00;  /* overloaded: pointer to room buffer, boolean flag, or float bits */
#else
        s32  unk00;
#endif
        s32  count;
...
#pragma region ModelTypes
    ...
    */
    ...
    #pragma region Model Node OpCode Definitions
        ...
        /**
         *  Opcode 17
         *  Non-rendered hit-volume node used by object shot tests.
         */
        typedef struct ModelRoData_Op17Record
        {
            s32       HitType;              /*0x0*/
            f32       BoundingVolumeRadius; /*0x4*/
            coord3d   pos;                  /*0x8*/
            ModelNode *RelatedNode;         /*0x14*/
            f32       Scale1;               /*0x18*/
            f32       Scale2;               /*0x1C*/
     ...
     */
#ifdef NATIVE_PORT
    /* On PC, RwData is allocated in a flat buffer with 4-byte stride matching
     * N64 indexing semantics. Reduce the union's alignment to 4 so the
     * compiler and sanitizers accept 4-byte-aligned accesses. */
    union __attribute__((packed, aligned(4))) ModelRwData
#else
    union ModelRwData
#endif
    {
...
#endif
#ifdef NATIVE_PORT
            void              *n64_filedata;          /* base address of loaded N64 binary file data */
            size_t             n64_filedata_size;      /* size of file data buffer */
            s32                requiredRenderPosCount; /* canonical live matrix count for render_pos */
            const char        *debugName;              /* source asset name for diagnostics */
            /* Persisted N64 offset→native pointer map for runtime lookups.
             * Weapon rendering code needs to access model nodes by their
             * N64 binary offset (the original code used flat byte arithmetic). */
            void             **nodeMapNative;          /* array of native pointers */
            u32               *nodeMapOffsets;         /* parallel array of N64 file offsets */
            s32                nodeMapCount;
            s32                rootNodeFileOffset;     /* byte offset of root node in n64_filedata */
#endif
        } ModelFileHeader;
...
#ifdef NATIVE_PORT
/* Declare here after struct types are complete (can't declare in chrobjdata.h due to circular include) */
...
extern struct ChrModelFileRecord c_item_entries[];
        ...
        */
        ...
        {                    // type 0x01
            s8      unk00;   /*0*/ /* must be s8, not u16: shares memory with ModelRwData_HeaderRecord */
            s8      unk01;   /*1*/
            s8      unk02;   /*2*/
            f32     ground;  /*4*/
...
#ifdef NATIVE_PORT
            uintptr_t         animflipfunc; /*0x98 — function pointer, widened for 64-bit */
            uintptr_t         unk9c;
            // 0xa0
            uintptr_t         unka0;        /* move callback pointer, widened for 64-bit */
#else
...
#pragma region ACT_TYPES
    ...
    /*
     * WARNING — 64-bit union offset hazard:
     *
     * Never use act_ubytes.padding[N], act_bytes.padding[N], or
     * act_init.padding[N] to access typed fields in the action union.
     * On N64 (32-bit pointers), padding indices happen to align with
     * typed struct members. On 64-bit, pointer expansion in structs
     * like act_gopos, act_attack, act_patrol shifts field offsets,
     * causing padding[N] to read/write the wrong memory silently.
     *
     * Always use the named struct member instead:
     *   WRONG:  self->act_ubytes.padding[45]
     *   RIGHT:  self->act_gopos.unk59
     *
     * Structs with pointers (broken padding on 64-bit):
     *   act_attack, act_attackwalk, act_attackroll, act_gopos, act_patrol
     *
     * Structs without pointers (padding stable, but still fragile):
     *   act_anim, act_die, act_dead, act_argh, act_preargh, act_runpos,
     *   act_sidestep, act_jumpout, act_stand, act_kneel, act_surrender,
     *   act_lookattarget, act_surprised, act_startalarm, act_throwgrenade
     */
...
#pragma region Prop, Chr and Definition Records
    ...
    typedef struct PropRecord
    {
        ...
        union {
            ...
            struct PropRecord *nextSibling;                 /*0x24*/
        };
    ...
    */
    ...
    typedef struct ChrRecord
    {
        ...
        //
#ifdef NATIVE_PORT
        PropRecord    *weapons_held[2]; /* 0x0160 0x0164 Right, Left */
        ALSoundState  *ptr_SEbuffer1;   /* 0x0168 */
        ALSoundState  *ptr_SEbuffer2;   /* 0x016C */
#else
    ...
    typedef struct CCTVRecord
    {
        struct ObjectRecord;
#ifndef NATIVE_PORT
        s32 pad;  /* duplicates ObjectRecord.pad — IDO allows shadowing */
#else
        s32 cctv_pad;
#endif
        Mtxf unk84;
