...
/* crash.c is deeply N64-specific (TLB fault handler, MIPS exception handling,
 * register dumping). Stubbed until N64 hardware layer is built. */

void *g_StackPtrs1[STACK_POINTER_COUNT] = {0};
void *g_StackPtrs2[STACK_POINTER_COUNT] = {0};
void *g_StackPtrs3[STACK_POINTER_COUNT] = {0};

void crashInit(void) {}
void crashMain(void *arg0) { (void)arg0; }
void crashAppendChar(unsigned char c) { (void)c; }
void crashScroll(s32 numlines) { (void)numlines; }
void crashInitBuffers(void) {}
void crashRenderFrame(u16 *buffer) { (void)buffer; }

#else /* !PORT_FIXME_STUBS */
...
/**
 * Crash-thread fault message type.
 */
...
u32 g_DebugOutputBitmaps[105] = {0}; /* MGB64: ROM-derived font data removed; supply from your own ROM */
