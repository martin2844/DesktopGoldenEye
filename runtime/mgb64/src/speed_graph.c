    ...
    *pcountAccumulator = localCountAccumulator;
    ...
    if (localCountAccumulator > COUNT_REQUIRED_FOR_OUTPUT)
    {
        *pcountAccumulator = localCountAccumulator;
        ...
        // utz %
        debmenuSetPos(8, 5);
        ge007_sprintf(buffer, "utz %2.0f%%\n", (((counters[1] - counters[3]) * 100.0f) / counters[0]));
        debmenuPrintString((const unsigned char *)buffer);

        // rsp %
        debmenuSetPos(8, 6);
        ge007_sprintf(buffer, "rsp %2.0f%%\n", (((counters[0] - counters[1]) * 100.0f) / counters[0]));
        debmenuPrintString((const unsigned char *)buffer);

        // tex %
        debmenuSetPos(8, 7);
        ge007_sprintf(buffer, "tex %2.0f%%", ((counters[3] * 100.0f) / counters[0]));
        debmenuPrintString((const unsigned char *)buffer);

        // hz (60 / framerate)
        // -- or 50 for PAL
        debmenuSetPos(28, 5);
        ge007_sprintf(buffer, "%2d hz", ((speedgraphframes == 0) ? 0 : (VICLOCK / speedgraphframes)));
        debmenuPrintString((const unsigned char *)buffer);

        // framerate
        debmenuSetPos(28, 6);
        ge007_sprintf(buffer, "%2d frames", speedgraphframes);
        debmenuPrintString((const unsigned char *)buffer);

        // (continues framerate output)
        if (speedgraphframes != g_speedGraphMaxSeenCount)
        {
            ge007_sprintf(buffer, " [%2d]", *pmaxSeenCount);
        }
        else
        {
            ge007_sprintf(buffer, "     ");
        }
