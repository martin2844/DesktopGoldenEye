...
{
#ifdef NATIVE_PORT
    /* On PC there is no separate controller polling thread, so skip the
     * inter-thread synchronization and call joyCheckStatus directly. */
    joyCheckStatus();
#else
    OSMesg  msg;
...
void joyCheckStatus(void)
{
    ...
    else
    {
        ...
        // Query controller status through the standard SI command sequence.
...
void joyConsumeSamplesWrapper(void)
{
    ...
    {
#ifdef NATIVE_PORT
        pcRamromTraceLoopPhaseWithDetails("joy_playback_func_before",
                                          g_ContData[CONTDATA_PLAYBACK].curstart,
                                          g_ContData[CONTDATA_PLAYBACK].curlast,
                                          g_ContData[CONTDATA_PLAYBACK].nextlast);
#endif
...
#ifdef NATIVE_PORT
        pcRamromTraceLoopPhaseWithDetails("joy_playback_func_after",
                                          g_ContData[CONTDATA_PLAYBACK].curstart,
                                          g_ContData[CONTDATA_PLAYBACK].curlast,
                                          g_ContData[CONTDATA_PLAYBACK].nextlast);
        pcRamromTraceLoopPhaseWithDetails("joy_playback_consume_before",
                                          g_ContData[CONTDATA_PLAYBACK].curstart,
                                          g_ContData[CONTDATA_PLAYBACK].curlast,
                                          g_ContData[CONTDATA_PLAYBACK].nextlast);
#endif
...
#ifdef NATIVE_PORT
        pcRamromTraceLoopPhaseWithDetails("joy_playback_consume_after",
                                          g_ContData[CONTDATA_PLAYBACK].curstart,
                                          g_ContData[CONTDATA_PLAYBACK].curlast,
                                          g_ContData[CONTDATA_PLAYBACK].nextlast);
#endif
...
#ifdef NATIVE_PORT
    pcRamromTraceLoopPhaseWithDetails("joy_regular_consume_before",
                                      g_ContData[CONTDATA_REGULAR].curstart,
                                      g_ContData[CONTDATA_REGULAR].curlast,
                                      g_ContData[CONTDATA_REGULAR].nextlast);
#endif
    joyConsumeSamples(&g_ContData[CONTDATA_REGULAR]);
#ifdef NATIVE_PORT
    pcRamromTraceLoopPhaseWithDetails("joy_regular_consume_after",
                                      g_ContData[CONTDATA_REGULAR].curstart,
                                      g_ContData[CONTDATA_REGULAR].curlast,
                                      g_ContData[CONTDATA_REGULAR].nextlast);
#endif
    ...
    {
#ifdef NATIVE_PORT
        pcRamromTraceLoopPhaseWithDetails("joy_record_func_before",
                                          g_ContData[CONTDATA_REGULAR].curstart,
                                          g_ContData[CONTDATA_REGULAR].curlast,
                                          g_ContData[CONTDATA_REGULAR].nextlast);
#endif
...
#ifdef NATIVE_PORT
        pcRamromTraceLoopPhaseWithDetails("joy_record_func_after",
                                          g_ContData[CONTDATA_REGULAR].curstart,
                                          g_ContData[CONTDATA_REGULAR].curlast,
                                          g_ContData[CONTDATA_REGULAR].nextlast);
#endif
...
#ifdef NATIVE_PORT
    pcRamromTraceLoopPhase("joy_wrapper_exit");
#endif
}
...
void joyPoll(void)
{
    ...
    if ((g_ContPollDisableCount == 0) && g_ContInitDone)
    {
        ...
        if (osRecvMesg(&g_ContInputMessageQueue, &msg, OS_MESG_NOBLOCK) == 0)
        {
            ...
            {
                if (((g_ContData[CONTDATA_REGULAR].samples[g_ContData[CONTDATA_REGULAR].nextlast].pads[i_s8].errnum == 0) && (g_ContData[CONTDATA_REGULAR].samples[g_ContData[CONTDATA_REGULAR].nextsecondlast].pads[i_s8].errnum != 0)) ||
                    ((g_ContData[CONTDATA_REGULAR].samples[g_ContData[CONTDATA_REGULAR].nextlast].pads[i_s8].errnum != 0) && (g_ContData[CONTDATA_REGULAR].samples[g_ContData[CONTDATA_REGULAR].nextsecondlast].pads[i_s8].errnum == 0)))
                {
                    joyCheckStatus();
