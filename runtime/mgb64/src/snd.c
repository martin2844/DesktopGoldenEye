...
static s16 sndPublicSfxIdToBankIndex(s16 soundIndex) {
    /* Gameplay submits SFX_ID values where 0 is NOTHING_SFX. The SFX bank
     * itself is zero-based and has no NOTHING_SFX placeholder, so public entry
     * points must translate once before touching soundArray. Keymap chained
     * references use the same public id space, with 0 as the chain terminator. */
    if (soundIndex <= 0) {
        return -1;
    }

    return (s16)(soundIndex - 1);
}

static s16 sndKeyMapNextPublicSfxId(const ALKeyMap *keyMap) {
    if (keyMap == NULL) {
        return 0;
    }

    return (s16)((s32)keyMap->velocityMin + ((s32)(keyMap->keyMin & 0xC0) * 4));
}

static s16 sndKeyMapNextBankIndex(const ALKeyMap *keyMap) {
 ...
 * fields, ALEventListItem, alGlobals, etc.) that our platform headers don't
 * provide. Stubbed until audio infrastructure is built. */

ALSndPlayer g_sndPlayer = {0};
ALSndPlayer *g_sndPlayerPtr = &g_sndPlayer;
s16 g_sndAllocatedVoicesCount = 0;
s8 g_sndBootswitchSound = 0;
f32 g_sndSfxVolumeScale = 1.0f;
s32 g_sndUnused800243E0 = 0;

typedef enum SndStubCounter_e {
    SND_STUB_NEW_PLAYER_INIT = 0,
    SND_STUB_GET_PLAYING_STATE,
    SND_STUB_DEACTIVATE,
    SND_STUB_DEACTIVATE_ALL_1,
    SND_STUB_DEACTIVATE_ALL_11,
    SND_STUB_DEACTIVATE_ALL_3,
    SND_STUB_CREATE_POST_EVENT,
    SND_STUB_PLAY_SFX,
    SND_STUB_DISPOSE_SOUND,
    SND_STUB_SET_PRIORITY,
    SND_STUB_UNLINK_CLEAR,
    SND_STUB_COUNT_ALLOC_LIST,
    SND_STUB_COUNT
} SndStubCounter;

static u32 s_sndStubCounters[SND_STUB_COUNT];
static u32 s_sndStubLegacyOverlapFallbackHits;
static s16 s_sndStubSlotVolume[SFX_SLOT_COUNT];
static u16 s_sndStubSlotNaturalVolume[SFX_SLOT_COUNT];
static u8 s_sndStubSlotVolumeInit;
static s32 s_sndStubAuthoredMix = -1;

static void sndDisposeSound(ALSoundState *state);
void sndSetSfxSlotVolume(u8 arg0, u16 arg1);
static struct SndStubStateMeta_s *sndStubStateFromHandle(ALSoundState *state);
static void sndStubEnsureSlotVolumes(void);

#define SND_STUB_HIT(counter) do { s_sndStubCounters[(counter)]++; } while (0)

u32 sndStubCounterGet(u32 index) {
    if (index >= SND_STUB_COUNT) {
        return 0;
    }
    return s_sndStubCounters[index];
}

u32 sndStubCounterGetTotal(void) {
    u32 i;
    u32 total = 0;
    for (i = 0; i < SND_STUB_COUNT; i++) {
        total += s_sndStubCounters[i];
    }
    return total;
}

u32 sndStubCounterGetLegacyOverlapFallbackHits(void) {
    return s_sndStubLegacyOverlapFallbackHits;
}
...
enum { SND_STUB_MAX_STATES = 128 };
enum { SND_STUB_MAX_VOICES_PER_STATE = 8 };
typedef struct SndStubStateMeta_s {
    ALSoundState state;
    struct SndStubStateMeta_s *nextAll;
    struct SndStubStateMeta_s *nextHash;
    ALSoundState **ownerSlot;
    s16 soundIndex;
    s16 voiceIndex;
    s16 voiceIndices[SND_STUB_MAX_VOICES_PER_STATE];
    ALSound *voiceSounds[SND_STUB_MAX_VOICES_PER_STATE];
    u8 voiceCount;
    u16 baseVolume;
    f32 posX;
    f32 posY;
...
enum { SND_STUB_HANDLE_HASH_SIZE = 257 };

/* Stub sound handles intentionally keep unique addresses for the lifetime of
 * the process. Gameplay stores raw ALSoundState* values and later passes them
 * back through native sndplayer-style chaining paths; reusing handle addresses
 * caused stale pointers to alias unrelated later sounds. */
static SndStubStateMeta *s_sndStubAllStates;
static SndStubStateMeta *s_sndStubHandleHash[SND_STUB_HANDLE_HASH_SIZE];
static s16 s_sndStubActiveStateCount;

#define SND_STUB_DEFAULT_SETUP_PITCH_SHIFT (-0x1770)

static f32 sndStubClamp01(f32 value) {
    if (value < 0.0f) return 0.0f;
    if (value > 1.0f) return 1.0f;
    return value;
}

static s32 sndStubAuthoredMixEnabled(void) {
    if (s_sndStubAuthoredMix < 0) {
        const char *value = getenv("GE007_SFX_AUTHORED_MIX");
        s_sndStubAuthoredMix = (value != NULL && value[0] == '0' && value[1] == '\0') ? 0 : 1;
    }

    return s_sndStubAuthoredMix;
}

static f32 sndStubComputePitchRatioForSound(const ALSoundState *state, const ALSound *sound) {
    f32 pitch = 1.0f;

    if (state != NULL) {
        /* Effective pitch = basepitch (from keymap) * bend ratio (dynamic).
         * Mirrors PD's n_sndplayer: state->pitch * state->basepitch */
        pitch = state->pitch_28 * state->pitch_2c;
    }
...
static s32 sndStubSoundUsesPitchEvents(const ALSound *sound) {
    return sound != NULL && sound->keyMap != NULL && (sound->keyMap->keyMax & 0x20) != 0;
}

static s32 sndStubFxNibbleForSound(const ALSound *sound) {
    if (sound == NULL || sound->keyMap == NULL) {
        return 0;
    }

    return (s32)(sound->keyMap->keyMax & 0xf);
}

static s32 sndStubComputeNativeFxMixForSound(const ALSoundState *state,
                                             const ALSound *sound) {
    s32 value;
    s32 base = state != NULL ? (s32)state->fxMix : AL_DEFAULT_FXMIX;

    value = (base + sndStubFxNibbleForSound(sound)) * 8;
    return MIN(AL_VOL_FULL, MAX(AL_DEFAULT_FXMIX, value));
}

static s32 sndStubSlotIndexForSound(const ALSound *sound) {
    if (sound == NULL || sound->keyMap == NULL) {
        return -1;
    }
...
static f32 sndStubComputeSlotScaleForSound(const ALSound *sound) {
    ...
    slot = sndStubSlotIndexForSound(sound);
    if (slot < 0 || slot >= SFX_SLOT_COUNT) {
        return 1.0f;
    }

    sndStubEnsureSlotVolumes();
    return sndStubClamp01((f32)g_sndSfxSlotVolume[slot] / 32767.0f);
}

static s32 sndStubSlotAppliedVolumeForSound(const ALSound *sound) {
    s32 slot;
...
static s32 sndStubComputeNativeEnvelopeVolumeForSound(const ALSoundState *state,
                                                      const ALSound *sound,
                                                      u32 envelopeVolume) {
    s32 stateVol;
    s32 sampleVol;
    ...
    stateVol = state != NULL ? state->vol : 0x7fff;
    sampleVol = sound != NULL ? sound->sampleVolume : AL_VOL_FULL;
    slotVol = sndStubSlotAppliedVolumeForSound(sound);

    vtmp = (stateVol * ((s32)envelopeVolume * sampleVol)) / (AL_VOL_FULL * AL_VOL_FULL);
    vtmp = (vtmp * slotVol) / 0x7fff;

    return MAX(0, vtmp - 1);
}

static ALMicroTime sndStubComputeReleaseTimeForSound(const ALSoundState *state,
                                                     const ALSound *sound) {
    f32 pitch;

    if (sound == NULL || sound->envelope == NULL) {
        return 0;
    }

    pitch = sndStubComputePitchRatioForSound(state, sound);
    if (pitch < 0.01f) {
        pitch = 0.01f;
    }

    return (ALMicroTime)((f32)sound->envelope->releaseTime / pitch);
}

static f32 sndStubComputeVolumeForSound(const ALSoundState *state, const ALSound *sound) {
    f32 vol = g_sndSfxVolumeScale;

    if (state == NULL || sound == NULL) {
        return vol;
    }

    if (sndStubAuthoredMixEnabled()) {
        vol = (f32)state->vol / 32767.0f;
        vol *= (f32)sound->sampleVolume / 127.0f;
        vol *= sndStubComputeSlotScaleForSound(sound);
    } else {
        /* Keep the legacy stub mixer simple and stable:
         * global SFX scale * per-sound bank volume. */
        vol *= (f32)sound->sampleVolume / 127.0f;
    }

    return sndStubClamp01(vol);
}

static f32 sndStubComputeVolume(const ALSoundState *state) {
    return sndStubComputeVolumeForSound(state, state != NULL ? state->sound : NULL);
}

static f32 sndStubComputePanForSound(const ALSoundState *state, const ALSound *sound) {
    s32 pan = AL_PAN_CENTER;

    if (state != NULL && sound != NULL) {
        pan = (s32)state->pan - AL_PAN_CENTER + (s32)sound->samplePan;
    }

    if (pan < AL_PAN_LEFT) pan = AL_PAN_LEFT;
    if (pan > AL_PAN_RIGHT) pan = AL_PAN_RIGHT;

    return (f32)pan;
}

static f32 sndStubComputePan(const ALSoundState *state) {
    return sndStubComputePanForSound(state, state != NULL ? state->sound : NULL);
}

static void sndStubEnsureSlotVolumes(void) {
    u8 i;

    if (g_sndSfxSlotVolume == NULL) {
        g_sndSfxSlotVolume = s_sndStubSlotVolume;
    }
    if (g_sndSfxSlotNaturalVolume == NULL) {
        g_sndSfxSlotNaturalVolume = s_sndStubSlotNaturalVolume;
    }

    if (!s_sndStubSlotVolumeInit) {
        for (i = 0; i < SFX_SLOT_COUNT; i++) {
            g_sndSfxSlotNaturalVolume[i] = 0x7fff;
            g_sndSfxSlotVolume[i] = (s16)((f32)g_sndSfxSlotNaturalVolume[i] * g_sndSfxVolumeScale);
        }
        s_sndStubSlotVolumeInit = 1;
    }
}

static void sndStubRefreshStateMixWithRamp(ALSoundState *state, ALMicroTime rampUsec) {
    SndStubStateMeta *meta = sndStubStateFromHandle(state);

    if (meta == NULL || !meta->active) {
        return;
    }

    for (u8 i = 0; i < meta->voiceCount; i++) {
        if (meta->voiceIndices[i] < 0 || meta->voiceSounds[i] == NULL) {
            continue;
        }

        if (meta->hasPosition) {
            if (sndStubAuthoredMixEnabled()) {
                portAudioUpdateVoicePanPositionRamp(meta->voiceIndices[i],
                                                    sndStubComputeVolumeForSound(state, meta->voiceSounds[i]),
                                                    meta->posX, meta->posY, meta->posZ,
                                                    rampUsec);
            } else {
                portAudioUpdateVoiceSpatialRamp(meta->voiceIndices[i],
                                                sndStubComputeVolumeForSound(state, meta->voiceSounds[i]),
                                                meta->posX, meta->posY, meta->posZ,
                                                rampUsec);
            }
        } else {
            portAudioUpdateVoiceMixRamp(meta->voiceIndices[i],
                                        sndStubComputeVolumeForSound(state, meta->voiceSounds[i]),
                                        sndStubComputePanForSound(state, meta->voiceSounds[i]),
                                        rampUsec);
        }
    }
}

static void sndStubRefreshStateMix(ALSoundState *state) {
    sndStubRefreshStateMixWithRamp(state, 0);
}

static void sndStubPrimeState(ALSoundState *state, ALSound *sound) {
    ALKeyMap *keyMap;
    ...
    state->vol = 0x7fff;
    state->fxMix = AL_DEFAULT_FXMIX;
    state->priority = 0x40;

    if (keyMap != NULL) {
        state->unk3e = (u8)(keyMap->keyMax & 0xf0);
        if ((state->unk3e & 0x20) != 0) {
            state->pitch_28 = alCents2Ratio((keyMap->keyBase * 100) + SND_STUB_DEFAULT_SETUP_PITCH_SHIFT);
        } else {
            state->pitch_28 = alCents2Ratio((keyMap->keyBase * 100) + keyMap->detune + SND_STUB_DEFAULT_SETUP_PITCH_SHIFT);
        }
    } else {
        state->unk3e = 0;
        state->pitch_28 = 1.0f;
...
static s16 sndStubNextChainedSoundIndex(const ALSound *sound) {
    if (sound == NULL || sound->keyMap == NULL) {
        return -1;
    }

    return sndKeyMapNextBankIndex(sound->keyMap);
}

static u32 sndStubChainedSoundDelayFrames(const ALSound *sound) {
    if (sound == NULL || sound->keyMap == NULL) {
        return 0;
    }

    return (u32)sound->keyMap->velocityMax;
}

static u32 sndStubHandleHashIndex(ALSoundState *state) {
    uintptr_t key = (uintptr_t)state;

    return (u32)((key >> 4) % SND_STUB_HANDLE_HASH_SIZE);
}

static void sndStubRegisterStateMeta(SndStubStateMeta *meta) {
    u32 bucket;
    ...
    bucket = sndStubHandleHashIndex(&meta->state);
    meta->nextHash = s_sndStubHandleHash[bucket];
    s_sndStubHandleHash[bucket] = meta;

    meta->nextAll = s_sndStubAllStates;
    s_sndStubAllStates = meta;
}

static void sndStubResetVoices(SndStubStateMeta *meta) {
    if (meta == NULL) {
        return;
    }
    ...
    meta->voiceIndex = -1;
    meta->voiceCount = 0;
    for (u8 i = 0; i < SND_STUB_MAX_VOICES_PER_STATE; i++) {
        meta->voiceIndices[i] = -1;
        meta->voiceSounds[i] = NULL;
    }
}

static SndStubStateMeta *sndStubStateFromHandle(ALSoundState *state) {
    SndStubStateMeta *meta;
    u32 bucket;
    ...
    bucket = sndStubHandleHashIndex(state);
    meta = s_sndStubHandleHash[bucket];
    while (meta != NULL) {
        if (&meta->state == state) {
            return meta;
        }
...
void sndStubInvalidateVoiceIndex(s32 voiceIdx) {
    SndStubStateMeta *meta;
    ...
    for (meta = s_sndStubAllStates; meta != NULL; meta = meta->nextAll) {
        if (!meta->active) {
            continue;
        }

        for (u8 j = 0; j < meta->voiceCount; j++) {
            if (meta->voiceIndices[j] == voiceIdx) {
                meta->voiceIndices[j] = -1;
                if (meta->voiceIndex == voiceIdx) {
                    meta->voiceIndex = -1;
                }
            }
        }
...
static ALSoundState *sndStubAllocState(void) {
    SndStubStateMeta *meta;

    if (s_sndStubActiveStateCount >= SND_STUB_MAX_STATES) {
        return NULL;
    }
    ...
    sndStubRegisterStateMeta(meta);
    meta->ownerSlot = NULL;
    meta->soundIndex = 0;
    sndStubResetVoices(meta);
    meta->baseVolume = 0x7fff;
    meta->posX = 0.0f;
    meta->posY = 0.0f;
    meta->posZ = 0.0f;
    meta->nextPitchFrame = 0;
    meta->hasPosition = 0;
    meta->active = 1;
    meta->state.playingState = AL_PLAYING;
    meta->state.unk3e |= 0x10;
    meta->state.priority = 0x40;
    s_sndStubActiveStateCount++;
    return &meta->state;
}

static void sndStubUnlinkParentState(SndStubStateMeta *meta) {
    ALSoundState *parent;
    ...
    if (parent != NULL && parent->link.next == (ALLink *)&meta->state) {
        parent->link.next = NULL;
    }
    meta->state.state = NULL;
}

static void sndStubReleaseStateMeta(SndStubStateMeta *meta) {
    if (meta == NULL || !meta->active) {
        return;
    }

    if (meta->ownerSlot != NULL && *meta->ownerSlot == &meta->state) {
    ...
    sndStubUnlinkParentState(meta);
    sndStubResetVoices(meta);
    meta->state.playingState = AL_STOPPED;
    meta->state.unk3e &= (u8)(~0x10);
    meta->ownerSlot = NULL;
    meta->hasPosition = 0;
    meta->nextPitchFrame = 0;
    meta->active = 0;
    if (s_sndStubActiveStateCount > 0) {
        s_sndStubActiveStateCount--;
    }
}

void sndNewPlayerInit(ALSeqpSfxConfig *cfg) {
    (void)cfg;
    SND_STUB_HIT(SND_STUB_NEW_PLAYER_INIT);
    s_sndStubSlotVolumeInit = 0;
    sndStubEnsureSlotVolumes();
}
u8 sndGetPlayingState(ALSoundState *state) {
    SndStubStateMeta *meta;
    extern s32 portAudioVoiceIsActive(s32 voiceIdx);
    u8 anyActive = 0;
    SND_STUB_HIT(SND_STUB_GET_PLAYING_STATE);
    meta = sndStubStateFromHandle(state);
    if (meta == NULL || !meta->active) {
        if (getenv("GE007_TRACE_CHR_AUDIO") != NULL && state != NULL) {
            printf("[CHR_AUDIO] get_state state=%p active=0\n", (void *)state);
        }
        return AL_STOPPED;
    }

    for (u8 i = 0; i < meta->voiceCount; i++) {
        if (meta->voiceIndices[i] >= 0 && portAudioVoiceIsActive(meta->voiceIndices[i])) {
            anyActive = 1;
            break;
        }
    }

    if (!anyActive && meta->voiceCount > 0) {
        if (getenv("GE007_TRACE_CHR_AUDIO") != NULL) {
            printf("[CHR_AUDIO] get_state state=%p active=0 dispose ownerSlot=%p sound=%d voices=%u\n",
                   (void *)state,
                   (void *)meta->ownerSlot,
                   meta->soundIndex,
                   (unsigned int)meta->voiceCount);
        }
        sndDisposeSound(state);
        return AL_STOPPED;
    }
    return meta->state.playingState;
}
...
s32 sndStateIsOwnedBySlot(ALSoundState *state, ALSoundState **slot) {
    ...
    if (state == NULL || slot == NULL || *slot != state) {
        return 0;
    }

    meta = sndStubStateFromHandle(state);
    return meta != NULL && meta->active && meta->ownerSlot == slot;
}
#endif

void sndDeactivate(ALSoundState *state) {
    SndStubStateMeta *meta;
    extern void portAudioReleaseVoice(s32 voiceIdx, ALMicroTime releaseUsec);
    SND_STUB_HIT(SND_STUB_DEACTIVATE);
    meta = sndStubStateFromHandle(state);
    if (meta == NULL || !meta->active) {
        return;
    }
    ...
    for (u8 i = 0; i < meta->voiceCount; i++) {
        if (meta->voiceIndices[i] >= 0) {
            portAudioReleaseVoice(
                meta->voiceIndices[i],
                sndStubComputeReleaseTimeForSound(state, meta->voiceSounds[i]));
        }
    }
    sndStubReleaseStateMeta(meta);
}
void sndDeactivateAllSfxByFlag_1(void) {
    SndStubStateMeta *item;
    SndStubStateMeta *next;

    SND_STUB_HIT(SND_STUB_DEACTIVATE_ALL_1);
    for (item = s_sndStubAllStates; item != NULL; item = next) {
        next = item->nextAll;
        if (item->active) {
            sndDeactivate(&item->state);
        }
    }
}
void sndDeactivateAllSfxByFlag_11(void) {
    SND_STUB_HIT(SND_STUB_DEACTIVATE_ALL_11);
    sndDeactivateAllSfxByFlag_1();
}
void sndDeactivateAllSfxByFlag_3(void) {
    SND_STUB_HIT(SND_STUB_DEACTIVATE_ALL_3);
    sndDeactivateAllSfxByFlag_1();
}
void sndCreatePostEvent(ALSoundState *state, s16 eventType, s32 arg2) {
    SND_STUB_HIT(SND_STUB_CREATE_POST_EVENT);
    if (state != NULL) {
        SndStubStateMeta *meta = sndStubStateFromHandle(state);

        if (meta != NULL && meta->active) {
            if (eventType == AL_SNDP_VOL_EVT) {
                if (arg2 < 0) {
                    arg2 = 0;
                } else if (arg2 > 0x7fff) {
                    arg2 = 0x7fff;
                ...
                portAudioTraceSfxJson(
                    ...
                    meta->soundIndex,
                    arg2,
                    (double)sndStubComputeVolume(state),
                    meta->hasPosition,
                    (double)meta->posX,
                    (double)meta->posY,
                    (double)meta->posZ,
                ...
                sndStubRefreshStateMixWithRamp(
                    state,
                    sndStubAuthoredMixEnabled() ? DELTA_1_MS : 0);
            } else if (eventType == AL_SNDP_PITCH_EVT) {
                union { s32 i; f32 f; } pun;
                pun.i = arg2;
                state->pitch_2c = pun.f;
                ...
                for (u8 i = 0; i < meta->voiceCount; i++) {
                    if (meta->voiceIndices[i] >= 0) {
                        portAudioUpdateVoicePitch(
                            meta->voiceIndices[i],
                            sndStubComputePitchRatioForSound(state, meta->voiceSounds[i]));
                    }
                }
            } else if (eventType == AL_SNDP_FX_EVT) {
                if (arg2 < 0) {
                    arg2 = 0;
                } else if (arg2 > AL_VOL_FULL) {
                    arg2 = AL_VOL_FULL;
                }
                ...
                portAudioTraceSfxJson(
                    ...
                    meta->soundIndex,
                    arg2,
                    sndStubComputeNativeFxMixForSound(state, meta->state.sound),
                    sndStubAuthoredMixEnabled());

                for (u8 i = 0; i < meta->voiceCount; i++) {
                    if (meta->voiceIndices[i] >= 0) {
                        portAudioUpdateVoiceFxMix(
                            meta->voiceIndices[i],
                            (u8)sndStubComputeNativeFxMixForSound(state, meta->voiceSounds[i]));
                    }
                }
            }
        }
...
void sndStubTick(void) {
    ...
    for (meta = s_sndStubAllStates; meta != NULL; meta = meta->nextAll) {
        ...
        if (!meta->active || meta->state.sound == NULL || meta->state.sound->keyMap == NULL) {
            continue;
        }
        if ((meta->state.unk3e & 0x20) == 0 || meta->nextPitchFrame <= 0) {
            continue;
        }
        if (g_GlobalTimer < meta->nextPitchFrame) {
            continue;
        }

        keyMap = meta->state.sound->keyMap;
        pun.f = alCents2Ratio(keyMap->detune) * meta->state.pitch_2c;
        sndCreatePostEvent(&meta->state, AL_SNDP_PITCH_EVT, pun.i);

        do {
            meta->nextPitchFrame += 2;
        } while (g_GlobalTimer >= meta->nextPitchFrame);
    }
}

/* Spatial audio position hint — set before calling sndPlaySfx for 3D positioning */
static s32 s_sndPosHintActive = 0;
static f32 s_sndPosX, s_sndPosY, s_sndPosZ;

void sndSetPositionHint(f32 x, f32 y, f32 z) {
    s_sndPosHintActive = 1;
    s_sndPosX = x;
    s_sndPosY = y;
    s_sndPosZ = z;
}
...
ALSoundState *sndPlaySfx(void *soundBank, s16 soundIndex, ALSoundState *pendingState) {
    return sndPlaySfxTagged(soundBank, soundIndex, pendingState, NULL, 0);
}
...
ALSoundState *sndPlaySfx(struct ALBankAlt_s *soundBank, s16 soundIndex, ALSoundState *pendingState) {
    ...
    SndStubStateMeta *meta;
    SndStubStateMeta *pendingMeta;
    s16 bankSoundIndex;
    static s32 s_allowLegacyOverlap = -1;
#ifdef PORT_SND_STUBS
    const char *traceCallerFile = callerFile != NULL ? callerFile : "";
    int traceCallerLine = callerLine;
#else
    const char *traceCallerFile = "";
    int traceCallerLine = 0;
#endif

    SND_STUB_HIT(SND_STUB_PLAY_SFX);
    bank = (struct ALBankAlt_s *)soundBank;
    sndStubEnsureSlotVolumes();
    ...
    meta = sndStubStateFromHandle(newState);
    meta->soundIndex = soundIndex;
    sndStubPrimeState(newState, sound);
    if (sndStubSoundUsesPitchEvents(sound)) {
        meta->nextPitchFrame = g_GlobalTimer + 2;
    }

    /* Submit through temporary backend adapter.
     * This isolates snd.c from direct callback-device ownership and gives us
     * a clean cut-over point for synth/CSP/AI queue routing. */
    if (portAudioIsReady()) {
        s16 chainIndex = bankSoundIndex;
        u32 delayFrames = 0;

        if (s_sndPosHintActive && meta != NULL) {
            meta->hasPosition = 1;
            meta->posX = s_sndPosX;
            meta->posY = s_sndPosY;
            meta->posZ = s_sndPosZ;
        ...
        while (chainIndex >= 0 && meta != NULL &&
            ...
            delaySamples = (s32)(delayFrames * 735);
            memset(&params, 0, sizeof(params));
            params.volume = sndStubComputeVolumeForSound(newState, chainSound);
            params.pan = sndStubComputePanForSound(newState, chainSound);
            params.pitch = sndStubComputePitchRatioForSound(newState, chainSound);
            params.delaySamples = delaySamples;
            params.fxMix = (u8)sndStubComputeNativeFxMixForSound(newState, chainSound);
            if (sndStubAuthoredMixEnabled() && chainSound->envelope != NULL) {
                params.useEnvelope = 1;
                params.attackVolume = chainSound->envelope->attackVolume;
            ...
            voiceIdx = portAudioPlaySfxDetailed(chainIndex, &params);

            slotIndex = sndStubSlotIndexForSound(chainSound);
            attackVol = chainSound->envelope ? (unsigned int)chainSound->envelope->attackVolume : AL_VOL_FULL;
            decayVol = chainSound->envelope ? (unsigned int)chainSound->envelope->decayVolume : AL_VOL_FULL;
            attackNative = sndStubComputeNativeEnvelopeVolumeForSound(newState, chainSound, attackVol);
            decayNative = sndStubComputeNativeEnvelopeVolumeForSound(newState, chainSound, decayVol);
            panNative = (s32)sndStubComputePanForSound(newState, chainSound);
            portAudioTraceSfxJson(
                "{\"event\":\"snd_submit\",\"frame\":%d,\"public_sound\":%d,\"public_bank\":%d,\"submit_bank\":%d,"
                "\"voice\":%d,\"state\":\"%p\",\"caller\":\"%s\",\"line\":%d,"
                "\"auth_mix\":%d,\"state_vol\":%d,\"sample_vol\":%u,"
                "\"slot\":%d,\"slot_applied\":%d,\"slot_natural\":%u,\"slot_scale\":%.6f,"
                "\"volume\":%.6f,\"pan\":%.3f,\"pitch\":%.6f,\"pitch_dynamic\":%d,\"detune\":%d,\"delay_samples\":%d,"
                "\"fx_base\":%u,\"fx_nibble\":%d,\"fx_mix\":%u,"
                ...
                sndStubAuthoredMixEnabled(),
                (int)newState->vol,
                (unsigned int)chainSound->sampleVolume,
                slotIndex,
                (slotIndex >= 0 && slotIndex < SFX_SLOT_COUNT) ? (int)g_sndSfxSlotVolume[slotIndex] : -1,
                (slotIndex >= 0 && slotIndex < SFX_SLOT_COUNT) ? (unsigned int)g_sndSfxSlotNaturalVolume[slotIndex] : 0U,
                (double)sndStubComputeSlotScaleForSound(chainSound),
                (double)params.volume,
                (double)params.pan,
                (double)params.pitch,
                sndStubSoundUsesPitchEvents(chainSound),
                chainSound->keyMap ? (int)chainSound->keyMap->detune : 0,
                params.delaySamples,
                (unsigned int)newState->fxMix,
                sndStubFxNibbleForSound(chainSound),
                (unsigned int)params.fxMix,
                chainSound->envelope ? (int)chainSound->envelope->attackTime : 0,
                chainSound->envelope ? (int)chainSound->envelope->decayTime : 0,
                chainSound->envelope ? (int)chainSound->envelope->releaseTime : 0,
                attackVol,
                decayVol,
                attackNative,
                decayNative,
            ...
            meta->voiceIndices[meta->voiceCount] = (s16)voiceIdx;
            meta->voiceSounds[meta->voiceCount] = chainSound;
            meta->voiceCount++;
            if (meta->voiceIndex < 0) {
                meta->voiceIndex = (s16)voiceIdx;
            }

            delayFrames += sndStubChainedSoundDelayFrames(chainSound);
            chainIndex = sndStubNextChainedSoundIndex(chainSound);
        }

        sndStubRefreshStateMix(newState);

        s_sndPosHintActive = 0;
    }

    if (getenv("GE007_TRACE_WEAPON_AUDIO") != NULL) {
        f32 slotScale = sndStubComputeSlotScaleForSound(sound);
        printf("[SND_STUB] play sound=%d bank=%d voice=%d pitch=%.3f vol=%.3f pan=%.1f chain_next=%d voices=%u stateVol=%d sampleVol=%u slotScale=%.3f authMix=%d\n",
               soundIndex,
               bankSoundIndex,
               meta ? meta->voiceIndex : -1,
               (double)sndStubComputePitchRatioForSound(newState, sound),
               (double)sndStubComputeVolumeForSound(newState, sound),
               (double)sndStubComputePan(newState),
               sndStubNextChainedSoundIndex(sound),
               meta ? (unsigned int)meta->voiceCount : 0U,
               (int)newState->vol,
               (unsigned int)sound->sampleVolume,
               (double)slotScale,
               sndStubAuthoredMixEnabled());
        if (sndStubAuthoredMixEnabled()) {
            printf("[SND_STUB_AUTH] sound=%d bank=%d stateVol=%d sampleVol=%u slotScale=%.6f baseVol=%.6f attackVol=%u decayVol=%u attackTime=%d decayTime=%d\n",
                   soundIndex,
                   bankSoundIndex,
                   (int)newState->vol,
                   (unsigned int)sound->sampleVolume,
                   (double)slotScale,
                   (double)sndStubComputeVolumeForSound(newState, sound),
                   sound->envelope ? (unsigned int)sound->envelope->attackVolume : 127U,
                   sound->envelope ? (unsigned int)sound->envelope->decayVolume : 127U,
                   sound->envelope ? (int)sound->envelope->attackTime : 0,
                   sound->envelope ? (int)sound->envelope->decayTime : 0);
        }
    }

    /* Writeback with real handles:
     * - If pendingState is a managed state, preserve native chaining semantics.
     * - Otherwise treat pendingState as ALSoundState* slot storage and write back
     *   the real allocated handle.
     * Legacy overlap fallback is opt-in only (GE007_SND_ALLOW_LEGACY_OVERLAP=1). */
    if (pendingState != NULL) {
        pendingMeta = sndStubStateFromHandle(pendingState);
        if (pendingMeta != NULL) {
            /* Native sndPlaySfx treats any real ALSoundState* as chaining
             * storage, even if that prior sound has already stopped. */
            pendingState->link.next = (void *)newState;
            newState->state = pendingState;
        } else {
            ALSoundState **slot = (ALSoundState **)pendingState;
            *slot = newState;
            sndStubStateFromHandle(newState)->ownerSlot = slot;
            if (getenv("GE007_TRACE_CHR_AUDIO") != NULL) {
                printf("[CHR_AUDIO] slot_write slot=%p state=%p sound=%d bank=%d voice=%d\n",
                       (void *)slot,
                       (void *)newState,
                       soundIndex,
                       bankSoundIndex,
            ...
            if (s_allowLegacyOverlap < 0) {
                s_allowLegacyOverlap = (getenv("GE007_SND_ALLOW_LEGACY_OVERLAP") != NULL) ? 1 : 0;
            }
            if (s_allowLegacyOverlap) {
                pendingState->link.next = (void *)newState;
                s_sndStubLegacyOverlapFallbackHits++;
            }
        }
    }
...
u16 sndGetSfxSlotFirstNaturalVolume(void) {
    sndStubEnsureSlotVolumes();
    return g_sndSfxSlotNaturalVolume[0];
}
void sndApplyVolumeAllSfxSlot(u16 arg0) {
    u8 i;
    sndStubEnsureSlotVolumes();
    for (i = 0; i < SFX_SLOT_COUNT; i++) {
        sndSetSfxSlotVolume(i, arg0);
    }
}
void sndSetScalerApplyVolumeAllSfxSlot(f32 arg0) {
    sndStubEnsureSlotVolumes();
    g_sndSfxVolumeScale = arg0;
    sndApplyVolumeAllSfxSlot(sndGetSfxSlotFirstNaturalVolume());
}
u16 sndGetSfxSlotNaturalVolume(u8 arg0) {
    sndStubEnsureSlotVolumes();
    return (arg0 < SFX_SLOT_COUNT) ? g_sndSfxSlotNaturalVolume[arg0] : 0x7FFF;
}
void sndSetSfxSlotVolume(u8 arg0, u16 arg1) {
    SndStubStateMeta *item;
    sndStubEnsureSlotVolumes();
    if (arg0 >= SFX_SLOT_COUNT) {
        return;
    }
    g_sndSfxSlotNaturalVolume[arg0] = arg1;
    ...
    for (item = s_sndStubAllStates; item != NULL; item = item->nextAll) {
        if (item->active && item->state.sound != NULL &&
            ...
            sndStubRefreshStateMixWithRamp(&item->state, rampUsec);
        }
    }
}
void sndDisposeSound(ALSoundState *state) {
    SndStubStateMeta *meta;
    SND_STUB_HIT(SND_STUB_DISPOSE_SOUND);
    meta = sndStubStateFromHandle(state);
    if (meta == NULL || !meta->active) {
        return;
    }
    if (meta->ownerSlot && *meta->ownerSlot == state) {
        *meta->ownerSlot = NULL;
    }
    if (meta->voiceCount > 0) {
        extern void portAudioStopVoice(s32 voiceIdx);
        for (u8 i = 0; i < meta->voiceCount; i++) {
            if (meta->voiceIndices[i] >= 0) {
                portAudioStopVoice(meta->voiceIndices[i]);
            }
        }
    }
    sndStubReleaseStateMeta(meta);
...
void sndSetPriority(ALSoundState *state, u8 priority) {
    SndStubStateMeta *meta;
    SND_STUB_HIT(SND_STUB_SET_PRIORITY);
    meta = sndStubStateFromHandle(state);
    if (meta == NULL || !meta->active) {
        return;
    }
    meta->state.priority = priority;
}
void sndUnlinkClearSound(ALSoundState *state) {
    SND_STUB_HIT(SND_STUB_UNLINK_CLEAR);
    sndDisposeSound(state);
}
s32 sndCountAllocList(s16 *allocListCount, s16 *freeListCount) {
    SND_STUB_HIT(SND_STUB_COUNT_ALLOC_LIST);
    if (allocListCount) *allocListCount = s_sndStubActiveStateCount;
    if (freeListCount) *freeListCount = (s16)(SND_STUB_MAX_STATES - s_sndStubActiveStateCount);
    return 0;
}
...
static s32 s_sndPosHintActive = 0;
static f32 s_sndPosX;
static f32 s_sndPosY;
static f32 s_sndPosZ;

static s32 sndStateIsLive(const ALSoundState *state);

static ALPan sndComputePanFromPosition(f32 x, f32 y, f32 z)
{
    f32 panF = (f32)AL_PAN_CENTER;

    portAudioComputeSpatialMix(x, y, z, NULL, &panF);

    if (panF < (f32)AL_PAN_LEFT) {
        panF = (f32)AL_PAN_LEFT;
    } else if (panF > (f32)AL_PAN_RIGHT) {
        panF = (f32)AL_PAN_RIGHT;
    }

    return (ALPan)(panF + 0.5f);
}

static void sndApplyStatePositionPan(ALSoundState *state, f32 x, f32 y, f32 z,
                                     s32 postIfPlaying)
{
    ALPan pan;
    ...
    pan = sndComputePanFromPosition(x, y, z);
    state->pan = pan;
    ...
    if (postIfPlaying && state->playingState == AL_PLAYING) {
        sndCreatePostEvent(state, AL_SNDP_PAN_EVT, (s32)pan);
    }
}

static void sndApplyPositionHint(ALSoundState *state)
{
    if (s_sndPosHintActive) {
        sndApplyStatePositionPan(state, s_sndPosX, s_sndPosY, s_sndPosZ, 0);
    }
}

static ALSoundState *sndPlaySfxReturn(ALSoundState *state)
{
    sndClearPositionHint();
    return state;
}

/**
 * Event payloads consumed by the local sound-player queue.
 */
...
union ALSndpSmallEvent_u {
    ...
    /*
     * alEvtqPostEvent copies sizeof(ALEvent) bytes from the source pointer.
     * Keep these compact SndPlayer events large enough for the native
     * 64-bit ALEvent layout while preserving the fields the handler uses.
     */
    ALEvent align_event;
};
...
struct D_800243E4_s {
    ...
    ALLink node;
    // address 800243EC
    ALSoundState *g_sndPlayerSoundStatePtr;
};
...
ALSndPlayer g_sndPlayer = {0};
ALSndPlayer *g_sndPlayerPtr = &g_sndPlayer;
static PortSndPlayerStats s_sndPlayerStats;
static u32 s_sndPlayerStateCount;

void sndGetPlayerStats(PortSndPlayerStats *stats)
{
    if (stats != NULL)
    {
        *stats = s_sndPlayerStats;
        stats->realPath = 1;
    }
}

static s32 sndStateInPool(const ALSoundState *state)
{
    const uintptr_t ptr = (uintptr_t)state;
    const uintptr_t base = (uintptr_t)g_sndPlayerPtr->sndState;
    const uintptr_t end = base + ((uintptr_t)s_sndPlayerStateCount * sizeof(ALSoundState));

    return state != NULL
        && base != 0U
        && ptr >= base
        && ptr < end
        && ((ptr - base) % sizeof(ALSoundState)) == 0U;
}

static s32 sndStateIsLive(const ALSoundState *state)
{
    return sndStateInPool(state)
...
static ALSoundState *sndPlaySfxInternal(struct ALBankAlt_s *soundBank, s16 soundIndex,
                                        ALSoundState *pendingState, s32 soundIndexIsPublic,
 ...
 * 8720    70007B20
 *
 * Initializes the local sound-player state pool and event queue.
 */
void sndNewPlayerInit(ALSeqpSfxConfig *sfxSeqpConfig)
{
     ...
     */
    memset(&s_sndPlayerStats, 0, sizeof(s_sndPlayerStats));
    s_sndPlayerStats.playerInits++;
    ...
    sState = alHeapAlloc(sfxSeqpConfig->heap, 1, sfxSeqpConfig->maybeSndStateCount * sizeof(ALSoundState));
    g_sndPlayerPtr->sndState = sState;
    s_sndPlayerStateCount = (sState != NULL) ? (u32)sfxSeqpConfig->maybeSndStateCount : 0U;
    ...
    for(i = 1; i < sfxSeqpConfig->maybeSndStateCount; i++)
    {
        ...
        sState = (ALSoundState *)g_sndPlayerPtr->sndState;
 ...
 * 89DC    70007DDC
 *
 * Runs queued sound-player events until the next nonzero delta.
 */
ALMicroTime sndPlayerVoiceHandler(void *node)
{
 ...
 * 8A80    70007E80
 *
 * Handles one queued sound-player event.
 *
 * decomp status:
 * - compiles: yes
 * - stack resize: ok
...
void sndHandleEvent(ALSndPlayer *sndp, ALSndpEvent *event)
{
    ...
    s32 startVolume;                    // native-port telemetry only
    ALSoundState *prevState = NULL;     // sp 0x78 = 120
        ...
        switch (event->common.type)
        {
            case (AL_SNDP_PLAY_EVT):
            {
                if ((state->playingState == AL_UNKOWN_5) || (state->playingState == AL_UNKOWN_4))
                {
                    ...
                    if (!allocVoiceSuccess)
                    {
                        ...
                        if (compare_result)
                        {
                            ...
                            do
                            {
                                ...
                                {
                                    playAllocEvent.msg.type = AL_SNDP_END_EVT;
                    ...
                    s_sndPlayerStats.playEvents++;
                    s_sndPlayerStats.voiceStarts++;
                    s_sndPlayerStats.activeVoices = (u32)g_sndAllocatedVoicesCount;
                    ...
                    stopEvent.msg.type = AL_SNDP_DECAY_EVT;
                ...
                )
                {
                    switch (state->playingState)
                    {
                        case (AL_PLAYING):
                        {
                            ...
                            if (delta)
                            {
                                stopEvent.msg.type = AL_SNDP_END_EVT;
                                stopEvent.msg.state = state;
                                alEvtqPostEvent(&sndp->evtq, (ALEvent *)&stopEvent, delta);
            ...
            case (AL_SNDP_DECAY_EVT):
            {
                if ((state->unk3e & 2) == 0)
                {
                    ...
                    stopEvent.msg.type = AL_SNDP_STOP_EVT;
                    stopEvent.msg.state = state;
                    alEvtqPostEvent(&sndp->evtq, (ALEvent *)&stopEvent, delta);
            ...
            case (AL_SNDP_PLAY_SFX_EVT):
            {
                ...
                {
                    sndPlaySfxInternal((struct ALBankAlt_s *)event->playSfx.soundBank,
                                       event->playSfx.soundIndex,
                                       state->state,
                                       0,
                                       event->playSfx.callerFile,
                                       event->playSfx.callerLine);
                }
            }
            break;
...
void sndDisposeSound(ALSoundState *state)
{
    ...
    if (hadVoice)
    {
        s_sndPlayerStats.activeVoices = (g_sndAllocatedVoicesCount > 0)
            ? (u32)g_sndAllocatedVoicesCount
            : 0U;
        portAudioTraceSfxJson(
...
/**
 * 9630     70008A30
 * Removes queued events of a matching type for one sound state.
 */
void sndRemoveEvents(ALEventQueue *evtq, ALSoundState *state, u16 eventType)
{
    ALLink              *thisNode;
...
s32 sndCountAllocList(s16 *allocListCount, s16 *freeListCount)
{
    ...
    for (counter1 = 0;
         freeListNodeForward != NULL && counter1 < s_sndPlayerStateCount;
         freeListNodeForward = freeListNodeForward->next)
    {
         counter1++;
    }

    for (counter2 = 0;
         allocListNodeForward != NULL && counter2 < s_sndPlayerStateCount;
         allocListNodeForward = allocListNodeForward->next)
    {
         counter2++;
    }

    for (returnCounter = 0;
         freeListNodeBackward != NULL && returnCounter < s_sndPlayerStateCount;
         freeListNodeBackward = freeListNodeBackward->prev)
    {
         returnCounter++;
    }
...
{
    (void)volume;
    return sndPlaySfxInternal(soundBank, soundIndex, pendingState, 1, file, line);
}
#else
/**
 * 9A08    70008E08
...
ALSoundState *sndPlaySfx(struct ALBankAlt_s *soundBank, s16 soundIndex, ALSoundState *pendingState)
{
    return sndPlaySfxInternal(soundBank, soundIndex, pendingState, 1, NULL, 0);
}
#endif
...
ALSoundState *sndPlaySfxTagged(struct ALBankAlt_s *soundBank,
                               ...
                               int callerLine)
{
    return sndPlaySfxInternal(soundBank, soundIndex, pendingState, 1, callerFile, callerLine);
}
#endif
                                        ...
                                        const char *callerFile, int callerLine)
{
    ...
    s16 publicSoundIndex;
    s16 entryBankSoundIndex;
    ...
    nextState = NULL;
    eventSoundIndex = 0;
    publicSoundIndex = soundIndexIsPublic ? soundIndex : 0;
    entryBankSoundIndex = 0;
    deltaTotal = 0;
    deltaLoop = 0;
    ...
    if (soundIndexIsPublic)
    {
        soundIndex = sndPublicSfxIdToBankIndex(soundIndex);
        if (soundIndex < 0)
        {
            return sndPlaySfxReturn(NULL);
        }
    ...
    entryBankSoundIndex = soundIndex;
    ...
    do
    {
        ...
        if (newState != NULL)
        {
            ...
            submitBankSoundIndex = soundIndex;
            entrySubmit = (nextState == NULL);
            deltaLoop = sound->keyMap->velocityMax * DELTA_33_MS;
            g_sndPlayerPtr->target = (s32)(intptr_t)newState;
            s_sndPlayerStats.submitEvents++;
            portAudioTraceSfxJson(
                "{\"event\":\"sndp_submit\",\"frame\":%d,\"sound\":%d,\"public_sound\":%d,"
                ...
                (int)submitBankSoundIndex + 1,
                (int)entryBankSoundIndex,
                (int)entryBankSoundIndex + 1,
                (int)entrySubmit,
                (void *)newState,
        ...
        keyMap = sound->keyMap;
        soundIndex = sndKeyMapNextBankIndex(keyMap);
...
void sndDeactivateAllSfxByFlag(u8 flag)
{
    ...
    for (i = 0; i < s_sndPlayerStateCount; i++)
    {
        ...
        if (sndStateIsLive(item) && ((item->unk3e & flag) == flag))
        {
...
#endif /* PORT_SND_STUBS */

#ifndef PORT_SND_STUBS
void sndStubTick(void) {
}
