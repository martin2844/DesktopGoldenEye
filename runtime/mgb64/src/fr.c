...
/**
 * Address 80060828.
 * Original VI mode captured before runtime adjustments.
 */
 ...
 *
 * Applies the current vertical shake offset to the VI start registers.
 */
...
{
#ifdef NATIVE_PORT
    static int trace_vi = -1;
    static int trace_vi_count = 0;
    if (trace_vi < 0) {
        trace_vi = (getenv("GE007_TRACE_MENU_VI") != NULL);
    }
    if (trace_vi && trace_vi_count < 512) {
        fprintf(stderr,
                "[MENU_VI] video_related_F x=%d y=%d buf=%d,%d view=%d,%d@%d,%d "
                "fovy=%.6f aspect=%.6f z=%.6f..%.6f usez=%d\n",
                g_ViBackData->x,
                g_ViBackData->y,
                g_ViBackData->bufx,
                g_ViBackData->bufy,
...
void viSetXY(s16 x, s16 y)
{
    ...
    if (trace_vi) {
        fprintf(stderr, "[MENU_VI] viSetXY(%d,%d)\n", x, y);
        fflush(stderr);
    }
#endif
    g_ViBackData->x = x;
...
void viSetViewSize(s16 x, s16 y)
{
    ...
    if (trace_vi) {
        fprintf(stderr, "[MENU_VI] viSetViewSize(%d,%d)\n", x, y);
        fflush(stderr);
    }
#endif
...
#ifdef NATIVE_PORT
    /* PC: apply to BOTH double-buffered VideoSettings to prevent the other
     * buffer from retaining stale menu znear after fog initialization. */
    {
        extern struct VideoSettings_s g_ViDataArray[];
        int i;
        for (i = 0; i < NUM_VIDEO_SETTINGS; i++) {
            g_ViDataArray[i].znear = near;
            g_ViDataArray[i].zfar = far;
        }
    }
#endif
    currentPlayerSetPerspective(g_ViBackData->znear, g_ViBackData->fovy, g_ViBackData->aspect);
...
void indyGrabJpg16bit(void)
{
    ...
    while (1)
    {
        ge007_sprintf(buffer, "grab.%d.jpeg", *pgrabnum);
        if (indycommHostCheckFileExists(buffer, &filesize) == 0)
        {
            break;
        }
    ...
    ge007_sprintf(buffer, "grab.%d.temp.uix", *pgrabnum);
    indycommHostSendDump(buffer, (u8*)g_ViBackData->framebuf, (viGetX() * viGetY() * 2));

    ge007_sprintf(buffer, "uix2pix grab.%d.temp.uix", *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "fromalias grab.%d.temp.pix grab.%d.temp.rgb", *pgrabnum, *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "imgcopy -fjfif grab.%d.temp.rgb grab.%d.jpeg", *pgrabnum, *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "rm grab.%d.temp.uix grab.%d.temp.pix grab.%d.temp.rgb", *pgrabnum, *pgrabnum, *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "imgview grab.%d.jpeg", *pgrabnum);
    indycommHostSendCmd(buffer);
#endif
}
...
void indyGrabJpg32bit(void)
{
    ...
    ge007_sprintf(buffer, "Uix2pix -xs%d grab.%d.temp.Uix", viGetX(), *pgrabnum);
    indycommHostSendCmd(buffer);
...
void indyGrabRgb16bit(void)
{
    ...
    while (1)
    {
        ge007_sprintf(buffer, "grab.%d.rgb", *pgrabnum);
        if (indycommHostCheckFileExists(buffer, &filesize) == 0)
        {
            break;
        }
    ...
    ge007_sprintf(buffer, "fromalias grab.%d.temp.pix grab.%d.rgb", *pgrabnum, *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "rm grab.%d.temp.uix grab.%d.temp.pix", *pgrabnum, *pgrabnum);
    indycommHostSendCmd(buffer);

    ge007_sprintf(buffer, "imgview grab.%d.rgb", *pgrabnum);
    indycommHostSendCmd(buffer);
#endif
}
