/*
 * Clean-room OS compatibility declarations and constants for the matching
 * target, plus a native-port forwarding shim for direct <PR/os.h> includes.
 */
#ifdef NATIVE_PORT
#ifndef _OS_H_
#define _OS_H_
...
/* NOTE: these fields are named `errno` in the real libultra ABI; renamed to
 * `errnum` here (and in src/platform/platform_os.h) because errno is a libc
 * macro (<errno.h>) and collides on hosted toolchains (e.g. MinGW). Pure
 * rename -- layout/codegen unchanged. */
typedef struct {
	u16     type;
	u8      status;
	u8	errnum;
