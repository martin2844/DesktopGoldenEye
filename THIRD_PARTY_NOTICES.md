...
| Jabo Direct3D 6/8 | optional legacy graphics plugins | Checksum-pinned upstream binary bundle | Not selected by DesktopGoldenEye. Noncommercial intent alone is not redistribution permission; do not publish an independently hosted package until permission/license provenance is documented |
...
The playable default is `DesktopGoldenEye.exe` + `GLideN64.dll` + `Mouse_Injector.dll` + `AziAudio.dll`. Jabo and other legacy plugins are not required for the quality profile and can later be offered as explicitly labeled compatibility options if their redistribution basis is recorded.
